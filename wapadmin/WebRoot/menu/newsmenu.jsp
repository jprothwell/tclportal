...
<table width="100%" border="1" align="center" cellpadding="1" cellspacing="1" class="dtree">
  ...
  <tr><td>id</td><td>名称</td><td>语言</td><td>发布日期</td></tr>
  <%
  if(contentid.equals("null")||contentid.length()==0)out.println("<tr><td colspan='4'>目前此栏目没有资源！</td></tr>");
  else {
    DBMap[] map = dao.newsList(contentid);
...
<DIV class=macrolong_aTab>
<table width="100%" border="1" align="center" cellpadding="1" cellspacing="1" class="dtree">
<tr><td colspan="4" align="center">共<%=total_num%>纪录 <%=per_page%>/<%=total_page%>页 <a href="newsmenu.jsp?id=<%=id%>&amp;contentid=<%=contentid%>&amp;perPage=1">首页</a>|
    <%
    if(per_page>1)out.println("<a href='newsmenu.jsp?id="+id+"&amp;contentid="+contentid+"&amp;perPage="+last_num+"'>上页</a>");
     else out.println("上页");
     %>
    |
        <%
    if(per_page<total_page)out.println("<a href='newsmenu.jsp?id="+id+"&amp;contentid="+contentid+"&amp;perPage="+next_num+"'>下页</a>");
     else out.println("下页");
     %>
    |<a href="newsmenu.jsp?id=<%=id%>&amp;contentid=<%=contentid%>&amp;perPage=<%=total_page%>">末页</a></td></tr>
    <tr>
  <tr><td>id</td><td>名称</td><td>语言</td><td>发布日期</td></tr>
...
<tr><td colspan="7" align="center">共<%=total_num%>纪录 <%=per_page%>/<%=total_page%>页 <a href="newsmenu.jsp?id=<%=id%>&amp;contentid=<%=contentid%>&amp;perPage=1">首页</a>|
    <%
    if(per_page>1)out.println("<a href='newsmenu.jsp?id="+id+"&amp;contentid="+contentid+"&amp;perPage="+last_num+"'>上页</a>");
     else out.println("上页");
     %>
    ...
    if(per_page<total_page)out.println("<a href='newsmenu.jsp?id="+id+"&amp;contentid="+contentid+"&amp;perPage="+next_num+"'>下页</a>");
     ...
     %>
    |<a href="newsmenu.jsp?id=<%=id%>&amp;contentid=<%=contentid%>&amp;perPage=<%=total_page%>">末页</a></td></tr></table>
</DIV>
</DIV>
<!--下面是引用代码-->
