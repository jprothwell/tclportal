...
<table width="100%" border="1" align="center" cellpadding="1" cellspacing="1" class="dtree">
  ...
  <tr><td>id</td><td>名称</td><td>文件</td><td>视频类型</td><td>视频尺寸</td><td>适合语言</td><td>视频说明</td></tr>
  <%
  if(contentid.equals("null")||contentid.length()==0)out.println("<tr><td colspan='7'>目前此栏目没有资源！</td></tr>");
  else {
    DBMap[] map = dao.videoList(contentid);
...
<DIV class=macrolong_aTab>
<table width="100%" border="1" align="center" cellpadding="1" cellspacing="1" class="dtree">
<tr><td colspan="7" align="center">共<%=total_num%>纪录 <%=per_page%>/<%=total_page%>页 <a href="videomenu.jsp?id=<%=id%>&amp;contentid=<%=contentid%>&amp;perPage=1">首页</a>|
    <%
    if(per_page>1)out.println("<a href='videomenu.jsp?id="+id+"&amp;contentid="+contentid+"&amp;perPage="+last_num+"'>上页</a>");
     else out.println("上页");
     %>
    |
        <%
    if(per_page<total_page)out.println("<a href='videomenu.jsp?id="+id+"&amp;contentid="+contentid+"&amp;perPage="+next_num+"'>下页</a>");
     else out.println("下页");
     %>
    |<a href="videomenu.jsp?id=<%=id%>&amp;contentid=<%=contentid%>&amp;perPage=<%=total_page%>">末页</a></td></tr>
    <tr>
 <tr><td>id</td><td>名称</td><td>文件</td><td>视频类型</td><td>视频尺寸</td><td>适合语言</td><td>视频说明</td></tr>
  </tr>
...
<tr><td colspan="7" align="center">共<%=total_num%>纪录 <%=per_page%>/<%=total_page%>页 <a href="videomenu.jsp?id=<%=id%>&amp;contentid=<%=contentid%>&amp;perPage=1">首页</a>|
    ...
    if(per_page<total_page)out.println("<a href='videomenu.jsp?id="+id+"&amp;contentid="+contentid+"&amp;perPage="+next_num+"'>下页</a>");
     ...
     %>
    |<a href="videomenu.jsp?id=<%=id%>&amp;contentid=<%=contentid%>&amp;perPage=<%=total_page%>">末页</a></td></tr></table>
</DIV>
</DIV>
<!--下面是引用代码-->
