...
public class Workspace extends ViewGroup implements DropTarget, DragSource,
	...
	// wisky@wanglijuan for preview{
	// sense zoom constants
	...
	private static int touchtype = SCROLL;
	private Paint mPaint;
	private PointF ptk1 = new PointF();
	private PointF ptk2 = new PointF();
	// end wisky@wanglijuan }
	...
	private void initWorkspace() {
		...
		Log.d(TAG, "initWorkspace.....Launcher.setScreen="+mCurrentScreen);
		...
		// 恢复退出前的所有信息
		SharedPreferences sharedPreferences = context.getSharedPreferences("status",
				Context.MODE_PRIVATE);
		mStatus = sharedPreferences.getInt("status", SENSE_CLOSED);
		isAnimating = sharedPreferences.getBoolean("isAnimating", false);
	...
	public void addView(View child, int width, int height) {
		Log.d(TAG, "addView .....width:"+width+",height:"+height);
		if (!(child instanceof CellLayout)) {
			throw new IllegalArgumentException(
					"A Workspace can only have CellLayout children.");
		}
	...
	Folder getOpenFolder() {
		...
		Log.d(TAG, "getOpenFolder,count:"+count);
	...
	ArrayList<Folder> getOpenFolders() {
		...
		Log.d(TAG, "getOpenFolder,screens:"+screens);
	...
	void setCurrentScreen(int currentScreen) {
		Log.d(TAG, "setCurrentScreen:"+currentScreen);
	...
	void addInCurrentScreen(View child, int x, int y, int spanX, int spanY) {
		Log.d(TAG, "addInCurrentScreen..x="+x+",y="+y+",spanX="+spanX+",spanY="+spanY);
	...
	void addInCurrentScreen(View child, int x, int y, int spanX, int spanY,
			boolean insert) {
		Log.d(TAG, "addInCurrentScreen..x="+x+",y="+y+",spanX="+spanX+",spanY="+spanY+",insert="+insert);
	...
	void addInScreen(View child, int screen, int x, int y, int spanX, int spanY) {
		Log.d(TAG, "addInCurrentScreen..x="+x+",y="+y+",spanX="+spanX+",spanY="+spanY+",screen="+screen);
	...
	void addInScreen(View child, int screen, int x, int y, int spanX,
		...
		Log.d(TAG, "addInCurrentScreen..x="+x+",y="+y+",spanX="+spanX+",spanY="+spanY+",screen="+screen+",insert="+insert);
	...
	private void updateWallpaperOffset() {
		Log.d(TAG, "updateWallpaperOffset");
		updateWallpaperOffset(getChildAt(getChildCount() - 1).getRight()
				- (mRight - mLeft));
	}

	private void updateWallpaperOffset(int scrollRange) {
		Log.d(TAG, "updateWallpaperOffset scrollRange="+scrollRange);
		IBinder token = getWindowToken();
		if (token != null) {
			mWallpaperManager.setWallpaperOffsetSteps(
					1.0f / (getChildCount() - 1), 0);
			mWallpaperManager.setWallpaperOffsets(getWindowToken(), Math.max(
					0.f, Math.min(mScrollX / (float) scrollRange, 1.f)), 0);
		}
	}
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		if (!mSensemode) { // wisky@wanglijuan20110620
			...
			// Context.MODE_PRIVATE文件存储方式下写入的内容会覆盖源文件的内容
			SharedPreferences sharedPreferences = getContext().getSharedPreferences(
					"status", Context.MODE_PRIVATE);
			// 获取编辑器
			SharedPreferences.Editor editor = sharedPreferences.edit();
			editor.putInt("status", mStatus);
			editor.putBoolean("isAnimating", isAnimating);
			editor.commit();
		}
		// end wisky@wanglijuan20110620}
		if (restore) {
	...
	protected void onLayout(boolean changed, int left, int top, int right,
		...
		//wisky@wanglijuan20110624 转屏后恢复缩略图模式
    	if(mStatus==3){
    		Log.d(TAG, "onLayout mStatus="+mStatus);
		 openSense(true);
		 invalidate();
    	}
    	//end wisky@wanglijuan20110624
	}
	...
	public boolean requestChildRectangleOnScreen(View child, Rect rectangle,
		...
		Log.d(TAG, "requestChildRectangleOnScreen screen="+screen);
		return false;
	}
	...
	protected boolean onRequestFocusInDescendants(int direction,
		...
		Log.d(TAG, "onRequestFocusInDescendants direction="+direction);
	...
	public void addFocusables(ArrayList<View> views, int direction,
			int focusableMode) {
		Log.d(TAG, "dispatchUnhandledMove views.size="+views.size()+",direction="+direction+",focusableMode="+focusableMode);
		if (!mLauncher.isAllAppsVisible()) {
			final Folder openFolder = getOpenFolder();
			if (openFolder == null) {
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		...
		if (ev.getAction() == MotionEvent.ACTION_DOWN) {
			if (mLauncher.isAllAppsVisible()) {
				Log.d(TAG,"dispatchTouchEvent isAllAppsVisible");
				return false;
			}
		}
	...
	// wisky@wanglijuan20110624 preview {
	private void findClickedPreview(float x, float y) {
		Log.d(TAG, "findClickedPreview x="+x+",y="+y);
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE: {
			 ...
			 * the down event.
			 */
			if(touchtype==SCROLL){ //wisky@wanglijuan20110621
			...
			//wisky@wanglijuan20110621
			}else{
				PointF ptk11 = new PointF();
			    PointF ptk22 = new PointF();
			    ptk11.x = ev.getX(0);
	       	    ptk11.y = ev.getY(0);
	       	    ptk22.x = ev.getX(1);
	       	    ptk22.y = ev.getY(1);
	       	 
				if((ptk11.x-ptk1.x)-(ptk22.x-ptk2.x)>10 || (ptk11.y-ptk1.y)-(ptk22.y-ptk2.y)>10){
					openSense(true);
					invalidate();
					touchtype = SCROLL;
	       	 }
...
//       	    Log.d("xx","ptk1.x="+ptk1.x+"ptk1.y="+ptk1.y);
			break;
			//end wisky@wanglijuan20110621
		}
	...
	private void onSecondaryPointerUp(MotionEvent ev) {
		...
		Log.d(TAG, "onSecondaryPointerUp..pointerIndex="+pointerIndex+",pointerId="+pointerId);
	}
	...
	void enableChildrenCache(int fromScreen, int toScreen) {
		...
		final int count = getChildCount();
		Log.d(TAG, "enableChildrenCache fromScreen:"+fromScreen+",toScreen:"+toScreen);
	...
	void clearChildrenCache() {
		final int count = getChildCount();
		Log.d(TAG, "clearChildrenCache count:"+count);
		for (int i = 0; i < count; i++) {
			final CellLayout layout = (CellLayout) getChildAt(i);
	...
	private void acquireVelocityTrackerAndAddMovement(MotionEvent ev) {
		Log.d(TAG, "acquireVelocityTrackerAndAddMovement");
		if (mVelocityTracker == null) {
	...
	private void snapToScreen(int whichScreen, int velocity, boolean settle) {
		...
		Log.d(TAG, "snapToScreen whichScreen="+whichScreen+",velocity="+velocity+",settle="+settle);
		...
		final int screenDelta = Math.max(1, Math.abs(whichScreen
				- mCurrentScreen));
	...
	void addApplicationShortcut(ShortcutInfo info, CellLayout.CellInfo cellInfo) {
		Log.d(TAG, "addApplicationShortcut");
	...
	void addApplicationShortcut(ShortcutInfo info,
		...
		Log.d(TAG, "addApplicationShortcut insertAtFirst="+insertAtFirst);
	...
	public void openSense(boolean open) {
		...
		Log.i(TAG,"openSense.distro_set:"+distro_set+",numRows="+numRows);
		...
		Log.i(TAG,"openSense.maxItemsPerRow:"+maxItemsPerRow);
		int maxPreviewHeight = ((getMeasuredHeight() - 70) / numRows);
		float w = (getMeasuredWidth() - 70) / maxItemsPerRow;
		int maxPreviewWidth = (int) w;
		Log.i(TAG,"maxPreviewHeight="+maxPreviewHeight+"maxPreviewWidth="+maxPreviewWidth);
		float scaleW = ((float) maxPreviewWidth / (float) getWidth());
		float scaleH = ((float) maxPreviewHeight / (float) getHeight());
		Log.i(TAG,"openSense.scaleW:"+scaleW+",scaleH:"+scaleH);
	...
	protected boolean drawChild(Canvas canvas, View child, long drawingTime) {
		Log.d(TAG, "drawChild drawingTime="+drawingTime);
	...
	private int getDropScreen(int x, int y) {
		int index = 0;
		for (int i = 0; i < getChildCount(); i++) {
			RectF tmp = getScaledChild(getChildAt(i));
			if (tmp.contains(x + getScrollX(), y + getScrollY())) {
	...
	public void onDrop(DragSource source, int x, int y, int xOffset,
		...
		Log.d(TAG, "onDrop...");
		final CellLayout cellLayout = getCurrentDropLayout(x, y); // wisky@wanglijuan20110621
		if (source != this) {
	 ...
	 */
	private CellLayout getCurrentDropLayout(int x, int y) {
		// wisky@wanglijuan20110621
		int index;
		if (!mSensemode) {
			index = mScroller.isFinished() ? mCurrentScreen : mNextScreen;
		} else {
			int mtempScreen = mCurrentScreen;
			mtempScreen = getDropScreen(x, y);
			int mtempnextScreen = mtempScreen + 1;
			if (mtempnextScreen >= getChildCount())
				mtempnextScreen = getChildCount() - 1;
			index = mScroller.isFinished() ? mtempScreen : mtempnextScreen;
		}
		// end wisky@wanglijuan20110621
	...
	public boolean acceptDrop(DragSource source, int x, int y, int xOffset,
			int yOffset, DragView dragView, Object dragInfo) {
		final CellLayout layout = getCurrentDropLayout(x, y); // wisky@wanglijuan20110621
