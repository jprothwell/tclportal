...
<head>
	...
	<TITLE><%=LangPage.langPage.getPageTitle("recommend")%></TITLE>
...
//			if(id != "" && id != "0") {
			...
			if(id != "" && id != "0"){
				if (document.all["tr"+id].style.display == ""){
					...
					sendG("../GetSubRFolder.do?ParentId="+id,getMenuResult);
				}
			}else{
				sendG("../GetSubRFolder.do?ParentId=0",getMenuResult);
			}
		}
...
<!-----------------添加应用类别------------------------------------>
		function showNew(){
			...
			newversionwindow.OpenURL('upload/newRecommend.jsp?ParentId='+cur_typecode);
		}
<!------------------------------------------------------------->
...
<!---------------------修改产品类别----------------------------->
		function showUpdate(){
			if(cur_typecode==""  || cur_typecode == "0"){
				...
				var a = new Date();
				updateversionwindow.OpenURL('upload/updateRecommend.jsp?FolderId='+cur_typecode+'&randomid='+a.getTime());
			}
		}
<!------------------------------------------------------------->
		...
		function deleteObj(){
			if(cur_typecode=="" || cur_typecode == "0"){
				...
				if(confirm("<%=LangPage.langPage.getDelete_confirm()%>")){
					sendG("../DeleteRFolder.do?FolderId="+cur_typecode,deleteResult);
				}
			}
		}
...
<!----------------读取指定类别的产品----------------------->
		function showBooks(){
			listPageURL.put('applist','../GetRFolderAppList.do?FolderId='+cur_typecode+'&index=#index#');
			openPage('applist','1');
		}
<!------------------------------------------------------------->
...
//			if(cur_typecode == "" || cur_typecode=="0"){
			...
			sendG("../AddAppToRFolder.do?FolderId="+cur_typecode+"&AppId="+sel_appid,newResult22);
			sel_appid = "";
		}
...
//			if(cur_typecode == "" || cur_typecode == "0"){
			if(cur_typecode == ""){
				...
				if(confirm("<%=LangPage.langPage.getDelete_confirm()%>")){
					sendG("../DeleteAppRFolder.do?RecId="+listCurrentKey.get('applist'),deleteResult1);
				}
			}
		}
...
<!------------------------------------------------------------->
		...
		function moveUp(){
			if(listCurrentKey.get('applist')==""){
				...
				sendG("../UpAppRFolder.do?RecId="+listCurrentKey.get('applist'),renovateResult);
			}
		}
		...
		function moveDown(){
			if(listCurrentKey.get('applist')==""){
				...
				sendG("../DownAppRFolder.do?RecId="+listCurrentKey.get('applist'),renovateResult);
			}
		}
		...
		function typeUp(){
			if(cur_typecode==""){
				...
				sendG("../UpRFolder.do?FolderId="+cur_typecode,renovateTypeResult);
			}
		}
		...
		function typeDown(){
			if(cur_typecode==""){
				...
				sendG("../DownRFolder.do?FolderId="+cur_typecode,renovateTypeResult);
			}
		}
...
<!------------------页面初始化---------------------->
		...
		function initList(){
			...
			listPageURL.put('applist','../GetRFolderAppList.do?FolderId=0&index=#index#');
...
<body onload="init();">
	...
	<tr>
		<td class="td_pagetitle" valign="middle"><%=LangPage.langPage.getPageTitle("recommend")%></td>
	</tr>
	</table>
