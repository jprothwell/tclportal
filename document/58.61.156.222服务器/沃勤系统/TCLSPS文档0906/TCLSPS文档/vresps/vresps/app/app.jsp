...
<!---------------添加一个新应用程序（游戏）------------------------->
		function showNew(){
			if(cur_cpid == ""){
				...
				newappwindow.Create('new_app_window','<%=LangPage.langPage.getWindowsTitle("app_addapp")%>',600,379,'');
				newappwindow.Show();
...
<!-------------------修改应用程序信息-------------------------->
		function showUpdate(){
			if(listCurrentKey.get('applist')==""){
				...
				updateappwindow.Create('update_app_window','<%=LangPage.langPage.getWindowsTitle("app_updateapp")%>',600,359,'');
