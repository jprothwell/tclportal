...
<%
	...
	else if(strEdit.equals("1"))
	{
				...
				if(updateSucc){
					...
					if(flowId != null){
						...
						if(oldApp.getInfo(appId)){
							...
							app.setValue("FreeFlag",oldApp.getValue("FreeFlag"));
							app.update();
						}
		...
		freeFlag=app.getValue("FreeFlag");
	}
%>
<html>
...
<!------------------页面初始化---------------------->
		...
		function submintForm(){
			if(NewForm.UploadFile.value == "" || NewForm.UploadFile.value == null){
				alert("<%=LangAction.langAction.getNoselect_uploadfile()%>");
			}else{
				NewForm.submit();
			}
		}
				...
				<form method="POST" action="updateApp.jsp?AppId=<%=appId%>&edit=1" name="NewForm" ENCTYPE="multipart/form-data">
					...
					<tr>
						...
						<td class="td_detailcontent0"><input class="input_edit" onkeydown="enterUnsigned();" style="width:100%;" type="hidden" name="TopPrice" value="<%=topPrice%>">
							收费<input type="radio" name="freeFlag" value = "0" 
							<%if("0".equals(freeFlag)){ %>
								checked="checked"
							<%}%> >&nbsp;&nbsp;&nbsp;&nbsp;
						          免费<input type="radio" name="freeFlag" value = "1" 
					...
					<tr>
						<td colspan="4" align="center" valign="middle" class="td_editbutton">
							<a href="#" onclick="submintForm();"><img src="../../<%=LangPage.langPage.getButton_img_path()%>submit_001.gif" border="0" align="top" vspace="1" /></a>
						</td>
					</tr>
				</form>
				</table>
