...
<head>
	...
	<TITLE><%=LangPage.langPage.getPageTitle("appblack")%></TITLE>
...
<!---------------添加一组产品到类别------------------------->
		function newSubmit22(){
			...
			sendG("../AddAppBlack.do?AppId="+sel_appid,newResult22);
			sel_appid = "";
		}
		...
		function deleteObj1(){
			if(listCurrentKey.get('applist')==""){
				...
				if(confirm("<%=LangPage.langPage.getDelete_confirm()%>")){
					sendG("../DeleteAppBlack.do?AppId="+listCurrentKey.get('applist'),deleteResult1);
				}
			}
		}
...
<!------------------------------------------------------------->
		...
		function showBlackList(){
			listPageURL.put('applist','../GetAppBlack.do?index=#index#');
			openPage('applist','1');
		}
...
<body onload="init();">
	...
	<tr>
		<td class="td_pagetitle" valign="middle"><%=LangPage.langPage.getPageTitle("appblack")%></td>
	</tr>
	</table>
