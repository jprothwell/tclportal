...
<head>
	...
	function showOptions(){
		...
		<!------------------根据所选厂商，更新机型UA下拉框--------------------------->
		function showUA(){
			sendG("../GetUabyMFId.do?MFId=0",getUAResult);
		}
	...
	<tr>
		<td valign="middle" style="font-size:9pt;color:#4796ee;">
			...
			<select name="reporttype" onchange="showOptions();">
				<option value="6"><%=LangPage.langPage.getSearch("search_bymobile")%></option>
				<option value="7" selected><%=LangPage.langPage.getSearch("search_byua")%></option>
			</select>
			</br>
		...
		<span id="useragent" >
			<%=LangPage.langPage.getSearch("search_useragent")%>
			<select name="UserAgent">
				<option value="0"><%=LangPage.langPage.getSelect("status_all")%></option>
			</select>
		</span>	
		<br>
