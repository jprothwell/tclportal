 ...
 *************************************************************************/	
	...
	function initHandleHTML(){
		...
		handleHTML = handleHTML + "<input type='button'  value='设置执行人' onclick='setHandle();' class='button' style='width:80;' onmouseover='this.className=\"buttonover\";'  onmouseout='this.className=\"button\";'>&nbsp;&nbsp;";
		handleHTML = handleHTML + "<input type='button'  value='提交申请' onclick='doApp();' class='button' style='width:80;' onmouseover='this.className=\"buttonover\";'  onmouseout='this.className=\"button\";'>";
	...
	function initSetHandleHTML(){
		...
		sethandleHTML = sethandleHTML + "<td class='td_edittitle' width='80'>步骤编号</td>";
		...
		sethandleHTML = sethandleHTML + "</tr>";
		sethandleHTML = sethandleHTML + "<tr>";
		sethandleHTML = sethandleHTML + "<td class='td_edittitle' width='80'>步骤名称</td>";
		...
		sethandleHTML = sethandleHTML + "</tr>";
		sethandleHTML = sethandleHTML + "<tr>";
		sethandleHTML = sethandleHTML + "<td class='td_edittitle' width='80'>执行人</td>";
		...
		sethandleHTML = sethandleHTML + "<input type='button'  value='确认' onclick='sethandleSubmit();' class='button' onmouseover='this.className=\"buttonover\";'  onmouseout='this.className=\"button\";'>&nbsp;&nbsp;";
		sethandleHTML = sethandleHTML + "<input type='button'  value='取消' onclick='sethandleCancel();' class='button' onmouseover='this.className=\"buttonover\";'  onmouseout='this.className=\"button\";'>";
		sethandleHTML = sethandleHTML + "</td>";
		sethandleHTML = sethandleHTML + "</tr>";
	...
	function initMstaffHTML(){
		...
		mstaffHTML = mstaffHTML + "<input type='button'  value='选择' onclick='doSelect1();' class='button' onmouseover='this.className=\"buttonover\";'  onmouseout='this.className=\"button\";'>";
	...
	function initSubList(){
		...
		listListXSL.put('sublist','../xsl/jwdf_list.xsl');
	...
	function openWFNodeSet(){
		handlewindow.Create('hdl_window','工作流-步骤',350,300,handleHTML);
		handlewindow.Show();
	...
	function setHandle(){
		if(listCurrentKey.get('sublist')==""){
			...
			sethandlewindow.Create('sethandle_window','设置执行人',280,150,sethandleHTML);
	...
	function getSetHandle(cartXML){
		...
		for(i=0;i<fields.length;i++){
			...
			if(name == "步骤编号") SetHandle.SubId.value = value;
			else if(name == "步骤名称") SetHandle.SubName.value = value;
			else if(name == "执行人") SetHandle.HandleStaff.value = value;
		}
	}
	...
	function showMStaff(){
		sel_staffs = "";
		mStaffWindow.Create('mstaff_window','选择执行人',300,250,mstaffHTML);
		mStaffWindow.Show();
		showMStaffList();
	}
