...
function detailResult(cartXML,area){
	...
	cpwindow.Create('detail_window_'+area,'Detail Information',listDetailWidth.get(area),listDetailHeight.get(area),cartXML.transformNode(xsl));
	cpwindow.Show();
}
...
function showSub(area,id,child,parent,name){
	...
	var event = treeSubEvent.get(area);
	if(event != null){
		new event;
	}
}
