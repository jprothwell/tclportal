...
<head>
	...
	<TITLE><%=LangPage.langPage.getPageTitle("wf_edit")%></TITLE>
		...
		function deleteWF(){
			if(listCurrentKey.get('workflowlist')==""){
				...
				if(confirm("<%=LangPage.langPage.getDelete_confirm()%>")){
					sendG("../RepealWorkFlow.do?FlowId="+listCurrentKey.get('workflowlist'),deleteWFResult);
				}
			}
		}
...
<!-------------------显示输入窗口------------------------------->
		...
		function getWFResult(cartXML){
			var xsl = loadXSL("../<%=LangPage.langPage.getXsl_path()%>wf_editinfo.xsl");
			document.all["workflow_new"].innerHTML = cartXML.transformNode(xsl);
			workflowwindow.Show();
		}
<!------------------------------------------------------------->

		function setHandleList(){
			workflowwindow.Hide();
...
<!------------------页面初始化---------------------->
		...
		function initList(){
			...
			listPageURL.put('workflowlist','../GetEditWF.do?index=#index#');
...
<body onload="init();">
	...
	<tr>
		<td class="td_pagetitle" valign="middle"><%=LangPage.langPage.getPageTitle("wf_edit")%></td>
	</tr>
	</table>
	...
	<tr>
		<td class="td_buttonbar" valign="middle">
			...
			<a href="#" onclick="deleteWF();"><img src="../<%=LangPage.langPage.getButton_img_path()%>delete_001.gif" border="0" align="top" vspace="1"/></a>
		</td>
	</tr>
	</table>
