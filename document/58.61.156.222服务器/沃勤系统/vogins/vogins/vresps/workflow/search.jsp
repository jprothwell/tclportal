...
<title><%=LangPage.langPage.getPageTitle("wf_search")%></title>
...
<!------------------搜索---------------------->
		function doSearch(){
			if(document.all["KeyWord"].value == ""){
				listPageURL.put('workflowlist','../FinishedWF.do?index=#index#');
				openPage('workflowlist','1');
			}else{
				listPageURL.put('workflowlist','../SearchWF.do?KeyWord='+encodeURIComponent(document.all['KeyWord'].value)+'&index=#index#');
				openPage('workflowlist','1');
			}
		}
<!------------------------------------------------>
...
<body onload="init();">
	...
	<tr>
		<td class="td_pagetitle" valign="middle"><%=LangPage.langPage.getPageTitle("wf_search")%></td>
	</tr>
	</table>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <td align="right"><table width="300" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td nowrap="nowrap" style="font-size:9pt;">输入关键字：</td>
        <td><input name="KeyWord" type="text" size="20" /></td>
        <td>
        	<a href="#" onclick="doSearch();"><img src="../<%=LangPage.langPage.getButton_img_path()%>search_003.gif"  border="0" align="top" vspace="1" /></a>
        </td>
      </tr>
    </table></td>
  </tr>
