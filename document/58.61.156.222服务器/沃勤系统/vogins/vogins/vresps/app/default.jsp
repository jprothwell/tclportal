...
<%
	DefaultInfo def=new DefaultInfo();
%>
<html>

<head>
	<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
	<TITLE><%=LangPage.langPage.getPageTitle("default")%></TITLE>
...
<!--------------------添加总表---------------------->
		...
		function newSubmit(){
			if(NewDefault.DefName.value == ""){
				alert("<%=LangPage.langPage.getPlease_input()%><%=def.getAliasByFiled("DefName")%>");
				NewDefault.DefName.focus();
				return false;
			}
			sendP("../AddDefault.do",getP(NewDefault),newResult);
		}
		function newResult(){
			newwindow.Close();
...
<!----------------修改总表------------------------>
		...
		function showUpdate(){
			if(listCurrentKey.get('deflist')==""){
				alert("<%=LangAction.langAction.getNoselect_default_applist()%>");
				return;
			}else{
				updatewindow.Create('update_window','<%=LangPage.langPage.getWindowsTitle("app_updatetable")%>',350,123,updateHTML);
				updatewindow.Show();
				sendG("../GetDefaultInfo.do?DefId="+listCurrentKey.get('deflist'),getUpdate);
			}
		}
		...
		function getUpdate(cartXML){
			...
			for(i=0;i<fields.length;i++){
				...
				if(name == "<%=def.getAliasByFiled("DefId")%>") UpdateDefault.DefId.value = value;
				if(name == "<%=def.getAliasByFiled("DefName")%>") UpdateDefault.DefName.value = value;
				if(name == "<%=def.getAliasByFiled("DefDesc")%>") UpdateDefault.DefDesc.value = value;
			}
		}
		...
		function updateSubmit(){
			...
			sendP("../UpdateDefault.do",getP(UpdateDefault),updateResult);
		}
<!------------------------------------------------------------->
		...
		function deleteObj(){
			if(listCurrentKey.get('deflist')==""){
				...
				if(confirm("<%=LangPage.langPage.getDelete_confirm()%>")){
					sendG("../DeleteDefault.do?DefId="+listCurrentKey.get('deflist'),deleteResult);
				}
			}
		}
...
<!------------------------------------------------------------->
		...
		function downloadFile(){
			if(listCurrentKey.get('deflist')==""){
				...
				window.open("../DefaultDownload.do?DefId="+listCurrentKey.get('deflist'),"DownloadFrame");
			}
		}
...
<!---------------控制显示或关闭总表配置窗口------------------------------>
		...
		function showFolderApp(){
			listPageURL.put('applist','../GetDefaultAppList.do?DefId='+cur_defid+'&FolderId='+treeCurrentId.get('foldertree')+'&index=#index#');
			openPage('applist','1');
		}
<!------------------------------------------------------------->
<!---------------添加一组产品到类别------------------------->
		function newSubmit22(){
			...
			sendG("../AddDefaultApp.do?DefId="+cur_defid+"&FolderId="+treeCurrentId.get('foldertree')+"&AppId="+sel_appid,newResult22);
			sel_appid = "";
		}
...
<!---------------从类别中删除产品------------------------>
		function deleteObj1(){
			if(treeCurrentId.get('foldertree') == "" || treeCurrentId.get('foldertree') == "0"){
				...
				if(confirm("<%=LangPage.langPage.getDelete_confirm()%>")){
					sendG("../DeleteDefaultApp.do?RecId="+listCurrentKey.get('applist'),deleteResult1);
				}
			}
		}
...
<!------------------------------------------------------------->
		...
		function moveUp(){
			if(listCurrentKey.get('applist')==""){
				...
				sendG("../UpDefaultApp.do?RecId="+listCurrentKey.get('applist'),renovateResult);
			}
		}
		...
		function moveDown(){
			if(listCurrentKey.get('applist')==""){
				...
				sendG("../DownDefaultApp.do?RecId="+listCurrentKey.get('applist'),renovateResult);
			}
		}
...
<!----------------显示产品版本----------------------->
		...
		function showVersion(){
			if(listCurrentKey.get('applist') == ''){
				...
				sendG("../GetDefaultVersion.do?RecId="+listCurrentKey.get('applist'),detailResult1);
...
<!--------------添加或删除版本--------------------------->
		function setAppVersion(verid){
			if(document.all["chk"+verid].checked){
				sendG("../AddDefaultVersion.do?RecId="+listCurrentKey.get('applist')+"&VerId="+verid,setVersionResult);
			}else{
				sendG("../DeleteDefaultVer.do?RecId="+listCurrentKey.get('applist')+"&VerId="+verid,setVersionResult);
			}
		}
...
<!------------------页面初始化---------------------->
		...
		function initList(){
			...
			listPageURL.put('deflist','../GetDefaultList.do?index=#index#');
			listDetailURL.put('deflist','../GetDefaultInfo.do?DefId=#id#');
			listCurrentId.put('deflist','');
			listCurrentKey.put('deflist','');
			listDetailWidth.put('deflist','300');
		...
		function initTree(){
			...
			treeSubURL.put('foldertree','../GetSubFolder.do?ParentId=#id#');
...
<body onload="init();">
	...
	<tr>
		<td class="td_pagetitle" valign="middle"><%=LangPage.langPage.getPageTitle("default")%></td>
	</tr>
	</table>
	...
	<div id="tb_new" class="datainfo">
		...
		<tr>
			<td class="td_edittitle" width="25%"><%=def.getAliasByFiled("DefName")%></td>
			<td class="td_editcontent" width="75%"><input class="input_edit" style="width:100%;" maxlength="20" type="text" name="DefName"></td>
		</tr>
		<tr>
			<td class="td_edittitle"><%=def.getAliasByFiled("DefDesc")%></td>
