...
<%
	...
	else if(strEdit.equals("1"))
	{
		...
		if(packVersion == null) packVersion = "AS2.0";
		...
		if(ver.getInfo(appstoreId)){
			...
			if(flow.checkExist("29", paras) || flow.checkExist("30", paras)){
				...
				if(!myContentFile.isMissing())
				{
					...
					ver.setValue("PublishType",Integer.toString(AppStore.PUBLISH_TYPE_TEST));
	...
	<table id="tb_new" class="table_edit" cellpadding="0" cellspacing="0" align="center">
		<tr>
			<td>
				...
				<form method="POST" action="updateAppStore.jsp?AppStoreId=<%=appstoreId%>&edit=1" name="NewForm" ENCTYPE="multipart/form-data">
					...
					<tr>
						...
						<td class="td_detailcontent0">
							<select name="PackVersion">
								<option<%if(packVersion.equals("AS2.0")){%> selected<%}%> value="AS2.0">2.0</option>
								<option<%if(packVersion.equals("AS3.0")){%> selected<%}%> value="AS3.0">3.0</option>
							</select>
						</td>
					</tr>
					<tr>
