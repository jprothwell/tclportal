...
<!---------------添加一个应用程序版本------------------------->
		function showNew(){
			...
			newversionwindow.OpenURL('upload/newAppStore2.jsp');
		}
<!------------------------------------------------------------->
...
<!-------------------修改应用程序信息-------------------------->
		function showUpdate(){
			if(listCurrentKey.get('applist')==""){
				...
				var a = new Date();
				updateversionwindow.OpenURL('upload/updateAppStore2.jsp?AppStoreId='+listCurrentKey.get('applist')+'&randomid='+a.getTime());
			}
		}
<!------------------------------------------------------------->
...
<!------------------页面初始化---------------------->
		...
		function initList(){
			...
			listPageURL.put('applist','../GetAppStore2List.do?index=#index#&publishType=1');
