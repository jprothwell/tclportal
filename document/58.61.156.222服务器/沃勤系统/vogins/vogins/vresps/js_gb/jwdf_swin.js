...
function SimpleWindow(){
	...
	function Create(name,title,width,height,html){
		...
		if(!simple_windows.containsKey("window_"+temp)){
			...
			strWindow = strWindow + "<td id='title3_" + this.name + "' class='swindowtitle_td' valign='middle' align='center' width='14'><div class='swindowtitle_div2' onclick=\"win_close('" + this.name + "','" + this.action + "');\"></div></td>";
			strWindow = strWindow + "</tr>";
			strWindow = strWindow + "<tr>";
	...
	function Hide(){
		if(this.name != ""){
			document.all["window_" + this.name].style.display = "none";
		}
	}
	...
	function Close(){
		win_close(this.name,this.action);
	}
}

/*--------------窗口关闭按钮处理 Start------------------*/
function win_close(winname,action){
	if(action == "1"){
		...
		document.all["window_" + winname].style.display = "none";
	}
}
/*--------------窗口关闭按钮处理 End------------------*/
