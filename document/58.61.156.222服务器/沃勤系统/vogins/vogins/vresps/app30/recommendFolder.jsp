...
<head>
	...
	<TITLE><%=LangPage.langPage.getPageTitle("recommendfolder")%></TITLE>
		...
		function showsubmenu(id){
			...
			if(id != "" && id != "0"){
				if (document.all["tr"+id].style.display == ""){
					...
					sendG("../GetSubRDFolder.do?ParentId="+id,getMenuResult);
				}
			}else{
				sendG("../GetSubRDFolder.do?ParentId=0",getMenuResult);
			}
		}
...
<!-----------------添加产品类别------------------------------------>
		function showNew(){
			...
			newversionwindow.OpenURL('upload/newRecommendDef.jsp?ParentId='+cur_typecode);
		}
<!------------------------------------------------------------->
...
<!---------------------修改产品类别----------------------------->
		function showUpdate(){
			if(cur_typecode==""  || cur_typecode == "0"){
				...
				var a = new Date();
				updateversionwindow.OpenURL('upload/updateRecommendDef.jsp?FolderId='+cur_typecode+'&randomid='+a.getTime());
			}
		}
<!------------------------------------------------------------->
		...
		function deleteObj(){
			if(cur_typecode=="" || cur_typecode == "0"){
				...
				if(confirm("<%=LangPage.langPage.getDelete_confirm()%>")){
					sendG("../DeleteRDFolder.do?FolderId="+cur_typecode,deleteResult);
				}
			}
		}
...
<!--------------------------上移/下移产品目录----------------------------------->
		...
		function typeUp(){
			if(cur_typecode==""){
				...
				sendG("../UpRDFolder.do?FolderId="+cur_typecode,renovateTypeResult);
			}
		}
		...
		function typeDown(){
			if(cur_typecode==""){
				...
				sendG("../DownRDFolder.do?FolderId="+cur_typecode,renovateTypeResult);
			}
		}
...
<body onload="init();">
	...
	<tr>
		<td class="td_pagetitle" valign="middle"><%=LangPage.langPage.getPageTitle("recommendfolder")%></td>
	</tr>
	</table>
