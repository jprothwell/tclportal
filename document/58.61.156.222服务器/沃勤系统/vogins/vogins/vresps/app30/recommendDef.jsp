...
<%@ page import="com.vogins.sps.db.app30.RecommendInfo"%>
<%
	RecommendInfo rec = new RecommendInfo();
%>
<html>

<head>
	<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
	<TITLE><%=LangPage.langPage.getPageTitle("recommenddef")%></TITLE>
...
<!--------------------添加总表---------------------->
		...
		function newSubmit(){
			if(NewDefault.DefName.value == ""){
				alert("<%=LangPage.langPage.getPlease_input()%><%=rec.getAliasByFiled("RecommendName")%>");
				NewDefault.DefName.focus();
				return false;
			}
			sendP("../AddRecDefault.do",getP(NewDefault),newResult);
		}
		function newResult(){
			newwindow.Close();
...
<!----------------修改总表------------------------>
		...
		function showUpdate(){
			if(listCurrentKey.get('deflist')==""){
				...
				updatewindow.Show();
				sendG("../GetRecDefInfo.do?DefId="+listCurrentKey.get('deflist'),getUpdate);
			}
		}
		...
		function getUpdate(cartXML){
			...
			for(i=0;i<fields.length;i++){
				...
				if(name == "<%=rec.getAliasByFiled("RecommendId")%>") UpdateDefault.DefId.value = value;
				if(name == "<%=rec.getAliasByFiled("RecommendName")%>") UpdateDefault.DefName.value = value;
				if(name == "<%=rec.getAliasByFiled("RecommendDesc")%>") UpdateDefault.DefDesc.value = value;
			}
		}
		...
		function updateSubmit(){
			if(UpdateDefault.DefName.value == ""){
				alert("<%=LangPage.langPage.getPlease_input()%><%=rec.getAliasByFiled("DefName")%>");
				UpdateDefault.DefName.focus();
				return false;
			}
			sendP("../UpdateRecDef.do",getP(UpdateDefault),updateResult);
		}
<!------------------------------------------------------------->
		...
		function deleteObj(){
			if(listCurrentKey.get('deflist')==""){
				...
				if(confirm("<%=LangPage.langPage.getDelete_confirm()%>")){
					sendG("../DeleteRecDef.do?DefId="+listCurrentKey.get('deflist'),deleteResult);
				}
			}
		}
...
<!--------------------------下载总表----------------------------------->
		...
		function downloadFile(){
			if(listCurrentKey.get('deflist')==""){
				...
				window.open("../RecDefDownload.do?DefId="+listCurrentKey.get('deflist'),"DownloadFrame");
			}
		}
...
<!---------------控制显示或关闭总表配置窗口------------------------------>
		...
		function showFolderApp(){
			listPageURL.put('applist','../GetRecDefAppList.do?DefId='+cur_defid+'&FolderId='+treeCurrentId.get('foldertree')+'&index=#index#');
			openPage('applist','1');
		}
<!---------------添加一组产品到类别------------------------->
		function newSubmit22(){
			...
			sendG("../AddRecDefApp.do?DefId="+cur_defid+"&FolderId="+treeCurrentId.get('foldertree')+"&AppId="+sel_appid,newResult22);
			sel_appid = "";
		}
...
<!---------------从类别中删除产品------------------------>
		function deleteObj1(){
			if(treeCurrentId.get('foldertree') == "" || treeCurrentId.get('foldertree') == "0"){
				...
				if(confirm("<%=LangPage.langPage.getDelete_confirm()%>")){
					sendG("../DeleteRecDefApp.do?RecId="+listCurrentKey.get('applist'),deleteResult1);
				}
			}
		}
...
<!--------------------------------上移/下移产品----------------------------->
		...
		function moveUp(){
			if(listCurrentKey.get('applist')==""){
				...
				sendG("../UpRecDefApp.do?RecId="+listCurrentKey.get('applist'),renovateResult);
			}
		}
		...
		function moveDown(){
			if(listCurrentKey.get('applist')==""){
				...
				sendG("../DownRecDefApp.do?RecId="+listCurrentKey.get('applist'),renovateResult);
			}
		}
...
<!----------------显示产品版本----------------------->
		...
		function showVersion(){
			if(listCurrentKey.get('applist') == ''){
				...
				sendG("../GetRecDefVersion.do?RecId="+listCurrentKey.get('applist'),detailResult1);
...
<!--------------添加或删除版本--------------------------->
		function setAppVersion(verid){
			if(document.all["chk"+verid].checked){
				sendG("../AddRecDefVersion.do?RecId="+listCurrentKey.get('applist')+"&VerId="+verid,setVersionResult);
			}else{
				sendG("../DeleteRecDefVer.do?RecId="+listCurrentKey.get('applist')+"&VerId="+verid,setVersionResult);
			}
		}
...
<!------------------页面初始化---------------------->
		...
		function initList(){
			...
			listPageURL.put('deflist','../GetRecDefList.do?index=#index#');
			listDetailURL.put('deflist','../GetRecDefInfo.do?DefId=#id#');
		...
		function initTree(){
			...
			treeSubURL.put('foldertree','../GetSubRDFolder.do?ParentId=#id#');
...
<body onload="init();">
	...
	<tr>
		<td class="td_pagetitle" valign="middle"><%=LangPage.langPage.getPageTitle("recommenddef")%></td>
	</tr>
	</table>
	...
	<tr>
		<td class="td_buttonbar" valign="middle">
			...
			<a href="#" onclick="location.href='recommendFolder.jsp'"><img src="../<%=LangPage.langPage.getButton_img_path()%>listmanage_001.gif"   border="0" align="top" vspace="1"/></a>
		</td>
	</tr>
	</table>
	...
	<div id="tb_new" class="datainfo">
		...
		<tr>
			<td class="td_edittitle" width="25%"><%=rec.getAliasByFiled("RecommendName")%></td>
			<td class="td_editcontent" width="75%"><input class="input_edit" style="width:100%;" maxlength="20" type="text" name="DefName"></td>
		</tr>
		<tr>
			<td class="td_edittitle"><%=rec.getAliasByFiled("RecommendDesc")%></td>
