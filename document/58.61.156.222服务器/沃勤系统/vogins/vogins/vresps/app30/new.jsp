...
<head>
	...
	<TITLE><%=LangPage.langPage.getPageTitle("new")%></TITLE>
		...
		function showsubmenu(id){
			...
			if(id != "" && id != "0"){
				if (document.all["tr"+id].style.display == ""){
					...
					sendG("../GetSubNFolder.do?ParentId="+id,getMenuResult);
				}
			}else{
				sendG("../GetSubNFolder.do?ParentId=0",getMenuResult);
			}
		}
...
<!-----------------添加应用类别------------------------------------>
		function showNew(){
			...
			newversionwindow.OpenURL('upload/newNew.jsp?ParentId='+cur_typecode);
		}
<!------------------------------------------------------------->
...
<!---------------------修改产品类别----------------------------->
		function showUpdate(){
			if(cur_typecode==""){
				...
				var a = new Date();
				updateversionwindow.OpenURL('upload/updateNew.jsp?FolderId='+cur_typecode+'&randomid='+a.getTime());
			}
		}
<!------------------------------------------------------------->
		...
		function deleteObj(){
			if(cur_typecode=="" || cur_typecode == "0"){
				...
				if(confirm("<%=LangPage.langPage.getDelete_confirm()%>")){
					sendG("../DeleteNFolder.do?FolderId="+cur_typecode,deleteResult);
				}
			}
		}
...
<!----------------读取指定类别的产品----------------------->
		function showBooks(){
			listPageURL.put('applist','../GetNFolderAppList.do?FolderId='+cur_typecode+'&index=#index#');
			openPage('applist','1');
		}
<!------------------------------------------------------------->
		...
		function typeUp(){
			if(cur_typecode==""){
				...
				sendG("../UpNFolder.do?FolderId="+cur_typecode,renovateTypeResult);
			}
		}
		...
		function typeDown(){
			if(cur_typecode==""){
				...
				sendG("../DownNFolder.do?FolderId="+cur_typecode,renovateTypeResult);
			}
		}
...
<!------------------页面初始化---------------------->
		...
		function initList(){
			...
			listDetailURL.put('applist','../GetAppNFolderInfo.do?RecId=#id#');
...
<body onload="init();">
	...
	<tr>
		<td class="td_pagetitle" valign="middle"><%=LangPage.langPage.getPageTitle("new")%></td>
	</tr>
	</table>
