...
<head>
	...
	<TITLE><%=LangPage.langPage.getPageTitle("more")%></TITLE>
		...
		function showsubmenu(id){
			...
			if(id != "" && id != "0"){
				if (document.all["tr"+id].style.display == ""){
					...
					sendG("../GetSubMFolder.do?ParentId="+id,getMenuResult);
				}
			}else{
				sendG("../GetSubMFolder.do?ParentId=0",getMenuResult);
			}
		}
...
<!-----------------添加应用类别------------------------------------>
		function showNew(){
			...
			newversionwindow.OpenURL('upload/newMore.jsp?ParentId='+cur_typecode);
		}
<!------------------------------------------------------------->
...
<!---------------------修改产品类别----------------------------->
		function showUpdate(){
			if(cur_typecode==""){
				...
				var a = new Date();
				updateversionwindow.OpenURL('upload/updateMore.jsp?FolderId='+cur_typecode+'&randomid='+a.getTime());
			}
		}
<!------------------------------------------------------------->
		...
		function deleteObj(){
			if(cur_typecode=="" || cur_typecode == "0"){
				...
				if(confirm("<%=LangPage.langPage.getDelete_confirm()%>")){
					sendG("../DeleteMFolder.do?FolderId="+cur_typecode,deleteResult);
				}
			}
		}
...
<!----------------读取指定类别的产品----------------------->
		function showBooks(){
			listPageURL.put('applist','../GetMFolderAppList.do?FolderId='+cur_typecode+'&index=#index#');
			openPage('applist','1');
		}
<!------------------------------------------------------------->
...
<!---------------添加一组产品到类别------------------------->
		function newSubmit22(){
			...
			sendG("../AddAppToMFolder.do?FolderId="+cur_typecode+"&AppId="+sel_appid,newResult22);
			sel_appid = "";
		}
...
<!---------------从类别中删除产品------------------------>
		function deleteObj1(){
			if(cur_typecode == "" || cur_typecode == "0"){
				...
				if(confirm("<%=LangPage.langPage.getDelete_confirm()%>")){
					sendG("../DeleteAppMFolder.do?RecId="+listCurrentKey.get('applist'),deleteResult1);
				}
			}
		}
...
<!------------------------------------------------------------->
		...
		function moveUp(){
			if(listCurrentKey.get('applist')==""){
				...
				sendG("../UpAppMFolder.do?RecId="+listCurrentKey.get('applist'),renovateResult);
			}
		}
		...
		function moveDown(){
			if(listCurrentKey.get('applist')==""){
				...
				sendG("../DownAppMFolder.do?RecId="+listCurrentKey.get('applist'),renovateResult);
			}
		}
		...
		function typeUp(){
			if(cur_typecode==""){
				...
				sendG("../UpMFolder.do?FolderId="+cur_typecode,renovateTypeResult);
			}
		}
		...
		function typeDown(){
			if(cur_typecode==""){
				...
				sendG("../DownMFolder.do?FolderId="+cur_typecode,renovateTypeResult);
			}
		}
...
<!------------------页面初始化---------------------->
		...
		function initList(){
			...
			listDetailURL.put('applist','../GetAppMFolderInfo.do?RecId=#id#');
...
<body onload="init();">
	...
	<tr>
		<td class="td_pagetitle" valign="middle"><%=LangPage.langPage.getPageTitle("more")%></td>
	</tr>
	</table>
