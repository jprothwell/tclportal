	...
	if(strEdit.equals("1"))
	{
		...
		if(folderName.equals("")){
			...
			if(!myContentFile.isMissing())
			{
				...
				if(level<=3){
					NewFolder subs[] = more.getSubFolder(parentId);
					if(subs.length<20){
...
<head>
<TITLE><%=LangPage.langPage.getPageTitle("newNew")%></TITLE>
	...
	<table id="tb_new" class="table_edit" cellpadding="0" cellspacing="0" align="center">
		<tr>
			<td>
				...
				<form method="POST" action="newNew.jsp?ParentId=<%=parentId%>&edit=1" name="NewForm" ENCTYPE="multipart/form-data">
					<tr>
						<td height="20" colspan="1" bgcolor="#8acef5"><b><%=LangPage.langPage.getPageTitle("newNew")%></b></td>
