...
#else
   kal_uint8 cheLastBlock[16];  //sw des and aes
   kal_uint8 cheLastReminder[16];//sw des and aes   
   kal_uint8 cheKeySchedule[128]; //des
   kal_uint8 iv[64]; //des
#endif   
}DES_CNXT;
...
//used for MD5,SHA1, MAC_MD5, MAC_SHA1, MD4, DHM
...
/*RC4_KEY: x, y: the swap index; data: keystream*/
typedef struct RC4_CNXT_T
{
   kal_uint32 x,y;
   kal_uint8 keyStream[256];
} RC4_CNXT;
...
typedef struct STCHE_T 
{
   ...
   kal_uint8 *cheWorkBuf; // pointe to a space used by each alog
   kal_uint32 cheBufLen; //depend on the encrypt alog
...
kal_bool che_duplicate(STCHE *src_che_context, STCHE *dst_che_context);
