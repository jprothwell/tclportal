...
void EntryPhnsetWallpaperSysView(void)
{
...

    {
        ...
        if (GetActiveScreenId() == SCR_WALLPAPER_VIEW)
        {
...
void PhnsetSubWallpaperForwardFromFmgr(void *path, FMGR_FILTER *filter, int is_short)
{
...
#ifdef JPG_DECODE
    {
        ...
        {
            if (FMGR_FILTER_IS_SET(filter, FMGR_TYPE_JPG) || FMGR_FILTER_IS_SET(filter, FMGR_TYPE_JPEG))
            {
				mmi_display_popup((UI_string_type) GetString(STR_GLOBAL_UNSUPPORTED_FORMAT), MMI_EVENT_FAILURE);
                return;
            }
...
void PhnsetWallpaperSelectFmgrCallBack(void *path, int is_short)
{
    ...
    {
		if (IsScreenPresent(g_phnset_cntx_p->ScrAnchor))
		{
...
void DispIdleScrWPBadFilePopup(void)
{
    ...
    if (error_str_ptr)
    {
		mmi_display_popup((UI_string_type) error_str_ptr, MMI_EVENT_FAILURE);
    }
    else
    {
		mmi_display_popup((UI_string_type) GetString(STR_IDLE_WP_DECODE_ERR), MMI_EVENT_FAILURE);
    }
    error_str_ptr = NULL;
...
void mmi_phnset_copy_wpss_check_device(void)
{
    ...
    else
    {
		...
		mmi_display_popup((UI_string_type) GetString(FMGR_FS_DISK_FULL_TEXT), MMI_EVENT_FAILURE);
...
void mmi_phnset_copy_wpss_entry_waiting_entry(void)
{
    ...
    EntryNewScreen(SCR_IDLE_SCR_DISP_WPSS_WAITING, NULL, mmi_phnset_copy_wpss_entry_waiting_entry, NULL);
...
void mmi_phnset_copy_wpss_rsp(void *info)
{
    ...
    else
    {
        ...
        {
			mmi_display_popup((UI_string_type) GetString(GetFileSystemErrorString(msgPtr->result)), MMI_EVENT_FAILURE);
        }
...
void mmi_phnset_del_dirty_file(void)
{
        ...
        && (mmi_ucs2cmp((PS8)phnset_wpss_default_file_1.wallpaper, (PS8)L"") != 0))
    {
        ...
        if (wp_fh >= 0)
        {
            ...
            FS_Close(wp_fh);
            temp_int = mmi_phnset_del_file((const unsigned short *) phnset_wpss_default_file_1.wallpaper);
        ...
        && (mmi_ucs2cmp((PS8)phnset_wpss_default_file_2.wallpaper, (PS8)L"") != 0))
    {
        ...
        if (wp_fh >= 0)
        {
            ...
            FS_Close(wp_fh);
            temp_int2 = mmi_phnset_del_file((const unsigned short *) phnset_wpss_default_file_2.wallpaper);
        ...
        && (mmi_ucs2cmp((PS8)phnset_wpss_default_file_1.screensave, (PS8)L"") != 0))
    {
        ...
        if (ss_fh >= 0)
        {
            ...
            temp_int = mmi_phnset_del_file((const unsigned short *) phnset_wpss_default_file_1.screensave);
        ...
        && (mmi_ucs2cmp((PS8)phnset_wpss_default_file_2.screensave, (PS8)L"") != 0))
    {
        ...
        if (ss_fh >= 0)
        {
            ...
            temp_int2 = mmi_phnset_del_file((const unsigned short *) phnset_wpss_default_file_2.screensave);
...
void mmi_phnset_init_folder(void)
{
    ...
    if (drive > 0)
    {
        ...
        memset(path_ascii, 0x00, 64);
        sprintf(path_ascii, "%c:\\%s", (S8)drive, FMGR_DEFAULT_FOLDER_SETTINGS);
        MMI_TRACE(MMI_COMMON_TRC_G7_SETTING, MMI_WALLPAPER_TRACE_051, path_ascii);
        mmi_asc_to_ucs2(path, path_ascii);
