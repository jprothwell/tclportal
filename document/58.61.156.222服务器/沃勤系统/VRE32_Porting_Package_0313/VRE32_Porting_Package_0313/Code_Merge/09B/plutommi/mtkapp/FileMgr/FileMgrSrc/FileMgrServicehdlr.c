...
const S32 g_fmgr_image_viewer_limit_arry[] = 
{
    ...
    IMG_M3D_LIMIT_ITEM(200, 0, 0),
#elif defined(MT6238) || defined(MT6239) || defined(MT6268) || defined(MT6268A)
    IMG_JPG_LIMIT_ITEM(12000, 4095, 4095),
    IMG_WBMP_LIMIT_ITEM(1536, 2048, 2048),
    IMG_BMP_LIMIT_ITEM(25000, 0, 0),
    IMG_GIF_LIMIT_ITEM(0, 2300, 2300),
    IMG_PNG_LIMIT_ITEM(0, 2047, 4096),
    IMG_SVG_LIMIT_ITEM(125, 0, 0),
    IMG_M3D_LIMIT_ITEM(200, 0, 0),
#elif defined(MT6253) || defined(MT6253T)
    IMG_JPG_LIMIT_ITEM(1000, 0, 0),
    IMG_WBMP_LIMIT_ITEM(1536, 2048, 2048),
    IMG_BMP_LIMIT_ITEM(1500, 0, 0),
    IMG_GIF_LIMIT_ITEM(0, 1200, 1200),
    IMG_PNG_LIMIT_ITEM(0, 700, 700),
    IMG_SVG_LIMIT_ITEM(125, 0, 0),
    IMG_M3D_LIMIT_ITEM(200, 0, 0),
#else
...
const S32 g_fmgr_vt_img_limit_arry[] = 
{
#if defined(MT6225)
    IMG_BMP_LIMIT_ITEM(0, UI_DEVICE_WIDTH, UI_DEVICE_HEIGHT),
    IMG_JPG_LIMIT_ITEM(256, 0, 0),
...
static void mmi_fmgr_to_phb_hdlr(void)
{
    ...
    if (FMGR_FILTER_IS_SET(&info.file_type, FMGR_TYPE_VCF))
    {
        ...
        if (mmi_syncml_is_phb_sync_now())
        {
            ...
            mmi_display_popup((UI_string_type)GetString(STR_ID_SYNC_CODE_FORBIDDEN), MMI_EVENT_FAILURE);
            return;
        }
    #endif
...
/* UDX_SUPPORT */
#ifdef __MMI_UDX_SUPPORT__
static void mmi_fmgr_to_udx_hdlr(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    {
        mmi_udx_filemgr_parse_file(filepath);
    }
    mmi_fmgr_close_option_screen();
}

void mmi_fmgr_highlight_to_udx(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    SetKeyHandler(mmi_fmgr_to_udx_hdlr, KEY_RIGHT_ARROW, KEY_EVENT_DOWN);
    SetLeftSoftkeyFunction(mmi_fmgr_to_udx_hdlr, KEY_EVENT_UP);
#ifdef __MMI_TOUCH_SCREEN__
    wgui_register_list_item_selected_callback(mmi_fmgr_to_udx_hdlr);
#endif 
}
#endif
/* UDX_SUPPORT */
...
#ifdef __SYNCML_SUPPORT__
    if (FMGR_FILTER_IS_SET(&info.file_type, FMGR_TYPE_VCS) && !mmi_vclndr_can_recv(filepath))
    {
        //DisplayPopup((U8*) GetString(STR_ID_SYNC_CODE_FORBIDDEN), IMG_GLOBAL_INFO, 0, 1000, WARNING_TONE);
        mmi_display_popup((UI_string_type)GetString(STR_ID_SYNC_CODE_FORBIDDEN), MMI_EVENT_FAILURE);
        return;
...
void mmi_fmgr_option_vcalendar_enabler(U16 menu_id, const FMGR_FILE_INFO_STRUCT* fileinfo)
{
#ifdef __MMI_VCALENDAR__
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    ...
    vType = mmi_vclndr_uti_get_vtype(filepath);
    
    if (vType == VCLNDR_TYPE_EVENT)
    {
        mmi_frm_hide_menu_item(MENU_ID_FMGR_FWD_TODOLIST);
        mmi_frm_unhide_menu_item(MENU_ID_FMGR_FWD_CLNDR);
    }
    else if (vType == VCLNDR_TYPE_TODO)
    {
        mmi_frm_hide_menu_item(MENU_ID_FMGR_FWD_CLNDR);
        mmi_frm_unhide_menu_item(MENU_ID_FMGR_FWD_TODOLIST);     
    } 
#endif  
}
...
static void mmi_fmgr_to_ems_hdlr(void)
{
    ...
    else
    {
        ...
        mmi_display_popup((UI_string_type)GetString(STR_FMGR_INVALID_EMS_FORMAT), MMI_EVENT_FAILURE);
    }
    mmi_fmgr_close_option_screen();        
}
...
/* under construction !*/
    ...
    type = mmi_fmgr_get_filetype_by_name((PS8) mmi_fmgr_get_current_fileinfo(NULL,NULL));
    can_insert = mmi_umms_check_mms_insert_postcard(type,mmi_fmgr_get_current_fileinfo(NULL,NULL));
    if(!can_insert)
    {
        mmi_frm_hide_menu_item(MENU_ID_UMMS_FMGR_SEND_TO_POSTCARD);
    }
...
#ifdef __MMI_MMS_POSTCARD__
static void mmi_fmgr_to_mms_postcard_hndlr(void)
{
    mms_content_entry_struct data = {0};

    data.file_path = (S8*)OslMalloc((FMGR_MAX_PATH_LEN + 1) *ENCODING_LENGTH);
    memcpy(data.file_path, mmi_fmgr_get_current_fileinfo(NULL,NULL),((FMGR_MAX_PATH_LEN + 1) *ENCODING_LENGTH));
...
void mmi_fmgr_highlight_to_postcard(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    SetKeyHandler(mmi_fmgr_to_mms_postcard_hndlr, KEY_RIGHT_ARROW, KEY_EVENT_DOWN);
    SetLeftSoftkeyFunction(mmi_fmgr_to_mms_postcard_hndlr, KEY_EVENT_UP);
#ifdef __MMI_TOUCH_SCREEN__
    wgui_register_list_item_selected_callback(mmi_fmgr_to_mms_postcard_hndlr);
#endif 
}
#endif
#endif
...
void mmi_fmgr_forward_peripheral_enabler(U16 menu_id, const FMGR_FILE_INFO_STRUCT* fileinfo)
{
    ...
    if(menu_id == MENU_ID_FMGR_FWD_BT || menu_id == MENU_ID_FMGR_MARK_OPTION_TO_BT)
    {
...
void mmi_fmgr_option_audio_enabler(U16 menu_id, const FMGR_FILE_INFO_STRUCT* fileinfo)
{
    ...
    if (!FMGR_FILTER_IS_SET(&fileinfo->file_type, FMGR_TYPE_IMY))
    {
        mmi_frm_hide_menu_item(MENU_ID_FMGR_AUD_OPTION_EDIT);
    }
...
void mmi_fmgr_option_image_enabler(U16 menu_id, const FMGR_FILE_INFO_STRUCT* fileinfo)
{
     ...
     */
    if(is_drm ||(!FMGR_FILTER_IS_SET(&fileinfo->file_type, FMGR_TYPE_PNG)&&(!FMGR_FILTER_IS_SET(&fileinfo->file_type, FMGR_TYPE_JPG) && !FMGR_FILTER_IS_SET(&fileinfo->file_type, FMGR_TYPE_JPEG))))
    {
...
static void mmi_fmgr_image_view_hdlr(void)
{
    ...
    mmi_ivex_view_image_hdlr(filepath, file_info, app_id);
    mmi_fmgr_close_option_screen();        
    return;
#endif
...
static void mmi_fmgr_image_entry_jpg_detail(void)
{
    ...
    if(FS_GetDevStatus(g_fmgr_image_jpg_detail_filepath[0], FS_MOUNT_STATE_ENUM) != FS_NO_ERROR)
    {
        OslMfree(g_fmgr_image_jpg_detail_filepath);
        g_fmgr_image_jpg_detail_filepath = NULL;
        GoBackHistory();
...
static void mmi_fmgr_image_entry_print_option(void)
{
    ...
    if(handle < 0)
    {
        ...
        mmi_display_popup((UI_string_type)GetString(GetFileSystemErrorString(handle)), MMI_EVENT_FAILURE);
        goto print_error;
    }
    ...
    if (nNumofItem == 0)
    {
        //DisplayPopup((U8*) GetString(STR_GLOBAL_UNSUPPORTED_FORMAT), IMG_GLOBAL_WARNING, 0, 1000, WARNING_TONE);
...
static void mmi_fmgr_audio_play_hdlr(void)
{
    ...
    mmi_media_app_play_audio(filepath,NULL,icon_id);
    mmi_fmgr_close_option_screen();
}
...
static void mmi_fmgr_audio_edit_hdlr(void)
{
    ...
    filepath = mmi_fmgr_get_current_fileinfo(&file_info, &app_id);
    mmi_fmgr_get_app_title_info(app_id, NULL, &icon_id, filepath);
    mmi_rngc_fmgr_edit_imy(file_info.file_name, filepath);
#ifdef __DRM_SUPPORT__
    if (DRM_METHOD_NONE != DRM_get_object_method(0, (PU16)filepath))
    {
        mmi_display_popup((UI_string_type) GetString(STR_GLOBAL_DRM_PROHIBITED), MMI_EVENT_FAILURE);
        mmi_fmgr_close_option_screen();
        return;
    }
#endif
...
{
#if defined(__MMI_RING_COMPOSER__) && defined(__MMI_IMELODY_SUPPORT__)
    SetKeyHandler(mmi_fmgr_audio_edit_hdlr, KEY_RIGHT_ARROW, KEY_EVENT_DOWN);
    SetLeftSoftkeyFunction(mmi_fmgr_audio_edit_hdlr, KEY_EVENT_UP);
#else /* defined(__MMI_RING_COMPOSER__) && defined(__MMI_IMELODY_SUPPORT__) */ 
    ASSERT(0);    
#endif /* defined(__MMI_RING_COMPOSER__) && defined(__MMI_IMELODY_SUPPORT__) */ 
}
...
static void mmi_fmgr_video_play_hdlr(void)
{
    ...
    {
        mmi_media_app_play_stream_from_sdp_file(filepath, file_info.is_short);
    }
    else if (FMGR_FILTER_IS_SET(&file_info.file_type, FMGR_TYPE_RAM))
    {
        mmi_media_app_play_stream_from_ram_file(filepath, file_info.is_short);
    }
    else /* other video type */
    {
        mmi_media_app_play_video(filepath,file_info.is_short);
    }
#else
...
void mmi_fmgr_highlight_java_package(void)
{
    SetKeyHandler(mmi_java_entry_send_screen_from_fgmr, KEY_RIGHT_ARROW, KEY_EVENT_DOWN);
    SetLeftSoftkeyFunction(mmi_java_entry_send_screen_from_fgmr, KEY_EVENT_UP);
#ifdef __MMI_TOUCH_SCREEN__
    wgui_register_list_item_selected_callback(mmi_java_entry_send_screen_from_fgmr);
#endif 
}
#endif    
...
static void mmi_fmgr_ebook_open_hdlr(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/      
    mmi_ebr_open_book_from_file_manager();
}
...
BOOL mmi_fmgr_audio_player_notify_hdlr(U32 notify_flag, void* para)
{
    ...
    mmi_fmgr_notify_unmount_info_struct     *unmount_info;
    mmi_fmgr_notify_string_struct           *str_path;
    mmi_fmgr_notify_delete_all_info_struct  *del_all_info;
    ...
    case MMI_FMGR_NOTIFY_SET_STORAGE_PRE_CHK:
        return mmi_audply_plst_notify_default_storage_change_pre_check(*((S8*)para));
    case MMI_FMGR_NOTIFY_SET_STORAGE:
        mmi_audply_plst_notify_default_storage_change(*((S8*)para));
        break;
    ...
    case MMI_FMGR_NOTIFY_DRV_POST_MOUNT:
        unmount_info = (mmi_fmgr_notify_unmount_info_struct*)para;
        mmi_audply_msdc_plug_in_hdlr(unmount_info->drv_num, unmount_info->drv_list);
        break;
        
    case MMI_FMGR_NOTIFY_DRV_POST_UNMOUNT:
        unmount_info = (mmi_fmgr_notify_unmount_info_struct*)para;
        mmi_audply_msdc_plug_out_hdlr(unmount_info->drv_num, unmount_info->drv_list);
        break;
       
    case MMI_FMGR_NOTIFY_PRE_DELETE:
    case MMI_FMGR_NOTIFY_PRE_MOVE:
        str_path = (mmi_fmgr_notify_string_struct*)para;
        if(mmi_audply_check_file_is_playing(str_path->path, str_path->is_folder, str_path->is_short, MMI_FALSE))
        {
            return FALSE;
        }
        break;

    case MMI_FMGR_NOTIFY_PRE_DEL_ALL:
        del_all_info = (mmi_fmgr_notify_delete_all_info_struct *)para;
        if (!FMGR_FILTER_IS_SET_AUDIO(&del_all_info->filter))
        {/* avoid: delete all image files. need not check*/
            return TRUE;
        }
        if(mmi_audply_check_file_is_playing(del_all_info->folder_path.path, 
                                            del_all_info->folder_path.is_folder,
                                            del_all_info->folder_path.is_short,
                                            MMI_TRUE))
        {
            return FALSE;
        }
    ...
    case MMI_FMGR_NOTIFY_POST_DELETE:
        ...
        {
            PS8 last_backlash = NULL;
            MMI_BOOL trg_folder = MMI_FALSE;
            MMI_BOOL src_folder = MMI_FALSE;
            
            last_backlash = mmi_ucs2rchr((const PS8)paths->trg_path.path, (U16)L'\\');
            MMI_ASSERT(last_backlash != NULL);
            
            if (2 > mmi_ucs2strlen((const PS8)last_backlash))
            {
                trg_folder = MMI_TRUE;
            }

            last_backlash = mmi_ucs2rchr((const PS8)paths->src_path.path, (U16)L'\\');
            MMI_ASSERT(last_backlash != NULL);
        ...
        else
		{
            ...
            mmi_audply_update_list_hdlr(paths->src_path.path, NULL, (U8)action, paths->src_path.is_folder);
		}
        break;
...
#if defined(__MMI_RING_COMPOSER__) && defined(__MMI_IMELODY_SUPPORT__)
BOOL mmi_rngc_notify_hdlr(U32 notify_flag, void* para)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
BOOL mmi_fmgr_wap_notify_hdlr(U32 notify_flag, void* para)
{
    ...
    /*----------------------------------------------------------------*/
#ifdef OBIGO_Q03C
    extern void wap_memory_card_plug_out_hdlr (void);
#endif
...
BOOL mmi_fmgr_ebook_notify_hdlr(U32 notify_flag, void* para)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    return mmi_ebr_fmgr_notify_hdlr(notify_flag, para);
}
#endif
...
BOOL mmi_fmgr_langln_notify_hdlr(U32 notify_flag, void* para)
{
    ...
    #ifdef __MMI_FMGR_SUPPORT_DEFAULT_STORAGE__
        case MMI_FMGR_NOTIFY_SET_STORAGE_PRE_CHK:
    		return mmi_langln_change_default_storage_pre_check(*((S8*)para));
    	case MMI_FMGR_NOTIFY_SET_STORAGE:
            mmi_langln_change_default_storage(*((S8*)para));
        	break;
    #endif
    case MMI_FMGR_NOTIFY_DRV_POST_UNMOUNT:
...
#ifdef __MMI_MEDIA_PLAYER__
BOOL mmi_fmgr_media_player_notify_hdlr(U32 notify_flag, void* para)
{
	/*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    #ifdef __MMI_FMGR_SUPPORT_DEFAULT_STORAGE__
        case MMI_FMGR_NOTIFY_SET_STORAGE_PRE_CHK:
    		return mmi_medply_plst_notify_default_storage_change_pre_check(*((S8*)para));
    	case MMI_FMGR_NOTIFY_SET_STORAGE:
            mmi_medply_plst_notify_default_storage_change(*((S8*)para));
        	break;
    #endif	/* __MMI_FMGR_SUPPORT_DEFAULT_STORAGE__ */
	    case MMI_FMGR_NOTIFY_DRV_PRE_FORMAT:
	        mmi_medply_plst_pre_format_hdlr(*((S8*)para));
	        break;
	               
	    case MMI_FMGR_NOTIFY_DRV_POST_UNMOUNT:
	        unmount_info = (mmi_fmgr_notify_unmount_info_struct*)para;
	        mmi_medply_plst_msdc_plug_out_hdlr(unmount_info->drv_num, unmount_info->drv_list);
	        break;
        case MMI_FMGR_NOTIFY_DRV_POST_MOUNT:
            unmount_info = (mmi_fmgr_notify_unmount_info_struct*)para;
	        mmi_medply_plst_msdc_plug_in_hdlr(unmount_info->drv_num, unmount_info->drv_list);
	        break;

        case MMI_FMGR_NOTIFY_ENTRY_UNMOUNT:
            mmi_medply_plst_msdc_entry_unmount(*((S8*)para));
   	}
...
BOOL mmi_fmgr_myfavorite_notify_hdlr(U32 notify_flag, void* para)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    {
    case MMI_FMGR_NOTIFY_DRV_POST_UNMOUNT:
        mmi_my_favorite_memorycard_plug_out_hdlr();
        break;
    }
    return TRUE;
}
...
#ifdef __MMI_AB_REPEATER__
BOOL mmi_fmgr_abrepeater_notify_hdlr(U32 notify_flag, void* para)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    #ifdef __MMI_FMGR_SUPPORT_DEFAULT_STORAGE__
        case MMI_FMGR_NOTIFY_SET_STORAGE_PRE_CHK:
    		return mmi_abrepeat_change_default_storage_pre_check(*((S8*)para));
    	case MMI_FMGR_NOTIFY_SET_STORAGE:
            mmi_abrepeat_change_default_storage(*((S8*)para));
        	break;
    #endif	/* __MMI_FMGR_SUPPORT_DEFAULT_STORAGE__ */
        case MMI_FMGR_NOTIFY_DRV_POST_UNMOUNT:
            mmi_abrepeat_storage_plug_out_hdlr();
            break;
    }
...
BOOL mmi_images_hyperlink_callback(U16 app_id, S8* path, fmgr_hyperlink_action_enum action, S32 para, S32* result)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
BOOL mmi_audio_hyperlink_callback(U16 app_id, S8* path, fmgr_hyperlink_action_enum action, S32 para, S32* result)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
BOOL mmi_video_hyperlink_callback(U16 app_id, S8* path, fmgr_hyperlink_action_enum action, S32 para, S32* result)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
BOOL mmi_mmsbox_hyperlink_callback(U16 app_id, S8* path, fmgr_hyperlink_action_enum action, S32 para, S32* result)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
BOOL mmi_theme_hyperlink_callback(U16 app_id, S8* path, fmgr_hyperlink_action_enum action, S32 para, S32* result)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
BOOL mmi_others_hyperlink_callback(U16 app_id, S8* path, fmgr_hyperlink_action_enum action, S32 para, S32* result)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
BOOL mmi_java_fmgr_hyperlink_callback(U16 app_id, S8* path, fmgr_hyperlink_action_enum action, S32 para, S32* result)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
BOOL mmi_fmgr_util_file_limit_check(const S32 *array_ptr, PS8 filename, PS8 *error_str_pp)
{
    ...
    *error_str_pp = g_fmgr_file_limit_error_str;
    ...
    if (fs_handle < 0)
    {
        FILE_LIMIT_FALSE_RETURN(mmi_fmgr_util_get_fs_error_string(fs_handle));
    }
	...
	if (FMGR_FILTER_IS_SET(&file_type, FMGR_TYPE_3GP))
        ...
        {
            FILE_LIMIT_FALSE_RETURN(STR_GLOBAL_UNSUPPORTED_FORMAT);
        }
      ...
      /*******************************************/
        if (check_image_width!=0 || check_image_height!=0)
        {
                ...
                ( image_width == 0 || image_height == 0))
            {
                ...
                {
                    FILE_LIMIT_FALSE_RETURN(STR_GLOBAL_NOT_ENOUGH_MEMORY);
                }
                else if (result == GDI_IMAGE_ERR_INVALID_IMG_TYPE ||
					image_width == 0 || 
					image_height == 0)
                {
                    FILE_LIMIT_FALSE_RETURN(STR_GLOBAL_INVALID_FORMAT);
                }
                else
                {
                    FILE_LIMIT_FALSE_RETURN(STR_GLOBAL_UNSUPPORTED_FORMAT);
    ...
    else if (res_group_type == FMGR_GROUP_VIDEO)
    {
        ...
        if (result < 0)
        {
            string_id = mdi_util_get_mdi_error_info(result,&event_id);
            FILE_LIMIT_FALSE_RETURN(string_id);
        }
        else
        {
...
U16 mmi_fmgr_query_custom_icon(BOOL is_large, U16 filetype, U16 grouptype, S8* filepath)
{
    ...
    /*----------------------------------------------------------------*/    
#ifdef __DRM_SUPPORT__
    if (grouptype == FMGR_TYPE_GRP_VIDEO ||
        grouptype == FMGR_TYPE_GRP_AUDIO)
    {
        perm = DRM_PERMISSION_PLAY;
    }
    else if (grouptype == FMGR_TYPE_GRP_IMAGE)
    {
...
static void mmi_fmgr_imps_open_hdlr(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    filepath = mmi_fmgr_get_current_fileinfo(&file_info, &app_id);
    mmi_fmgr_get_app_title_info(app_id, NULL, &icon_id, filepath);
    mmi_frm_open_im_from_file_manager((S8*)filepath, icon_id);
    mmi_fmgr_close_option_screen();  
}


void mmi_fmgr_highlight_im_open(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    SetLeftSoftkeyFunction(mmi_fmgr_imps_open_hdlr, KEY_EVENT_UP);
    SetKeyHandler(mmi_fmgr_imps_open_hdlr, KEY_RIGHT_ARROW, KEY_EVENT_DOWN);
#ifdef __MMI_TOUCH_SCREEN__
    wgui_register_list_item_selected_callback(mmi_fmgr_imps_open_hdlr);
#endif 
}
#endif

#if defined(__MMI_FMGR_MULTI_SELECT_SUPPORT__) && defined(__MMI_BT_SEND_SUPPORT__)

static void mmi_fmgr_bt_multi_send_next(void* hdl, U32 fmgr_id)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    btsend_obj_struct next_obj;
    ...
    memset(&next_obj, 0, sizeof(next_obj));
    next_obj.data.obj_type = BTSEND_USE_FILE;
    next_obj.data.u.file.path = (U16 *)item.filepath;
    next_obj.data.u.file.is_sfn = item.is_native;
    mmi_bt_send_obj(hdl, &next_obj);
    
    mmi_fmgr_multi_sel_unlock_curr(fmgr_id, &item);
}

static void mmi_fmgr_bt_multi_send_notify(U32 status, void* hdl, void* app_data)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    U32 fmgr_id = (U32)app_data;
    ...
    switch(status)
    {
    case BTSEND_STATUS_READY:
        mmi_fmgr_bt_multi_send_next(hdl, fmgr_id);
        break;

    case BTSEND_STATUS_SUCC:
...
void mmi_fmgr_bt_multi_send(U32 fmgr_ms_id)
{
    ...
    /* begin the sending */
    objs.total = mmi_fmgr_multi_sel_get_remain_count(fmgr_ms_id);
    objs.obj = NULL;
    app.appID = APP_FILEMANAGER;
    app.fNotify = mmi_fmgr_bt_multi_send_notify;
    app.userData = (void*)fmgr_ms_id;
#ifdef __MMI_BT_AUTO_PROMPT_SWITCH__    
    mmi_bt_send_begin(&objs, &app, BTSEND_PROF_ALL, BTSEND_OPT_DEFAULT_DEVICE);
#else
    mmi_bt_send_begin(&objs, &app, BTSEND_PROF_ALL, BTSEND_OPT_NORMAL);
#endif
