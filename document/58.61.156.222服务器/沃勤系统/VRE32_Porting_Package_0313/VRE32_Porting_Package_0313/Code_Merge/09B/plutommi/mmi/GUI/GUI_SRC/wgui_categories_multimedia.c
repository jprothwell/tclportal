...
void DrawCateVdoCategoryControlArea(dm_coordinates *coordinate)
{
    ...
    {
        /* resize */
        if (cat_vdoply_is_full_screen || (wgui_video_info.width > content_width) || (wgui_video_info.height > content_height))
        {
    ...
    if (wgui_is_touch_title_bar_buttons())
    {
        ...
        /* set status icon with double bufer */
        {
            ...
            set_status_icon_bar_layer(0, wgui_catvdo_top_layer);
            ...
            wgui_status_icon_set_alpha_layer(wgui_base_layer);
        }
#endif /* defined(GUI_COMMON_SHOW_STATUS_ICON) */
                ...
                void *gui_buffer)
{
   ...
   ********************************************************************************/
    if (!is_fullscreen)
    {
        ...
        /* init title */
        dm_add_title(get_string(title_str), get_image(title_icon));
        wgui_title_set_menu_shortcut_number(-1);
...
/* 073007 abm dialing End */
    ...
    {
        wgui_status_icon_set_alpha_layer(cat_swflash_layer);
...
static MMI_BOOL mmi_cat229_hfp_volume_callback(U8 volume, MMI_BOOL is_mute)
{
    ...
    if (volume < LEVEL7 && is_mute == MMI_FALSE)
    {
