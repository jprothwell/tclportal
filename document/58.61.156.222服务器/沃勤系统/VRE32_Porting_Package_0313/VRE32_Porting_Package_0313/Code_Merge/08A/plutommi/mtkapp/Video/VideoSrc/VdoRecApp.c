...
void mmi_vdorec_draw_panel_delete(void);
        ...
        void (*result_callback)(MMI_BOOL result, S8 *filepath))
{
    ...
    /* video cant record so accurate, -10K for margin  */
    if (g_vdorec_cntx.ext_req_size > 15)
    {
        g_vdorec_cntx.ext_req_size -= 10;
    }
    else
    {
    ...
    if (g_vdorec_cntx.ext_req_size < 10)
    {
...
static void mmi_vdorec_record_result_hdlr_callback(MDI_RESULT result)
{
    ...
    if (g_vdorec_cntx.is_ext_req)
    {
        ...
        {
            mmi_vdorec_disply_popup(
                (PU8)GetString(STR_ID_VDOREC_NOTIFY_EXCEED_APP_LIMIT),
                IMG_GLOBAL_INFO,
                1,
                ST_NOTIFYDURATION,
                WARNING_TONE);
    ...
    switch (state)
    {
        ...
        case VDOREC_STATE_RECORD:
        {
            ...
            mmi_vdorec_set_softkey(
                ...
                IMG_ID_VDOREC_LSK_OPTION,
                IMG_ID_VDOREC_RSK_BACK,
                IMG_ID_VDOREC_CK_STOP);
        #else /* defined(__DIRECT_SENSOR_SUPPORT__) */
            mmi_vdorec_set_softkey(
                (PS8)GetString(STR_GLOBAL_OPTIONS),
            ...
            {
            #ifdef __VDOREC_FEATURE_RECORD_START_SND__
...
/* under construction !*/
        ...
        case VDOREC_STATE_VIDEO_PREVIEW_PLAY:
            ...
            {
                ClearKeyHandler(VDOREC_SWITCH_TO_CAMERA_KEY, KEY_EVENT_UP);
            }
            if (VDOREC_DELETE_KEY != -1)
            {
                SetKeyHandler(mmi_vdorec_del_key_press_hdlr, (U16)VDOREC_DELETE_KEY, KEY_EVENT_DOWN);
...
static void mmi_vdorec_entry_app_internal(void)
{
    ...
    {
        mmi_vdorec_disply_popup(
            (PU8)GetString(STR_ID_VDOREC_NOTIFY_BEARER_IS_ACTIVE),
            IMG_GLOBAL_WARNING,
            1,
            ST_NOTIFYDURATION,
            WARNING_TONE);
...
#ifndef __VDOREC_FEATURE_SHOW_STATUS_BAR__
    entry_full_screen();
#endif
   ...
   ********************************************************************/
   ...
   /************ base layer layer ************/
...
#ifdef __DIRECT_SENSOR_SUPPORT__
    g_vdorec_cntx.preview_wnd_width = IMAGE_SENSOR_PREVIEW_WIDTH;
    g_vdorec_cntx.preview_wnd_height = IMAGE_SENSOR_PREVIEW_HEIGHT;

    g_vdorec_cntx.preview_wnd_offset_x = g_vdorec_osd_cntx.preview_wnd.rect.offset_x;
    g_vdorec_cntx.preview_wnd_offset_x += 
          ((g_vdorec_osd_cntx.preview_wnd.rect.width - IMAGE_SENSOR_PREVIEW_WIDTH) >> 1);        
    
    g_vdorec_cntx.preview_wnd_offset_y = g_vdorec_osd_cntx.preview_wnd.rect.offset_y;
    g_vdorec_cntx.preview_wnd_offset_y += 
        ((g_vdorec_osd_cntx.preview_wnd.rect.height - IMAGE_SENSOR_PREVIEW_HEIGHT) >> 1);            
    ...
    gdi_layer_set_position(g_vdorec_cntx.preview_wnd_offset_x, g_vdorec_cntx.preview_wnd_offset_y);
...
#ifdef __VDOREC_FEATURE_SHOW_STATUS_BAR__
    wgui_status_icon_set_display_layer(g_vdorec_cntx.bg_region_0_layer_handle);
    show_status_icon_bar(0);
#endif
...
#ifdef __VDOREC_FEATURE_SHOW_STATUS_BAR__
    wgui_status_icon_set_display_layer(g_vdorec_cntx.bg_layer_handle);
    show_status_icon_bar(0);
#endif    
...
static void mmi_vdorec_exit_app(void)
{
    ...
    mmi_frm_scrmem_free(g_vdorec_cntx.bg_region_0_layer_buf_ptr);
    mmi_frm_scrmem_free(g_vdorec_cntx.bg_region_1_layer_buf_ptr);
    g_vdorec_cntx.bg_region_0_layer_buf_ptr = NULL;
    g_vdorec_cntx.bg_region_1_layer_buf_ptr = NULL;    
#else /* __VDOREC_FEATURE_MULTI_REGION_LAYER_BG__ */
...
static void mmi_vdorec_init_ui(void)
{
    ...
    g_vdorec_cntx.bg_region_0_layer_buf_ptr = (PU8) mmi_frm_scrmem_alloc(region_0_buf_size);
    ...
    gdi_layer_create_using_outside_memory(
        ...
        rect_ptr->height,        
        &g_vdorec_cntx.bg_region_0_layer_handle,   
        (PU8)g_vdorec_cntx.bg_region_0_layer_buf_ptr,
        (S32)region_0_buf_size);
    ...
    g_vdorec_cntx.bg_region_1_layer_buf_ptr = (PU8) mmi_frm_scrmem_alloc(region_1_buf_size);
    ...
    gdi_layer_create_using_outside_memory(
        ...
        rect_ptr->height,        
        &g_vdorec_cntx.bg_region_1_layer_handle,   
        (PU8)g_vdorec_cntx.bg_region_1_layer_buf_ptr,
        (S32)region_1_buf_size);
...
do {                                                                        \
    ...
    gdi_image_get_dimension_id(IMG_ID_VDOREC_DEL, &touch_p->width, &touch_p->height);
#endif /* __VDOREC_FEATURE_PREVIEW_SAVED_FILE__ */
    ...
    #ifdef __VDOREC_FEATURE_PREVIEW_SAVED_FILE__
    ASSIGN_DRAW_LAYER(panel_ptr->zoom.offset_x, panel_ptr->zoom.offset_y, g_vdorec_cntx.del_draw_layer);    
    #endif    
    ...
    #ifdef __VDOREC_FEATURE_PREVIEW_SAVED_FILE__
    g_vdorec_cntx.del_draw_layer = g_vdorec_cntx.bg_layer_handle;
    #endif
...
#if (defined(__MMI_VDOREC_MMS_SUPPORT__) && !defined(__VDOREC_FEATURE_DIRECT_SAVE__))
    /* support link to MMS */
...
static void mmi_vdorec_entry_option_screen(void)
{
    ...
    GetSequenceImageIds_Ext(MENU_ID_VDOREC_OPTION, ItemIcons);
...
static void mmi_vdorec_entry_set_storage_screen_internal(void)
{
    ...
    gui_buffer = GetCurrGuiBuffer(SCR_ID_VDOREC_STORAGE);
    ...
    ShowCategory353Screen(
        (PU8)GetString(STR_GLOBAL_STORAGE),
        GetRootTitleIcon(MENU_ID_VDOREC_APP),
        STR_GLOBAL_OK,
        IMG_GLOBAL_OK,
        STR_GLOBAL_BACK,
...
void mmi_vdorec_delete_saved_vdo_hdlr(void)
{
    ...
    {
        mmi_vdorec_disply_popup((PU8)GetString(STR_GLOBAL_DELETED), IMG_GLOBAL_ACTIVATED, 1, ST_NOTIFYDURATION, SUCCESS_TONE);
    }
    else
    {
        mmi_vdorec_disply_popup(
            ...
            IMG_GLOBAL_WARNING,
            1,
            ST_NOTIFYDURATION, WARNING_TONE);
    }
...
void mmi_vdorec_delete_saved_vdo_confirm(void)
{
    ...
    DisplayConfirmRotated(
       ...
       STR_GLOBAL_NO,
       IMG_GLOBAL_NO,
       get_string(STR_ID_VDOREC_NOTIFY_DELETE_CONFIRM),
       IMG_GLOBAL_QUESTION,
       WARNING_TONE,
       MMI_FRM_SCREEN_ROTATE_270);
#else /* defined(__VDOREC_FEATURE_HORIZONTAL_VIEW__) && defined(__MMI_SCREEN_ROTATE__) */ 
...
void mmi_vdorec_entry_preview_option_screen(void)
{
    ...
    /* bluetooth */
#ifdef __MMI_BT_SEND_SUPPORT__
    if (mmi_bootup_get_active_flight_mode() || g_vdorec_cntx.is_ext_req)
    {
    ...
    GetSequenceImageIds_Ext(MENU_ID_VDOREC_PREVIEW_OPTION, ItemIcons);
...
void mmi_vdorec_entry_preview_video_screen(void)
{
   ...
   ********************************************************************/
    ...
    GpioCtrlReq(MMIAPI_DEV_CTRL_GET);                       /* stop LED patten */
    gdi_layer_clear(GDI_COLOR_BLACK);
...
//    mdi_video_ply_open_clip_file(g_vdorec_cntx.filepath, &info);
...
void mmi_vdorec_draw_panel_delete(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    gdi_layer_push_and_set_active(g_vdorec_cntx.del_draw_layer);
    gdi_layer_get_position(&layer_offset_x, &layer_offset_y);
