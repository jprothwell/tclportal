...
#ifdef __REL5__
S8 gSimEccNumList[MAX_ECC_NUMBER_LIST_LENGTH + 16][(EMERGENCY_NUMBER_LENGTH + 1)];
#else
...
#ifdef __MMI_DUAL_SIM_MASTER__
#ifdef __REL5__
S8 gSimEccNumList_2[MAX_ECC_NUMBER_LIST_LENGTH + 16][(EMERGENCY_NUMBER_LENGTH + 1)];
#else
...
#ifdef __REL5__
S8 gSimEccNameList[MAX_ECC_NUMBER_LIST_LENGTH][(MAX_PB_NAME_LENGTH + 1) * ENCODING_LENGTH];
U8 gSimEccCategoryList[MAX_ECC_NUMBER_LIST_LENGTH + 16];
#ifdef __MMI_DUAL_SIM_MASTER__
S8 gSimEccNameList_2[MAX_ECC_NUMBER_LIST_LENGTH][(MAX_PB_NAME_LENGTH + 1) * ENCODING_LENGTH];
U8 gSimEccCategoryList_2[MAX_ECC_NUMBER_LIST_LENGTH + 16];
#endif
U8 gSimEccIndex = 0;
#endif /* __REL5__ */
...
U16 EmergencyNumber[ ][5] = 
{
    {0x0031, 0x0031, 0x0032, 0x0000, 1}, /* 112 */
    {0x0039, 0x0031, 0x0031, 0x0000, 1}, /* 911 */
#ifdef __MEXICO_ECC__
    {0x0030, 0x0038, 0x0030, 0x0000, 0}, /* 080 */
    {0x0030, 0x0036, 0x0030, 0x0000, 0}, /* 060 */
...
#ifdef __REL5__
    SetProtocolEventHandler(NwECCInfoSave, MSG_ID_MMI_NW_UPDATE_ECC_IND);
#endif
...
#ifdef __REL5__
    SetSlaveProtocolEventHandler(MTPNP_PFAL_NW_ECCInfoSave, MSG_ID_MMI_NW_UPDATE_ECC_IND);
#endif
...
#ifdef __REL5__
    SetProtocolEventHandler(MTPNP_PFAL_NW_ECCInfoSave, MSG_ID_MMI_NW_UPDATE_ECC_IND);
#endif
...
void mmi_sim_error_hide_menu_items(void)
{
    ...
    mmi_frm_hide_menu_item(MENU_ID_SECSET_ACL_MAIN);
#endif
...
#endif
    mmi_frm_hide_menu_item(MAIN_MENU_CALL_HISTORY);
#endif /* __MMI_VOIP__ */
 ...
 *  void
 *****************************************************************************/
void mmi_sim_error_process_pending_events(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
void InitSimErrorScreen(void)
{
    ...
    else if (IsScreenPresent(IDLE_SCREEN_ID))
    {
        DisplayPopup((U8*) GetString(simErrorDisplayString), IMG_GLOBAL_ERROR, 0, 2000, (U8) ERROR_TONE);
    #ifdef __MMI_DUAL_SIM_MASTER__
        HistoryReplace(IDLE_SCREEN_ID, SCR_SIM_INSERTION, mmi_idle_entry_dual_sim_root_screen);
    #else
        HistoryReplace(IDLE_SCREEN_ID, SCR_SIM_INSERTION, EntryScrnSimErrorScreen);
    #endif
 ...
 * DESCRIPTION
 *  Send verify phone lock request to MOD_L4C_2
 * PARAMETERS
 *  void
 * RETURNS
 *  void
...
/*****************************************************************************
 * FUNCTION
 *  mmi_sim_error_popup_flight_mode
 * DESCRIPTION
 *  This function displays warning pop-up when the user wants to dial SOS in
 *  flight mode. This screen needs to handle manually keypad lock.
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void mmi_sim_error_popup_flight_mode(void)
{
#ifdef __FLIGHT_MODE_SUPPORT__
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
...
void EntryScrnSimErrorScreen(void)
{
    ...
    else
    {
        if (g_keylock_context.gKeyPadLockFlag == 0) /* keypad is not locked */
        {
            ...
            {
                SetRightSoftkeyFunction(mmi_sim_error_popup_flight_mode, KEY_EVENT_UP);
            }
            else
            {
...
/* under construction !*/
    ...
    #ifndef __MMI_SIM_OPTION_MENU_NO_MULTIMEDIA__
        IMG_ID_OPTION_MENU_MULTIMEDIA_ICON,
    #endif
    #if defined(__MMI_FILE_MANAGER__) && !defined(__MMI_OP11_MENU_12MATRIX__)
        IMG_ID_OPTION_MENU_FILEMNGR_ICON,
    #endif
    #ifndef __MMI_SIM_OPTION_MENU_NO_GAME__
        IMG_ID_OPTION_MENU_FUNANDGAMES_ICON,
    #endif
        IMG_ID_OPTION_MENU_PROFILES_ICON,
        IMG_ID_OPTION_MENU_ORGANIZER_ICON,
    #ifndef __MMI_SIM_OPTION_MENU_NO_EXTRA__
        ...
        IMG_GLOBAL_L10, 
        IMG_GLOBAL_L11,
        IMG_GLOBAL_L12  /* max 12 items in SIM option menu */
...
/* under construction !*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
#if defined(__MMI_BI_DEGREE_MAIN_MENU_STYLE__) && defined(__MMI_VERSION_2__) && !defined(__MMI_SERVICE_MENU_NUMERIC_LIST_ICON__)
    if (GetChildMenuIDIndexByParentMenuID_Ext(MENU_SIM_OPTION_ID, MAIN_MENU_OPTION_AVZONE_MENUID) == -1)
    {
        U8 i;
        for (i = 0; i < MAX_SUB_MENUS - 1; i++)
        {
            MenuOptionIconList[i] = MenuOptionIconList[i + 1];
        }
    }
#endif /* defined(__MMI_BI_DEGREE_MAIN_MENU_STYLE__) && defined(__MMI_VERSION_2__) */
    ...
    nNumofItem = GetNumOfChild_Ext(MENU_SIM_OPTION_ID);

    nDispAttribute = GetDispAttributeOfItem(MENU_SIM_OPTION_ID);

    GetSequenceStringIds_Ext(MENU_SIM_OPTION_ID, nStrItemList);
...
void EntryScrEnterPIN(void)
{
    ...
    SetKeyHandler(mmi_sim_cancel_pin_lock, KEY_END, KEY_EVENT_DOWN);
#endif
}
...
#endif
    {
        /* Reset dialpad buffer if the user inputs * to start SSC. */
        g_idle_context.DialPadCallBuffer[0] = 0;
        g_idle_context.DialPadCallBuffer[1] = 0;
    }
...
void EntryScrEnterNewPINFirstTime(void)
{
	...
	if (mmi_ucs2strlen(gInputNewPinBufferFirstTime) != 0)
	{    
		gui_buffer = GetCurrGuiBuffer(SCR_ENTER_NEW_PIN);
	}
...
void EntryScrEnterNewPINSecondTime(void)
{
	...
	if (mmi_ucs2strlen(gInputNewPinBufferSecondTime) != 0)
	{
    	gui_buffer = GetCurrGuiBuffer(SCR_ENTER_CONFIRM_PIN);
    }
...
void ValidateFirstAndSecondPin(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (!mmi_ucs2cmp(gInputNewPinBufferFirstTime, gInputNewPinBufferSecondTime))
    {
        ValidatePUK();
    }
    else
...
void EntryScrPinDoNotMatch(void)
{
    ...
    DeleteNScrId(SCR_ENTER_NEW_PIN);
    GenericExitScreen(SCR_ENTER_NEW_PIN, EntryScrEnterNewPINFirstTime);
}
...
void HandleInsertSIMScrDigitEntry(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    /* Reset dialpad buffer before the user enter dialer screen. */
    g_idle_context.DialPadCallBuffer[0] = 0;
    g_idle_context.DialPadCallBuffer[1] = 0;
...
void DialSOSNumber(void)
{
    ...
    {
    #ifdef __MMI_DUAL_SIM_MASTER__
        mmi_idle_set_dual_sim_dialing_keypad_call_handler(EntryScrEmergencyCallDialing, EntryScrEmergencyCallDialing);
    #else
...
void EntryScrEmergencyCallDialing(void)
{
    ...
    {
        g_pwr_context.ForCMToIdentifySOSCall = MMI_TRUE;
...
#endif
{
    ...
    {
        /* compare with SIM ECC and NW ECC first */
    #ifdef __MMI_DUAL_SIM_MASTER__
        ...
        for (i = 0; i < EmergencyNumber[i][0] != 0x00; i++)
        {
 ...
 *  void
 *****************************************************************************/
void mmi_ecc_prepare_number(U8 *bcd_ecc, U8 *digit_ecc)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    *(digit_ecc + 0) = (*(bcd_ecc + 0) & 0x0F) == 0x0F ? 0 : (U8) (0x30 + (*(bcd_ecc + 0) & 0x0F));
    *(digit_ecc + 1) = (*(bcd_ecc + 0) & 0xF0) == 0xF0 ? 0 : (U8) (0x30 + ((*(bcd_ecc + 0) & 0xF0) >> 4));
    *(digit_ecc + 2) = (*(bcd_ecc + 1) & 0x0F) == 0x0F ? 0 : (U8) (0x30 + (*(bcd_ecc + 1) & 0x0F));
    *(digit_ecc + 3) = (*(bcd_ecc + 1) & 0xF0) == 0xF0 ? 0 : (U8) (0x30 + ((*(bcd_ecc + 1) & 0xF0) >> 4));
    *(digit_ecc + 4) = (*(bcd_ecc + 2) & 0x0F) == 0x0F ? 0 : (U8) (0x30 + (*(bcd_ecc + 2) & 0x0F));
    *(digit_ecc + 5) = (*(bcd_ecc + 2) & 0xF0) == 0xF0 ? 0 : (U8) (0x30 + ((*(bcd_ecc + 2) & 0xF0) >> 4));
}


/*****************************************************************************
 * FUNCTION
 *  mmi_ecc_prepare_identifier
 * DESCRIPTION
 *  
 * PARAMETERS
 *  ecc_identifier  [IN]        
 ...
 *  void
 *****************************************************************************/
void mmi_ecc_prepare_identifier(U8 *ecc_identifier, U8 ecc_length, U8 ecc_dcs, U8 *ecc_output)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    mmi_ucs2ncpy((S8*) ecc_output,(S8*) tmp_identifier, MAX_PB_NAME_LENGTH);
...
/*****************************************************************************
 * FUNCTION
 *  mmi_ecc_is_ecc_number_with_identifier
 * DESCRIPTION
 *  
 * PARAMETERS
 *  eccnumber       [IN]        
 ...
 *****************************************************************************/
#ifdef __MMI_DUAL_SIM_MASTER__
MMI_BOOL mmi_ecc_is_ecc_number_with_identifier(S8 *eccnumber, S8 *eccname, U8 simInterface)
#else
MMI_BOOL mmi_ecc_is_ecc_number_with_identifier(S8 *eccnumber, S8 *eccname)
#endif
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
...
#ifdef __MMI_DUAL_SIM_MASTER__
    rtn = CheckValidEmergencyNo1(eccnumber, simInterface);
#else
    rtn = CheckValidEmergencyNo1(eccnumber);
#endif

#ifdef __REL5__
    if (gSimEccIndex == 0xFF || gSimEccIndex >= MAX_ECC_NUMBER_LIST_LENGTH)
    {
        memset(eccname, 0, 2);
        return rtn;
    }
    ...
    {
        mmi_ucs2ncpy(eccname, (S8*) gSimEccNameList_2[gSimEccIndex], MAX_PB_NAME_LENGTH);
    }
    else
#endif /* __MMI_DUAL_SIM_MASTER__ */
    {
        mmi_ucs2ncpy(eccname, (S8*) gSimEccNameList[gSimEccIndex], MAX_PB_NAME_LENGTH);
    }
#endif /* __REL5__ */
    return rtn;
}
 ...
 *  SimECCInfoSave
 * DESCRIPTION
 *  Callback of MSG_ID_MMI_NW_UPDATE_ECC_IND
 * PARAMETERS
 *  inMsg       [IN]        
 * RETURNS
 *  void
 *****************************************************************************/
void NwECCInfoSave(void *inMsg)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    mmi_nw_update_ecc_ind_struct *local_data = (mmi_nw_update_ecc_ind_struct*) inMsg;
    U8 Count = 0;

    /* reset all network ECC list */
    for (Count = 0; Count < 16; Count++)
    {
        memset((void*) gSimEccNumList[Count + MAX_ECC_NUMBER_LIST_LENGTH], 0, EMERGENCY_NUMBER_LENGTH + 1);
        gSimEccCategoryList[Count + MAX_ECC_NUMBER_LIST_LENGTH] = 0;
    }

    /* prepare valid network ECC list */
    if (local_data->no_ecc > 0)
    {
        for (Count = 0; Count < local_data->no_ecc; Count++)
        {
            MMI_ASSERT(local_data->ecc_list[Count].tel_number.addr_length <= 3);
            mmi_ecc_prepare_number(local_data->ecc_list[Count].tel_number.addr_bcd, (U8*) gSimEccNumList[Count + MAX_ECC_NUMBER_LIST_LENGTH]);
            gSimEccCategoryList[Count + MAX_ECC_NUMBER_LIST_LENGTH] = local_data->ecc_list[Count].category;
        }
    }
}
#endif /* __REL5__ */
...
void SimECCInfoSave(void *inMsg)
{
    ...
    if (local_data->num > 0)
    {
        ...
        {
            mmi_ecc_prepare_number(local_data->ecc_entry[Count].ecc, (U8*) gSimEccNumList[Count]);
        #ifdef __REL5__
            mmi_ecc_prepare_identifier(local_data->ecc_entry[Count].name, local_data->ecc_entry[Count].name_length, local_data->ecc_entry[Count].name_dcs, (U8*) gSimEccNameList[Count]);
            gSimEccCategoryList[Count] = local_data->ecc_entry[Count].esc;
        #endif /* __REL5__ */
        }
    }
...
void SimPasswdReqResponse(void *inMsg)
{
    ...
    if (g_pwr_context.PrevScreenIndicator == ENTER_SIM_INSERTION ||
        g_pwr_context.PrevScreenIndicator == ENTER_SIM_BLOCK ||
    #ifdef __MMI_WLAN_FEATURES__
       (g_pwr_context.PrevScreenIndicator == ENTER_IDLE_SCR && mmi_bootup_is_sim_valid() == MMI_FALSE) ||
    #endif
        MMI_FALSE)
    {
...
/* under construction !*/
    ...
    {
        MMI_TRACE(MMI_COMMON_TRC_G3_BOOTUP, MMI_POWER_ON_RECV_SIM_PSWD);
        ...
        if (!mmi_bootup_is_power_on_animation_complete() && !mmi_bootup_is_sim_refresh())
        {
    ...
    MMI_TRACE(MMI_COMMON_TRC_G3_BOOTUP, MMI_POWER_ON_PROC_SIM_PSWD);
    ...
    switch (local_data->type)
    {
	    ...
	    /* set the card1 status to work */
            ...
            {
                g_phb_cntx.phb_ready = MMI_FALSE;   /* MTK 2003-12-22 Wilson, for SIM refresh */
        ...
        #endif
        #if 0//def __MMI_SML_UNLOCK_AFTER_VERIFIED__
/* under construction !*/
        #endif
        ...
        case MMI_TYPE_NP:
        #ifdef __MMI_SML_UNLOCK_AFTER_VERIFIED__
            mmi_sml_unlock_after_verified_paswd(SML_CAT_N);
        #endif
            ...
            if (gCPHSRetryCount == 0)
            {
                ...
                simErrorDisplayString = STR_SIM_BLOCK_MSG;
            #ifdef __MMI_DUAL_SIM_MASTER__
                MTPNP_PFAL_Entry_SIM1_Personalization_Blocked();
            #endif
                InitSimBlockedScreen();
            }
            else
        ...
        case MMI_TYPE_NSP:
        #ifdef __MMI_SML_UNLOCK_AFTER_VERIFIED__
            mmi_sml_unlock_after_verified_paswd(SML_CAT_NS);
        #endif

        #ifdef __MMI_SML_NP_ONLY__
            mmi_bootup_sim_entry_pending_nsp_screen();
        #else /* __MMI_SML_NP_ONLY__ */ 
            if (gCPHSRetryCount == 0)
            {
                g_pwr_context.PowerOnMMIStatus = MMI_POWER_ON_IDLE; /* 0x80: insert/blocked SIM idle screen */
        ...
        case MMI_TYPE_SP:
        #ifdef __MMI_SML_UNLOCK_AFTER_VERIFIED__
            mmi_sml_unlock_after_verified_paswd(SML_CAT_SP);
        #endif

        #ifdef __MMI_SML_NP_ONLY__
            mmi_bootup_sim_entry_pending_sp_screen();
        #else /* __MMI_SML_NP_ONLY__ */ 
            if (gCPHSRetryCount == 0)
            {
                g_pwr_context.PowerOnMMIStatus = MMI_POWER_ON_IDLE; /* 0x80: insert/blocked SIM idle screen */
        ...
        case MMI_TYPE_CP:
        #ifdef __MMI_SML_UNLOCK_AFTER_VERIFIED__
            mmi_sml_unlock_after_verified_paswd(SML_CAT_C);
        #endif

        #ifdef __MMI_SML_NP_ONLY__
            mmi_bootup_sim_entry_pending_cp_screen();
        #else /* __MMI_SML_NP_ONLY__ */ 
            if (gCPHSRetryCount == 0)
            {
                g_pwr_context.PowerOnMMIStatus = MMI_POWER_ON_IDLE; /* 0x80: insert/blocked SIM idle screen */
        ...
        case MMI_TYPE_IMSI_LOCK:
        #ifdef __MMI_SML_UNLOCK_AFTER_VERIFIED__
            mmi_sml_unlock_after_verified_paswd(SML_CAT_SIM);
        #endif

        #ifdef __MMI_SML_NP_ONLY__
            mmi_bootup_sim_entry_pending_simp_screen();
        #else /* __MMI_SML_NP_ONLY__ */ 
            if (gCPHSRetryCount == 0)
            {
                g_pwr_context.PowerOnMMIStatus = MMI_POWER_ON_IDLE; /* 0x80: insert/blocked SIM idle screen */
        ...
        case MMI_TYPE_LINK_NS_SP:
        #ifdef __MMI_SML_UNLOCK_AFTER_VERIFIED__
            mmi_sml_unlock_after_verified_paswd(SML_CAT_NS_SP);
        #endif

        #ifdef __MMI_SML_NP_ONLY__
            mmi_bootup_sim_entry_pending_nssp_screen();
        #else /* __MMI_SML_NP_ONLY__ */ 
            if (gCPHSRetryCount == 0)
            {
                g_pwr_context.PowerOnMMIStatus = MMI_POWER_ON_IDLE; /* 0x80: insert/blocked SIM idle screen */
        ...
        case MMI_TYPE_LINK_SIM_C:
        #ifdef __MMI_SML_UNLOCK_AFTER_VERIFIED__
            mmi_sml_unlock_after_verified_paswd(SML_CAT_SIM_C);
        #endif

        #ifdef __MMI_SML_NP_ONLY__
            mmi_bootup_sim_entry_pending_simcp_screen();
        #else /* __MMI_SML_NP_ONLY__ */ 
            if (gCPHSRetryCount == 0)
            {
                g_pwr_context.PowerOnMMIStatus = MMI_POWER_ON_IDLE; /* 0x80: insert/blocked SIM idle screen */
...
void SimQueryFailureRsp(void *inMsg)
{
    ...
    if (mmi_bootup_is_power_on_animation_complete() == MMI_FALSE)
    {
        MMI_TRACE(MMI_COMMON_TRC_G3_BOOTUP, MMI_POWER_ON_RECV_SIM_FAIL);
    ...
    MMI_TRACE(MMI_COMMON_TRC_G3_BOOTUP, MMI_POWER_ON_PROC_SIM_FAIL);
    ...
    switch (local_data->cause)
    {
        ...
        case DUALSIM_DISCONNECTED:
            ...
            {
                g_pwr_context.PowerOnSIMStatus |= MMI_SIM_STATUS_DUALSIM_DISCONNECTED;
                g_pwr_context.CurrentServiceType = LIMITED_SERVICE;
...
void ShowWelcomeScr(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_COMMON_TRC_G3_BOOTUP, MMI_POWER_ON_START_WELCOME);
...
void ExitWelcomeScreen(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_COMMON_TRC_G3_BOOTUP, MMI_POWER_ON_STOP_WELCOME);
...
void mmi_ready_to_idle_screen_ind_hdlr(void *msg)
{
    ...
    if (isInCall())
    {
        ...
        MMI_TRACE(MMI_COMMON_TRC_G3_BOOTUP, MMI_POWER_ON_CALL_BEFORE_IDLE);
        return;
    }

    MMI_TRACE(MMI_COMMON_TRC_G3_BOOTUP, MMI_POWER_ON_READY_TO_IDLE);
...
void mmi_ready_to_idle_screen_ind(void) /* this function is call by MMI */
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_COMMON_TRC_G3_BOOTUP, MMI_POWER_ON_SEND_MSG_2_MMI);
...
void RemoveStatusIconsForSimRemoved(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_COMMON_TRC_G3_BOOTUP, MMI_POWER_ON_REMOVE_ICONS);
...
void PowerOnBatteryIndicationCompletePhase2(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    g_pwr_context.PowerOnMMIStatus = MMI_POWER_ON_LOW_BATTERY;
...
void BatteryCheckAfterPowerOnAnimation(void)
{
        ...
        g_charbat_context.chargerLevel == BATTERY_LOW_WARNING)
    {
        ...
        DisplayPopupCallBackExt(
            (PU8) GetString(STR_LOW_BATTERY), 
            mmi_get_event_based_image(EVENT_BATTERY_WARNING), 
            0, 
            LOW_BATTERY_SCR_TIMEOUT, 
            (U8) mmi_get_event_based_sound(EVENT_BATTERY_WARNING), 
            mmi_bootup_exit_low_battery_warning, /* timeout handler */
            ExitLowBatteryScrAtPowerOn);         /* interrupt handler */
...
void CallBackPowerOnAnimationComplete(GDI_RESULT ret)
{
    ...
    extern pBOOL g_DisableTouchScreenCalibration;
#endif
    ...
    /* animation stop -> CallBackPowerOnAnimationComplete -> PowerOnBatteryIndicationCompletePhase2 -> (security check) */
    ...
    #ifdef __MMI_TOUCH_SCREEN__
    #ifndef __MTK_TARGET__
        if (PhnsetNeedFactoryDefaultCalibration() && !AUTO_POWER && !g_DisableTouchScreenCalibration) /* skip pen calibration for MoDIS auto power on test */
    #else
...
#ifdef __MMI_ACL_SUPPORT__
    mmi_frm_unhide_menu_item(MENU_ID_SECSET_ACL_MAIN);
#endif
...
void mmi_sim_error_reset_input_password_guibuffer(void)
{
    ...
    if (GetActiveScreenId() == SCR_ENTER_PHONELOCK ||
        ...
        GetActiveScreenId() == SCR_ENTER_NP ||
        GetActiveScreenId() == SCR_ENTER_NSP ||
        GetActiveScreenId() == SCR_ENTER_CP ||
        GetActiveScreenId() == SCR_ENTER_SP ||
        GetActiveScreenId() == SCR_ENTER_SIMP ||
        GetActiveScreenId() == SCR_ENTER_NS_SP ||
        GetActiveScreenId() == SCR_ENTER_SIM_C)
    {
        ...
        if (GetHistoryScrID(SCR_ENTER_PHONELOCK, &History) == ST_SUCCESS ||
            ...
            GetHistoryScrID(SCR_ENTER_NP, &History) == ST_SUCCESS ||
            GetHistoryScrID(SCR_ENTER_NSP, &History) == ST_SUCCESS ||
            GetHistoryScrID(SCR_ENTER_CP, &History) == ST_SUCCESS ||
            GetHistoryScrID(SCR_ENTER_SP, &History) == ST_SUCCESS ||
            GetHistoryScrID(SCR_ENTER_SIMP, &History) == ST_SUCCESS ||
            GetHistoryScrID(SCR_ENTER_NS_SP, &History) == ST_SUCCESS ||
            GetHistoryScrID(SCR_ENTER_SIM_C, &History) == ST_SUCCESS)
        {
...
/*****************************************************************************
 * FUNCTION
 *  mmi_sim_cancel_pin_lock_poweron_rsp_hdlr
 * DESCRIPTION
 *  This function is the response handler for the cancel lock request.
 * PARAMETERS
 *  void        msg         The response message
 * RETURNS
 *  void
 *****************************************************************************/
static void mmi_sim_cancel_pin_lock_poweron_rsp_hdlr(void *msg)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    switch (g_pwr_context.cancel_pin_lock_state)
    {
        case TYPE_PHB_SMS_PARAM_ONLY_INIT:
        case TYPE_PHB_NORMAL_INIT:
            /* Continue to initialize PHB/SMS only if the response is ok. */
            if (rsp->result)
            {
                mmi_sim_cancel_pin_lock_poweron_req_hdlr();
            }
            
            break;
            
        case TYPE_SMS_NORMAL_INIT:
            /* Initialization finishes. Reset the handler. */
            ClearProtocolEventHandler(PRT_EQ_CANCEL_LOCK_POWERON_RSP);
            break;
            
        default:
            MMI_EXT_ASSERT(0, g_pwr_context.cancel_pin_lock_state, 0, 0);
            break;
    }
}
 ...
 *  mmi_sim_cancel_pin_lock_poweron_req_hdlr
 * DESCRIPTION
 *  This function starts the PHB/SMS after the user cancels the PIN lock.
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void mmi_sim_cancel_pin_lock_poweron_req_hdlr(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    switch (g_pwr_context.cancel_pin_lock_state)
    {
        case 0xFF: /* 0xFF: init value */
            g_pwr_context.cancel_pin_lock_state = TYPE_PHB_SMS_PARAM_ONLY_INIT;
            break;
            
        case TYPE_PHB_SMS_PARAM_ONLY_INIT:
            g_pwr_context.cancel_pin_lock_state = TYPE_PHB_NORMAL_INIT;
            break;
            
        case TYPE_PHB_NORMAL_INIT:
            g_pwr_context.cancel_pin_lock_state = TYPE_SMS_NORMAL_INIT;
            break;
    ...
    SetProtocolEventHandler(
        mmi_sim_cancel_pin_lock_poweron_rsp_hdlr, 
        PRT_EQ_CANCEL_LOCK_POWERON_RSP);

    /* Send to primitive to init PHB/SMS after the user cancels the PIN lock. */
    req = OslConstructDataPtr(sizeof(mmi_eq_cancel_lock_poweron_req_struct));
    req->type = g_pwr_context.cancel_pin_lock_state;
    
    msg_struct.oslSrcId = MOD_MMI;
    msg_struct.oslDestId = MOD_L4C;
    msg_struct.oslMsgId = PRT_EQ_CANCEL_LOCK_POWERON_REQ;
    msg_struct.oslSapId = MMI_L4C_SAP;
    msg_struct.oslDataPtr = (oslParaType *)req;
    msg_struct.oslPeerBuffPtr = NULL;
    OslMsgSendExtQueue(&msg_struct);
}


/*****************************************************************************
 * FUNCTION
 *  mmi_sim_cancel_pin_lock
 * DESCRIPTION
 *  This function cancels the PIN lock in the power-on period. The handset will
 *  entry the "Insert SIM" idle screen mode. Need to initialize PHB/SMS based on
 *  the following MSC:
 *
 *    MMI                                                       L4C
 *     | CANCEL_LOCK_POWERON_REQ(PHB_SMS_PARAM_ONLY_INIT)        |
 *     |-------------------------------------------------------->|
 *     |                                 CANCEL_LOCK_POWERON_RSP |
 *     |<--------------------------------------------------------|
 *     |                                                         |
 *     | CANCEL_LOCK_POWERON_REQ(TYPE_PHB_NORMAL_INIT)           |
 *     |-------------------------------------------------------->|
 *     |                                 CANCEL_LOCK_POWERON_RSP |
 *     |<--------------------------------------------------------|
 *     |                                                         |
 *     | CANCEL_LOCK_POWERON_REQ(TYPE_SMS_NORMAL_INIT)           |
 *     |-------------------------------------------------------->|
 *     |                                 CANCEL_LOCK_POWERON_RSP |
 *     |<--------------------------------------------------------|
 *     |                                                         |
 ...
 *  void
 *****************************************************************************/
void mmi_sim_cancel_pin_lock(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    /* Start the PHB/SMS initialization process. */
    mmi_sim_cancel_pin_lock_poweron_req_hdlr();

    /* Enter the "Insert SIM" idle screen mode. */
    msg.cause = MMI_SIM_CARD_REMOVED;
    SimQueryFailureRsp(&msg);
}
#endif /* __CANCEL_LOCK_POWERON__ */

/* Vogins Start */
#ifdef __VRE__
MMI_BOOL mmi_sim_get_home_plmn_num(
    ...
    const U32 buffer_size)
{
    ...
    /*----------------------------------------------------------------*/
#ifdef __MMI_DUAL_SIM_MASTER__
    extern S8 gHomePLMNNum_2[]; /* In MTPNP_PFAL_Gemini_Network.c */
#endif
    ...
    if (buffer_size < MAX_PLMN_LEN_MMI + 1)
    {
        home_plmn_num[0] = '\0';
        return MMI_FALSE;
    }
    ...
    if (required_sim == SIM1)
    {
        if (g_pwr_context.testRoaming == 3) /* IMSI is available, as well as PLMN */
        {
            memcpy(home_plmn_num, gHomePLMNNum, MAX_PLMN_LEN_MMI + 1);
            valid = MMI_TRUE;
        }
    }
#ifdef __MMI_DUAL_SIM_MASTER__
    else if (required_sim == SIM2)
    {
        if (g_pwr_context_2.testRoaming == 3)
        {
            memcpy(home_plmn_num, gHomePLMNNum_2, MAX_PLMN_LEN_MMI + 1);
            valid = MMI_TRUE;
        }
    }
#endif

    home_plmn_num[MAX_PLMN_LEN_MMI] = '\0';
