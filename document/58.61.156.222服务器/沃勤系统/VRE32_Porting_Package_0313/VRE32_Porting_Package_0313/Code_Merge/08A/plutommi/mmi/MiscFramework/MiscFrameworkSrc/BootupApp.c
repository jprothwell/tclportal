...
pwr_context_struct g_pwr_context = 
{
    ...
    0xFF,               /* cancel_pin_lock_state, 0xFF: invalid */
#endif
 ...
 * DESCRIPTION
 *  This function checks whether the SIM card is in dual SIM disconnected mode.
 * PARAMETERS
 *  void
 * RETURNS
 *  On disconnected mode, return MMI_TRUE; otherwise, return MMI_FALSE.
 *****************************************************************************/
MMI_BOOL mmi_bootup_is_dualsim_disconnected(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (g_pwr_context.PowerOnSIMStatus & MMI_SIM_STATUS_DUALSIM_DISCONNECTED)
    {
        return MMI_TRUE;
    }
    else
...
void mmi_bootup_entry_disk_check(void)
{
    ...
    /*----------------------------------------------------------------*/
    /*
     * This is the first function to be executed after keypad power on.
     * Therefore, let the handset vibrate once if necessary.
     */
    ...
    if (result == FS_NO_ERROR)
    {
        ...
        {
            FS_Close(fh);
#if 0 /* move check disk to engineering mode and not perform check disk in power on period */
...
void mmi_bootup_entry_animation(void)
{
    ...
    mmi_pwron_access_nvram_in_animation();
}
...
#ifdef __VRE__
#ifdef __MMI_DUAL_SIM_MASTER__
U8* mmi_bootup_get_imei_value(sim_interface_enum sim_interface)
#else
U8* mmi_bootup_get_imei_value(void)
#endif
{
    U8 temp_imei[8];
    U8 i;
    ...
    {
        if (g_pwr_context_2.imei[0] == 0)
        {
            nvram_get_imei_value(8, temp_imei, 2);
            for (i = 0; i < 8; i++)
            {
                g_pwr_context_2.imei[i*2] = 0x30 + (temp_imei[i] & 0x0F);
                g_pwr_context_2.imei[(i*2) + 1] = 0x30 + (temp_imei[i] >> 4);
            }
            g_pwr_context_2.imei[(i*2) - 1] = 0;        
}
        return g_pwr_context_2.imei; // 15 digits with null terminator
    }
    else
#endif /* __MMI_DUAL_SIM_MASTER__ */
    {
        if (g_pwr_context.imei[0] == 0)
        {
            nvram_get_imei_value(8, temp_imei, 1);
            for (i = 0; i < 8; i++)
            {
                g_pwr_context.imei[i*2] = 0x30 + (temp_imei[i] & 0x0F);
                g_pwr_context.imei[(i*2) + 1] = 0x30 + (temp_imei[i] >> 4);
            }
            g_pwr_context.imei[(i*2) - 1] = 0; /* add terminated character. */
        }
        return g_pwr_context.imei; // 15 digits with null terminator
     }
}
#ifdef __MMI_DUAL_SIM_MASTER__
U8* mmi_bootup_get_imsi_value(sim_interface_enum sim_interface)
#else
U8* mmi_bootup_get_imsi_value(void)
#endif
{
#ifdef __MMI_DUAL_SIM_MASTER__
    if (sim_interface == SIM2)
        return g_pwr_context_2.imsi;
    else
#endif
        return g_pwr_context.imsi; 
}
#endif
/* Vogins End */
