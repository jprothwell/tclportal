...
#if defined(__AGPS_SUPPORT__) && defined(__SUPL_SUPPORT__)
    {0, 0, 0, 0, IMG_ID_GPS_AGPS_STATE_ICON, 0, 0, 0, 0, NULL, 0,},
#endif
/* Vogins Start */
#ifdef __VRE30_AM__
...
void arrange_status_icons(void)
{
    ...
    x2 = MMI_status_icons[MMI_status_icons_list1[1]].x - STATUS_ICON_X_GAP;
        ...
        t_x1 = x2 - WGUI_CLOCK_SMALL_WIDTH;
        t_y1 = y1;
        t_x2 = t_x1 + WGUI_CLOCK_SMALL_WIDTH - 1;
        t_y2 = y2;
    ...
    while (!done)
    {
        ...
        {
        #ifdef WGUI_STATUS_ICON_SHOW_TIME_IN_RIGHT
            if ((x + MMI_status_icons[k].width) > x2 - WGUI_CLOCK_SMALL_WIDTH)   
...
#ifdef WGUI_STATUS_ICON_SHOW_TIME_IN_RIGHT
    x = x2 - STATUS_ICON_X_GAP - WGUI_CLOCK_SMALL_WIDTH;
#else/* WGUI_STATUS_ICON_SHOW_TIME_IN_RIGHT */
        ...
        if (MMI_status_icon_bars[i].flags & STATUS_ICON_BAR_DISPLAY)    /* stopwatch icon changed because re-arragne earlier */
        {
            if (MMI_status_icon_bars[i].graphics_context == &main_LCD_device_bitmap)
            {
                if (status_icon_display_area != SUBLCD_ONLY)
                {
                    ...
                    gdi_layer_push_clip();
                    get_status_icon_bar_clip((U8)i, &cx1, &cy1, &cx2, &cy2);
...
void refresh_fixed_status_icon(S16 icon_id, S16 bar_id)
{
    ...
    if ((status_icon_display_ON) && (MMI_status_icon_bars[bar_id].flags & STATUS_ICON_BAR_DISPLAY))
    {
        ...
        else
        {
            ...
            get_status_icon_bar_clip(0, &cx1, &cy1, &cx2, &cy2);
            set_status_icon_bar_clip(0, x1, y1, x2, y2);
            gdi_layer_push_clip();
            gdi_layer_set_clip(x1, y1, x2, y2);
            MMI_hide_status_icon_bars[bar_id]();
            gdi_layer_pop_clip();
            set_status_icon_bar_clip(0, cx1, cy1, cx2, cy2);
                    }
            ...
            for (i = 0; i < MMI_status_icon_bars[j].n_icons; i++)
            {
                ...
                if (MMI_status_icons[id].icon_ID != IMAGE_ID_NULL)
                {
                    ...
                    else if (MMI_status_icons[id].flags & STATUS_ICON_BLINK)
                    {
                        ...
                        {
                            gdi_image_draw(ix1, iy1, MMI_status_icons[id].icon_data);
                        }
                    }
                    else
                    {
           ...
           /*     if ((!mmi_idle_is_on_idle_screen()) || (mmi_idle_is_on_idle_screen() && (!mmi_bootup_is_idle_screen_reached())))*/
                {
                    ...
                    }
                    {
                        ...
                        gdi_layer_push_clip();
                        get_status_icon_bar_clip((U8)j, &cx1, &cy1, &cx2, &cy2);
                        gdi_layer_set_clip(cx1, cy1, cx2, cy2);
                    ...
                    {
                        wgui_clock_create(WGUI_CLOCK_MAINLCD_CLOCK5, 
                            t_x1, t_y1, t_x2 - t_x1 + 1, t_y2 - t_y1 + 1, 
                            GUI_CLOCK_TYPE_DIGITAL_TIME_SMALL);

                        wgui_clock_set_hide_function(WGUI_CLOCK_MAINLCD_CLOCK5, wgui_status_icon_time_small_hide_callback);
                        
                        if(get_status_icon_bar_layer((U8)j)!=GDI_ERROR_HANDLE)
                        {
                            wgui_clock_set_target_layer(WGUI_CLOCK_MAINLCD_CLOCK5, 
                    ...
                    wgui_clock_show_clock(WGUI_CLOCK_MAINLCD_CLOCK5);
    ...
    for (i = 0; i < MAX_STATUS_ICON_BARS; i++)
    {
        if (MMI_status_icon_bars[i].flags & STATUS_ICON_BAR_DISPLAY)
        {
            if (MMI_status_icon_bars[i].graphics_context == current_LCD_device_bitmap)
            {
                ...
                #ifdef WGUI_STATUS_ICON_SHOW_TIME_IN_RIGHT
                wgui_clock_close(WGUI_CLOCK_MAINLCD_CLOCK5);
...
void status_icon_timer_callback(void)
{
    ...
    if (!(get_status_icon_bar_type(0) & STATUS_ICON_BAR_DISPLAY) && !(g_wgui_status_bar_integrated_disabled))
    {
        if (MMI_redraw_integrated_status_icon_bars && !gdi_nb_is_non_blocking_decoding())
        {
...
#endif
    {
        if (!gdi_nb_is_non_blocking_decoding())
        {
 ...
 *  
 *****************************************************************************/
U32 get_status_icon_bar_layer(S32 status_bar_ID)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
void move_status_icons(S32 index, S32 x1, S32 y1, S32 x2, S32 y2)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
void wgui_status_icon_integrated_get_area(S16 icon_ID, S32 *width, S32 *height)
{
    ...
    /*----------------------------------------------------------------*/
#ifdef MMI_SHOW_STATUS_ICON_IN_TITLE
    if (!(get_status_icon_bar_type(0) & STATUS_ICON_BAR_DISPLAY) && 
        (MMI_status_icons[icon_ID].flags & STATUS_ICON_DISPLAY) &&
...
void wgui_status_icon_integrated_show(S16 icon_ID, S32 x, S32 y)
{
        ...
        !(g_wgui_status_bar_integrated_disabled))
    {
        ...
        else if (MMI_status_icons[icon_ID].flags & STATUS_ICON_DISPLAY)
        {
            ...
            {
                gdi_image_draw(x, y, MMI_status_icons[icon_ID].icon_data);
            }
