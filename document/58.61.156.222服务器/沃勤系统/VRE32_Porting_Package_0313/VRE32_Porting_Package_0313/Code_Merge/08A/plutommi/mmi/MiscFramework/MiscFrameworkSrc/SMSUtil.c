...
static mmi_frm_sms_deliver_data_struct mmi_frm_sms_data[MMI_FRM_SMS_DATA_SIZE + 1];
...
static U8 mmi_frm_sms_data_counter[MMI_FRM_SMS_DATA_SIZE + 1];
...
const U8 NoStandardAsciiToDefaultArray[] = 
{
    ...
    31, 69, 69, 69, 73, 73, 73, 73, 68, 93,/* 200 ~ 209 */
...
void mmi_frm_sms_add_new_sms(mmi_frm_sms_deliver_msg_struct *entry, U16 length, U8 *data)
{
    ...
    U8 segment = 1;
	U8 maxSeg = g_frm_sms_cntx.mmi_frm_sms_msg_seg_size;
	...
	{
		maxSeg = g_frm_sms_cntx.mmi_frm_sms_msg_view_seg_size;
	}
#endif /* __EMS_NON_STD_7BIT_CHAR__ */	
    ...
    if (((concatinfo->seg) % maxSeg) == 0)
    {
        segment = (U8) maxSeg;
    }
    else
    {
        segment = (concatinfo->seg) % maxSeg;
    }

    /* change seg info to start seg */
    concatinfo->seg =
        ((concatinfo->seg - 1) / maxSeg) * maxSeg + 1;
    ...
    {
	    MMI_TRACE(MMI_COMMON_TRC_G6_MSG, TRC_MMI_SMS_FRM_SMS_ADD_NEW_SMS, index);
        ...
        if (mmi_frm_sms_index_list[index].prevtype == MMI_FRM_SMS_AWAITS)
        {
		    ...
		    MMI_TRACE(MMI_COMMON_TRC_G6_MSG, TRC_MMI_SMS_FRM_SMS_ADD_NEW_SMS_1);
            ...
            {
			    MMI_TRACE(MMI_COMMON_TRC_G6_MSG, TRC_MMI_SMS_FRM_SMS_ADD_NEW_SMS_2);
            ...
            else
            {
			    ...
			    MMI_TRACE(MMI_COMMON_TRC_G6_MSG, TRC_MMI_SMS_FRM_SMS_ADD_NEW_SMS_3, awaited_index);

                mmi_frm_sms_remove_sms_entry(index);
				mmi_frm_sms_interrupt_callback(MOD_MMI, PRT_MSG_ID_MMI_SMS_SYNC_MSG_IND, NULL, L4C_AT_SMS_DELETE);                
            }
        }
        else if (mmi_frm_sms_index_list[index].prevtype == MMI_FRM_SMS_NOBOX)
        {
		    ...
		    MMI_TRACE(MMI_COMMON_TRC_G6_MSG, TRC_MMI_SMS_FRM_SMS_ADD_NEW_SMS_4);
            ...
            if (mmi_frm_sms_index_list[L4index].prevtype == MMI_FRM_SMS_AWAITS)
            {
			    ...
			    MMI_TRACE(MMI_COMMON_TRC_G6_MSG, TRC_MMI_SMS_FRM_SMS_ADD_NEW_SMS_5);
			    ...
			    MMI_TRACE(MMI_COMMON_TRC_G6_MSG, TRC_MMI_SMS_FRM_SMS_ADD_NEW_SMS_6, segment, content_index, pre_content_index);
                if (content_index != MMI_FRM_SMS_INVALID_NUM && pre_content_index != MMI_FRM_SMS_INVALID_NUM)
                {
                    ...
                    mmi_frm_sms_content[content_index].nextindex = MMI_FRM_SMS_INVALID_NUM;
                    mmi_frm_sms_content[content_index].segment = MMI_FRM_SMS_INVALID_NUM;            
				    MMI_TRACE(MMI_COMMON_TRC_G6_MSG, TRC_MMI_SMS_FRM_SMS_ADD_NEW_SMS_7);
            ...
            {
			    MMI_TRACE(MMI_COMMON_TRC_G6_MSG, TRC_MMI_SMS_FRM_SMS_ADD_NEW_SMS_8);
                mmi_frm_sms_remove_sms_entry(index);
				mmi_frm_sms_interrupt_callback(MOD_MMI, PRT_MSG_ID_MMI_SMS_SYNC_MSG_IND, NULL, L4C_AT_SMS_DELETE);
            }
        }
        ...
        {
		    MMI_TRACE(MMI_COMMON_TRC_G6_MSG, TRC_MMI_SMS_FRM_SMS_ADD_NEW_SMS_9);
            mmi_frm_sms_remove_sms_entry(index);
			mmi_frm_sms_interrupt_callback(MOD_MMI, PRT_MSG_ID_MMI_SMS_SYNC_MSG_IND, NULL, L4C_AT_SMS_DELETE);
        }
    }
...
#ifdef __MMI_DUAL_SIM_MASTER__
	mmi_frm_sms_data[MMI_FRM_SMS_DATA_SIZE].isSlave = entry->isSlave;
#endif

    mmi_frm_sms_data[MMI_FRM_SMS_DATA_SIZE].dest_mod_id = entry->dest_mod_id;
    mmi_frm_sms_data[MMI_FRM_SMS_DATA_SIZE].dest_port = entry->dest_port;
    mmi_frm_sms_data[MMI_FRM_SMS_DATA_SIZE].dcs = entry->dcs;
    mmi_frm_sms_data[MMI_FRM_SMS_DATA_SIZE].mti = entry->mti;
    memcpy(&(mmi_frm_sms_data[MMI_FRM_SMS_DATA_SIZE].concat_info), concatinfo, sizeof(smsal_concat_struct));

    memcpy((S8*) & (mmi_frm_sms_data[MMI_FRM_SMS_DATA_SIZE].oa), (S8*) & (entry->oa), sizeof(l4c_number_struct));
    memcpy((S8*) (mmi_frm_sms_data[MMI_FRM_SMS_DATA_SIZE].scts), (S8*) (entry->scts), 7);

    mmi_frm_sms_data[MMI_FRM_SMS_DATA_SIZE].data[0] = MMI_FRM_SMS_MSG_CONTENT_NUM;
    mmi_frm_sms_data[MMI_FRM_SMS_DATA_SIZE].message_len = entry->message_len;
...
void mmi_frm_sms_free_new_data(void)
{
    ...
    mmi_frm_sms_content[MMI_FRM_SMS_MSG_CONTENT_NUM].segment = MMI_FRM_SMS_INVALID_NUM;
    memset(
        (S8*) & mmi_frm_sms_data[MMI_FRM_SMS_DATA_SIZE],
        MMI_FRM_SMS_INVALID_NUM,
...
void mmi_frm_sms_convert_new_sms(MMI_FRM_SMS_DELIVER_MSG_IND_STRUCT *data, mmi_frm_sms_deliver_msg_struct *entry)
{
	...
	if (entry->pid == SMSAL_EMAIL_PID)
	{		
		memset((S8*) &entry->addr_number, 0, sizeof(l4c_number_struct));
	}	
...
void mmi_frm_sms_convert_mt_to_entry(mmi_frm_sms_deliver_msg_struct *data, mmi_frm_sms_msgbox_struct *entry)
{
	...
	{
	    entry->startseg =
	        ((concat->seg - 1) / g_frm_sms_cntx.mmi_frm_sms_msg_view_seg_size) * g_frm_sms_cntx.mmi_frm_sms_msg_view_seg_size + 1;
	}
	else
#endif /* __EMS_NON_STD_7BIT_CHAR__ */
	{
...
#ifdef __MMI_MESSAGES_SMS_EMAIL__
	if ((data->sendrequire & MMI_FRM_SMS_SEND_EMAIL) == MMI_FRM_SMS_SEND_EMAIL)
	{
		entry->pid = SMSAL_EMAIL_PID;
		memset((S8*) entry->number, 0, 21);
	}	
#endif /* __MMI_MESSAGES_SMS_EMAIL__ */
...
#if defined(__MMI_UNIFIED_MESSAGE__) && !defined(__MMI_MSG_UM_LOW_MEMORY_COST__)
    {
        U8 *content = NULL;
...
#endif /* defined(__MMI_UNIFIED_MESSAGE__) && !defined(__MMI_MSG_UM_LOW_MEMORY_COST__) */ 
...
void mmi_frm_sms_remove_sms_from_list(U16 index, U16 type)
{
    ...
    {
	    MMI_TRACE(MMI_COMMON_TRC_G6_MSG, TRC_MMI_SMS_FRM_SMS_REMOVE_SMS_FROM_LIST);
        return;
    }
...
U16 mmi_frm_sms_get_index(U16 index, U8 *index_bitmap)
{
    ...
    U16 maxSize = (g_frm_sms_cntx.mmi_frm_sms_msg_box_size >> 3) + 1;
    ...
    for (i = start_byte_idx; i < maxSize; i++)
    {
...
void mmi_frm_sms_get_sms_index(mmi_frm_sms_msgbox_enum type, U16 index, U16 *l4_index)
{
    ...
    /*----------------------------------------------------------------*/
#ifdef __EMS_NON_STD_7BIT_CHAR__
    for (i = 0; i < g_frm_sms_cntx.mmi_frm_sms_msg_view_seg_size; i++)
#else
...
U8 mmi_frm_sms_add_awaited_segment(U8 index, U16 addindex, U8 segment, mmi_frm_sms_deliver_msg_struct *entry)
{
    ...
    MMI_ASSERT(mmi_frm_sms_awaited[index].no_msg_data <= MMI_SMS_MAX_VIEW_MSG_SEG * MMI_FRM_SMS_MSG_LEN);
...
U8 mmi_frm_sms_check_awaited_complete(U8 index)
{
	...
	total_seg = ((concat_info->total_seg - concat_info->seg + 1) > maxseg) ?
        maxseg : (concat_info->total_seg - concat_info->seg + 1);
#endif /* __EMS_NON_STD_7BIT_CHAR__ */
...
U8 mmi_frm_sms_check_awaited(void)
{
    ...
    /*----------------------------------------------------------------*/
#ifdef __EMS_NON_STD_7BIT_CHAR__
	if (mmi_frm_sms_awaited[MMI_FRM_SMS_ACTION_SIZE].dcs == SMSAL_UCS2_DCS)
	{
		maxseg = g_frm_sms_cntx.mmi_frm_sms_msg_view_seg_size;
	}
    if (((concatinfo->total_seg % maxseg) == 1) &&
			...
			))
        {
            ...
            if (inbox_index != MMI_FRM_SMS_INVALID_INDEX)
            {
            #if defined(__MMI_UNIFIED_MESSAGE__) && !defined(__MMI_MSG_UM_LOW_MEMORY_COST__)
                {
                    ...
                    ucs2_content_len = ((mmi_msg_get_max_content_size() * 2 + 1) * ENCODING_LENGTH);
                    ...
                    else
                    {
                        ...
                        {
                        	U8 *ucs2_escape_content = NULL;
							U16 escape_content_len;
							
							escape_content_len = (mmi_msg_get_max_content_size() * 2 + 1) * ENCODING_LENGTH;
							ucs2_escape_content = OslMalloc(escape_content_len);
							memset(ucs2_escape_content, 0, escape_content_len);

                            mmi_asc_n_to_ucs2((S8*)ucs2_escape_content, (S8*)content, (U32)(mmi_msg_get_max_content_size() * 2));
							mmi_frm_sms_remove_escape_symbol_for_extension_char(
								(U16*)ucs2_content,
								(U16*)ucs2_escape_content,
								(U16)(mmi_msg_get_max_content_size() * 2));
							OslMfree(ucs2_escape_content);
                        }
                    }
                    ...
                    OslMfree(data);
				#ifndef __EMS_NON_STD_7BIT_CHAR__
					OslMfree(content);
				#else
                    applib_mem_screen_free(content);
				#endif
...
U8 mmi_frm_sms_check_sms_complete(U16 type, U16 index)
{
...
#ifdef __EMS_NON_STD_7BIT_CHAR__ 
	if (mmi_frm_sms_msg_box[msgindex].dcs == SMSAL_UCS2_DCS)
	{
	    total_seg =
	        ((mmi_frm_sms_msg_box[msgindex].totalseg - mmi_frm_sms_msg_box[msgindex].startseg + 1) >
	         g_frm_sms_cntx.mmi_frm_sms_msg_view_seg_size) ? g_frm_sms_cntx.mmi_frm_sms_msg_view_seg_size : (mmi_frm_sms_msg_box[msgindex].totalseg - mmi_frm_sms_msg_box[msgindex].startseg +
	                                    1);
	}
	else
#endif /* __EMS_NON_STD_7BIT_CHAR__ */
	{
	    total_seg =
	        ((mmi_frm_sms_msg_box[msgindex].totalseg - mmi_frm_sms_msg_box[msgindex].startseg + 1) >
	         g_frm_sms_cntx.mmi_frm_sms_msg_seg_size) ? g_frm_sms_cntx.mmi_frm_sms_msg_seg_size : (mmi_frm_sms_msg_box[msgindex].totalseg - mmi_frm_sms_msg_box[msgindex].startseg +
	                                    1);
	} /* __EMS_NON_STD_7BIT_CHAR__ */
...
U8 mmi_frm_sms_get_data_index(module_type mod, U16 port, U8 startindex)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    for (i = startindex; i < MMI_FRM_SMS_DATA_SIZE; i++)
    {
...
void mmi_frm_sms_remove_data(module_type mod, U16 port)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    for (i = 0; i < MMI_FRM_SMS_DATA_SIZE; i++)
    {
        if ((mmi_frm_sms_data[i].dest_mod_id == mod) && (mmi_frm_sms_data[i].dest_port == port))
        {
...
U8 mmi_frm_sms_add_data_entry(void)
{
    ...
    while (i < MMI_FRM_SMS_DATA_SIZE)
    {
        if (mmi_frm_sms_data[i].data[0] == MMI_FRM_SMS_INVALID_NUM)
        {
            memcpy(
                &(mmi_frm_sms_data[i]),
                &(mmi_frm_sms_data[MMI_FRM_SMS_DATA_SIZE]),
                sizeof(mmi_frm_sms_deliver_data_struct));
...
U8 mmi_frm_sms_check_data(void)
{
    ...
    U8 i = 0;
    mmi_frm_sms_deliver_data_struct *datainfo = &mmi_frm_sms_data[MMI_FRM_SMS_DATA_SIZE];
    mmi_frm_sms_concat_struct *concatinfo =
        ((mmi_frm_sms_concat_struct*) & (mmi_frm_sms_data[MMI_FRM_SMS_DATA_SIZE].concat_info));
    U8 thisseg = mmi_frm_sms_content[MMI_FRM_SMS_MSG_CONTENT_NUM].segment;
...
U8 mmi_frm_sms_check_data_complete(U8 index)
{
    ...
    /*----------------------------------------------------------------*/
    /* if the message has only one segment */
    if (index == MMI_FRM_SMS_DATA_SIZE)
    {
        return TRUE;
    }
    while (dataindex != MMI_FRM_SMS_INVALID_NUM)
...
U8 mmi_frm_sms_set_sms_status(U16 type, U16 index, U8 newtype)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_COMMON_TRC_G6_MSG, TRC_MMI_SMS_FRM_SMS_SET_SMS_STATUS, type, index, newtype);
...
U8 mmi_frm_sms_get_sms_size(U16 type, U16 index)
{
	...
	if (mmi_frm_sms_msg_box[msgindex].dcs == SMSAL_UCS2_DCS)
	{
	    return ((totalseg > g_frm_sms_cntx.mmi_frm_sms_msg_view_seg_size) ? g_frm_sms_cntx.mmi_frm_sms_msg_view_seg_size : totalseg);
	}
	else
#endif /* __EMS_NON_STD_7BIT_CHAR__ */
	{
...
BOOL mmi_frm_sms_check_UCS2(U16 *buf, U16 charNum)
{
    ...
    while (count < charNumber)
    {
        c = buffer[count];
	#ifdef __EMS_NON_STD_7BIT_CHAR__
	    if (mmi_frm_sms_is_7bit_setting() == TRUE)
	    {
	    	if (mmi_frm_sms_is_no_standard_character(c) != TRUE && UI_TEST_UCS2_CHARACTER(c))
	    	{
	        	return KAL_TRUE;
	    	}
	    }
...
U8 mmi_frm_sms_get_read_sms_content(MMI_FRM_SMS_GET_MSG_RSP_STRUCT *entry, U8 *content)
{
    ...
    if (buffLen)
    {
        ...
        if (result == EMS_OK)
        {
            ...
            content_len = (pEMSdata->textLength < (MMI_FRM_SMS_MSG_LEN - ENCODING_LENGTH)) ? pEMSdata->textLength : (MMI_FRM_SMS_MSG_LEN - ENCODING_LENGTH);

			mmi_frm_sms_remove_escape_symbol_for_extension_char(
				(U16*)content,
				(U16*)pEMSdata->textBuffer,
				(U16)(content_len / ENCODING_LENGTH));
        }
        else
        {
            memset((S8*) content, 0, ENCODING_LENGTH);
...
void mmi_frm_sms_highlight_ind(U8 msg_box_type, U16 index)
{
    ...
    if (msg_box_type == MMI_FRM_SMS_NOBOX)
    {
		...
		else if (type == MMI_FRM_SMS_UNSENT)
		{
	...
	sms_msgbox_type = mmi_msg_um_box_type_to_sms((UmMsgBoxType)type);
	msg_id = mmi_msg_sms_list_index_to_um_msg_id(sms_msgbox_type, msg_list_index);

    mmi_um_highlight_msg_ind(UM_MSG_TYPE_SMS, (UmMsgBoxType)type, msg_id);
}
...
void mmi_frm_sms_init_msgbox(void)
{
    ...
    memset(
        (S8*) mmi_frm_sms_data,
        MMI_FRM_SMS_INVALID_NUM,
        sizeof(mmi_frm_sms_deliver_data_struct) * (MMI_FRM_SMS_DATA_SIZE + 1));
    memset((S8*) mmi_frm_sms_data_counter, MMI_FRM_SMS_INVALID_NUM, sizeof(U8) * (MMI_FRM_SMS_DATA_SIZE + 1));
...
#ifdef __UNIFIED_MESSAGE_SIMBOX_SUPPORT__
	mmi_frm_sms_simbox_size = 0;
    if (mmi_frm_sms_sim_list != NULL)
    {
        OslMfree(mmi_frm_sms_sim_list);
		mmi_frm_sms_sim_list = NULL;
    }
#endif /* __UNIFIED_MESSAGE_SIMBOX_SUPPORT__ */

#ifdef __MMI_MESSAGES_SMS_EMAIL__
	mmi_frm_init_sms_email_sc();
#endif /* __MMI_MESSAGES_SMS_EMAIL__ */
...
U16 mmi_frm_sms_get_archive_content_file_size()
{
    ...
    MMI_TRACE(MMI_COMMON_TRC_G6_MSG, TRC_MMI_SMS_FRMSMS_GET_ARCHIVE_CONTENT_FILE_SIZE, pEMSdata->textLength);
    while (currObj != NULL)
    {                    	
		switch (currObj->Type)
		{
    ...
    MMI_TRACE(MMI_COMMON_TRC_G6_MSG, TRC_MMI_SMS_FRMSMS_GET_ARCHIVE_CONTENT_FILE_SIZE_1, filesize);
    return filesize;
}
...
FS_HANDLE mmi_frm_sms_check_archive_pub_folder_created(void)
{
    ...
    {
	    MMI_TRACE(MMI_COMMON_TRC_G6_MSG, TRC_MMI_SMS_FRM_SMS_CHECK_ARCHIVE_PUB_FOLDER_CREATED, file_handle);
    }
    else
    {
    ...
    *pFsErrCode = FS_NO_ERROR;
    ...
    if (file_handle < 0)
    {
        *pFsErrCode = file_handle;
    	MMI_TRACE(MMI_COMMON_TRC_G6_MSG, TRC_MMI_SMS_FRM_SMS_NEW_ARCHIVE_CONTENT_FILE, file_handle);
        return FALSE;  
    }     
...
U8 mmi_frm_sms_save_one_archive_msg(mmi_frm_sms_msgbox_enum fromboxtype, U16 srcIndex, FS_HANDLE *pFsErrCode) 
{
    ...
    *pFsErrCode = mmi_frm_sms_check_archive_pub_folder_created();
	...
	{
    	MMI_TRACE(MMI_COMMON_TRC_G6_MSG, TRC_MMI_SMS_FRM_SMS_SAVE_ONE_ARCHIVE_MSG);
		return FALSE;
	}
...
U8 mmi_frm_sms_update_whole_archive_index_file(void)
{
    ...
    {
    	MMI_TRACE(MMI_COMMON_TRC_G6_MSG, TRC_MMI_SMS_FRM_UPDATE_WHOLE_ARCHIVE_INDEX_FILE, file_handle);
        return FALSE;
    }
...
//1--- create arch folder in sys drive for storing idx file
    ...
    {
    	MMI_TRACE(MMI_COMMON_TRC_G6_MSG, TRC_MMI_SMS_FRM_SMS_CHECK_ARCHIVE_FOLDER_CREATED, file_handle);
    }    
    FS_SetAttributes((PU16) archive_folder_path, FS_ATTR_DIR | FS_ATTR_HIDDEN);
...
U8 mmi_frm_sms_remove_archive_content_file(U16 fileId)
{
    ...
    if (file_handle < 0)
    {
	    MMI_TRACE(MMI_COMMON_TRC_G6_MSG, TRC_MMI_SMS_FRM_SMS_REMOVE_ARCHIVE_CONTENT_FILE, file_handle);
        return FALSE;
    }
    else 
    {
...
U8 mmi_frm_sms_init_archive_index_List(void)
{
    ...
    {
	    MMI_TRACE(MMI_COMMON_TRC_G6_MSG, TRC_MMI_SMS_FRM_SMS_INIT_ARCHIVE_INDEX_LIST);
    ...
    if (file_handle < 0)
    {
	    MMI_TRACE(MMI_COMMON_TRC_G6_MSG, TRC_MMI_SMS_FRM_SMS_INIT_ARCHIVE_INDEX_LIST_1, file_handle);
        return FALSE;  
    }        
    ...
    {
	    MMI_TRACE(MMI_COMMON_TRC_G6_MSG, TRC_MMI_SMS_FRM_SMS_INIT_ARCHIVE_INDEX_LIST_2);
        FS_Close(file_handle);
 ...
 * DESCRIPTION
 *  to remove all files in this folder  (archive folder in system drv)
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void mmi_frm_sms_format_archive(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_COMMON_TRC_G6_MSG, TRC_MMI_SMS_FRM_SMS_FORMAT_ARCHIVE);
    sprintf(tmp_str, "%c:%s", MMI_SYSTEM_DRV, MMI_SMS_ARCH_SYS_PATH);
    mmi_asc_to_ucs2((S8*) archive_folder_path, (S8*) tmp_str);
...
/*****************************************************************************
 * FUNCTION
 *  mmi_frm_sms_get_archive_entry_index
 * DESCRIPTION
 *  get archive entry index
 * PARAMETERS
 *  result      [IN]       archive list index
 * RETURNS
 *  U16
 *****************************************************************************/
U16 mmi_frm_sms_get_archive_entry_index(U16 list_index)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	return mmi_frm_sms_archive_list[list_index];
}


/*****************************************************************************
 * FUNCTION
 *  mmi_frm_sms_get_archive_list_index
 * DESCRIPTION
 *  get archive list index
 * PARAMETERS
 *  entry_index      [IN]     archive entry index   
 * RETURNS
 *  void
 *****************************************************************************/
U16 mmi_frm_sms_get_archive_list_index(U16 entry_index)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	for (i = 0; i < mmi_frm_sms_archive_size; i++)
	{
		if (mmi_frm_sms_archive_list[i] == entry_index)
		{
			break;
		}
	}
...
/*****************************************************************************
 * FUNCTION
 *  mmi_frm_sms_convert_l4_num_to_ascii_num
 * DESCRIPTION
 ...
 *  void
 *****************************************************************************/
void mmi_frm_sms_convert_l4_num_to_ascii_num(U8 *ascii_number, l4c_number_struct *l4_number, U8 max_ascii_num_len)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
BOOL mmi_frm_sms_get_unsent_sms_waitting_index(U16 *index)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_COMMON_TRC_G6_MSG, TRC_MMI_SMS_FRM_SMS_GET_UNSENT_SMS_WAITING_INDEX);
    if (index == NULL)
    {
        return FALSE;
    }
    ...
    for (i = mmi_frm_sms_unsent_size - 1; i >= 0; i--)
    {
        ...
        {
            *index = i;
		    MMI_TRACE(MMI_COMMON_TRC_G6_MSG, TRC_MMI_SMS_FRM_SMS_GET_UNSENT_SMS_WAITING_INDEX_1, *index, mmi_frm_sms_unsent_list[i]);
            return TRUE;
        }
    }
...
/*****************************************************************************
 * FUNCTION
 *  mmi_frm_sms_get_sms_segment_number
 * DESCRIPTION
 *  Get SMS segment
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/ 
U8 mmi_frm_sms_get_sms_segment_number(U16 UCS2_count, U16 char_count, U16 extension_char_count)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (UCS2_count)
    {
        if (EMSCalculateSeg(SMSAL_UCS2_DCS, (U16)(char_count * 2), &segment) == EMS_OK)
        {
            return segment;
        }
        else
    ...
    {
        if (EMSCalculateSeg(SMSAL_DEFAULT_DCS, (U16)((char_count + extension_char_count) * 2), &segment) == EMS_OK)
        {
            return segment;
        }
        else
...
/*****************************************************************************
 * FUNCTION
 *  mmi_frm_sms_get_content_from_entry
 * DESCRIPTION
 *  Get sms content from the entry
 * PARAMETERS
 *  entry       	[IN]     Message entry info
 *  content     	[OUT]    Message content
 *  content_size 	[IN]	 Message Content size wanted to get
 * RETURNS
 *  object present
 *****************************************************************************/
void mmi_frm_sms_get_content_from_entry(MMI_FRM_SMS_GET_MSG_RSP_STRUCT *entry, S8 *content, U8 content_size)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (buffLen)
    {
        ...
        if (result == EMS_OK)
        {
			...
			if ((pEMSdata->textLength / ENCODING_LENGTH) < content_size)
            {
				content_len = pEMSdata->textLength / ENCODING_LENGTH;
			}
			else
			{
 ...
 *  return MMI_TRUE or MMI_FALSE to mean get successfully or not
 *****************************************************************************/
U8 mmi_frm_sms_get_archive_msg_content(U16 index, U8 *content, U8 content_size)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	U8* archive_buff;
    U16 entryindex = mmi_frm_sms_archive_list[index];
	U32 buff_length = content_size * ENCODING_LENGTH;
	U8 result;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	if (mmi_frm_sms_archiveIndex_list[entryindex].textLength < buff_length)
	{
		buff_length = mmi_frm_sms_archiveIndex_list[entryindex].textLength;
	}

	/* the first two byte of the archive file is the textLength of the EMSData */
	buff_length += 2;
	
	archive_buff = (U8*)OslMalloc(buff_length);
	memset(archive_buff, 0, buff_length);	   

	result = mmi_frm_sms_read_archive_content_file(archive_buff,  entryindex, buff_length);

	if (result == TRUE)
	{
		//Skip the textLength
		memcpy(content, (archive_buff + 2), (buff_length - 2));
	}
	
	OslMfree(archive_buff);
	return result;
}
#endif /* __UNIFIED_MESSAGE_ARCHIVE_SUPPORT__*/
...
/*****************************************************************************
 * FUNCTION
 *  mmi_frm_sms_get_unread_sms_num
 * DESCRIPTION
 *  get the unread sms number
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
U16 mmi_frm_sms_get_unread_sms_num(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/    
	...
	{
		U16 msgbox_index = mmi_frm_sms_inbox_list[i];

		if (mmi_frm_sms_msg_box[msgbox_index].msgtype & MMI_FRM_SMS_UNREAD)
		{
			count++;
		}
	}
 ...
 *  mmi_frm_init_sms_email_sc
 * DESCRIPTION
 *  To Initialize sms email sc number
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void mmi_frm_init_sms_email_sc(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    U8 i = 0;
    S16 error;
    mmi_frm_nvram_sms_email_struct infoStruct[MAX_NO_OF_SMS_EMAIL_SC];

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    memset(&infoStruct, 0, MAX_NO_OF_SMS_EMAIL_SC * sizeof(mmi_frm_nvram_sms_email_struct));
    for (i = 0; i < MAX_NO_OF_SMS_EMAIL_SC; i++)
    {
        U16 index = (U16) i + 1;

        if (ReadRecord
            (NVRAM_EF_SMS_EMAIL_LID, index, &infoStruct[i], NVRAM_SMS_EMAIL_RECORD_SIZE, &error) < 0)
        {
            memset(&infoStruct[i], 0, sizeof(infoStruct));
            WriteRecord(
 ...
 *  status
 *****************************************************************************/
BOOL mmi_frm_sms_is_sms_email(U16 type, U16 index)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	...
	if (mmi_frm_sms_msg_box[entryindex].pid == SMSAL_EMAIL_PID)
	{
		return TRUE;
	}
    return FALSE;
 ...
 *  
 *****************************************************************************/
S32 mmi_frm_sms_get_ems_remaining_length(EMSData *data)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
#else /* defined(__MMI_MESSAGES_CONCAT_SMS__) */ 
        return (data->Reminder.totalRemainingOctet -
                ((EMS_UCS2_DCS_ALLOCATED_LENGTH << 1) - (EMS_UCS2_DCS_AVAILABLE_LENGTH << 1)))>>1;
#endif /* defined(__MMI_MESSAGES_CONCAT_SMS__) */ 
}
#endif/*__MMI_MESSAGES_SMS_EMAIL__*/
 ...
 *  
 *****************************************************************************/
BOOL mmi_frm_sms_is_7bit_setting(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	ReadValue(NVRAM_SMS_MISCELL_SETTING, &nvramByte, DS_BYTE, &errorCode);
	if (nvramByte&MMI_FRM_SMS_SETTING_7BIT)
	{
		return TRUE;
	}	
...
/*****************************************************************************
 * FUNCTION
 *  mmi_frm_sms_is_no_standard_character
 * DESCRIPTION
 *  
 * PARAMETERS
 *  type        [?]     
 ...
 *  
 *****************************************************************************/
BOOL mmi_frm_sms_is_no_standard_character(U16 UCS2_char)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 	U8 highByte = (U8)((UCS2_char&0XFF00)>>8);
	U8 lowBytyte = (U8)(UCS2_char&0X00FF);

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/	
	if(highByte == 0 && NoStandardAsciiToDefaultArray[lowBytyte] != AsciiToDefaultArray[lowBytyte])
	{
		return TRUE;
	}
	return FALSE;	
...
/*****************************************************************************
 * FUNCTION
 *  mmi_frm_sms_encode_no_sandard_char
 * DESCRIPTION
 *  convert no-standard character to standard character
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
U16 mmi_frm_sms_encode_no_sandard_char(U8 highByte, U8 lowByte)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	static BOOL isextChar = FALSE;
	...
	if (highByte == 0)
	{
		if (lowByte == EMS_ESCAPE_CHARACTER)
		{
			isextChar = TRUE;			
		}
		else
		{
			if (isextChar == TRUE)
			{
				lowByte = ExtendedAsciiToDefaultArray[lowByte];
			}
			else
			{
				lowByte = NoStandardAsciiToDefaultArray[lowByte];
			}
			isextChar = FALSE;
		}	
	}
 ...
 *  void
 *****************************************************************************/
U16 mmi_frm_sms_set_cur_Segment(EMSData *emsData)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	U8 seg = g_frm_sms_cntx.mmi_frm_sms_msg_seg_size;
	U16 result;
	...
	{
		if (mmi_frm_sms_check_UCS2((U16 *)emsData->textBuffer, (U16)(emsData->textLength >> 1)) == FALSE)
		{
			result = EMSSetDCS(emsData, SMSAL_DEFAULT_DCS);
		}
	}
	if (mmi_frm_sms_get_miscell_setting(MMI_FRM_SMS_SETTING_LONG_SMS) == FALSE)
	{
		seg = 1;
	}
	result = EMSSetEditSegs(emsData, seg);
...
/*****************************************************************************
 * FUNCTION
 *  mmi_frm_sms_get_max_used_segment
 * DESCRIPTION
 ...
 *  void
 *****************************************************************************/
U8 mmi_frm_sms_get_max_used_segment(U8 seg)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/	
	...
	return (U8)(((seg*153) /  67 ) + 1);
}
#endif /*__EMS_NON_STD_7BIT_CHAR__ */


#if defined(__EMS_NON_STD_7BIT_CHAR__) || defined(__EMS_DYNAMIC_SEG_NUM__)
/*****************************************************************************
 * FUNCTION
 *  mmi_frm_sms_get_miscell_setting
 * DESCRIPTION
 *  
 * PARAMETERS
 *  type        [?]     
 ...
 *  
 *****************************************************************************/
BOOL mmi_frm_sms_get_miscell_setting(mmi_frm_miscell_setting_enum type)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
/*****************************************************************************
 * FUNCTION
 *  mmi_frm_sms_set_miscell_setting
 * DESCRIPTION
 *  
 * PARAMETERS
 *  type        [?]
 ...
 *  
 *****************************************************************************/
BOOL mmi_frm_sms_set_miscell_setting(mmi_frm_miscell_setting_enum type, BOOL flag)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	...
	WriteValue(NVRAM_SMS_MISCELL_SETTING, &nvramByte, DS_BYTE, &errorCode);
	
	return TRUE;
	
}
#endif /* defined(__EMS_NON_STD_7BIT_CHAR__) || defined(__EMS_DYNAMIC_SEG_NUM__) */
...
/* Vogins Start */
#ifdef __VRE__
U16 mmi_frm_sms_get_msg_index(U16 type, U16 list_index)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    U16 msgbox_index = MMI_FRM_SMS_INVALID_INDEX;
    ...
    {        
        case MMI_FRM_SMS_NOBOX:
            msgbox_index = list_index;
            break;

        case MMI_FRM_SMS_INBOX:
            msgbox_index = mmi_frm_sms_inbox_list[list_index];
            break;


        case MMI_FRM_SMS_OUTBOX:
             msgbox_index = mmi_frm_sms_outbox_list[list_index];
            break;
            
    #ifdef __MMI_MESSAGES_DRAFT_BOX__
        case MMI_FRM_SMS_DRAFTS:
            msgbox_index = mmi_frm_sms_drafts_list[list_index];
            break;
    #endif /* __MMI_MESSAGES_DRAFT_BOX__ */
    
    #ifdef __MMI_UNIFIED_MESSAGE__
        case MMI_FRM_SMS_UNSENT:
            msgbox_index = mmi_frm_sms_unsent_list[list_index];
            break;
    #endif /* __MMI_UNIFIED_MESSAGE__ */

    #ifdef __UNIFIED_MESSAGE_SIMBOX_SUPPOR
        case MMI_FRM_SMS_SIM:
            msgbox_index = mmi_frm_sms_sim_list[list_index];
            break;
    #endif /* __UNIFIED_MESSAGE_SIMBOX_SUPPORT__ */
        default:
            MMI_ASSERT(0);
