...
U8 phnset_wp_default_file[(FMGR_MAX_PATH_LEN + 1) * ENCODING_LENGTH];  
U8 phnset_ss_default_file[(FMGR_MAX_PATH_LEN + 1) * ENCODING_LENGTH]; 
...
U8 PHNSET_DEFAULT_WP_PATH_1[(FMGR_MAX_PATH_LEN + 1) * ENCODING_LENGTH];
U8 PHNSET_DEFAULT_WP_PATH_2[(FMGR_MAX_PATH_LEN + 1) * ENCODING_LENGTH];
U8 PHNSET_DEFAULT_SS_PATH_1[(FMGR_MAX_PATH_LEN + 1) * ENCODING_LENGTH];
U8 PHNSET_DEFAULT_SS_PATH_2[(FMGR_MAX_PATH_LEN + 1) * ENCODING_LENGTH];
...
static U8 phnset_dispchar_wp_cp_file[(FMGR_MAX_PATH_LEN + 1) * ENCODING_LENGTH];
static U8 phnset_dispchar_ss_cp_file[(FMGR_MAX_PATH_LEN + 1) * ENCODING_LENGTH];
...
extern void mmi_phnset_get_postfix(U8 *postfix, U8 *inBuffer);
extern void mmi_phnset_copy_wpss_refresh_screen(void);
/* Vogins Start */
#ifdef __VRE_V30__
void vre_phnset_copy_wpss_check_device(void);
void vre_phnset_copy_wpss_rsp(void *info);
#endif
/* Vogins End */
#endif /* __MMI_FILE_MANAGER__ */
...
void PhnsetWallpaperSysHiliteHandler(S32 nIndex)
{
    ...
    if (nIndex > wp_item_amount - MAX_DISPCHAR_SWFLASH - 1)
        ChangeLeftSoftkey(STR_ID_DISPCHAR_PLAY, 0);
    else
...
void PhnsetWallpaperForwardFromFmgr(void *path, FMGR_FILTER *filter, int is_short)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_COMMON_TRC_G7_SETTING, MMI_PHNSET_WP_FWFROM_FMGR, path, is_short);
    ...
    if (!FMGR_FILTER_IS_SET(filter, FMGR_TYPE_GIF) &&
        ...
        !FMGR_FILTER_IS_SET(filter, FMGR_TYPE_WBM) )
    {
...
void PhnsetSubWallpaperForwardFromFmgr(void *path, FMGR_FILTER *filter, int is_short)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_COMMON_TRC_G7_SETTING, MMI_PHNSET_SUB_WP_FWFROM_FMGR, path, is_short);
...
BOOL mmi_fmgr_phnset_notify_hdlr(U32 notify_flag, void* para)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING, "mmi_fmgr_phnset_notify_hdlr");
    ...
    case MMI_FMGR_NOTIFY_DRV_POST_UNMOUNT:
        ...
        {
            MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING, "phnset_dispchar_strp == NULL");
            return TRUE;
        }
        
        MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING, "phnset_dispchar_strp[0] = c%", phnset_dispchar_strp[0]);

        temp = FS_GetDevStatus((U8)phnset_dispchar_strp[0], FS_MOUNT_STATE_ENUM);
        MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING, "FS_GetDevStatus phnset_dispchar_strp[0] = c%", temp);

        temp = FS_GetDevStatus(FS_DEVICE_TYPE_CARD, FS_MOUNT_STATE_ENUM);
        MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING, "FS_GetDevStatus FS_DEVICE_TYPE_CARD = c%", temp);
        ...
        {
            MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING, "FS_GetDevStatus == FS_NO_ERROR");
            return TRUE;
        }

        MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING, "g_phnset_cntx_p->wpss_type = d%", g_phnset_cntx_p->wpss_type);
        ...
        case MMI_PHNSET_TYPE_WP:
            {
                ...
                MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING, "SCR_MAIN_WALLPAPER");

                if (IsScreenPresent(SCR_IDLE_SCR_DISP_WP_PREVIEW))
                {
                    MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING,"set storage invalid TRUE");
                    DeleteScreenIfPresent(SCR_IDLE_SCR_DISP_WP_PREVIEW);
        ...
        case MMI_PHNSET_TYPE_SS:
            {
                /* preview screen exist and storage error */
                MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING, "SCR_MAIN_SCREENSAVER");

                if (IsScreenPresent(SCR_SCREENSAVER_DOWNLOAD_VIEW))
                {
                    MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING,"set storage invalid TRUE");
                    DeleteScreenIfPresent(SCR_SCREENSAVER_DOWNLOAD_VIEW);
                    g_phnset_cntx_p->storage_invalid = MMI_TRUE;
                }
        ...
        case MMI_PHNSET_TYPE_PWRON:
            {
                /* preview screen exist and storage error */
                MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING, "SCR_ID_PHNSET_POWER_ON");
                if (IsScreenPresent(SCR_ID_PHNSET_POWER_ON_USER_VIEW))
                {
                    MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING,"set storage invalid TRUE");
                    DeleteScreenIfPresent(SCR_ID_PHNSET_POWER_ON_USER_VIEW);
                    g_phnset_cntx_p->storage_invalid = MMI_TRUE;
                }
        ...
        case MMI_PHNSET_TYPE_PWROFF:
            {
                /* preview screen exist and storage error */
                MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING, "SCR_ID_PHNSET_POWER_OFF");
                if (IsScreenPresent(SCR_ID_PHNSET_POWER_OFF_USER_VIEW))
                {
                    MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING,"set storage invalid TRUE");
                    DeleteScreenIfPresent(SCR_ID_PHNSET_POWER_OFF_USER_VIEW);
                    g_phnset_cntx_p->storage_invalid = MMI_TRUE;
                }
...
void PhnsetWpSsPlayCallBack(S32 result)
{
    ...
    #ifdef __MMI_FILE_MANAGER__
        if (g_phnset_cntx_p->storage_invalid == MMI_FALSE)
    #endif
        {
...
PS8 PhnsetGetCurWallpaperName(void)
{
    ...
    if (g_phnset_cntx_p->curtWpID == 0)
    {
        if (phnset_wpss_filename.isWPShort == 1)
        {
            ...
            MMI_TRACE(MMI_COMMON_TRC_G7_SETTING, MMI_PHNSET_GET_CUR_WP_SHORT_NAME, p, type);
        ...
        {
            MMI_TRACE(MMI_COMMON_TRC_G7_SETTING, MMI_PHNSET_GET_CUR_WP_NAME, phnset_wpss_filename.wallpaper);
...
PS8 PhnsetGetCurSubWallpaperName(void)
{
    ...
    if (g_phnset_cntx_p->curtSubWpID == 0)
    {
        if (phnset_wpss_sub_filename.isWPShort == 1)
        {
            ...
            MMI_TRACE(MMI_COMMON_TRC_G7_SETTING, MMI_PHNSET_GET_CUR_SUBWP_SHORT_NAME, p, type);
            return p;
        }
        else
        {
            MMI_TRACE(MMI_COMMON_TRC_G7_SETTING, MMI_PHNSET_GET_CUR_SUBWP_NAME, phnset_wpss_sub_filename.wallpaper);
...
void mmi_phnset_copy_wpss_timeout(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING,"[wallpaper.c]mmi_phnset_copy_wpss_timeout");
    ...
    {
        MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING,"[wallpaper.c]waiting exist and responsed!");
...
void mmi_phnset_copy_wpss_check_device(void)
{
    ...
    S8 file_path[10];
    U8 path_asc[FMGR_MAX_PATH_LEN + 1];
    U8 postfix[PHNSET_MAX_EXT_LEN];
    U8 path_ucs2[(FMGR_MAX_PATH_LEN + 1) * ENCODING_LENGTH];
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING,"[wallpaper.c]mmi_phnset_copy_wpss_check_device");
    if (mmi_fmgr_forwardable_path((PS8) phnset_dispchar_strp))
    {
        error_str_ptr = (PS8) GetString(STR_ID_PHNSET_DISPCHAR_INVALID_PATH);
        DispIdleScrWPBadFilePopup();
    ...
    if (drive > 0)
    {
        ...
        MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING,"[wallpaper.c]path_ascii = %s", path_ascii);
        ...
        else
        {
            ...
            {
                MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING,"[wallpaper.c]create fold result = %d", result);
            }
        }
    }
    ...
    {
        phnset_wpss_default_file_1.isSSShort = 0;
    ...
    if(fh >= 0)
    {
        ...
        MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING,"[wallpaper.c]Get file size = %d", new_file_len);
        if(new_file_len == 0)
        {
            FS_Delete((const unsigned short *) phnset_dispchar_strp);
        }
    }
    ...
    /* check device status */
    {
        ...
        if (fs_ret >= 0)
        {
            ...
            MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING,"[wallpaper.c]Get Disk info free space = %d", disk_free_space);
            MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING,"[wallpaper.c]Get Disk info total space = %d", disk_total_space);
        }
        else
        {
            MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING,"[wallpaper.c]Get Disk info error");
        }
    }
    ...
    {
        MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING,"[wallpaper.c]start find empty file");
        ...
        /* copy new file to default folder */
        {
            ...
            if (g_phnset_cntx_p->wpss_type == MMI_PHNSET_TYPE_WP)
            {
                ...
                if ((phnset_wpss_default_file_1.isWPShort == 0) 
                    && (mmi_ucs2cmp((PS8)phnset_wpss_default_file_1.wallpaper, (PS8)L"") == 0))
                {
                    MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING,"[wallpaper.c]phnset_wpss_default_file_1 is empty");
                    phnset_wpss_default_file_1.isWPShort = 1;
                    sprintf((PS8) path_asc, "%s.%s", PHNSET_DEFAULT_WP_PATH_1, postfix);
                }
                else if ((phnset_wpss_default_file_2.isWPShort == 0)
                    && (mmi_ucs2cmp((PS8)phnset_wpss_default_file_2.wallpaper, (PS8)L"") == 0))
                {
                    MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING,"[wallpaper.c]phnset_wpss_default_file_2 is empty");
                    phnset_wpss_default_file_2.isWPShort = 1;
                    sprintf((PS8) path_asc, "%s.%s", PHNSET_DEFAULT_WP_PATH_2, postfix);
                }

                MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING,"[wallpaper.c]wp_path_asc : %s", path_asc);
                mmi_asc_to_ucs2((PS8) path_ucs2, (PS8) path_asc);
                mmi_ucs2cpy((PS8) phnset_dispchar_wp_cp_file, (PS8) path_ucs2);
            }
            else if (g_phnset_cntx_p->wpss_type == MMI_PHNSET_TYPE_SS)
            {
                memset(phnset_dispchar_ss_cp_file, 0x00, ((FMGR_MAX_PATH_LEN + 1) * ENCODING_LENGTH));

                /* default file 1 is in use */
                if ((phnset_wpss_default_file_1.isSSShort == 0)
                    && (mmi_ucs2cmp((PS8)phnset_wpss_default_file_1.screensave, (PS8)L"") == 0))
                {
                    MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING,"[wallpaper.c]phnset_wpss_default_file_1 is empty");
                    phnset_wpss_default_file_1.isSSShort = 1;
                    sprintf((PS8) path_asc, "%s.%s", PHNSET_DEFAULT_SS_PATH_1, postfix);
                }
                else if ((phnset_wpss_default_file_2.isSSShort == 0)
                    && (mmi_ucs2cmp((PS8)phnset_wpss_default_file_2.screensave, (PS8)L"") == 0))
                {
                    MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING,"[wallpaper.c]phnset_wpss_default_file_2 is empty");
                    phnset_wpss_default_file_2.isSSShort = 1;
                    sprintf((PS8) path_asc, "%s.%s", PHNSET_DEFAULT_SS_PATH_2, postfix);
                }

                MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING,"[wallpaper.c]ss_path_asc : %s", path_asc);
                mmi_asc_to_ucs2((PS8) path_ucs2, (PS8) path_asc);
            ...
            /* delete exist file with same name */
            {
                ...
                if (fh >= FS_NO_ERROR)
                {
                    ...
                    FS_Close(fh);
                    ret_del = FS_Delete((const unsigned short *)path_ucs2);

                    MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING,"[wallpaper.c]ret_del = %d", ret_del);
                }
                else
                {
                    if ((fh != FS_FILE_NOT_FOUND) && (fh != FS_PATH_NOT_FOUND))
                    {
                        MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING,"[wallpaper.c]FS_Open file = %d", fh);
                    }
                }
            }
        }
        ...
        mmi_fmgr_send_copy_req(
            FMGR_ACTION_COPY, 
            (PU8)phnset_dispchar_strp, 
            (PU8)path_ucs2, 
            (PsFuncPtr)mmi_phnset_copy_wpss_rsp);
    ...
    else
    {
        ...
        DisplayPopup((PU8)GetString(FMGR_FS_DISK_FULL_TEXT), IMG_GLOBAL_ERROR, 0, ST_NOTIFYDURATION, ERROR_TONE);
...
void mmi_phnset_copy_wpss_entry_waiting_entry(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING,"[wallpaper.c]mmi_phnset_copy_wpss_entry_waiting_entry");

    EntryNewScreen(SCR_IDLE_SCR_DISP_WPSS_WAITING, mmi_phnset_copy_wpss_entry_waiting_exit, NULL, NULL);
...
void mmi_phnset_copy_wpss_entry_waiting_exit(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING,"[wallpaper.c]mmi_phnset_copy_wpss_entry_waiting_exit");

    /* not receive response and not pop up */
    if ((phnset_waiting_receive_rsp == MMI_FALSE) && (POPUP_SCREENID != GetActiveScreenId()))
    {
        MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING,"[wallpaper.c]send abort copy req");
    ...
    else
    {
        ...
        wait_screen.scrnID = SCR_IDLE_SCR_DISP_WPSS_WAITING;
        wait_screen.entryFuncPtr = mmi_phnset_copy_wpss_entry_waiting_entry;
        AddNHistory(wait_screen, inputBufferSize); 
    }
...
void mmi_phnset_copy_wpss_save_setting(U8* file_path)
{
    ...
    if (g_phnset_cntx_p->wpss_type == MMI_PHNSET_TYPE_WP)
    {
        ...
        if (phnset_wpss_default_file_1.isWPShort == 1)
        {
            mmi_ucs2cpy((PS8)phnset_wpss_default_file_1.wallpaper, (PS8)phnset_wp_default_file);
        }
        else if (phnset_wpss_default_file_2.isWPShort == 1)
        {
            mmi_ucs2cpy((PS8)phnset_wpss_default_file_2.wallpaper, (PS8)phnset_wp_default_file);
        }
    ...
    else if (g_phnset_cntx_p->wpss_type == MMI_PHNSET_TYPE_SS)
    {
        ...
        if (phnset_wpss_default_file_1.isSSShort == 1)
        {
            mmi_ucs2cpy((PS8)phnset_wpss_default_file_1.screensave, (PS8)phnset_ss_default_file);
        }
        else if (phnset_wpss_default_file_2.isSSShort == 1)
        {
            mmi_ucs2cpy((PS8)phnset_wpss_default_file_2.screensave, (PS8)phnset_ss_default_file);
        }

        PhnsetSetScrSvrByFileName((PS8) phnset_dispchar_strp);
    }

    WriteRecord(NVRAM_EF_WPSS_FILENAME_LID, 4, &phnset_wpss_default_file_1, sizeof(phnset_wpss_default_file_1), &error);
    WriteRecord(NVRAM_EF_WPSS_FILENAME_LID, 5, &phnset_wpss_default_file_2, sizeof(phnset_wpss_default_file_2), &error);
}
...
void mmi_phnset_copy_wpss_refresh_screen(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (GetActiveScreenId() == SCR_IDLE_SCR_DISP_WPSS_WAITING)
    {
        DisplayPopup(
            (PU8) GetString(STR_GLOBAL_DONE),
            IMG_GLOBAL_ACTIVATED,
    ...
    DeleteScreenIfPresent(SCR_IDLE_SCR_DISP_WPSS_WAITING);
...
void mmi_phnset_copy_wpss_rsp(void *info)
{
    ...
    MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING,"[wallpaper.c]mmi_phnset_copy_wpss_rsp : %d", msgPtr->result);
    ...
    else
    {
        if (g_phnset_cntx_p->wpss_type == MMI_PHNSET_TYPE_WP)
        {
            ...
            ret_del = FS_Delete((const unsigned short *)phnset_dispchar_wp_cp_file);
        ...
        else if (g_phnset_cntx_p->wpss_type == MMI_PHNSET_TYPE_SS)
        {
            ...
            ret_del = FS_Delete((const unsigned short *)phnset_dispchar_ss_cp_file);
        ...
        if (!isInCall())
        {
            DisplayPopup((PU8)GetString(GetFileSystemErrorString(msgPtr->result)), IMG_GLOBAL_ERROR, 0, ST_NOTIFYDURATION, ERROR_TONE);
        }
...
void mmi_phnset_get_wpss_postfix(U8 *postfix)
{
    ...
    if (g_phnset_cntx_p->wpss_type == MMI_PHNSET_TYPE_WP)
    {
        mmi_ucs2_to_asc((PS8)path_asc, (PS8)phnset_wpss_filename.wallpaper);
    }
    else if (g_phnset_cntx_p->wpss_type == MMI_PHNSET_TYPE_SS)
    {
        mmi_ucs2_to_asc((PS8)path_asc, (PS8)phnset_wpss_filename.screensave);
    }
 ...
 *  void
 *****************************************************************************/
void mmi_phnset_get_postfix(U8 *postfix, U8 *inBuffer)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
void mmi_phnset_del_dirty_file(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING,"[wallpaper.c]mmi_phnset_del_dirty_file");
        ...
        && (mmi_ucs2cmp((PS8)phnset_wpss_default_file_1.wallpaper, (PS8)L"") != 0))
    {
        ...
        wp_fh = FS_Open((const unsigned short *) phnset_wpss_default_file_1.wallpaper, FS_READ_ONLY);
        ...
        if (wp_fh >= 0)
        {
            ...
            FS_Close(wp_fh);
            temp_int = FS_Delete((const unsigned short *) phnset_wpss_default_file_1.wallpaper);
            
            if (temp_int >= FS_NO_ERROR)
            {
                mmi_ucs2cpy((PS8)phnset_wpss_default_file_1.wallpaper, (PS8)L"");
            }

            MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING,"[wallpaper.c]temp_int = %d", temp_int);
        }
        else
        {
        ...
        MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING,"[wallpaper.c]wp_fh_1 = %d", wp_fh);
    }
    
    if ((phnset_wpss_default_file_2.isWPShort == 0)
        && (mmi_ucs2cmp((PS8)phnset_wpss_default_file_2.wallpaper, (PS8)L"") != 0))
    {
        FS_HANDLE wp_fh;
        
        wp_fh = FS_Open((const unsigned short *) phnset_wpss_default_file_2.wallpaper, FS_READ_ONLY);
        ...
        if (wp_fh >= 0)
        {
            ...
            FS_Close(wp_fh);
            temp_int2 = FS_Delete((const unsigned short *) phnset_wpss_default_file_2.wallpaper);
            
            if (temp_int2 >= FS_NO_ERROR)
            {
                mmi_ucs2cpy((PS8)phnset_wpss_default_file_2.wallpaper, (PS8)L"");
            }

            MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING,"[wallpaper.c]temp_int2 = %d", temp_int2);
        }
        else
        {            
            if ((wp_fh == FS_FILE_NOT_FOUND) || (wp_fh == FS_PATH_NOT_FOUND))
        ...
        MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING,"[wallpaper.c]wp_fh_2 = %d", wp_fh);
    }
        ...
        && (mmi_ucs2cmp((PS8)phnset_wpss_default_file_1.screensave, (PS8)L"") != 0))
    {
        ...
        ss_fh = FS_Open((const unsigned short *) phnset_wpss_default_file_1.screensave, FS_READ_ONLY);
        ...
        if (ss_fh >= 0)
        {
            ...
            temp_int = FS_Delete((const unsigned short *) phnset_wpss_default_file_1.screensave);
            
            if (temp_int >= FS_NO_ERROR)
            {
                mmi_ucs2cpy((PS8)phnset_wpss_default_file_1.screensave, (PS8)L"");
            }
        ...
        MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING,"[wallpaper.c]ss_fh_1 = %d", ss_fh);
    }
    
    if ((phnset_wpss_default_file_2.isSSShort == 0)
        && (mmi_ucs2cmp((PS8)phnset_wpss_default_file_2.screensave, (PS8)L"") != 0))
    {
        FS_HANDLE ss_fh;
        
        ss_fh = FS_Open((const unsigned short *) phnset_wpss_default_file_2.screensave, FS_READ_ONLY);
        ...
        if (ss_fh >= 0)
        {
            ...
            temp_int2 = FS_Delete((const unsigned short *) phnset_wpss_default_file_2.screensave);

            if (temp_int2 >= FS_NO_ERROR)
            {
                mmi_ucs2cpy((PS8)phnset_wpss_default_file_2.screensave, (PS8)L"");
            }
        ...
        MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING,"[wallpaper.c]ss_fh_2 = %d", ss_fh);
    }
}
...
void mmi_phnset_init_folder(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING,"[wallpaper.c]mmi_phnset_init_folder");

    ReadRecord(NVRAM_EF_WPSS_FILENAME_LID, 4, &phnset_wpss_default_file_1, sizeof(phnset_wpss_default_file_1), &error);
    ...
    if (drive > 0)
    {
        ...
        /* get postfix of copied file in default folder and set to idle wallpaper */
        {
            ...
            if (phnset_wpss_default_file_1.isWPShort == 1)
            {
                mmi_ucs2cpy((PS8)path_ucs2, (PS8)phnset_wpss_default_file_1.wallpaper);
            }
            else if (phnset_wpss_default_file_2.isWPShort == 1)
            {
                mmi_ucs2cpy((PS8)path_ucs2, (PS8)phnset_wpss_default_file_2.wallpaper);
            }
            ...
            {
                MMI_PRINT(MOD_MMI_COMMON_APP, MMI_COMMON_TRC_G7_SETTING,"[wallpaper.c]no wallpaper in default folder");
            }
        }
        ...
        /* init default screen saver file name and save in global structure */
        {
            ...
            if (phnset_wpss_default_file_1.isSSShort == 1)
            {
                mmi_ucs2cpy((PS8)path_ucs2, (PS8)phnset_wpss_default_file_1.screensave);
            }
            else if (phnset_wpss_default_file_2.isSSShort == 1)
            {
                mmi_ucs2cpy((PS8)path_ucs2, (PS8)phnset_wpss_default_file_2.screensave);
            }
