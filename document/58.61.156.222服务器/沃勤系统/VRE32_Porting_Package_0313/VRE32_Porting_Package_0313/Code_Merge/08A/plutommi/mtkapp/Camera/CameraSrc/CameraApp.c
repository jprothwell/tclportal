...
#define __MMI_CAMERA_DEBUG_ON__
#endif
...
#if defined(__CAMERA_OSD_HORIZONTAL__)
#define MMI_CAMERA_DISPLAY_POPUP(v,w,x,y,z)  DisplayPopupRotated((PU8)v,w,x,y,z,g_camera_cntx.osd_rotate);
#else /* defined(__CAMERA_OSD_HORIZONTAL__) */ 
#define MMI_CAMERA_DISPLAY_POPUP(v,w,x,y,z)  DisplayPopup((PU8)v,w,x,y,z);
...
#define CAMERA_ERR_BEARER_IS_ACTIVE    (-8)
...
static void mmi_camera_check_sensor_orientation(void);
#endif
...
void mmi_camera_load_setting(void)
{
    ...
    if (!g_camera_cntx.is_setting_loaded)
    {
        ...
        {
            old_storage = mmi_imgview_get_storage();

            mmi_camera_restore_setting();

            if (old_storage != 0xffff)
            {
                mmi_imgview_set_storage(old_storage);
            }
        }
...
static void mmi_camera_display_error_popup(void)
{
    ...
    if (g_camera_cntx.last_error != MDI_RES_CAMERA_SUCCEED)
    {
        ...
        MMI_CAMERA_DISPLAY_POPUP(GetString(string_id), IMG_GLOBAL_WARNING, 1, ST_NOTIFYDURATION, WARNING_TONE);
...
void mmi_camera_entry_app_screen(void)
{
    ...
    {
        g_camera_cntx.last_error = CAMERA_ERR_BEARER_IS_ACTIVE;
...
#if (defined(__MMI_FTS_SUPPORT__)||defined(__MMI_OPP_SUPPORT__)) && defined(MMI_CAMERA_CHECK_CONCURRENCY)
...
static void mmi_camera_post_init_app(void)
{
    ...
    mmi_camera_preview_update_capture_size();
        ...
        if ((g_camera_cntx.is_fullscreen && g_camera_cntx.setting.add_frame != 0)||(g_camera_cntx.setting.cont_shot == CAMERA_SETTING_CONT_SHOT_TILE)|| is_in_bg_call == TRUE)
        #else
   ...
   /************ enter next state ************/
    ...
    else if (mmi_ucm_is_background_call() == TRUE)
    {
        ...
        mmi_camera_preview_set_hint(
            (PS8) GetString((U16)STR_GLOBAL_CURRENTLY_NOT_AVAILABLE),
            NULL,
            0);
...
static void mmi_camera_preview_set_key_hdlr(void)
{
    ...
    #ifdef __MMI_CAMERA_RECORDER_ONE_KEY_TOGGLE__
    if (CAMERA_SWITCH_TO_VDOREC_KEY != -1 && g_camera_cntx.is_external_request == FALSE)
    {
...
static void mmi_camera_incall_popup_error(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    DisplayPopup((PU8) GetString(STR_GLOBAL_CURRENTLY_NOT_AVAILABLE), IMG_GLOBAL_WARNING, 1, ST_NOTIFYDURATION, WARNING_TONE);
}
    ...
    if (is_previewing)
    {
        ...
        else
        {
            ...
            gdi_layer_flatten_to_base(
                g_camera_cntx.base_layer_handle,
                g_camera_cntx.preview_layer_handle,
                g_camera_cntx.osd_layer_handle0, //Vincent: should be 0?
                0);
        #else /* defined(__MMI_CAMERA_BASELAYER_SET_COLOR_SUPPORT__) */ 
            gdi_layer_flatten_to_base(
                g_camera_cntx.base_layer_handle,
...
#define CHECK_OSD_LAYER_END()
    ...
    #if 0 // need to fix due to split imageviewer.c
/* under construction !*/
/* under construction !*/
/* under construction !*/
/* under construction !*/
...
static void mmi_camera_entry_burstshot_capture_state(void)
{
    ...
    #ifdef __MMI_CAMERA_RECORDER_ONE_KEY_TOGGLE__
    if (g_camera_cntx.setting.shutter_sound != CAMERA_SETTING_SHUTTER_SOUND_OFF && IsSilentModeActivated()==FALSE)
    #else
    if (g_camera_cntx.setting.shutter_sound != CAMERA_SETTING_SHUTTER_SOUND_OFF)
    #endif
    {
...
static void mmi_camera_entry_delete_confirm_state(void)
{
...
#else /* defined(__MMI_CAMERA_FORWARD_SUPPORT__) */ 
    /* clearn center key - since if not clear, this key will mappign to LSK */
    if (CAMERA_CAPTURE_KEY != -1)
    {
        SetKeyHandler(mmi_camera_dummy_func, CAMERA_CAPTURE_KEY, KEY_EVENT_DOWN);
    ...
    else
    {
        ...
        if (g_camera_cntx.app_prev_state == CAMERA_STATE_FORWARD_TO)
        {
            /* get file dimension */
...
#endif
        {
            ...
            if (result >= 0)
            {
                ...
                else
                {
                    ...
                    {
    #if MDI_CAMERA_MT6228_SERIES
                        if (g_camera_cntx.capture_path == CAPTURE_FROM_LAYER)
                        {
                            gdi_image_jpeg_draw_resized(
                                resized_offset_x + image_region_offset_x,
                                resized_offset_y + image_region_offset_y,
    ...
    //MMI_TRACE(MMI_MEDIA_TRC_G2_APP,MMI_TRC_CAMERA_DECODE_JPEG_TIME, g_mmi_cam_duration,__LINE__);
...
static void mmi_camera_delete_confirm_delete_file(void)
{
...
#endif
    ExitCategoryFunction();
    ShowCategory65Screen((PU8) GetString(STR_GLOBAL_DELETING), IMG_GLOBAL_DELETED, NULL);
    g_camera_cntx.is_cat65_show = TRUE;
    ...
    else
    {
        ...
        if (file_check != FS_NO_ERROR)
        {
            ...
            MMI_CAMERA_DISPLAY_POPUP(GetString(str_id),IMG_GLOBAL_WARNING,1,ST_NOTIFYDURATION,WARNING_TONE);
...
static void mmi_camera_entry_delete_done_state(void)
{
    ...
    {
        ShowCategory65Screen((PU8) GetString(STR_GLOBAL_DELETED), IMG_GLOBAL_ACTIVATED, NULL);
    }
    ...
    gui_start_timer(DONE_POPUP_FADE_TIME, mmi_camera_delete_done_timeout);
}
        ...
        case CAMERA_SETTING_CONT_SHOT_OFF:
        {
            ...
            /* for reducing tearing effect on WQVGA lcm */
                ...
                if (g_camera_cntx.capture_path == CAPTURE_FROM_MEMORY)
                    gdi_layer_blt(g_camera_cntx.base_layer_handle,g_camera_cntx.osd_layer_handle0, g_camera_cntx.osd_layer_handle4, 0, 0, 0,g_camera_cntx.base_UI_device_width - 1, g_camera_cntx.base_UI_device_height - 1);
                else
                    gdi_layer_blt_previous(
                        0,
                        0,
...
static void mmi_camera_entry_scr_snapshot_screen(void)
{
    ...
    switch (result)
    {
        case MDI_RES_CAMERA_SUCCEED:
            ...
            MMI_CAMERA_DISPLAY_POPUP((PU8) storage_filepath, IMG_GLOBAL_ACTIVATED, 1, ST_NOTIFYDURATION, SUCCESS_TONE);
            return;
            break;

        default:
            str_id = mmi_camera_get_error_string_by_error_enum(result);
            MMI_CAMERA_DISPLAY_POPUP((PU8) GetString(str_id), IMG_GLOBAL_WARNING, 1, ST_NOTIFYDURATION, WARNING_TONE);
            return;
            break;
    }
