...
* Global Variable
*****************************************************************************/
/* The flag to check if suspend to process key events in MMI framework */
static MMI_BOOL g_mmi_suspend_key_flag = MMI_FALSE;
...
static MMI_BOOL is_allow_key_action = MMI_TRUE;
...
void GetkeyInfo(PU16 keyCode, PU16 keyType)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    *keyCode = frm_p->currKeyCode;
    *keyType = frm_p->currKeyType;
}
...
void SetKeyHandler(FuncPtr funcPtr, U16 keyCode, U16 keyType)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_FW_TRC_G6_FRM_DETAIL, MMI_FRM_INFO_EVENT_SETKEY_HDLR, keyCode, keyType, funcPtr);
...
void SetKeyHandlerEx(FuncPtr funcPtr, U16 keyCode, U16 keyType, U16 flag)
{
    ...
    if (keyCode == KEY_END && flag == MMI_FRM_APP_FULL_CONTROL)
    {
        /* currently, we only supoort KEY_END in SetKeyHandlerEx() */
        SetBit(g_mmi_frm_cntx.end_key_flag, keyType);
    }
...
void SetGroupKeyHandler(FuncPtr funcPtr, PU16 keyCodes, U8 len, U16 keyType)
{
    ...
    MMI_TRACE(MMI_FW_TRC_G6_FRM_DETAIL, MMI_FRM_INFO_EVENT_SETGROUPKEY_HDLR, len, keyType, funcPtr);
...
void ClearKeyHandler(U16 keyCode, U16 keyType)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_FW_TRC_G6_FRM_DETAIL, MMI_FRM_INFO_EVENT_CLRKEY_HDLR, keyCode, keyType);
...
void ClearAllKeyHandler(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_FW_TRC_G6_FRM_DETAIL, MMI_FRM_INFO_EVENT_CLRALLKEY_HDLR);
 ...
 *  void
 *****************************************************************************/
void RegisterEndKeyHandlerInCall(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
void ProcessKeyEvent(U32 MsgType, U16 DeviceKeyCode)
{
    ...
    MMI_TRACE(MMI_FW_TRC_G6_FRM_DETAIL, MMI_FRM_INFO_PROC_KEYEVENT_HDLR, nKeyPadStatus[prevKeyMapIndex], nKeyPadMap[KeyMapIndex].nMMIKeyCode);
...
void ExecuteCurrKeyHandler(S16 keyCode, S16 keyType)
{
    ...
    if (!gEmerencyNoDialed)
    {
        ...
        /* If the handset is in voice call, END key will always to disconnect the call */
        if (frm_p->currKeyType == KEY_EVENT_DOWN && 
            isInCall() && !GetWapCallPresent() && 
            IsBitReset(g_mmi_frm_cntx.end_key_flag, frm_p->currKeyType) )
        {
            /*
             * We only change KEY_END, KEY_EVENT_DOWN handler
             * We don't need to restore original KEY_END, KEY_EVENT_DOWN because the screen will change.
             */
            RegisterEndKeyHandlerInCall();
        }
        ...
        {
            /* 
             * Process repeat key as key down, have to set pressKey to FULL_DOWN_STATUS,
             * Because after switching screen, pressKey is reset to FULL_UP_STATUS in clearkeyevents by application,
             * then event up will be drop in new screen.
             */
...
void mmi_frm_convert_process_key_event(U32 Keyevent, U16 DeviceKeyCode)
{
    ...
    {
        if (mmi_mem_mon_profiling_get_enable() == MMI_EM_MEMORY_MONITOR_ASM)
        {
            mmi_mem_mon_profiling_memory_monitor(MMI_FRM_MEM_ASM);
        }
        else if (mmi_mem_mon_profiling_get_enable() == MMI_EM_MEMORY_MONITOR_OSL)
        {    
            mmi_mem_mon_profiling_memory_monitor(MMI_FRM_MEM_OSL);
        }
    }
 ...
 *  mmi_frm_enable_key
 * DESCRIPTION
 *  This function is used to enable key process
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void mmi_frm_enable_key(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
void mmi_frm_key_handle(void *paraBuff)
{
    ...
    {
        /*
         * If we doesn't process any keys in the touch screen project after power on and 
         * mmi_suspend_handle_key_process (TRUE) was invoke by the pen event,
         * it will cause keypad_ptr is still NULL.
         */
        if ((g_mmi_suspend_key_flag == MMI_FALSE) && keypad_ptr &&((*(keypad_ptr))(&k) == MMI_TRUE))
        {
            if (k.Keydata[0] != KEY_INVALID)
            {
            ...
            #if defined(__MMI_TOUCH_SCREEN__) || defined(__MMI_HANDWRITING_PAD__)
                /* Disable key when pen is down */
                if (is_pen_down && (k.Keyevent == WM_KEYPRESS))
                {
                    /* Update key status even if pen is down */
                    U16 KeyMapIndex;
                ...
                /* 
                 * Enable key when pen is up or pen stroke up.
                 * By this way, key could disable pen when key is long or repeated pressed.
                 */
...
MMI_BOOL mmi_suspend_handle_key_process(MMI_BOOL enable)
{
    if (enable == MMI_TRUE)
    {
        g_mmi_suspend_key_flag = MMI_TRUE;
    }
    else
    {
        MYQUEUE Message;

        g_mmi_suspend_key_flag = MMI_FALSE;
...
void ClearKeyEvents(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_FW_TRC_G6_FRM_DETAIL, MMI_FRM_INFO_CLEAR_KEY_EVENTS_HDLR);
...
S16 mmi_kbd_reverse_code(S16 mmi_key_code)
{
    ...
    MMI_TRACE(MMI_FW_TRC_G6_FRM_DETAIL, MMI_FRM_KEY_REVERSE_CODE, nKeyPadMap[i].nKeyCode);
...
U8 mmi_kbn_key_tone_hdlr(KEYBRD_MESSAGE *eventKey)
{
    ...
    MMI_TRACE(MMI_FW_TRC_G6_FRM_DETAIL, MMI_FRM_KEY_REVERSE_CODE, eventKey->nKeyCode,
                         eventKey->nMsgType);
...
MMI_BOOL mmi_kbd_set_concurrent_mode(U32 mode)
{
    ...
    {
        MMI_TRACE(MMI_FW_TRC_G6_FRM_DETAIL, MMI_FRM_KEY_CONCURRENT_MODE, mode);
...
void mmi_frm_set_default_power_onoff_key(void)
{
    ...
    SetKeyHandler(NULL, KEY_END, KEY_REPEAT);
    SetKeyHandler(ShutdownSystemOperation, KEY_POWER, KEY_LONG_PRESS);
    SetKeyHandler(ShutdownSystemOperation, KEY_POWER, KEY_REPEAT);
#else
    SetKeyHandler(ShutdownSystemOperation, KEY_END, KEY_LONG_PRESS);
    SetKeyHandler(ShutdownSystemOperation, KEY_END, KEY_REPEAT);
...
static void KeyEventHandler(KEYBRD_MESSAGE *eventKey)
{
    ...
    if (g_mmi_frm_cntx.kbd_pre_func)
    {
        ...
        if (!is_hdlr_enabled)
        {
            MMI_TRACE(MMI_FW_TRC_G6_FRM_DETAIL, MMI_FRM_KEY_HDLR_DISABLED, eventKey->nKeyCode, eventKey->nMsgType, g_mmi_frm_cntx.kbd_pre_func);
        }        
    }
