...
#endif 
#ifdef __HOMEZONE_SUPPORT__
    SetSlaveProtocolEventHandler(MTPNP_PFAL_HOMEZONE_Status_Indication, PRT_MMI_HZ_TAG_IND);
#endif 
...
#endif
#ifdef __HOMEZONE_SUPPORT__
    SetProtocolEventHandler(MTPNP_PFAL_HOMEZONE_Status_Indication, PRT_MMI_HZ_TAG_IND);
#endif  
#endif /* __MMI_DUAL_SIM_DUAL_CALL_SLAVE__ */
...
void NetWorkAttachedIndication(void *inMsg)
{
    ...
    /* 
     * Ignore normal service indication in SAP and dual-sim disconnected mode. 
     */
    ...
    switch (local_data->status) /* l4c_rac_response_enum */
    {
        case MMI_RAC_OK:    /* normal service */
            ...
            if (strcmp((S8 *)gPLMN, (S8 *)lastPLMN) != 0 && lastPLMN[0] != 0)
            {
                mmi_imps_network_ind();
            }
        #endif /* __MMI_IMPS__ */
        ...
        if (g_idle_context.IsOnIdleScreen && g_pwr_context.testRoaming == 3)
        {
        ...
        #endif
            {
                if (mmi_bootup_is_idle_screen_reached())
                {
                    ...
                    /* LSK and RSK of the OP11 Homescreen do not change according
...
/* under construction !*/
...
#endif
    {
		...
		/* No matter the camp-on tone is played or not, if the network is 
		   attached after the idle screen is reached, turn on the flag. */
        if (mmi_bootup_is_idle_screen_reached())
        { 
        	g_pwr_context.IsCampOnTonePlayed = 1;
	    #ifdef __MMI_DUAL_SIM_MASTER__        
...
void NetWorkSignalLevelIndication(void *inMsg)
{
    ...
    {
    #ifdef __MMI_DUAL_SIM_MASTER__
        /* update signal icon even if still in mode switch period */
        if (DualModeIsSwitching())
        {
            if (MTPNP_AD_Get_Card1CurrLineID() == MTPNP_AD_LINE_1)
            {
                MTPNP_PFAL_Replace_StatusICON(STATUS_ICON_SIGNAL_STRENGTH, IMG_SI_MASTER_SIGNAL_STRENGTH_L1);
            }
            else if (MTPNP_AD_Get_Card1CurrLineID() == MTPNP_AD_LINE_2)
            {
...
void ChangeRoamingState(void)
{
    ...
    #ifdef MMS_SUPPORT
        mms_roaming_status_indication_callback(0/* SIM1 */, (U32) 1);
    #endif 
    }
    else
    {
    ...
    #ifdef MMS_SUPPORT
        mms_roaming_status_indication_callback(0/* SIM1 */, (U32) 0);
    #endif 
    }
...
/* Vogins Start */
#ifdef __VRE__
	memcpy(g_pwr_context.imsi, local_data->imsi, 17);
#endif
/* Vogins End */
	
    #ifdef MMS_SUPPORT
        mms_plmn_number_indication_callback((const U8*)gHomePLMNNum);
    #endif 
...
/*****************************************************************************
 * FUNCTION
 *  mmi_netset_replace_hplmm_by_ens_acting_hplmn
 * DESCRIPTION
 *  
 * PARAMETERS
 *  ens_acting_hplmn
 ...
 *  void
 *****************************************************************************/
void mmi_netset_replace_hplmm_by_ens_acting_hplmn(U8 *ens_acting_hplmn)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_eons_convert_plmn_bcd_format_to_string(ens_acting_hplmn, (U8*) gHomePLMNNum);
}
#endif /* __ENS__ */
