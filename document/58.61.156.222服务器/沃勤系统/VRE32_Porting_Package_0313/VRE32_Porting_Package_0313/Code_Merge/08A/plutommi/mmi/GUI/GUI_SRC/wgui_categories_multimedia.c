...
#include "ProfileGprots.h" /* to get current profile */
...
#endif /* defined(__MMI_MAINLCD_176X220__) || defined (__MMI_MAINLCD_240X320__) || defined (__MMI_MAINLCD_320X240__) || defined (__MMI_MAINLCD_240X400__) */ 
...
#else /* defined(__MMI_MAINLCD_176X220__) || defined (__MMI_MAINLCD_240X320__) || defined (__MMI_MAINLCD_320X240__) || defined (__MMI_MAINLCD_240X400__) */ 
 ...
 *  title_icon                  [IN]        Title image buffer
 *  left_softkey                [IN]        Lsk string buffer
 *  left_softkey_icon           [IN]        Lsk image buffer
 *  right_softkey               [IN]        Rsk string buffer
 *  right_softkey_icon          [IN]        Rsk image buffer
 *  background_color            [IN]        Background color for content region
...
static void Cat222DrawTitle(void)
{
    ...
    if (cat222_title == NULL)
    {
        ...
        mmi_ucs2ncpy((PS8) buf_filename_no_ext, (PS8) (&(cat222_file_name_ptr[index])), FMGR_MAX_FILE_LEN);
 ...
 * DESCRIPTION
 *  disable the error message display of category 222, using before show category function
 * PARAMETERS
 *  void
 * RETURNS
 *  void
...
static void Cat222DecodeImage(void)
{
    ...
    if (result >= 0 && cat222_is_fullscr_tvout == TRUE)
    {
        ...
        if (mdi_tvout_is_enable())
        {
            ...
            tvout_resized_offset_x = 0;
#ifdef MDI_TVOUT_MT6228_SERIES
            if ((tvout_resized_width % 8) != 0)     /* for hardware limitation */
            {
                S32 temp_w;
                
                temp_w = ((tvout_resized_width + 7) >> 3 ) << 3;
                tvout_resized_offset_x = (temp_w - tvout_resized_width) >> 1;
                tvout_resized_width = temp_w;
            }
        ...
        U8 *history_buffer)
{
    ...
    ExitCategoryFunction = wgui_cat223_exit;
...
void DrawCateVdoCategoryControlArea(dm_coordinates *coordinate)
{
    ...
    *******************************************************************************/
    ...
    if (wgui_is_touch_title_bar_buttons())
    {
        ...
        show_status_icons();
    #endif /* defined(__MMI_MAINLCD_176X220__) || defined(__MMI_MAINLCD_240X320__) || defined (__MMI_MAINLCD_240X400__) */ 
    }
...
static U8* wgui_catvdoply_get_history(U8* buffer)
{
    ...
    if (buffer != NULL)
    {
        ...
        {
            if (cat_vdoply_is_video_finished == TRUE)
            {
                /* seek to end */
                ...
                void *gui_buffer)
{
   ...
   ********************************************************************************/
    ...
    GetCategoryHistory = wgui_catvdoply_get_history;
    GetCategoryHistorySize = wgui_catvdoply_get_history_size;
    ...
    if (gui_buffer != NULL)
    {
        ...
        if (history_valid(h->history_id, dm_data.s32CatId))
        {
    ...
    if (cat_vdoply_is_video_play == TRUE)
    {
        ...
        {
        /* 061506 double buffer Start */
        #if (defined(__MMI_MAINLCD_176X220__) || defined(__MMI_MAINLCD_240X320__) || defined (__MMI_MAINLCD_240X400__)) && ((defined(GUI_COMMON_USE_SPECIFIC_STYLE) && defined(GUI_COMMON_SHOW_STATUS_ICON)) || (!defined(GUI_COMMON_USE_SPECIFIC_STYLE)))
        ...
        #else /* defined(__MMI_MAINLCD_176X220__) || defined(__MMI_MAINLCD_240X320__) || defined (__MMI_MAINLCD_240X400__) */ 
            /* black-video-base */
            video_ret = mdi_video_ply_play(
                            cat_vdoply_video_layer,
...
/* 073007 abm dialing Start */
    #if (( defined ( __MMI_MAINLCD_176X220__ ) || defined (__MMI_MAINLCD_240X320__) || defined (__MMI_MAINLCD_320X240__) || defined (__MMI_MAINLCD_240X400__)) && defined (__GDI_MEMORY_PROFILE_2__) )
...
static void CatSWFlashVolUp(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    silent = (BOOL)IsSilentModeActivated() || (BOOL)IsMeetingModeActivated();
    if (!silent && cat_swflash_audio_volumn < LEVEL7)
    {
...
static void CatSWFlashVolDown(void)
{
    ...
    /*----------------------------------------------------------------*/
    silent = (BOOL)IsSilentModeActivated() || (BOOL)IsMeetingModeActivated();
    if (!silent && cat_swflash_audio_volumn > LEVEL1)
    {
        if (cat_swflash_audio_volumn == LEVEL2)
        {
...
static void DrawCate334Content(void)
{
    ...
    gdi_image_draw(0, 0, g_wgui_cat334_info_p->bg_image);
    ...
    bar_x = g_wgui_cat334_info_p->bar_start_x + 
                (bar_w + g_wgui_cat334_info_p->bar_hor_gap)*g_wgui_cat334_info_p->curr_act_bar - 1;
 ...
 * DESCRIPTION
 *  for application to update the screen of category 334
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 ...
 *  MMI_BOOL
 *****************************************************************************/
static MMI_BOOL Cate334CategoryControlAreaPenDownAndMoveHandler(mmi_pen_point_struct point)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 * DESCRIPTION
 *  Show category funciton for editable equalizer screen
 * PARAMETERS
 ...
 *  right_softkey           [IN]        Right softkey label  
 *  right_softkey_icon      [IN]        Icon for the right softkey  
 *  cat_info                [IN]        Category information provided by application
 *  history_buffer          [IN]        History buffer(not used)
 * RETURNS
 *  void
 *****************************************************************************/
...
#if defined(__MMI_TOUCH_SCREEN__)
    wgui_register_category_screen_control_area_pen_handlers(Cate334CategoryControlAreaPenDownAndMoveHandler, MMI_PEN_EVENT_DOWN);
    wgui_register_category_screen_control_area_pen_handlers(Cate334CategoryControlAreaPenDownAndMoveHandler, MMI_PEN_EVENT_MOVE);
#endif /* defined(__MMI_TOUCH_SCREEN__) */
...
static void wgui_redraw_inputbox_header_img(MMI_BOOL is_draw, S32 yy, S32 height, S32 x1, S32 y1, S32 x2, S32 y2)
{
    ...
    if (wgui_cat44x_header_img != NULL)
    {
        ...
        ret = gdi_image_draw_resized_mem(x1, yy, img_width, img_height, (U8*) wgui_cat44x_header_img, (U8)image_type, wgui_cat44x_header_img_length);
    }
    else
    {
        /* Use resource image */
        ret = gdi_image_draw_id(x1, yy, wgui_cat44x_header_img_id);
    }
    if (ret != GDI_SUCCEED)
    {
        gdi_color line_color = GDI_COLOR_RED;
        
        gdi_draw_rect(x1, yy, x1 + img_width - 1, yy + img_height - 1, line_color);
        gdi_draw_line(x1, yy, x1 + img_width - 1, yy + img_height - 1, line_color);
        gdi_draw_line(x1 + img_width - 1, yy, x1, yy + img_height - 1, line_color);
    }
...
static U8 *wgui_cat440_get_history(U8 *history_buffer)
{
    ...
    s = (s + 3) / 4;
    s *= 4;
    get_multiline_inputbox_category_history(MMI_CATEGORY440_ID, (U8*) (history_buffer + s), mmi_imm_get_curr_input_mode());
    return (history_buffer);
}
...
static U8 wgui_cat440_set_history(U16 history_ID, U8 *history_buffer, mmi_imm_input_mode_enum * input_mode)
{
    ...
    s = (s + 3) / 4;
    s *= 4;
    h_flag = set_multiline_inputbox_category_history(MMI_CATEGORY440_ID, (U8*)(history_buffer + s), input_mode);
    return h_flag;
}


static color old_selected_text_color;
static color old_selector_color;
/*****************************************************************************
 * FUNCTION
        ...
        U8 *history_buffer)
{
    ...
    if (inputbox_area_data->hilite_str_number != 0)
    {
        ...
        {
            gui_multi_line_input_box_go_to_hilite(&MMI_multiline_inputbox, 0, inputbox_area_data->jump_cur_hilite);
        }
    }
        ...
        U8 *history_buffer)
{
    ...
    h_flag = set_multiline_inputbox_category_history(MMI_CATEGORY445_ID, history_buffer, &input_mode);
        ...
        U8 *history_buffer)
{
    ...
    /* Set multiline inputbox history */
    h_flag = set_multiline_inputbox_category_history(MMI_CATEGORY447_ID, history_buffer, &input_mode);
        ...
        U8 *history_buffer)
{
    ...
    MMI_multiline_inputbox.page_offset = page_info_d->text_offset;
    /* Set multiline inputbox history */
    h_flag = set_multiline_inputbox_category_history(MMI_CATEGORY448_ID, history_buffer, &input_mode);
        ...
        U8 *history_buffer)
{
    ...
    MMI_multiline_inputbox.page_offset = page_info_d->text_offset;
    /* Set multiline inputbox history */
    h_flag = set_multiline_inputbox_category_history(MMI_CATEGORY449_ID, history_buffer, &input_mode);
