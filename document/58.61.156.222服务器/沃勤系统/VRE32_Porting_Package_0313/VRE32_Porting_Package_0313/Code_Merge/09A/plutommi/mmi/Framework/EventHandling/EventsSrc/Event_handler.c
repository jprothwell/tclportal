 ...
 * removed!
 * removed!
 * in nvram procedure, we need to judge the current key msg's protocol event  handler to handler key events. This issue is: the protocol event handler is mmi_frm_key_handle, but in nvram procedure because the condition to do key handle for before poweron is wrong, so some key event is cleared.
 *
 * removed!
 * removed!
