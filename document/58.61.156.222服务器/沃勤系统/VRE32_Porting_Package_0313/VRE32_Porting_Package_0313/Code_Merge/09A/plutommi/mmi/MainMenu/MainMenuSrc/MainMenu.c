...
/* Vogins Start */
#ifdef __VRE__
		extern void mmi_vre_start(void);
#endif
/* Vogins End */
...
static MMI_BOOL g_venus_main_menu_enabled = MMI_FALSE;

static void goto_venus_main_menu_style1(void);
static void exit_venus_main_menu_style1(void);
#endif
...
static set_main_menu_softkey_handler(void(*lsk)(void), void(*rsk)(void))
{
    ...
    /*----------------------------------------------------------------*/
#ifdef __MMI_VUI_MAINMENU__
#ifdef __MMI_VUI_MAINMENU_FOR_ALL_THEME__
...
#ifdef __MMI_THEMES_APPLICATION__
#ifdef __MMI_THEMES_V2_SUPPORT__
    curr_theme = mmi_tm_get_current_theme_id();
    curr_theme --;
#else /* __MMI_THEMES_V2_SUPPORT__ */           
    curr_theme = g_thm_cntx.CurrTheme;
#endif /* __MMI_THEMES_V2_SUPPORT__ */
#endif /* __MMI_THEMES_APPLICATION__ */    

    if (curr_theme == 0 && mmi_idle_is_main_menu())
    {
        MMI_main_menu_lsk_handler = lsk;
        MMI_main_menu_rsk_handler = rsk;
        change_left_softkey(NULL, NULL);
...
void highlight_mainmenu_messages(void)
{
    ...
    //SetLeftSoftkeyFunction(mmi_um_entry_main_message_menu, KEY_EVENT_UP);     /* UM-code */
#else /* __MMI_UNIFIED_MESSAGE__ */
    set_main_menu_softkey_handler(EntryScrMessagesMenuList, handle_mainmenu_right_softkey_up);
...
void highlight_mainmenu_msgclub(void)
{
    ...
    set_main_menu_softkey_handler(mmi_msg_club_entry_level_1, handle_mainmenu_right_softkey_up);
#endif 
    //SetRightSoftkeyFunction(handle_mainmenu_right_softkey_up, KEY_EVENT_UP);
...
void highlight_mainmenu_vre(void)
{
    ...
    //SetRightSoftkeyFunction(handle_mainmenu_right_softkey_up, KEY_EVENT_UP);
/* Vogins Start */
    set_main_menu_softkey_handler(mmi_vre_start, handle_mainmenu_right_softkey_up);
    if (GetActiveScreenId() == SCR_SIM_OPTION)
    {
...
void highlight_mainmenu_shortcuts(void)
{
    ...
    //SetRightSoftkeyFunction(handle_mainmenu_right_softkey_up, KEY_EVENT_UP);
    set_main_menu_softkey_handler(EntryShctInMainMenu, handle_mainmenu_right_softkey_up);
...
void highlight_mainmenu_WiFi_wizard(void)
{
    ...
    //SetRightSoftkeyFunction(handle_mainmenu_right_softkey_up, KEY_EVENT_UP);
    set_main_menu_softkey_handler(mmi_wlan_entry_wlan_wizard, handle_mainmenu_right_softkey_up);
...
void goto_main_menu(void)
{
...
#ifdef __MMI_VUI_MAINMENU_FOR_ALL_THEME__
    g_venus_main_menu_enabled = MMI_TRUE;
    goto_venus_main_menu_style1();
    ...
    g_venus_main_menu_enabled = MMI_FALSE;
#endif
#endif

#if defined( __MMI_OP12_MENU_12MATRIX__)
    #ifdef __SAT__
    if (IsSATPresent() && mmi_bootup_get_active_flight_mode() == 0)
    {
        mmi_frm_unhide_menu_item(MAIN_MENU_SAT_MENUID);
    }
    else
    {
        mmi_frm_hide_menu_item(MAIN_MENU_SAT_MENUID);
    }
    #endif
#endif
...
#ifdef __MMI_VUI_MAINMENU_FOR_ALL_THEME__
    exit_venus_main_menu_style1();
    return;
#else
    
    if (g_venus_main_menu_enabled == MMI_TRUE)
    {
        exit_venus_main_menu_style1();
        return;
    }
...
void highlight_mainmenu_callcenter(void)
{
    ...
    //SetRightSoftkeyFunction(handle_mainmenu_right_softkey_up, KEY_EVENT_UP);
    set_main_menu_softkey_handler(mmi_ucm_entry_call_center, handle_mainmenu_right_softkey_up);
...
void highlight_mainmenu_callhistory(void)
{
    ...
    //SetLeftSoftkeyFunction(mmi_chist_entry_general_main_menu, KEY_EVENT_UP);
    //SetRightSoftkeyFunction(handle_mainmenu_right_softkey_up, KEY_EVENT_UP);
    set_main_menu_softkey_handler(mmi_chist_entry_general_main_menu, handle_mainmenu_right_softkey_up);
}

#if defined( __MMI_OP12_MENU_12MATRIX__)
/*****************************************************************************
 * FUNCTION
 *  highlight_mainmenu_simtoolkit
 * DESCRIPTION
 *  
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
void highlight_mainmenu_simtoolkit(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
void highlight_mainmenu_Camera(void)
{
    ...
    SetLeftSoftkeyFunction(EntryMainCamera, KEY_EVENT_UP);
    SetRightSoftkeyFunction(GoBackHistory, KEY_EVENT_UP);
...
/* under construction !*/
    ...
    if (GetActiveScreenId() == SCR_SIM_OPTION)
    {
        SetKeyHandler(EntryMainCamera, KEY_RIGHT_ARROW, KEY_EVENT_UP);
    }
#endif /* __MMI_OP01_MENU_9MATRIX__ */
}
...
#define  SCR_AVZONE        SCR_ID_AVZONE
#endif /* #if 0 */
...
void EntryMainMultimedia(void)
{
...
#else
		gInsertSimAppFlag /* no sim */
        || g_pwr_context.CurrentServiceType != FULL_SERVICE /* invalid sim */
#endif /* __MMI_DUAL_SIM_MASTER__ */
	...
	{
		mmi_frm_hide_menu_item(MENU_ID_GGLIVE);
	}
	else
	{
		mmi_frm_unhide_menu_item(MENU_ID_GGLIVE);
	}
#endif /* __GGLIVE_SUPPORT__ */
...
void EntryMainCamera(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    EntryNewScreen(SCR_CAMERA, NULL, EntryMainCamera, NULL);

    guiBuffer = GetCurrGuiBuffer(SCR_CAMERA);
    nItems = GetNumOfChild_Ext(MAIN_MENU_CAMERA_MENUID);
    GetSequenceStringIds_Ext(MAIN_MENU_CAMERA_MENUID, ItemList);
    GetSequenceImageIds_Ext(MAIN_MENU_CAMERA_MENUID, ItemIcons);
    SetParentHandler(MAIN_MENU_CAMERA_MENUID);
    RegisterHighlightHandler(ExecuteCurrHiliteHandler);
    ...
    if (mmi_bootup_is_sim_valid() == MMI_FALSE)
    {
        ...
        MenuItemId = GetChildMenuIDIndexByParentMenuID(MAIN_MENU_CAMERA_MENUID, MENU_ID_PHB_CALLER_PIC_1);
        if (MenuItemId != -1)   /* Menu Exists. */
        {
            nItems -= 1;
        }
    ...
    /* MTK Leo add, mini_tab_bar */
#ifdef __MMI_WGUI_MINI_TAB_BAR__
    wgui_enable_mini_tab_bar(MAIN_MENU_CAMERA_MENUID);
#endif 
    /* MTK Leo end */

    ShowCategory52Screen(
        MAIN_MENU_CAMERA_TEXT,
        GetRootTitleIcon(MAIN_MENU_CAMERA_MENUID),
        STR_GLOBAL_OK,
        IMG_GLOBAL_OK,
        STR_GLOBAL_BACK,
        IMG_GLOBAL_BACK,
...
extern void vf_test_main_menu_style1_entry(void);
extern void vf_test_main_menu_style1_exit(void);
/*****************************************************************************
 * FUNCTION
 *  goto_venus_main_menu_style1
 * DESCRIPTION
...
static void goto_venus_main_menu_style1(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    vf_test_main_menu_style1_entry();
}
