/*******************************************************************************
*  Copyright Statement:
*  --------------------
*  This software is protected by Copyright and the information contained
*  herein is confidential. The software may not be copied and the information
*  contained herein may not be used or disclosed except with the written
*  permission of MediaTek Inc. (C) 2002
*
*******************************************************************************/

/*******************************************************************************
 * Filename:
 * ---------
 * Wgui_categories_util.h
 *
 * Project:
 * --------
 *   MAUI
 *
 * Description:
 * ------------
 *   This file is intends for common utilities for categoires.
 *
 * Author:
 * -------
 * -------
 *
 *==============================================================================
 *     HISTORY
 * Below this line, this part is controlled by PVCS VM. DO NOT MODIFY!! 
 *------------------------------------------------------------------------------
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 * removed!
 *
 *------------------------------------------------------------------------------
 * Upper this line, this part is controlled by PVCS VM. DO NOT MODIFY!! 
 *==============================================================================
 *******************************************************************************/
#ifndef __WGUI_CATEGORIES_UTIL_H__
#define __WGUI_CATEGORIES_UTIL_H__

#include "MMI_features.h"
#include "CustMenuRes.h"
#include "gui_config.h"
#include "wgui.h"
#include "gui_menu_shortcut.h"
#include "gui_title.h"
#include "wgui_inputs.h"
#include "wgui_inline_edit.h"
#include "wgui_categories_defs.h"

#include "gui_typedef.h"

/***************************************************************************** 
* Extern Global Function
*****************************************************************************/

/*****************************************************************************
* FUNCTION
*  dummy_get_history
* DESCRIPTION
*  Dummy function of get history
* PARAMETERS
*  buffer      : [IN]        History buffer
* RETURNS
*  byte*
 *****************************************************************************/
extern U8 *dummy_get_history(U8 *buffer);


/*****************************************************************************
 * FUNCTION
 *  dummy_get_history_size
 * DESCRIPTION
 *  Dummy function of get history size
 * PARAMETERS
 *  void
 * RETURNS
 *  Size of history
 *****************************************************************************/
extern S32 dummy_get_history_size(void);



/*****************************************************************************
 * FUNCTION
 *  get_image
 * DESCRIPTION
 *  Wrapper for GetImage() function, get image for given id
 * PARAMETERS
 *  i       : [IN]        Id of image
 * RETURNS
 *  Pointer to the image
 *****************************************************************************/
extern PU8 get_image(MMI_ID_TYPE i);


/*****************************************************************************
 * FUNCTION
 *  get_audio
 * DESCRIPTION
 *  wrapper for GetAudio() function
 * PARAMETERS
 *  i          : [IN]        Audio id
 *  type       : [OUT]       Audio type
 *  filelen    : [OUT]       File length
 * RETURNS
 *  pointer to the audio raw data corresponding to the given audio ID
 *****************************************************************************/
extern U8 *get_audio(MMI_ID_TYPE i, U8 *type, U32 *filelen); 


/*****************************************************************************
 * FUNCTION
 *  get_string
 * DESCRIPTION
 *  Wrapper for GetString() function, get string for given id
 * PARAMETERS
 *  i       : [IN]        The id of string
 * RETURNS
 *  Pointer to the string
 *****************************************************************************/
extern UI_string_type get_string(MMI_ID_TYPE i);


/*****************************************************************************
 * FUNCTION
 *  initialize_category_screens
 * DESCRIPTION
 *  Called once during bootup to initialize category screens
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void initialize_category_screens(void);


/*****************************************************************************
 * FUNCTION
 *  reset_category_functions
 * DESCRIPTION
 *  Reset all MMI objects and category screens
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void reset_category_functions(void);

/* DOM-NOT_FOR_SDK-BEGIN */

/*****************************************************************************
 * FUNCTION
 *  SetCategoryScreenLayout
 * DESCRIPTION
 *  Set the layout of category screen
 * PARAMETERS
 *  flags       : [IN]       New value of layout flag
 * RETURNS
 *  void
 *****************************************************************************/
extern void SetCategoryScreenLayout(U32 flags);

/* DOM-NOT_FOR_SDK-END */


/*****************************************************************************
 * FUNCTION
 *  wgui_show_animation
 * DESCRIPTION
 *  Show an animated icon that was set previously
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_show_animation(void);


/*****************************************************************************
 * FUNCTION
 *  wgui_show_transparent_animation
 * DESCRIPTION
 *  Show an animated icon that was set previously, with transparency
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_show_transparent_animation(void);

/* DOM-NOT_FOR_SDK-BEGIN */

/*****************************************************************************
 * FUNCTION
 *  wgui_set_animation
 * DESCRIPTION
 *  Set up the animated icon to be displayed in the category screens
 * PARAMETERS
 *  x           : [IN]        X coordinate of animation
 *  y           : [IN]        Y coordinate of animation
 *  img         : [IN]        Id of animation
 * RETURNS
 *  void
 * REMARKS
 *  The position of animated icon is centered in horizontal and vertical
 *****************************************************************************/
extern void wgui_set_animation(S32 x, S32 y, MMI_ID_TYPE img);


/*****************************************************************************
 * FUNCTION
 *  wgui_set_animation_y
 * DESCRIPTION
 *  Set up the animated icon to be displayed in the category screens
 * PARAMETERS
 *  x           : [IN]        X coordinate of animation
 *  y           : [IN]        Y coordinate of animation
 *  img         : [IN]        Id of animation     
 * RETURNS
 *  void
 * REMARKS
 *  The position of animated icon is centered in horizontal
 *****************************************************************************/
extern void wgui_set_animation_y(S32 x, S32 y, MMI_ID_TYPE img);

/* DOM-NOT_FOR_SDK-END */


/*****************************************************************************
 * FUNCTION
 *  wgui_set_animation_image
 * DESCRIPTION
 *  Set up the animated icon to be displayed in the category screens
 * PARAMETERS
 *  x           : [IN]        X coordinate of animation
 *  y           : [IN]        Y coordinate of animation
 *  img         : [IN]        Image of animation
 * RETURNS
 *  void
 * REMARKS
 *  The position of animated icon is centered in horizontal and vertical
 *****************************************************************************/
extern void wgui_set_animation_image(S32 x, S32 y, PU8 img);


/*****************************************************************************
 * FUNCTION
 *  wgui_set_animation_image_y
 * DESCRIPTION
 *  Set up the animated icon to be displayed in the category screens
 * PARAMETERS
 *  x           : [IN]        X coordinate of animation
 *  y           : [IN]        Y coordinate of animation
 *  img         : [IN]        Image of animation
 * RETURNS
 *  void
 * REMARKS
 *  The position of animated icon is centered in horizontal
 *****************************************************************************/
extern void wgui_set_animation_image_y(S32 x, S32 y, PU8 img);

/* DOM-NOT_FOR_SDK-BEGIN */

/*****************************************************************************
 * FUNCTION
 *  wgui_measure_set_animation
 * DESCRIPTION
 *  Sets up the animated icon to be displayed in the category screens
 * PARAMETERS
 *  x           : [IN]        Soordinates around which the icon is centered horizontally
 *  y           : [IN]        Position from which the icon is drawn centered vertically.
 *  width       : [OUT]       The width of animate  
 *  height      : [OUT]       The height of animate  
 *  img         : [IN]        The id of icon to be displayed  
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_measure_set_animation(S32 x, S32 y, S32 *width, S32 *height, MMI_ID_TYPE img);


/*****************************************************************************
 * FUNCTION
 *  wgui_measure_set_animation_y
 * DESCRIPTION
 *  Sets up the animated icon to be displayed in the category screens
 * PARAMETERS
 *  x           : [IN]        Coordinates around which the icon is centered horizontally
 *  y           : [IN]        Position from which the top of the icon is drawn
 *  width       : [OUT]       The width of animate  
 *  height      : [OUT]       The height of animate  
 *  img         : [IN]        The id of icon to be displayed   
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_measure_set_animation_y(S32 x, S32 y, S32 *width, S32 *height, MMI_ID_TYPE img);

/* DOM-NOT_FOR_SDK-END */


/*****************************************************************************
 * FUNCTION
 *  wgui_measure_set_animation_image
 * DESCRIPTION
 *  Measure the given animation
 * PARAMETERS
 *  x           : [IN]        X coordinate of animation
 *  y           : [IN]        Y coordinate of animation
 *  width       : [OUT]       Pointer of parameter to get width of animation  
 *  height      : [OUT]       Pointer of parameter to get height of animation
 *  img         : [IN]        Image of animation
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_measure_set_animation_image(S32 x, S32 y, S32 *width, S32 *height, PU8 img);


/*****************************************************************************
 * FUNCTION
 *  wgui_measure_set_animation_image_y
 * DESCRIPTION
 *  Measure the given animation
 * PARAMETERS
 *  x           : [IN]        X coordinate of animation
 *  y           : [IN]        Y coordinate of animation
 *  width       : [OUT]       Pointer of parameter to get width of animation  
 *  height      : [OUT]       Pointer of parameter to get height of animation
 *  img         : [IN]        Image of animation    
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_measure_set_animation_image_y(S32 x, S32 y, S32 *width, S32 *height, PU8 img);


/*****************************************************************************
 * FUNCTION
 *  wgui_hide_animation
 * DESCRIPTION
 *  Hide the Category screen animation stop the animation
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_hide_animation(void);



/*****************************************************************************
 * FUNCTION
 *  wgui_fill_rectangle_clip
 * DESCRIPTION
 *  Show a rectangle with the given color
 * PARAMETERS
 *  x1      : [IN]        X coordinate of left-top corner
 *  y1      : [IN]        Y coordinate of left-top corner
 *  x2      : [IN]        X coordinate of right-bottom corner
 *  y2      : [IN]        Y coordinate of right-bottom corner
 *  c       : [IN]        Color is used to fill the rectangle
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_fill_rectangle_clip(S32 x1, S32 y1, S32 x2, S32 y2, color c);


/*****************************************************************************
 * FUNCTION
 *  clear_screen
 * DESCRIPTION
 *  Clear the screen with white color
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void clear_screen(void);


/*****************************************************************************
 * FUNCTION
 *  clear_screen_with_color
 * DESCRIPTION
 *  Clear the screen with specific color
 * PARAMETERS
 *  c       : [IN]        The color is used to clear screen
 * RETURNS
 *  void
 *****************************************************************************/
extern void clear_screen_with_color(color c);


/*****************************************************************************
 * FUNCTION
 *  clear_buttonbar
 * DESCRIPTION
 *  Clear softkey button bar with white color
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void clear_buttonbar(void);


/*****************************************************************************
 * FUNCTION
 *  set_button_bar_height
 * DESCRIPTION
 *  Set the height of softkey button bar
 * PARAMETERS
 *  height                      : [IN]            New height of softkey button bar
 * RETURNS
 *  void
 *****************************************************************************/
extern void set_button_bar_height(S32 height);


/*****************************************************************************
 * FUNCTION
 *  clear_contentarea
 * DESCRIPTION
 *  Clear the content area with white color
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void clear_contentarea(void);


/*****************************************************************************
 * FUNCTION
 *  draw_arrow_indication
 * DESCRIPTION
 *  Show arrow indicate
 * PARAMETERS
 *  left        : [IN]        Whether show left arrow or not
 *  right       : [IN]        Whether show right arrow or not
 *  up          : [IN]        Whether show up arrow or not
 *  down        : [IN]        Whether show down arrow or not
 * RETURNS
 *  void
 *****************************************************************************/
extern void draw_arrow_indication(S8 left, S8 right, S8 up, S8 down);


/*****************************************************************************
 * FUNCTION
 *  SetHighlightedItem
 * DESCRIPTION
 *  Highlight a given item in a menu screen
 * PARAMETERS
 *  item_index      : [IN]        Index (zero based) of the item to be highlighted
 * RETURNS
 *  void
 *****************************************************************************/
extern void SetHighlightedItem(S32 item_index);


/*****************************************************************************
 * FUNCTION
 *  GetHighlightedItem
 * DESCRIPTION
 *  Get the index of the highlighted item
 * PARAMETERS
 *  void
 * RETURNS
 *  Index of the highlighted item
 *****************************************************************************/
extern S32 GetHighlightedItem(void);


/*****************************************************************************
 * FUNCTION
 *  get_item_text
 * DESCRIPTION
 *  Get the text of the item at the given index in the currently displayed menu
 * PARAMETERS
 *  index       : [IN]         Index(zero based) of the item
 * RETURNS
 *  Pointer to the text of the specified item
 *****************************************************************************/
extern UI_string_type get_item_text(S32 index);


/*****************************************************************************
 * FUNCTION
 *  get_dynamic_item_text
 * DESCRIPTION
 *  Get dynamic item text
 * PARAMETERS
 *  index       : [IN]        Index(zero base) of the item
 * RETURNS
 *  UI_string_type
 *****************************************************************************/
extern UI_string_type get_dynamic_item_text(S32 index);

/* DOM-NOT_FOR_SDK-BEGIN */
extern PU8 get_dynamic_item_icon(S32 index);
/* DOM-NOT_FOR_SDK-END */

#ifdef ENABLE_BOTTOM_NAVIGATION_KEYS

/* DOM-NOT_FOR_SDK-BEGIN */

/*****************************************************************************
 * FUNCTION
 *  gui_show_key_navigation
 * DESCRIPTION
 *  show key navigation
 * PARAMETERS
 *  index       : [IN]        Show which key, up, down, right or left
 * RETURNS
 *  void
 *****************************************************************************/
extern void gui_show_key_navigation(S32 index);


/*****************************************************************************
 * FUNCTION
 *  gui_show_navigation_keys
 * DESCRIPTION
 *  show navigation keys
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void gui_show_navigation_keys(void);
#endif /* ENABLE_BOTTOM_NAVIGATION_KEYS */ 


/*****************************************************************************
 * FUNCTION
 *  DisableCategoryScreenScrollBar
 * DESCRIPTION
 *  Disable showing scroll bar
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void DisableCategoryScreenScrollBar(void);

/* DOM-NOT_FOR_SDK-END */


/*****************************************************************************
 * FUNCTION
 *  SetupCategoryKeyHandlers
 * DESCRIPTION
 *  Setup default softkey handler
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void SetupCategoryKeyHandlers(void);


/* DOM-NOT_FOR_SDK-BEGIN */

/*****************************************************************************
 * FUNCTION
 *  wgui_set_java_screen
 * DESCRIPTION
 *  Set the flag which indicate in java screen
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_set_java_screen(void); /* Java title */


/*****************************************************************************
 * FUNCTION
 *  wgui_unset_java_screen
 * DESCRIPTION
 *  Unset the indicate java screen flag
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_unset_java_screen(void);

/* DOM-NOT_FOR_SDK-END */


/*****************************************************************************
 * FUNCTION
 *  entry_full_screen
 * DESCRIPTION
 * Entry full screen
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 * REMARKS
 *  This funciton will hide horizontal and vertical status icon
 *****************************************************************************/
extern void entry_full_screen(void);


/*****************************************************************************
 * FUNCTION
 *  leave_full_screen
 * DESCRIPTION
 *  Leave full screen
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void leave_full_screen(void);


/*****************************************************************************
 * FUNCTION
 *  in_idle_screen
 * DESCRIPTION
 *  Set the flag which is used to indicate in idle screen
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void in_idle_screen(void);


/*****************************************************************************
 * FUNCTION
 *  leave_idle_screen
 * DESCRIPTION
 *  Leave idle screen 
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void leave_idle_screen(void);


/*****************************************************************************
 * FUNCTION
 *  if_full_screen_hide_status_icons
 * DESCRIPTION
 *  To check whether hide status icon or not when in full screen
 * PARAMETERS
 *  void
 * RETURN VALUES
 *  MMI_TRUE    :  Should hide status icon bar
 *  MMI_FALSE   :  Should not hide status icon bar
 *****************************************************************************/
extern MMI_BOOL if_full_screen_hide_status_icons(void);


/*****************************************************************************
 * FUNCTION
 *  reset_title_status_icon
 * DESCRIPTION
 *  Reset full screen flag
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 * REMARKS
 *  when full screen flag be reset, indicate leave the full screen
 *****************************************************************************/
extern void reset_title_status_icon(void);


/*****************************************************************************
 * FUNCTION
 *  close_title_status_icon
 * DESCRIPTION
 *  Close status icon bar
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 * REMARKS
 *  This function will hide horizontal and vertical status icon bar
 *****************************************************************************/
extern void close_title_status_icon(void);


/*****************************************************************************
 * FUNCTION
 *  show_title_status_icon
 * DESCRIPTION
 *  Show status icon bar
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void show_title_status_icon(void);


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_register_update_callback
 * DESCRIPTION
 * Register status icon update function.
 * PARAMETERS
 *  f       : [IN]    New update function of status icon bar
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_status_icon_register_update_callback(void (*f)(void));


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_reset_update_callback
 * DESCRIPTION
 * Clear status icon update function.
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_status_icon_reset_update_callback(void);


/*****************************************************************************
 * FUNCTION
 *  history_valid
 * DESCRIPTION
 *  To check history id whether valid or not
 * PARAMETERS
 *  history_ID              : [IN]        Id of history
 *  category_screen_ID      : [IN]        Id of category screen
 * RETURN VALUES
 *  MMI_TRUE  : The given history ID is valid
 *  MMI_FALSE : The given history ID is not valid
 *****************************************************************************/
extern U8 history_valid(U16 history_ID, U16 category_screen_ID);


/*****************************************************************************
 * FUNCTION
 *  wgui_cat_block_list_effect
 * DESCRIPTION
 *  Block list effect
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_cat_block_list_effect(void);


/*****************************************************************************
 * FUNCTION
 *  change_list_menu_category_history
 * DESCRIPTION
 *  Change the highlight index of category screen
 * PARAMETERS
 *  history_buffer              : [IN]        History buffer
 *  new_highlighted_index       : [IN]        New highlighted index
 *  new_n_items                 : [IN]        Number of items
 *  flag                        : [IN]        whether let the new hihglight index become the first display item
 * RETURNS
 *  void
 *****************************************************************************/
extern void change_list_menu_category_history(U8 *history_buffer, S32 new_highlighted_index, S32 new_n_items, U8 flag);

/* DOM-NOT_FOR_SDK-BEGIN */

/*****************************************************************************
 * FUNCTION
 *  create_list_menu_category_history
 * DESCRIPTION
 *  change the highlight index of category screen
 * 
 *  Remark: deprecated API
 * PARAMETERS
 *  history_ID                  : [IN]        History id
 *  history_buffer              : [IN]        History buffer
 *  new_highlighted_index       : [IN]        New highlighted index
 *  new_n_items                 : [IN]        Number of items
 *  flag                        : [IN]        whether let the new highlight index become the first display item
 * RETURNS
 *  void
 *****************************************************************************/
extern void create_list_menu_category_history(
                S32 history_ID,
                U8 *history_buffer,
                S32 new_highlighted_index,
                S32 new_n_items,
                U8 flag);
/* DOM-NOT_FOR_SDK-END */

/*****************************************************************************
 * FUNCTION
 *  set_list_menu_category_history
 * DESCRIPTION
 *  Set the history data for the list menu from the given history buffer
 * PARAMETERS
 *  history_ID          : [IN]        Id of history
 *  history_buffer      : [IN]        The buffer of history
 * RETURN VALUES
 *  MMI_TRUE    :   Set success 
 *  MMI_FALSE   :   Set fail
 *****************************************************************************/
extern U8 set_list_menu_category_history(U16 history_ID, U8 *history_buffer);


/*****************************************************************************
 * FUNCTION
 *  set_list_menu_category_history_state
 * DESCRIPTION
 *  Set the state of history data for list
 * PARAMETERS
 *  history_ID          : [IN]        Id of history
 *  history_buffer      : [IN]        Buffer of history
 *  state               : [IN]        New state of history
 * RETURNS
 *  void
 *****************************************************************************/
extern void set_list_menu_category_history_state(U16 history_ID, U8 *history_buffer, S8 state);        

/* DOM-NOT_FOR_SDK-BEGIN */

/*****************************************************************************
 * FUNCTION
 *  change_cat184_list_menu_history_highlighted_item
 * DESCRIPTION
 *  Change the highlighted item stored in history data. (for SMS updating list)
 * PARAMETERS
 *  index               : [IN]        New index
 *  history_buffer      : [IN]        The history buffer wanted to be changed.
 * RETURNS
 *  void
 *****************************************************************************/
extern void change_cat184_list_menu_history_highlighted_item(S32 index, U8 *history_buffer);   /* for SMS updating list */

/* DOM-NOT_FOR_SDK-END */

/*****************************************************************************
 * FUNCTION
 *  get_list_menu_category_history
 * DESCRIPTION
 *  Get the history data for list menu and store into the given history buffer
 * PARAMETERS
 *  history_ID          : [IN]        Id of history
 *  history_buffer      : [OUT]        Buffer of history
 * RETURNS
 *  void
 *****************************************************************************/
extern void get_list_menu_category_history(U16 history_ID, U8 *history_buffer);


/*****************************************************************************
 * FUNCTION
 *  get_list_menu_category_history_size
 * DESCRIPTION
 *  Get the size of history
 * PARAMETERS
 *  void
 * RETURNS
 *  Return the size of list menu history
 *****************************************************************************/
extern S32 get_list_menu_category_history_size(void);


/*****************************************************************************
 * FUNCTION
 *  get_list_menu_category_history_state
 * DESCRIPTION
 *  Get the history state
 * PARAMETERS
 *  history_ID          : [IN]        Id of history
 *  history_buffer      : [IN]        Buffer of history
 * RETURNS
 *  The history state
 *****************************************************************************/
extern S8 get_list_menu_category_history_state(U16 history_ID, U8 *history_buffer);    


/*****************************************************************************
 * FUNCTION
 *  set_matrix_menu_category_history
 * DESCRIPTION
 *  Set the history data for the matrix menu from the given history buffer
 * PARAMETERS
 *  history_ID          : [IN]        Id of history
 *  history_buffer      : [IN]        Buffer of history
 * RETURN VALUES
 *  MMI_TRUE    :   Set history success
 *  MMI_FALSE   :   Set history fail
 *****************************************************************************/
extern U8 set_matrix_menu_category_history(U16 history_ID, U8 *history_buffer);


/*****************************************************************************
 * FUNCTION
 *  get_matrix_menu_category_history
 * DESCRIPTION
 *  Get the history data for the matrix menu and store into the given history buffer
 * PARAMETERS
 *  history_ID          : [IN]        Id of history
 *  history_buffer      : [OUT]        Buffer of history
 * RETURNS
 *  void
 *****************************************************************************/
extern void get_matrix_menu_category_history(U16 history_ID, U8 *history_buffer);


/*****************************************************************************
 * FUNCTION
 *  set_object_list_category_history
 * DESCRIPTION
 *  Set history of object list
 * PARAMETERS
 *  history_ID          : [IN]        Id of history
 *  history_buffer      : [IN]        Buffer of history
 *  index               : [OUT]       Pointer of parameter to get index of history 
 * RETURN VALUES
 *  MMI_TRUE    :   Set history success
 *  MMI_FALSE   :   Set history fail
 *****************************************************************************/
extern U8 set_object_list_category_history(U16 history_ID, U8 *history_buffer, S32 *index);


/*****************************************************************************
 * FUNCTION
 *  get_object_list_category_history
 * DESCRIPTION
 *  Get history of object list
 * PARAMETERS
 *  history_ID          : [IN]        Id of history
 *  history_buffer      : [IN]        Buffer of history
 *  index               : [IN]        Index of history
 * RETURNS
 *  void
 *****************************************************************************/
extern void get_object_list_category_history(U16 history_ID, U8 *history_buffer, S32 *index);


/*****************************************************************************
 * FUNCTION
 *  get_slide_control_category_history
 * DESCRIPTION
 *  Get the history of slide control
 * PARAMETERS
 *  history_ID          : [IN]        Id of history
 *  history_buffer      : [OUT]        Buffer of history     
 * RETURNS
 *  void
 *****************************************************************************/
extern void get_slide_control_category_history(U16 history_ID, U8 *history_buffer);


/*****************************************************************************
 * FUNCTION
 *  set_slide_control_category_history
 * DESCRIPTION
 *  Set the history of slide control
 * PARAMETERS
 *  history_ID          : [IN]        Id of history
 *  history_buffer      : [IN]        Buffer of history      
 * RETURNS
 *  The number of items
 *****************************************************************************/
extern U8 set_slide_control_category_history(U16 history_ID, U8 *history_buffer);



/*****************************************************************************
* FUNCTION
*  wgui_fixed_list_register_get_flags_handler
* DESCRIPTION
*  register get flags handler
* PARAMETERS
*  get_flags_handler     : [IN]   Get flags of menuitem
* RETURNS
*  void
*****************************************************************************/
extern void wgui_fixed_list_register_get_flags_handler(GetMenuitemFlags get_flags_handler);

/* for new list_matrix menu style */
#ifdef __MMI_BI_DEGREE_MAIN_MENU_STYLE__


/*****************************************************************************
 * FUNCTION
 *  set_list_matrix_menu_category_history
 * DESCRIPTION
 *  Set the history data for the list_matrix main menu from the given history buffer
 * PARAMETERS
 *  history_ID          : [IN]        Id of history
 *  history_buffer      : [IN]        Buffer of history
 * RETURN VALUES
 *  MMI_TRUE    :   Set history success
 *  MMI_FALSE   :   Set history fail
 *****************************************************************************/
extern U8 set_list_matrix_menu_category_history(U16 history_ID, U8 *history_buffer);


/*****************************************************************************
 * FUNCTION
 *  get_list_matrix_menu_category_history
 * DESCRIPTION
 *  Get the history data for the list_matrix main menu and stores into the given history buffer
 * PARAMETERS
 *  history_ID          : [IN]        Id of history
 *  history_buffer      : [OUT]        Buffer of history
 * RETURNS
 *  void
 *****************************************************************************/
extern void get_list_matrix_menu_category_history(U16 history_ID, U8 *history_buffer);
#endif /* __MMI_BI_DEGREE_MAIN_MENU_STYLE__ */ 



/*****************************************************************************
 * FUNCTION
 *  wgui_save_key_handlers
 * DESCRIPTION
 *  Save all key handlers
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_save_key_handlers(void);


/*****************************************************************************
 * FUNCTION
 *  wgui_execute_key_handler
 * DESCRIPTION
 *  Execute key handler function
 * PARAMETERS
 *  key_code        : [IN]        The key code
 *  key_event       : [IN]        The key event
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_execute_key_handler(S32 key_code, S32 key_event);


/*****************************************************************************
 * FUNCTION
 *  wgui_restore_key_handlers
 * DESCRIPTION
 *  Restore  all key handlers
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_restore_key_handlers(void);

/* support wallpaper ob bottom */


/*****************************************************************************
 * FUNCTION
 *  hide_status_icons_bar0_by_transparent
 * DESCRIPTION
 *  Hide horizontal status icon bar by transparent color
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void hide_status_icons_bar0_by_transparent(void);


/*****************************************************************************
 * FUNCTION
 *  wgui_set_wallpaper_on_bottom
 * DESCRIPTION
 *  Set wallpaper on bottom or not
 * PARAMETERS
 *  state       : [IN]        Whether have wallpaper on bottom or not
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_set_wallpaper_on_bottom(MMI_BOOL state);


/*****************************************************************************
 * FUNCTION
 *  wgui_reset_wallpaper_on_bottom
 * DESCRIPTION
 *  Reset wallpaper on bottom
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_reset_wallpaper_on_bottom(void);


/*****************************************************************************
 * FUNCTION
 *  wgui_is_wallpaper_on_bottom
 * DESCRIPTION
 *  To check wallpaper on bottom or not
 * PARAMETERS
 *  void
 * RETURN VALUES
 *  MMI_TRUE    :   Have wallpaper on bottom 
 *  MMI_FALSE   :   Have not wallpaper on bottom
 *****************************************************************************/
extern MMI_BOOL wgui_is_wallpaper_on_bottom(void);


/* DOM-NOT_FOR_SDK-BEGIN */

/*****************************************************************************
 * FUNCTION
 *  ShowStatusIconsTitle
 * DESCRIPTION
 *  Show staus icon bar on main lcd screen
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void ShowStatusIconsTitle(void);
/* DOM-NOT_FOR_SDK-END */


/*****************************************************************************
 * FUNCTION
 *  fixed_list_goto_item
 * DESCRIPTION
 *  Go to the specified item.
 * PARAMETERS
 *  item_index      : [IN]        Item index
 * RETURNS
 *  void
 *****************************************************************************/
extern void fixed_list_goto_item(S32 item_index);

/* DOM-NOT_FOR_SDK-BEGIN */

/*****************************************************************************
 * FUNCTION
 *  dynamic_list_goto_item
 * DESCRIPTION
 *  Go to the specified item.
 * PARAMETERS
 *  item_index      : [IN]        Item index
 * RETURNS
 *  void
 *****************************************************************************/
extern void dynamic_list_goto_item(S32 item_index);


/*****************************************************************************
 * FUNCTION
 *  dynamic_list_goto_previous_item
 * DESCRIPTION
 *  Go to the previous item.
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void dynamic_list_goto_previous_item(void);

/* DOM-NOT_FOR_SDK-END */

/*****************************************************************************
 * FUNCTION
 *  dynamic_list_goto_item_no_redraw
 * DESCRIPTION
 *  Go to a specified item without redrawing. For example, set the highlighted item
 *  before the dynamic list is shown by the draw manager.
 * PARAMETERS
 *  item_index      : [IN]        Item index
 * RETURNS
 *  void
 *****************************************************************************/
extern void dynamic_list_goto_item_no_redraw(S32 item_index);

/* DOM-NOT_FOR_SDK-BEGIN */

extern void dynamic_matrix_goto_item_no_redraw(S32 item_index);


/*****************************************************************************
 * FUNCTION
 *  show_dynamic_list
 * DESCRIPTION
 *  Show the dynamic list.
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void show_dynamic_list(void);

/* DOM-NOT_FOR_SDK-END */

/*****************************************************************************
 * FUNCTION
 *  resize_fixed_list_to_content_height
 * DESCRIPTION
 *  Resize fixed list to content height
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void resize_fixed_list_to_content_height(void);


/*****************************************************************************
 * FUNCTION
 *  toggle_check_list_handle_left_softkey_up
 * DESCRIPTION
 *  The toggle check list left softkey handler
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 * REMARKS
 *  Change the string of left softkey according to stat of check box of current
 *  highlight item and change the string of right softkey to Done and
 *  set the function handler of right softkey
 *****************************************************************************/
extern void toggle_check_list_handle_left_softkey_up(void);


/*****************************************************************************
 * FUNCTION
 *  special_check_list_handle_left_softkey_up
 * DESCRIPTION
 *  The special check list left softkey handler
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 * REMARKS
 *  Change the string of left softkey according to stat of check box of current
 *  highlight item and change the string of right sofkkey to Done and
 *  set the function handler of right softkey
 *****************************************************************************/
extern void special_check_list_handle_left_softkey_up(void);


/*****************************************************************************
 * FUNCTION
 *  toggle_check_list_handle_item_select
 * DESCRIPTION
 *  Handler of toggle check list select
 * PARAMETERS
 *  h       : [IN]        Index of highlight item
 * RETURNS
 *  void
 *****************************************************************************/
extern void toggle_check_list_handle_item_select(S32 h);


/*****************************************************************************
 * FUNCTION
 *  SetCheckboxToggleRightSoftkeyFunctions
 * DESCRIPTION
 *  Set the right softkey function handlers of check box screen
 * PARAMETERS
 *  done_function      : [IN]        The done function pointer
 *  back_function      : [IN]        The back function pointer
 * RETURNS
 *  void
 *****************************************************************************/
void SetCheckboxToggleRightSoftkeyFunctions(void (*done_function) (void), void (*back_function) (void));


/*****************************************************************************
 * FUNCTION
 *  standard_check_list_handle_item_select
 * DESCRIPTION
 *  The standard handle of check list selected
 * PARAMETERS
 *  h       : [IN]        Index of the highlight item
 * RETURNS
 *  void
 * REMARKS
 *  Handles the selection of an item in a check list and draw the check list
 *  Toggles the given item between selected and unselected modes
 *****************************************************************************/
extern void standard_check_list_handle_item_select(S32 h);


/*****************************************************************************
 * FUNCTION
 *  standard_check_list_handle_left_softkey_up
 * DESCRIPTION
 *  The standard handler of check list response  left softkey up event
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void standard_check_list_handle_left_softkey_up(void);


/*****************************************************************************
 * FUNCTION
 *  standard_check_list_handle_left_softkey_up
 * DESCRIPTION
 *  The standard handler of check list response  left softkey up event
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void standard_check_list_handle_left_softkey_up(void);


/*****************************************************************************
 * FUNCTION
 *  RegisterHighlightHandler
 * DESCRIPTION
 *  Registers the highlight handler to be called in menu screens
 * PARAMETERS
*  f             : [IN]        Is a pointer to a highlight handler function
 * RETURNS
 *  void
 *****************************************************************************/
void RegisterHighlightHandler(void (*f) (S32 item_index));


/*****************************************************************************
 * FUNCTION
 *  MMI_dummy_highlight_handler
 * DESCRIPTION
 *  Dummy function of highlight handler
 * PARAMETERS
 *  item_index      : [IN]        Index of highlight item
 * RETURNS
 *  void
 *****************************************************************************/
extern void MMI_dummy_highlight_handler(S32 item_index);


/*****************************************************************************
 * FUNCTION
 *  ClearHighlightHandler
 * DESCRIPTION
 *  Clear all highlight handler
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void ClearHighlightHandler(void);



/*****************************************************************************
 * FUNCTION
 *  wgui_override_list_menu_slim_style
 * DESCRIPTION
 *  Override the current list menu icon style
 * PARAMETERS
 *  style       : [IN]        New style of menu
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_override_list_menu_slim_style(wgui_list_menu_slim_style_enum style);


/*****************************************************************************
 * FUNCTION
 *  wgui_restore_list_menu_slim_style
 * DESCRIPTION
 *  Restore the current list menu icon style
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_restore_list_menu_slim_style(void);



/*****************************************************************************
 * FUNCTION
 *  wgui_slide_control_set_focus
 * DESCRIPTION
 *  Set the focus on particular slide
 * PARAMETERS
 *  s       : [IN]        The pointer of slide_control
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_slide_control_set_focus(slide_control *s);


/*****************************************************************************
 * FUNCTION
 *  wgui_show_slide_control
 * DESCRIPTION
 *  Show slide control and blt out
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_show_slide_control(void);


/*****************************************************************************
 * FUNCTION
 *  wgui_show_slide_control_focus
 * DESCRIPTION
 *  Show the focus area of current slide control
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_show_slide_control_focus(void);


/*****************************************************************************
 * FUNCTION
 *  wgui_hide_slide_control_focus
 * DESCRIPTION
 *  Hide the focus area of current slide control
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_hide_slide_control_focus(void);


/*****************************************************************************
 * FUNCTION
 *  wgui_slide_control_previous
 * DESCRIPTION
 *  Move to previous slide control on screen
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_slide_control_previous(void);


/*****************************************************************************
 * FUNCTION
 *  wgui_slide_control_next
 * DESCRIPTION
 *  Move to next slide control on screen
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_slide_control_next(void);


/*****************************************************************************
 * FUNCTION
 *  wgui_show_slide_control_value
 * DESCRIPTION
 *  Show the value of current slide control
 * PARAMETERS
 *  x1          : [IN]        X coordinate of left-top corner of show area
 *  y1          : [IN]        Y coordinate of left-top corner of show area
 *  x2          : [IN]        X coordinate of right-bottom corner of show area
 *  y2          : [IN]        Y coordinate of right-bottom corner of show area
 *  value       : [IN]        The value to show
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_show_slide_control_value(S32 x1, S32 y1, S32 x2, S32 y2, S32 value);


/*****************************************************************************
 * FUNCTION
 *  wgui_slide_control_set_value
 * DESCRIPTION
 *  Set current slide control's value
 * PARAMETERS
 *  value       : [IN]        New value of slide control
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_slide_control_set_value(S32 value);



/*****************************************************************************
 * FUNCTION
 *  GetRootTitleIcon
 * DESCRIPTION
 *  Get root menu item icon in main menu
 * PARAMETERS
 *  ItemId          : [IN]        Index of main menu item 
 * RETURNS
 *  TitleIcon's id
 *****************************************************************************/
extern U16 GetRootTitleIcon(U16 ItemId);



/*****************************************************************************
 * FUNCTION
 *  MMI_apply_current_theme
 * DESCRIPTION
 *  Apply the current MMI theme to all MMI objects
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void MMI_apply_current_theme(void);



/*****************************************************************************
 * FUNCTION
 *  show_vertical_bar
 * DESCRIPTION
 *  Show volume vertical bar
 * PARAMETERS
 *  volume_level        : [IN]        Volume level
 * RETURNS
 *  void
 *****************************************************************************/
extern void show_vertical_bar(S32 volume_level);


/*****************************************************************************
 * FUNCTION
 *  show_vertical_bar_previous
 * DESCRIPTION
 *  Show volume vertical bar downgraded by 1
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void show_vertical_bar_previous(void);


/*****************************************************************************
 * FUNCTION
 *  show_vertical_bar_next
 * DESCRIPTION
 *  Show volume vertical bar upgraded by 1
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void show_vertical_bar_next(void);



/*****************************************************************************
 * FUNCTION
 *  set_multiline_inputbox_change
 * DESCRIPTION
 *  Set the changed flag of multiline inputbox
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void set_multiline_inputbox_change(void);


/*****************************************************************************
 * FUNCTION
 *  get_mutliline_inputbox_change_status
 * DESCRIPTION
 *  Get the multi line input box changed flag
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern S32 get_mutliline_inputbox_change_status(void);


/*****************************************************************************
 * FUNCTION
 *  set_multiline_inputbox_change_callback
 * DESCRIPTION
 *  Set multi line box contents changed flag
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void set_multiline_inputbox_change_callback(void);

#ifdef __MMI_UI_HINTS_IN_MENUITEM__


/*****************************************************************************
 * FUNCTION
 *  wgui_get_current_pop_up_description_string
 * DESCRIPTION
 *  Get pop up description's string
 * PARAMETERS
 *  void
 * RETURNS
 *  The popup description's string
 *****************************************************************************/
extern UI_string_type wgui_get_current_pop_up_description_string(void);


/*****************************************************************************
 * FUNCTION
 *  set_force_all_hints_in_menuitem
 * DESCRIPTION
 *  Force hints on all hints within menu item
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void set_force_all_hints_in_menuitem(void);


/*****************************************************************************
 * FUNCTION
 *  reset_force_all_hints_in_menuitem
 * DESCRIPTION
 *  Reset hints on all hints within menu item
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void reset_force_all_hints_in_menuitem(void);


/*****************************************************************************
 * FUNCTION
 *  is_set_force_all_hints_in_menuitem
 * DESCRIPTION
 *   To check whether force hints on all hints within menu item or not
 * PARAMETERS
 *  void
 * RETURNS
 *  whether this flag on or not
 *****************************************************************************/
extern S32 is_set_force_all_hints_in_menuitem(void);


/*****************************************************************************
 * FUNCTION
 *  set_force_hints_on_highlight_in_menuitem
 * DESCRIPTION
 *  Force hints on highlight only within menu item
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void set_force_hints_on_highlight_in_menuitem(void);


/*****************************************************************************
 * FUNCTION
 *  reset_force_hints_on_highlight_in_menuitem
 * DESCRIPTION
 *  Reset hints on highlight only within menu item
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void reset_force_hints_on_highlight_in_menuitem(void);


/*****************************************************************************
 * FUNCTION
 *  is_set_force_hints_on_highlight_in_menuitem
 * DESCRIPTION
 *  To check whether force hints on highlight only whithin menu item or not
 * PARAMETERS
 *  void
 * RETURNS
 *  Whether this flag on or not
 *****************************************************************************/
extern S32 is_set_force_hints_on_highlight_in_menuitem(void);


/*****************************************************************************
 * FUNCTION
 *  set_force_icon_on_highlight_only_in_menuitem
 * DESCRIPTION
 *  Force icon on highlight only within menu item
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void set_force_icon_on_highlight_only_in_menuitem(void);


/*****************************************************************************
 * FUNCTION
 *  reset_force_icon_on_highlight_only_in_menuitem
 * DESCRIPTION
 *  Reset icon on highlight only within menu item
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void reset_force_icon_on_highlight_only_in_menuitem(void);


/*****************************************************************************
 * FUNCTION
 *  is_set_force_icon_on_highlight_only_in_menuitem
 * DESCRIPTION
 *  To check whether force icon on highlight only within menu item or not
 * PARAMETERS
 *  void
 * RETURNS
 *  Whether this flag on or not
 *****************************************************************************/
extern S32 is_set_force_icon_on_highlight_only_in_menuitem(void);



/*****************************************************************************
 * FUNCTION
 *  reset_all_force_flags_for_hints
 * DESCRIPTION
 *  Reset all forced hint flag
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void reset_all_force_flags_for_hints(void);
#endif /* __MMI_UI_HINTS_IN_MENUITEM__ */ 


/*****************************************************************************
 * FUNCTION
 *  wgui_is_display_medium_font_string
 * DESCRIPTION
 *  To check whether the given string can be displayed by medium font or not
 * PARAMETERS
 *  string   : [IN]   Pointer of string
 *  len      : [IN]   Length of string
 * RETURNS
 *  MMI_BOOL
 *****************************************************************************/
extern MMI_BOOL wgui_is_display_medium_font_string(U8* string, S32 len);


/*****************************************************************************
 * FUNCTION
 *  wgui_category_clear_region_with_transparent_color
 * DESCRIPTION
 *  Clear the given region with transparent color
 * PARAMETERS
 *  x1    : [IN]    X coordinate of left-top corner of given region
 *  y1    : [IN]    Y coordinate of left-top corner of given region
 *  x2    : [IN]    X coordinate of right-bottom corner of given region
 *  y2    : [IN]    Y coordinate of right-bottom corner of given region
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_category_clear_region_with_transparent_color(S32 x1, S32 y1, S32 x2, S32 y2);

/* DOM-NOT_FOR_SDK-BEGIN */

/*****************************************************************************
 * FUNCTION
 *  wgui_category_dummy_callback
 * DESCRIPTION
 *  Dummy function
 * PARAMETERS
 *  result    : [IN]    GDI result
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_category_dummy_callback(GDI_RESULT result);

/* DOM-NOT_FOR_SDK-END */

/*****************************************************************************
* FUNCTION
*  wgui_fixed_list_register_display_callback
* DESCRIPTION
*  Register asyncdynamic list display callback handler
* PARAMETERS
*  async_display_callback      : [IN]     Display callback function to call after show asyncdynamic list
* RETURNS
*  void
*****************************************************************************/
extern void wgui_fixed_list_register_display_callback(AsyncDisplayCallback async_display_callback);


/*****************************************************************************
* FUNCTION
*  wgui_async_list_register_update_callback
* DESCRIPTION
*  Register asyncdynamic list update callback handler
* PARAMETERS
*  async_update_callback     : [IN]  Update callback function to update highlight index
* RETURNS
*  void
*****************************************************************************/
extern void wgui_async_list_register_update_callback(AsyncUpdateCallback async_update_callback);

/*****************************************************************************
* FUNCTION
*  wgui_fixed_list_register_indicator_display_callback
* DESCRIPTION
*  Register inline indicator display callback handler
* PARAMETERS
*  f      : [IN]     Display callback function to draw one indicator
* RETURNS
*  void
*****************************************************************************/
extern void wgui_fixed_list_register_indicator_display_callback(void (*f)(S32 index, S32 x, S32 y));

/*--------------------------------------------------------------------------------------------*/
/* Title                                                                                      */
/*--------------------------------------------------------------------------------------------*/


/*****************************************************************************
 * FUNCTION
 *  wgui_title_change
 * DESCRIPTION
 *  Change title left icon, right icon and text
 * PARAMETERS
 *  icon_ID         : [IN]       New image id of left icon
 *  icon2_ID        : [IN]       New image id of right icon 
 *  text            : [IN]       New text of title 
 *  flag            : [IN]       The flag is used to decide which function will be changed
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_title_change(U16 icon_ID, U16 icon2_ID, U8 *text, U32 flag);


/*****************************************************************************
 * FUNCTION
 *  wgui_title_set_thick
 * DESCRIPTION
 *  Set title thick or not
 * PARAMETERS
 *  thick       : [IN]    Whether set title thick or not
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_title_set_thick(MMI_BOOL thick); 

/* DOM-NOT_FOR_SDK-BEGIN */
extern void draw_title(void);
extern void wgui_stop_title_animation(void);
extern void draw_title_wap(void);
extern void ChangeTitleString(U8 *string);
extern void ChangeTitleIcon(U16 image_ID);
extern void ChangeTitle(U16 image_ID, U8 *string);
/* DOM-NOT_FOR_SDK-END */

#if defined(__MMI_TOUCH_SCREEN__)


/*****************************************************************************
 * FUNCTION
 *  register_touch_button_handlers
 * DESCRIPTION
 *  Register the button handlers for up and down event.
 * PARAMETERS
 *  f1_up       : [IN]        Up event handler of left button.
 *  f1_down     : [IN]        Down event handler of left button.
 *  f2_up       : [IN]        Up event handler of right button.
 *  f2_down     : [IN]        Down event handler of right button.
 * RETURNS
 *  void
 *****************************************************************************/
extern void register_touch_button_handlers(FuncPtr f1_up, FuncPtr f1_down, FuncPtr f2_up, FuncPtr f2_down);
#endif /* defined(__MMI_TOUCH_SCREEN__) */ 


/*****************************************************************************
 * FUNCTION
 *  wgui_set_touch_title_bar_buttons
 * DESCRIPTION
 *  Set the flag on which decide whether show buttons in title or not
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_set_touch_title_bar_buttons(void);


/*****************************************************************************
 * FUNCTION
 *  wgui_reset_touch_title_bar_buttons
 * DESCRIPTION
 *  Reset the flag off which decide whether show buttons in title or not
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_reset_touch_title_bar_buttons(void);


/*****************************************************************************
 * FUNCTION
 *  wgui_is_touch_title_bar_buttons
 * DESCRIPTION
 *  To check whether show buttons in title or not
 * PARAMETERS
 *  void
 * RETURN VALUES
 *  MMI_TRUE    :   Show buttons in title 
 *  MMI_FLASE   :   Not show buttons in title
 *****************************************************************************/
extern MMI_BOOL wgui_is_touch_title_bar_buttons(void);



/*****************************************************************************
 * FUNCTION
 *  wgui_title_set_menu_shortcut_number
 * DESCRIPTION
 *  Set the input number of shortcut
 * PARAMETERS
 *  menu_shortcut_number    : [IN]    The value of input number
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_title_set_menu_shortcut_number(S32 menu_shortcut_number);


/*****************************************************************************
 * FUNCTION
 *  wgui_title_get_menu_shortcut_number
 * DESCRIPTION
 *  Get the input number of shortcut
 * PARAMETERS
 *  void
 * RETURNS
 *  the shortcut number
 *****************************************************************************/
extern S32 wgui_title_get_menu_shortcut_number(void);


/*--------------------------------------------------------------------------------------------*/
/* Status icons                                                                               */
/*--------------------------------------------------------------------------------------------*/

/* Application specific APIs  */


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_show_status_icon
 * DESCRIPTION
 *  Set particular icon display flag
 * PARAMETERS
 *  icon_ID     : [IN]        Id of icon to set display flag
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_status_icon_show_status_icon(S16 icon_ID);


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_hide_status_icon
 * DESCRIPTION
 *  Reset particular icon flag
 * PARAMETERS
 *  icon_ID     : [IN]    Id of icon to reset flag     
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_status_icon_hide_status_icon(S16 icon_ID);


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_update_status_icons
 * DESCRIPTION
 *  Rearrange and refresh status icons
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_status_icon_update_status_icons(void);


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_update_SubLCD_status_icons
 * DESCRIPTION
 *  Rearrange and refresh sub LCD status icons
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_status_icon_update_SubLCD_status_icons(void);


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_change_status_icon_level
 * DESCRIPTION
 *  Change the level of status icon like battery icon ,signal icon
 * PARAMETERS
 *  icon_ID                 : [IN]        Id of icon to be change 
 *  level_percentage        : [IN]        Level of increase
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_status_icon_change_status_icon_level(S16 icon_ID, U8 level_percentage);


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_change_status_icon_image
 * DESCRIPTION
 *  Change the image of status icon like signal icon with line switch
 * PARAMETERS
 *  icon_ID         : [IN]        Id of icon to be change    
 *  image_ID        : [IN]        New image id of icon
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_status_icon_change_status_icon_image(S16 icon_ID, U16 image_ID);


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_animate_status_icon
 * DESCRIPTION
 *  Animate the status icon
 * PARAMETERS
 *  icon_ID     : [IN]     Id of icon to animate 
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_status_icon_animate_status_icon(S16 icon_ID);


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_blink_status_icon
 * DESCRIPTION
 *  Blinking particular icon
 * PARAMETERS
 *  icon_ID     : [IN]     Id of icon to blink   
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_status_icon_blink_status_icon(S16 icon_ID);


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_get_status_icon_flag
 * DESCRIPTION
 *  Get icon's flag
 * PARAMETERS
 *  icon_ID     : [IN]   Id of icon     
 * RETURNS
 *  The flag of status icon
 *****************************************************************************/
extern U32 wgui_status_icon_get_status_icon_flag(S16 icon_ID);


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_set_status_icon_flag
 * DESCRIPTION
 *  Set flag of icon
 * PARAMETERS
 *  icon_ID     : [IN]        Id of icon to set new flag  
 *  value       : [IN]        New flag of icon
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_status_icon_set_status_icon_flag(S16 icon_ID, U32 value);


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_set_status_icon_bar_layer
 * DESCRIPTION
 *  Set status icon bar's target layer
 * PARAMETERS
 *  status_bar_id           : [IN]        Id of status icon bar
 *  gdi_layer               : [IN]        New target layer of status icon bar
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_status_icon_set_status_icon_bar_layer(U8 status_bar_ID, GDI_HANDLE gdi_layer);


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_set_alpha_layer
 * DESCRIPTION
 *  Set alpha blending(abm) layer of status icon bar
 * PARAMETERS
 *  alpha_layer    : [IN]    New alpha blending(abm) layer of status icon bar
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_status_icon_set_alpha_layer(gdi_handle alpha_layer);


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_reset_alpha_layer
 * DESCRIPTION
 *  Clear alpha blending(abm) layer of status icon bar
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_status_icon_reset_alpha_layer(void);

#ifdef __MMI_TOUCH_SCREEN__


/*****************************************************************************
 * FUNCTION
 *  wgui_register_status_icon_pen_event_hdlr
 * DESCRIPTION
 *  Register pen event handlers for status icons.
 * PARAMETERS
 *  icon_id         : [IN]        Icon id of register pen event handler
 *  event_type      : [IN]        Event of status icon bar
 *  f               : [IN]        The pointer of event handler
 * RETURNS
 *  void
 * SEE ALSO
 *  wgui_status_icon_bar_pen_enum
 *****************************************************************************/
extern void wgui_register_status_icon_pen_event_hdlr(
                S16 icon_id,
                wgui_status_icon_bar_pen_enum event_type,
                FuncPtr f);


/*****************************************************************************
 * FUNCTION
 *  wgui_execute_status_icon_pen_event_hdlr
 * DESCRIPTION
 *  Execute pen event handlers for status icons.
 * PARAMETERS
 *  icon_id         : [IN]        Id of icon get the pen event
 *  event_type      : [IN]        Type of event
 * RETURN VALUES
 *  MMI_TRUE    :   The event handler exists and execute the pen event handler success
 *  MMI_FALSE   :   The event handler not exists
 * SEE ALSO
 *  wgui_status_icon_bar_pen_enum
 *****************************************************************************/
extern MMI_BOOL wgui_execute_status_icon_pen_event_hdlr(S16 icon_id, wgui_status_icon_bar_pen_enum event_type);


/*****************************************************************************
 * FUNCTION
 *  wgui_reset_status_icon_pen_event_hdlr
 * DESCRIPTION
 *  Clear all status icons pen event handlers
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_reset_status_icon_pen_event_hdlr(void);


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_translate_pen_event
 * DESCRIPTION
 *  Translate pen events for status icons.
 * PARAMETERS
 *  pen_event       : [IN]            The pen event
 *  x               : [IN]            X coordinate of pen
 *  y               : [IN]            Y coordinate of pen
 *  icon_id         : [OUT]           Icon id of be touch
 *  event_type      : [OUT]           Status icon bar's event 
 * RETURN VALUES
 *  MMI_TRUE    :   Response the pen event 
 *  MMI_FALSE   :   Not response the pen event
 * SEE ALSO
 *  mmi_pen_event_type_enum,wgui_status_icon_bar_pen_enum
 *****************************************************************************/
extern BOOL wgui_status_icon_translate_pen_event(
                mmi_pen_event_type_enum pen_event,
                S16 x,
                S16 y,
                S16 *icon_id,
                wgui_status_icon_bar_pen_enum *event_type);
#endif /* __MMI_TOUCH_SCREEN__ */ 

/* DOM-NOT_FOR_SDK-BEGIN */
#define ShowStatusIcon          wgui_status_icon_show_status_icon
#define HideStatusIcon          wgui_status_icon_hide_status_icon
#define UpdateStatusIcons       wgui_status_icon_update_status_icons
#define UpdateSubLCDStatusIcons wgui_status_icon_update_SubLCD_status_icons
#define ChangeStatusIconLevel   wgui_status_icon_change_status_icon_level
#define ChangeStatusIconImage   wgui_status_icon_change_status_icon_image    
#define AnimateStatusIcon       wgui_status_icon_animate_status_icon
#define BlinkStatusIcon         wgui_status_icon_blink_status_icon
#define get_status_icon_flag    wgui_status_icon_get_status_icon_flag   
#define set_status_icon_flag    wgui_status_icon_set_status_icon_flag   
#define set_status_icon_bar_layer       wgui_status_icon_set_status_icon_bar_layer  
/* DOM-NOT_FOR_SDK-END */


/*--------------------------------------------------------------------------------------------*/
/* Tab bar                                                                                */
/*--------------------------------------------------------------------------------------------*/

/*****************************************************************************
 * FUNCTION
 *  wgui_horizontal_tab_bar_store_first_displayed_item
 * DESCRIPTION
 *  Store the first display item index of horiizonta tab bar,use in left and right arrow key handler 
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_horizontal_tab_bar_store_first_displayed_item(void);


/*****************************************************************************
 * FUNCTION
 *  MMI_horizontal_tab_bar_first_displayed_on_screen_switched
 * DESCRIPTION
 *  Set the horizontal tab bar first displayed item index
 * PARAMETERS
 *  index			: [IN]	 Index of horizontal tab bar first displayed item    
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_horizontal_tab_bar_set_first_displayed_item(S8 index);


/*****************************************************************************
 * FUNCTION
 *  wgui_horizontal_tab_bar_get_first_displayed_item
 * DESCRIPTION
 *  Get the horizontal tab bar first displayed item index
 * PARAMETERS
 *  void    
 * RETURNS
 *  The tab bar first displayed item index
 *****************************************************************************/
S8 wgui_horizontal_tab_bar_get_first_displayed_item(void);


/*****************************************************************************
 * FUNCTION
 *  wgui_horizontal_tab_bar_set_use_pen_switch
 * DESCRIPTION
 *  Set the horizontal tab bar use pen switch item or not
 * PARAMETERS
 *  use_pen_switch		: [IN]		Whether use pen switch or not   
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_horizontal_tab_bar_set_use_pen_switch(MMI_BOOL use_pen_switch);


/*****************************************************************************
 * FUNCTION
 *  wgui_horizontal_tab_bar_get_use_pen_switch
 * DESCRIPTION
 *  Get the horizontal tab bar use pen switch item or not
 * PARAMETERS
 *  void    
 * RETURNS
 *  Whether use pen switch or not
 *****************************************************************************/
MMI_BOOL wgui_horizontal_tab_bar_get_use_pen_switch(void);


/*--------------------------------------------------------------------------------------------*/
/* Menu                                                                                  */
/*--------------------------------------------------------------------------------------------*/

//wgui_dynamic_menus.h


/*****************************************************************************
 * FUNCTION
 *  DynamicListIncreaseItem
 * DESCRIPTION
 *  Increase the total number of dynamic item.
 * PARAMETERS
 *  steps       : [IN]        Number of items to be increased
 * RETURNS
 *  void
 *****************************************************************************/
extern void DynamicListIncreaseItem(S16 steps);


/*****************************************************************************
 * FUNCTION
 *  DynamicListGetItems
 * DESCRIPTION
 *  Return total number of dynamic items and the requested item text from the dynamic buffer.
 *  The caller must take care of the validity of the specified item in the buffer.
 * PARAMETERS
 *  string          : [OUT]       Requested item text
 *  item_index      : [IN]        Index of the requested item
 * RETURNS
 *  total number of dynamic items
 *****************************************************************************/
extern S32 DynamicListGetItems(U8 *string, U16 item_index);


/*****************************************************************************
 * FUNCTION
 *  DynamicListGetHighlightedItem
 * DESCRIPTION
 *  Return the index and text of the highlighted item.
 * PARAMETERS
 *  string      : [OUT]       Highlighted item text
 * RETURNS
 *  Highlighted item index
 *****************************************************************************/
extern S32 DynamicListGetHighlightedItem(U8 *string);


/*****************************************************************************
 * FUNCTION
 *  DynamicGetFirstDisplayedItem
 * DESCRIPTION
 *  Return the index and text of first displayed item.
 * PARAMETERS
 *  string      : [OUT]       First displayed item text
 * RETURNS
 *  First displayed item index
 *****************************************************************************/
extern S32 DynamicGetFirstDisplayedItem(U8 *string);

#ifdef __MMI_UI_TWO_LINE_MENUITEM_STYLES__


/*****************************************************************************
 * FUNCTION
 *  wgui_set_pfn_to_get_current_menu_item_properties
 * DESCRIPTION
 *  Set the callback function to get current menu item property.
 * PARAMETERS
 *  pfn     : [IN]        callback to get the two-line item property
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_set_pfn_to_get_current_menu_item_properties(wgui_get_two_line_menu_item_properties pfn);


/*****************************************************************************
 * FUNCTION
 *  wgui_set_pfn_to_get_display_style
 * DESCRIPTION
 *  Set the callback function to get current menu item display style.
 * PARAMETERS
 *  pfn     : [IN]        get display style callback
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_set_pfn_to_get_display_style(wgui_get_display_style pfn);
#endif /* __MMI_UI_TWO_LINE_MENUITEM_STYLES__ */ 


/*--------------------------------------------------------------------------------------------*/
/* calendar                                                                                   */
/*--------------------------------------------------------------------------------------------*/
/* Calendar wrappers        */

/*****************************************************************************
 * FUNCTION
 *  wgui_calendar_set_bg
 * DESCRIPTION
 *  Set background image for normal full screen calendar.
 * REMARKS
 *  At present, normal calendar will use sub menu's background filler as its background.
 *  But if user call this API, calendar will use the given image as its background.
 * PARAMETERS
 *  bg_id            : [IN] Is the image ID of background
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_calendar_set_bg(UI_image_ID_type bg_id);

/*****************************************************************************
 * FUNCTION
 *  wgui_calendar_set_select_horizontal1
 * DESCRIPTION
 *  Set calendar's left horizontal select.
 * REMARKS
 *  If icon image of select is NULL, we will use a color to fill a solid rectangle.
 * PARAMETERS
 *  left_up              : [IN] Is the image ID of left icon in its pen up state
 *  left_down            : [IN] Is the image ID of left icon in its pen down state
 *  right_up             : [IN] Is the image ID of right icon in its pen up state
 *  right_down           : [IN] Is the image ID of right icon in its pen down state
 *  left_filler_index    : [IN] Is the filler's color index of left icon 
 *  right_filler_index   : [IN] Is the filler's color index of right icon 
 *  string               : [IN] Is the text content
 *  align_type           : [IN] Is the align type of text
 * RETURNS
 *  void
 * SEE ALSO
 *  You should also know GUI_CALENDAR_SELECT_ALIGN_TYPE as well.
 *****************************************************************************/
extern void wgui_calendar_set_select_horizontal1(
            UI_image_ID_type left_up,
            UI_image_ID_type left_down,
            UI_image_ID_type right_up,
            UI_image_ID_type right_down,
             U8 left_filler_index,
             U8 right_filler_index,
            U8 *string,
            U32 align_type);

/*****************************************************************************
 * FUNCTION
 *  wgui_calendar_set_select_horizontal1_string
 * DESCRIPTION
 *  Set string to calendar's left horizontal select.
 * PARAMETERS
 *  string               : [IN] Is the text content
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_calendar_set_select_horizontal1_string(U8 *string);

/*****************************************************************************
 * FUNCTION
 *  wgui_calendar_set_select_horizontal1_handler
 * DESCRIPTION
 *  Set callback function handler to calendar's left horizontal select.
 * PARAMETERS
 *  left_handler         : [IN] Is the callback function handler of left icon
 *  right_handler        : [IN] Is the callback function handler of right icon
 * RETURNS
 *  void
 * SEE ALSO
 *  You should also know GUICalendarV2EventFuncPtr as well.
 *****************************************************************************/
extern void wgui_calendar_set_select_horizontal1_handler(
            GUICalendarV2EventFuncPtr left_handler,
            GUICalendarV2EventFuncPtr right_handler);

/*****************************************************************************
 * FUNCTION
 *  wgui_calendar_set_select_horizontal2
 * DESCRIPTION
 *  Set calendar's right horizontal select.
 * REMARKS
 *  If icon image of select is NULL, we will use a color to fill a solid rectangle.
 * PARAMETERS
 *  left_up              : [IN] Is the image ID of left icon in its pen up state
 *  left_down            : [IN] Is the image ID of left icon in its pen down state
 *  right_up             : [IN] Is the image ID of right icon in its pen up state
 *  right_down           : [IN] Is the image ID of right icon in its pen down state
 *  left_filler_index    : [IN] Is the filler's color index of left icon 
 *  right_filler_index   : [IN] Is the filler's color index of right icon 
 *  string               : [IN] Is the text content
 *  align_type           : [IN] Is the align type of text
 * RETURNS
 *  void
 * SEE ALSO
 *  You should also know GUI_CALENDAR_SELECT_ALIGN_TYPE as well.
 *****************************************************************************/
extern void wgui_calendar_set_select_horizontal2(
            UI_image_ID_type left_up,
            UI_image_ID_type left_down,
            UI_image_ID_type right_up,
            UI_image_ID_type right_down,
             U8 left_filler_index,
             U8 right_filler_index,
            U8 *string,
            U32 align_type);

/*****************************************************************************
 * FUNCTION
 *  wgui_calendar_set_select_horizontal2_string
 * DESCRIPTION
 *  Set string to calendar's right horizontal select.
 * PARAMETERS
 *  string               : [IN] Is the text content
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_calendar_set_select_horizontal2_string(U8 *string);

/*****************************************************************************
 * FUNCTION
 *  wgui_calendar_set_select_horizontal2_handler
 * DESCRIPTION
 *  Set callback function handler to calendar's right horizontal select.
 * PARAMETERS
 *  left_handler         : [IN] Is the callback function handler of left icon
 *  right_handler        : [IN] Is the callback function handler of right icon
 * RETURNS
 *  void
 * SEE ALSO
 *  You should also know GUICalendarV2EventFuncPtr as well.
 *****************************************************************************/
extern void wgui_calendar_set_select_horizontal2_handler(
            GUICalendarV2EventFuncPtr left_handler,
            GUICalendarV2EventFuncPtr right_handler);

/*****************************************************************************
 * FUNCTION
 *  wgui_calendar_set_select_vertical
 * DESCRIPTION
 *  Set calendar's vertical select.
 * PARAMETERS
 *  top_up           : [IN] is the image ID of top icon in its pen up state
 *  top_down         : [IN] Is the image ID of top icon in its pen down state
 *  bottom_up        : [IN] Is the image ID of bottom icon in its pen up state
 *  bottom_down      : [IN] Is the image ID of bottom icon in its pen down state
 *  string_list      : [IN] Is the text content of vertical select's every row
 *  align_type       : [IN] Is the align type of text
 * RETURNS
 *  void
 * SEE ALSO
 *  You should also know GUI_CALENDAR_SELECT_ALIGN_TYPE as well.
 *****************************************************************************/
extern void wgui_calendar_set_select_vertical(
            UI_image_ID_type top_up,
            UI_image_ID_type top_down,
            UI_image_ID_type bottom_up,
            UI_image_ID_type bottom_down,
            U8 **string_list,
            U32 align_type);

/*****************************************************************************
 * FUNCTION
 *  wgui_calendar_set_select_vertical_string
 * DESCRIPTION
 *  Set string to calendar's vertical select.
 * PARAMETERS
 *  string_list          : [IN] Is the text content of vertical select's every row
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_calendar_set_select_vertical_string(U8 **string_list);

/*****************************************************************************
 * FUNCTION
 *  wgui_calendar_set_select_vertical_handler
 * DESCRIPTION
 *  Set callback function handler to calendar's vertical select.
 * PARAMETERS
 *  top_handler          : [IN] Is the callback function handler of top icon
 *  bottom_handler       : [IN] Is the callback function handler of bottom icon
 * RETURNS
 *  void
 * SEE ALSO
 *  You should also know GUICalendarV2EventFuncPtr as well.
 *****************************************************************************/
extern void wgui_calendar_set_select_vertical_handler(
            GUICalendarV2EventFuncPtr top_handler,
            GUICalendarV2EventFuncPtr bottom_handler);

/*****************************************************************************
 * FUNCTION
 *  wgui_calendar_set_normal_title
 * DESCRIPTION
 *  Set title to calendar.
 * PARAMETERS
 *  string       : [IN] Is the text content of title
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_calendar_set_normal_title(U8 *string);


#ifdef __MMI_MENSTRUAL__
/*****************************************************************************
 * FUNCTION
 *  wgui_calendar_set_menstrual_title_property
 * DESCRIPTION
 *  
 * PARAMETERS
 *  left_up        : [IN] Is the image ID of left icon in its pen up state
 *  left_down      : [IN] Is the image ID of left icon in its pen down state
 *  right_up       : [IN] Is the image ID of right icon in its pen up state
 *  right_down     : [IN] Is the image ID of right icon in its pen down state
 *  left_handler   : [IN] Is the callback function handler of left icon
 *  right_handler  : [IN] Is the callback function handler of right icon
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_calendar_set_menstrual_title_property(
             UI_image_ID_type left_up,
             UI_image_ID_type left_down,
             UI_image_ID_type right_up,
             UI_image_ID_type right_down,
             GUICalendarV2EventFuncPtr left_handler,
             GUICalendarV2EventFuncPtr right_handler);
#endif /* __MMI_MENSTRUAL__ */


/*****************************************************************************
 * FUNCTION
 *  wgui_calendar_set_horizontal_string
 * DESCRIPTION
 *  Set horizontal string to calendar.
 * PARAMETERS
 *  string_list          : [IN] Is the text content of horizontal string
 *  underline_index      : [IN] Is the index of underline which represnets the current date of calendar
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_calendar_set_horizontal_string(U8 **string_list, S32 underline_index);

/*****************************************************************************
 * FUNCTION
 *  wgui_calendar_set_cell_array
 * DESCRIPTION
 *  Set content to calendar cell.
 * PARAMETERS
 *  cell                         : [IN] Is the cell of calendar
 *  cell_bg_fill_type            : [IN] Is the background fill style of one cell
 *  cell_text_display_type       : [IN] Is the text display style of one cell
 *  cell_icon_display_type       : [IN] Is the icon display style of one cell
 * RETURNS
 *  void
 * SEE ALSO
 *  You should also know GUI_CALENDAR_CELL_ICON_DISPLAY_TYPE,
 *  GUI_CALENDAR_CELL_TEXT_DISPLAY_TYPE & GUI_CALENDAR_CELL_BG_FILL_TYPE
 *  as well.
 *****************************************************************************/
extern void wgui_calendar_set_cell_array(
            gui_calendar_cell_struct *cell,
            U8 cell_bg_fill_type,
            U8 cell_text_display_type,
            U8 cell_icon_display_type);

/*****************************************************************************
 * FUNCTION
 *  wgui_calendar_set_current
 * DESCRIPTION
 *  Set row and column number of current day to calendar.
 * PARAMETERS
 *  current_row          : [IN] Is the row number of current day
 *  current_column       : [IN] Is the column number of current day
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_calendar_set_current(U32 current_row, U32 current_column);

/*****************************************************************************
 * FUNCTION
 *  wgui_calendar_set_highlight
 * DESCRIPTION
 *  Set row and column number of highlighted day to calendar.
 * PARAMETERS
 *  current_row          : [IN] Is the row number of highlighted day
 *  current_column       : [IN] Is the column number of highlighted day
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_calendar_set_highlight(U32 highlighted_row, U32 highlighted_column);

/*****************************************************************************
 * FUNCTION
 *  wgui_calendar_set_popup
 * DESCRIPTION
 *  Set popup information to calendar.
 * PARAMETERS
 *  message1         : [IN] Is the popup information of first line
 *  message2         : [IN] Is the popup information of second line
 *  message3         : [IN] Is the popup information of third line
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_calendar_set_popup(U8 *message1, U8 *message2, U8 *message3);

#ifdef __MMI_UI_CALENDAR_WITH_INFO_BOX__

/*****************************************************************************
 * FUNCTION
 *  wgui_calendar_set_infobox
 * DESCRIPTION
 *  Set content of infobox to calendar.
 * REMARKS
 *  This API is only vailable when calendar supports infobox.
 * PARAMETERS
 *  icon_list        : [IN] Is the icon of infobox
 *  string_list      : [IN] Is the text of infobox
 *  entry_num        : [IN] Is the entry number of infobox
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_calendar_set_infobox(UI_image_ID_type *icon_list, U8 **string_list, U8 entry_num);

#endif /*__MMI_UI_CALENDAR_WITH_INFO_BOX__*/

/*****************************************************************************
 * FUNCTION
 *  wgui_calendar_register_cell_highlight_handler
 * DESCRIPTION
 *  Set the highlight handler of cell to calendar.
 * PARAMETERS
 *  app_cell_highlight_handler   : [IN] Is the highlight handler of cell
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_calendar_register_cell_highlight_handler(MMI_BOOL (*app_cell_highlight_handler)(U32 highlighted_row, U32 highlighted_column));

/*****************************************************************************
 * FUNCTION
 *  wgui_calendar_register_left_arrow_key_handler
 * DESCRIPTION
 *  Set the key handler of left arrow key to calendar.
 * PARAMETERS
 *  app_cell_highlight_handler   : [IN] Is the key handler of left arrow key
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_calendar_register_left_arrow_key_handler(MMI_BOOL (*app_left_arrow_key_handler)(U32 highlighted_row, U32 highlighted_column));

/*****************************************************************************
 * FUNCTION
 *  wgui_calendar_register_right_arrow_key_handler
 * DESCRIPTION
 *  Set the key handler of right arrow key to calendar.
 * PARAMETERS
 *  app_cell_highlight_handler   : [IN] Is the key handler of right arrow key
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_calendar_register_right_arrow_key_handler(MMI_BOOL (*app_right_arrow_key_handler)(U32 highlighted_row, U32 highlighted_column));

/*****************************************************************************
 * FUNCTION
 *  wgui_calendar_register_up_arrow_key_handler
 * DESCRIPTION
 *  Set the key handler of up arrow key to calendar.
 * PARAMETERS
 *  app_cell_highlight_handler   : [IN] Is the key handler of up arrow key
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_calendar_register_up_arrow_key_handler(MMI_BOOL (*app_up_arrow_key_handler)(U32 highlighted_row, U32 highlighted_column));

/*****************************************************************************
 * FUNCTION
 *  wgui_calendar_register_down_arrow_key_handler
 * DESCRIPTION
 *  Set the key handler of down arrow key to calendar.
 * PARAMETERS
 *  app_cell_highlight_handler   : [IN] Is the key handler of down arrow key
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_calendar_register_down_arrow_key_handler(MMI_BOOL (*app_down_arrow_key_handler)(U32 highlighted_row, U32 highlighted_column));

/*****************************************************************************
 * FUNCTION
 *  wgui_calendar_register_up_side_key_handler
 * DESCRIPTION
 *  Set the key handler of up side key to calendar.
 * PARAMETERS
 *  app_cell_highlight_handler   : [IN] Is the key handler of up side key
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_calendar_register_up_side_key_handler(MMI_BOOL (*app_up_side_key_handler)(U32 highlighted_row, U32 highlighted_column));

/*****************************************************************************
 * FUNCTION
 *  wgui_calendar_register_down_side_key_handler
 * DESCRIPTION
 *  Set the key handler of down side key to calendar.
 * PARAMETERS
 *  app_cell_highlight_handler   : [IN] Is the key handler of down side key
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_calendar_register_down_side_key_handler(MMI_BOOL (*app_down_side_key_handler)(U32 highlighted_row, U32 highlighted_column));

#if defined (__MMI_CALENDAR_ON_IDLE_SCREEN__)

/*****************************************************************************
 * FUNCTION
 *  wgui_calendar_set_detail
 * DESCRIPTION
 *  Set detail content to idle calendar.
 * REMARKS
 *  This API only available when the calendar's type is GUI_CALENDAR_IDLE_EXTEND.
 * PARAMETERS
 *  day              : [IN] Is the detail information of current day
 *  current          : [IN] Is the detail informstion of current date
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_calendar_set_detail(U8 *day, U8 *current);

/*****************************************************************************
 * FUNCTION
 *  wgui_calendar_idle_set_title
 * DESCRIPTION
 *  Set title to idle calendar.
 * REMARKS
 *  This API only available when the calendar's type is GUI_CALENDAR_IDLE_NORMAL.
 * PARAMETERS
 *  month           : [IN] Is the month information
 *  year            : [IN] Is the year informstion
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_calendar_idle_set_title(U8 *month, U8 *year);

/*****************************************************************************
 * FUNCTION
 *  wgui_calendar_set_idle_target_layer
 * DESCRIPTION
 *  
 * PARAMETERS
 *  target_layer    [IN]    Is the target layer
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_calendar_set_idle_target_layer(GDI_HANDLE target_layer);

#if defined (__MMI_VUI_HOMESCREEN__)
/*****************************************************************************
 * FUNCTION
 *  wgui_calendar_set_in_venus
 * DESCRIPTION
 *  Set idle calendar is in venus or not.
 * REMARKS
 *  This API only available when the calendar's type is GUI_CALENDAR_IDLE_NORMAL.
 * PARAMETERS
 *  is_in_venus         : [IN] Is in venus nor not
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_calendar_set_in_venus(MMI_BOOL is_in_venus);

/*****************************************************************************
 * FUNCTION
 *  wgui_calendar_register_update_screen_callback
 * DESCRIPTION
 *  Register calendar's update screen callback of venus.
 * REMARKS
 *  This API only available when the calendar's type is GUI_CALENDAR_IDLE_NORMAL.
 * PARAMETERS
 *  update_screen_callback          : [IN] Is update screen callback
 *  update_screen_handle            : [IN] In update screen handle
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_calendar_register_update_screen_callback(
             void (*update_screen_callback)(void *handle),
             void *update_screen_handle);

/*****************************************************************************
 * FUNCTION
 *  wgui_calendar_set_venus_update
 * DESCRIPTION
 *  The update of idle calendar is triggered by venus.
 * REMARKS
 *  This API only available when the calendar's type is GUI_CALENDAR_IDLE_NORMAL.
 * PARAMETERS
 *  is_venus_update         : [IN] The update is ocme from venus
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_calendar_set_venus_update(MMI_BOOL is_venus_update);
#endif /* __MMI_VUI_HOMESCREEN__ */

#endif /* (__MMI_CALENDAR_ON_IDLE_SCREEN__) */

/*--------------------------------------------------------------------------------------------*/
/* datetime                                                                                   */
/*--------------------------------------------------------------------------------------------*/
/* datetime */

/*****************************************************************************
 * FUNCTION
 *  date_string
 * DESCRIPTION
 *  Use flag that passed in to get different string of date information.
 * PARAMETERS
 *  t            : [IN] Is the time which to be converted to a string 
 *  s            : [IN/OUT] Is the pointer of parameter to get string
 *  flags        : [IN] Is the flag of DT_DISPLAY_TYPE
 * RETURNS
 *  void
 * SEE ALSO
 *  You should also know DT_DISPLAY_TYPE as well.
 *****************************************************************************/
extern void date_string(UI_time *t, UI_string_type s, U32 flags);


/*****************************************************************************
 * FUNCTION
 *  time_string
 * DESCRIPTION
 *  Use flag that passed in to get different string of time information.
 * PARAMETERS
 *  t            : [IN] Is the time which to be converted to a string 
 *  s            : [IN/OUT] Is the pointer of parameter to get string
 *  flags        : [IN] Is the flag of DT_DISPLAY_TYPE
 * RETURNS
 *  void
 * SEE ALSO
 *  You should also know DT_DISPLAY_TYPE as well.
 *****************************************************************************/
extern void time_string(UI_time *t, UI_string_type s, U32 flags);


/*****************************************************************************
 * FUNCTION
 *  duration_string
 * DESCRIPTION
 *  Use flag that passed in to get different string of duration information.
 * PARAMETERS
 *  t            : [IN] Is the time which to be converted to a string 
 *  s            : [IN/OUT] Is the pointer of parameter to get string
 *  flags        : [IN] Is the flag of DT_DISPLAY_TYPE
 * RETURNS
 *  void
 * SEE ALSO
 *  You should also know DT_DISPLAY_TYPE as well.
 *****************************************************************************/
extern void duration_string(UI_time *t, UI_string_type s, U32 flags);


/*****************************************************************************
 * FUNCTION
 *  set_inline_date_boundary
 * DESCRIPTION
 *  Set boundary value of date input.
 * PARAMETERS
 *  date_year_max        : [IN] Is the year's max boundary value of date input
 *  date_year_min        : [IN] Is the year's min boundary value of date input
 *  date_month_max       : [IN] Is the month's max boundary value of date input
 *  date_month_min       : [IN] Is the month's min boundary value of date input
 * RETURNS
 *  void
 *****************************************************************************/
extern void set_inline_date_boundary(S32 date_year_max, S32 date_year_min, S32 date_month_max, S32 date_month_min);


/*****************************************************************************
 * FUNCTION
 *  update_mainlcd_dt_display
 * DESCRIPTION
 *  Update date and time's display of main LCD.
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void update_mainlcd_dt_display(void);


/*****************************************************************************
 * FUNCTION
 *  update_sublcd_dt_display
 * DESCRIPTION
 *  Update date and time's display of sub LCD.
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void update_sublcd_dt_display(void);

/* DOM-NOT_FOR_SDK-BEGIN */

/*****************************************************************************
 * FUNCTION
 *  wgui_datetime_update_mainlcd_cm_duration
 * DESCRIPTION
 *  Update call time duration.
 * REMARKS
 *  This API is only provided for CM to refresh the duration of call screen.
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_datetime_update_mainlcd_cm_duration(void);

/* DOM-NOT_FOR_SDK-END */

/* DOM-NOT_FOR_SDK-BEGIN */

/*****************************************************************************
 * FUNCTION
 *  set_time_duration
 * DESCRIPTION
 *  Set the time to duration object of main LCD.
 * PARAMETERS
 *  t                : [IN] Is the start time of duration
 *  duration         : [IN] Is the start duration
 * RETURNS
 *  void
 *****************************************************************************/
extern void set_time_duration(UI_time *t, U32 duration);
/* DOM-NOT_FOR_SDK-END */

/* DOM-NOT_FOR_SDK-BEGIN */

/*****************************************************************************
 * FUNCTION
 *  get_time_duration
 * DESCRIPTION
 *  Get duration object of main LCD.
 * PARAMETERS
 *  t        : [IN]  Is the time duration object
 * RETURNS
 *  void
 *****************************************************************************/
extern void get_time_duration(UI_time *t);

/* DOM-NOT_FOR_SDK-END */


/* DOM-NOT_FOR_SDK-BEGIN */

/*****************************************************************************
 * FUNCTION
 *  time_update_duration
 * DESCRIPTION
 *  Update time duration
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void time_update_duration(void);

/* DOM-NOT_FOR_SDK-END */

/*****************************************************************************
 * FUNCTION
 *  set_time_display
 * DESCRIPTION
 *  Set position and dimension to duration object.
 * PARAMETERS
 *  flags        : [IN] Is the flag of DT_DISPLAY_TYPE
 *  x            : [IN] Is the x position of duration
 *  y            : [IN] Is the y position of duration
 *  width        : [IN] Is the width of duration
 *  height       : [IN] Is the height of duration
 * RETURNS
 *  void
 *****************************************************************************/
extern void set_time_display(U32 flags, S32 x, S32 y, S32 width, S32 height);


/*****************************************************************************
 * FUNCTION
 *  close_main_LCD_time_display
 * DESCRIPTION
 *  Disable duration's display of main LCD.
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void close_main_LCD_time_display(void);


/*****************************************************************************
 * FUNCTION
 *  CategoryScreenUpdateDuration
 * DESCRIPTION
 *  Update the duration of category 19.
 * PARAMETERS
 *  t        : [IN] Is the begin time of duration
 * RETURNS
 *  void
 *****************************************************************************/
extern void Cat19UpdateDuration(MYTIME *t);

#ifdef __MMI_UI_TECHNO_IDLESCREEN_BAR__
/*****************************************************************************
 * FUNCTION
 *  wgui_set_clock_type
 * DESCRIPTION
 *  Set the type of the current clock to be displayed.
 * PARAMETERS
 *  clock_type       : [IN] Is one type of MMI_CLOCK_TYPE
 * RETURNS
 *  void
 * SEE ALSO
 *  You should also know MMI_CLOCK_TYPE as well.
 *****************************************************************************/
extern void wgui_set_clock_type(U8 clock_type);

#endif /* __MMI_UI_TECHNO_IDLESCREEN_BAR__ */


/*--------------------------------------------------------------------------------------------*/
/* icon_bar                                                                                   */
/*--------------------------------------------------------------------------------------------*/

/*****************************************************************************
 * FUNCTION
 *  wgui_icon_bar_register_highlight_handler
 * DESCRIPTION
 *  Set highlight callback fuction to icon bar object.
 * PARAMETERS
 *  f            : [IN] Is the handler of highlight function
 * RETURNS
 *  void
 *****************************************************************************/
extern void wgui_icon_bar_register_highlight_handler(void (*f)(S32 index));


/*--------------------------------------------------------------------------------------------*/
/* softkey                                                                                    */
/*--------------------------------------------------------------------------------------------*/
/* softkey */

/*****************************************************************************
 * FUNCTION
 *  set_left_softkey_function
 * DESCRIPTION
 *  Set the key event function to left softkey.
 * PARAMETERS
 *  FUNC      : [IN] Is the function handler which will be called when one specific key event occurs
 *  KEY_TYPE  : [IN] Is the key event
 * RETURNS
 *  void
 *****************************************************************************/
#define set_left_softkey_function(FUNC, KEY_TYPE)       set_softkey_function(FUNC, KEY_TYPE, MMI_LEFT_SOFTKEY)

/*****************************************************************************
 * FUNCTION
 *  set_right_softkey_function
 * DESCRIPTION
 *  Set the key event function to right softkey.
 * PARAMETERS
 *  FUNC      : [IN] Is the function handler which will be called when one specific key event occurs
 *  KEY_TYPE  : [IN] Is the key event
 * RETURNS
 *  void
 *****************************************************************************/
#define set_right_softkey_function(FUNC, KEY_TYPE)      set_softkey_function(FUNC, KEY_TYPE, MMI_RIGHT_SOFTKEY)

/*****************************************************************************
 * FUNCTION
 *  get_left_softkey_function
 * DESCRIPTION
 *  Get the key event function of left softkey.
 * REMARKS
 *  This API is the corresponding API of set_left_softkey_function.
 * PARAMETERS
 *  k        : [IN] Is the key event
 * RETURNS
 *  FuncPtr
 * RETURN VALUES
 *  FuncPtr : Is the function handler which will be called when one specific key event occurs.
 *****************************************************************************/
#define get_left_softkey_function(KEY_TYPE)             get_softkey_function(KEY_TYPE, MMI_LEFT_SOFTKEY)

/*****************************************************************************
 * FUNCTION
 *  get_right_softkey_function
 * DESCRIPTION
 *  Get the key event function of right softkey.
 * REMARKS
 *  This API is the corresponding API of set_right_softkey_function.
 * PARAMETERS
 *  k        : [IN] Is the key event
 * RETURNS
 *  FuncPtr
 * RETURN VALUES
 *  FuncPtr : Is the function handler which will be called when one specific key event occurs.
 *****************************************************************************/
#define get_right_softkey_function(KEY_TYPE)            get_softkey_function(KEY_TYPE, MMI_RIGHT_SOFTKEY)


/*****************************************************************************
 * FUNCTION
 *  execute_left_softkey_function
 * DESCRIPTION
 *  Execute key event function of left softkey when particular softkey event occurs.
 * PARAMETERS
 *  k        : [IN] Is the key event
 * RETURNS
 *  void
 *****************************************************************************/
#define execute_left_softkey_function(KEY_TYPE)         execute_softkey_function(KEY_TYPE,MMI_LEFT_SOFTKEY)

/*****************************************************************************
 * FUNCTION
 *  execute_right_softkey_function
 * DESCRIPTION
 *  Execute key event function of right softkey when particular softkey event occurs.
 * PARAMETERS
 *  k        : [IN] Is the key event
 * RETURNS
 *  void
 *****************************************************************************/
#define execute_right_softkey_function(KEY_TYPE)        execute_softkey_function(KEY_TYPE,MMI_RIGHT_SOFTKEY)


/*****************************************************************************
 * FUNCTION
 *  redraw_left_softkey
 * DESCRIPTION
 *  Redraw the left softkey's content to main LCD and blt it to main LCD.
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
#define redraw_left_softkey()                           redraw_softkey(MMI_LEFT_SOFTKEY)

/*****************************************************************************
 * FUNCTION
 *  redraw_right_softkey
 * DESCRIPTION
 *  Redraw the right softkey's content to main LCD and blt it to main LCD.
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
#define redraw_right_softkey()                          redraw_softkey(MMI_RIGHT_SOFTKEY)

#ifdef __MMI_WGUI_CSK_ENABLE__

/*****************************************************************************
 * FUNCTION
 *  redraw_center_softkey
 * DESCRIPTION
 *  Redraw the center softkey's content to main LCD and blt it to main LCD.
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
#define redraw_center_softkey()                         redraw_softkey(MMI_CENTER_SOFTKEY)
#endif


/*****************************************************************************
 * FUNCTION
 *  set_softkey_function
 * DESCRIPTION
 *  Set the key event function to one specific softkey.
 * REMARKS
 *  This API is a corresponding API of get_softkey_function.
 * PARAMETERS
 *  f        : [IN] Is the function handler which will be called when one specific key event occurs
 *  k        : [IN] Is the key event
 *  key      : [IN] Is the softkey
 * RETURNS
 *  void
 * SEE ALSO
 *  You should also know WGUI_SOFTKEY_ENUM as well.
 *****************************************************************************/
extern void set_softkey_function(void (*f) (void), MMI_key_event_type k, WGUI_SOFTKEY_ENUM key);

/*****************************************************************************
 * FUNCTION
 *  get_softkey_function
 * DESCRIPTION
 *  Set the key event function of one specific softkey.
 * REMARKS
 *  This API is a corresponding API of set_softkey_function.
 * PARAMETERS
 *  k        : [IN] Is the key event
 *  key      : [IN] Is the softkey
 * RETURNS
 *  FuncPtr
 * RETURN VALUES
 *  FuncPtr : Is the key event function handler.
 * SEE ALSO
 *  You should also know WGUI_SOFTKEY_ENUM as well.
 *****************************************************************************/
extern FuncPtr get_softkey_function(MMI_key_event_type k, WGUI_SOFTKEY_ENUM key);

/*****************************************************************************
 * FUNCTION
 *  execute_softkey_function
 * DESCRIPTION
 *  Execute key event function of one specific softkey when particular softkey event occurs.
 * PARAMETERS
 *  k        : [IN] Is the key event
 *  key      : [IN] Is the softkey
 * RETURNS
 *  void
 * SEE ALSO
 *  You should also know WGUI_SOFTKEY_ENUM as well.
 *****************************************************************************/
extern void execute_softkey_function(MMI_key_event_type k, WGUI_SOFTKEY_ENUM key);

/*****************************************************************************
 * FUNCTION
 *  redraw_softkey
 * DESCRIPTION
 *  Redraw the one specific softkey's content to main LCD and blt it to main LCD.
 * PARAMETERS
 *  key      : [IN] Is the softkey
 * RETURNS
 *  void
 * SEE ALSO
 *  You should also know WGUI_SOFTKEY_ENUM as well.
 *****************************************************************************/
extern void redraw_softkey(WGUI_SOFTKEY_ENUM key);


/*****************************************************************************
 * FUNCTION
 *  ChangeLeftSoftkey
 * DESCRIPTION
 *  Changes the string and icon of left softkey and redraw.
 * PARAMETERS
 *  s        : [IN] Is the string of left softkey
 *  i        : [IN] Is the icon of left softkey
 * RETURNS
 *  void
 *****************************************************************************/
extern void ChangeLeftSoftkey(U16 s, U16 i);

/*****************************************************************************
 * FUNCTION
 *  ChangeRightSoftkey
 * DESCRIPTION
 *  Changes the string and icon of right softkey and redraw.
 * PARAMETERS
 *  s        : [IN] Is the string of right softkey
 *  i        : [IN] Is the icon of right softkey
 * RETURNS
 *  void
 *****************************************************************************/
extern void ChangeRightSoftkey(U16 s, U16 i);

/*****************************************************************************
 * FUNCTION
 *  EnableLeftSoftkey
 * DESCRIPTION
 *  Set string and icon to left softkey, redraw left softkey and regiester left softkey to KEY_LSK.
 * PARAMETERS
 *  s        : [IN] Is the string of left softkey
 *  i        : [IN] Is the image of left softkey
 * RETURNS
 *  void
 *****************************************************************************/
extern void EnableLeftSoftkey(U16 s, U16 i);

/*****************************************************************************
 * FUNCTION
 *  DisableLeftSoftkey
 * DESCRIPTION
 *  Disable the left softkey, set string and icon to left softkey and redraw left softkey
 *  with disabled color.
 * PARAMETERS
 *  s        : [IN] Is the string of left softkey
 *  i        : [IN] Is the image of left softkey
 * RETURNS
 *  void
 *****************************************************************************/
extern void DisableLeftSoftkey(U16 s, U16 i);

/*****************************************************************************
 * FUNCTION
 *  EnableRightSoftkey
 * DESCRIPTION
 *  Set string and icon to right softkey, redraw right softkey and regiester right softkey to KEY_RSK.
 * PARAMETERS
 *  s        : [IN] Is the string of right softkey
 *  i        : [IN] Is the image of right softkey
 * RETURNS
 *  void
 *****************************************************************************/
extern void EnableRightSoftkey(U16 s, U16 i);

/*****************************************************************************
 * FUNCTION
 *  DisableRightSoftkey
 * DESCRIPTION
 *  Disable the right softkey, set string and icon to right softkey and redraw right softkey
 *  with disabled color.
 * PARAMETERS
 *  s        : [IN] Is the string of right softkey
 *  i        : [IN] Is the image of right softkey
 * RETURNS
 *  void
 *****************************************************************************/
extern void DisableRightSoftkey(U16 s, U16 i);

/*****************************************************************************
 * FUNCTION
 *  SetLeftSoftkeyFunction
 * DESCRIPTION
 *  Set the key event function to left softkey and register left softkey to KEY_LSK.
 * PARAMETERS
 *  f        : [IN] Is the key event function handler which will be called when one specific key event occurs
 *  k        : [IN] Is the key event
 * RETURNS
 *  void
 *****************************************************************************/
extern void SetLeftSoftkeyFunction(void (*f) (void), MMI_key_event_type k);

/*****************************************************************************
 * FUNCTION
 *  SetRightSoftkeyFunction
 * DESCRIPTION
 *  Set the key event function to right softkey and register right softkey to KEY_RSK.
 * PARAMETERS
 *  f        : [IN] Is the key event function handler which will be called when one specific key event occurs
 *  k        : [IN] Is the key event
 * RETURNS
 *  void
 *****************************************************************************/
extern void SetRightSoftkeyFunction(void (*f) (void), MMI_key_event_type k);

#ifdef __MMI_WGUI_CSK_ENABLE__
/*****************************************************************************
 * FUNCTION
 *  EnableCenterSoftkey
 * DESCRIPTION
 *  Set string and icon to center softkey, redraw center softkey and regiester center softkey to KEY_CSK.
 * PARAMETERS
 *  s        : [IN] Is the string of center softkey
 *  i        : [IN] Is the image of center softkey
 * RETURNS
 *  void
 *****************************************************************************/    
    extern void EnableCenterSoftkey(U16 s, U16 i);

/*****************************************************************************
 * FUNCTION
 *  ResetCenterSoftkey
 * DESCRIPTION
 *  Reset center softkey's content to NULL and set its state to normal.
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
    extern void ResetCenterSoftkey(void);

/*****************************************************************************
 * FUNCTION
 *  SetCenterSoftkeyFunction
 * DESCRIPTION
 *  Set the key event function to center softkey and register center softkey to KEY_CSK.
 * REMARKS
 *  This API will help to rigister center softkey to Enter key for non-touch projects.
 * PARAMETERS
 *  f        : [IN] Is the key event function handler which will be called when one specific key event occurs
 *  k        : [IN] Is the key event
 * RETURNS
 *  void
 *****************************************************************************/
    extern void SetCenterSoftkeyFunction(void (*f) (void), MMI_key_event_type k);

/*****************************************************************************
 * FUNCTION
 *  ChangeCenterSoftkey
 * DESCRIPTION
 *  Changes the string and icon of center softkey.
 * PARAMETERS
 *  s       : [IN] Is the new string of center softkey
 *  i       : [IN] Is the new icon of center softkey
 * RETURNS
 *  void
 *****************************************************************************/
    extern void ChangeCenterSoftkey(U16 s, U16 i);

#endif /* __MMI_WGUI_CSK_ENABLE__ */

/* Vogins Start */
#ifdef __VRE_V30__
extern U16 GetIconImageId(S16 icon_ID);
extern void ChangeStatusIconImageData(S16 icon_ID, PU8 image_data);
#endif
/* Vogins End */

#endif /* __WGUI_CATEGORIES_UTIL_H__ */ 

