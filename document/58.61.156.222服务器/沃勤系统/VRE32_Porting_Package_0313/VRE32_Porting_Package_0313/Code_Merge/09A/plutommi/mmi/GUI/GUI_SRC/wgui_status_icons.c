/*****************************************************************************
*  Copyright Statement:
*  --------------------
*  This software is protected by Copyright and the information contained
*  herein is confidential. The software may not be copied and the information
*  contained herein may not be used or disclosed except with the written
*  permission of MediaTek Inc. (C) 2005
*
*  BY OPENING THIS FILE, BUYER HEREBY UNEQUIVOCALLY ACKNOWLEDGES AND AGREES
*  THAT THE SOFTWARE/FIRMWARE AND ITS DOCUMENTATIONS ("MEDIATEK SOFTWARE")
*  RECEIVED FROM MEDIATEK AND/OR ITS REPRESENTATIVES ARE PROVIDED TO BUYER ON
*  AN "AS-IS" BASIS ONLY. MEDIATEK EXPRESSLY DISCLAIMS ANY AND ALL WARRANTIES,
*  EXPRESS OR IMPLIED, INCLUDING BUT NOT LIMITED TO THE IMPLIED WARRANTIES OF
*  MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE OR NONINFRINGEMENT.
*  NEITHER DOES MEDIATEK PROVIDE ANY WARRANTY WHATSOEVER WITH RESPECT TO THE
*  SOFTWARE OF ANY THIRD PARTY WHICH MAY BE USED BY, INCORPORATED IN, OR
*  SUPPLIED WITH THE MEDIATEK SOFTWARE, AND BUYER AGREES TO LOOK ONLY TO SUCH
*  THIRD PARTY FOR ANY WARRANTY CLAIM RELATING THERETO. MEDIATEK SHALL ALSO
*  NOT BE RESPONSIBLE FOR ANY MEDIATEK SOFTWARE RELEASES MADE TO BUYER'S
*  SPECIFICATION OR TO CONFORM TO A PARTICULAR STANDARD OR OPEN FORUM.
*
*  BUYER'S SOLE AND EXCLUSIVE REMEDY AND MEDIATEK'S ENTIRE AND CUMULATIVE
*  LIABILITY WITH RESPECT TO THE MEDIATEK SOFTWARE RELEASED HEREUNDER WILL BE,
*  AT MEDIATEK'S OPTION, TO REVISE OR REPLACE THE MEDIATEK SOFTWARE AT ISSUE,
*  OR REFUND ANY SOFTWARE LICENSE FEES OR SERVICE CHARGE PAID BY BUYER TO
*  MEDIATEK FOR SUCH MEDIATEK SOFTWARE AT ISSUE. 
*
*  THE TRANSACTION CONTEMPLATED HEREUNDER SHALL BE CONSTRUED IN ACCORDANCE
*  WITH THE LAWS OF THE STATE OF CALIFORNIA, USA, EXCLUDING ITS CONFLICT OF
*  LAWS PRINCIPLES.  ANY DISPUTES, CONTROVERSIES OR CLAIMS ARISING THEREOF AND
*  RELATED THERETO SHALL BE SETTLED BY ARBITRATION IN SAN FRANCISCO, CA, UNDER
*  THE RULES OF THE INTERNATIONAL CHAMBER OF COMMERCE (ICC).
*
*****************************************************************************/

/*****************************************************************************
 *
 * Filename:
 * ---------
 * wgui_status_icons.c
 *
 * Project:
 * --------
 *   MAUI
 *
 * Description:
 * ------------
 *   This file is intends for status icon bars control.
 *
 * Author:
 * -------
 * -------
 *
 *============================================================================
 *             HISTORY
 * Below this line, this part is controlled by PVCS VM. DO NOT MODIFY!!
 *------------------------------------------------------------------------------
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 * removed!
 * removed!
 * removed!
 *
 *------------------------------------------------------------------------------
 * Upper this line, this part is controlled by PVCS VM. DO NOT MODIFY!!
 *============================================================================
 ****************************************************************************/


#include "wgui_status_icons.h"
#include "WguiCategoryGprot.h"
/* To be removed later..and put in boot up init file..Right now needed for 
   initializing status icon data */
#include "CustDataRes.h"
#include "StatusIconRes.h"
#include "MMI_features.h"
#include "SimDetectionGexdcl.h"
#include "SimDetectionGprot.h"
#include "IdleAppDef.h"
#include "IdleAppProt.h"

#ifdef __MMI_TOUCH_SCREEN__
#include "mmi_frm_gprot.h"
#endif /* __MMI_TOUCH_SCREEN__ */ 

#if defined(__MMI_UI_STATUS_BAR_AT_BOTTOM__)
#endif 

#ifdef __MMI_DOWNLOADABLE_THEMES_SUPPORT__
#endif

#ifdef __MMI_WLAN_FEATURES__
#include "DataAccountResDef.h"
extern U8 mmi_netset_get_active_preferred_mode(void);
#endif /* __MMI_WLAN_FEATURES__ */

#include "gpioInc.h"  

#if defined(__AGPS_SUPPORT__) && defined(__SUPL_SUPPORT__)
#include "GPSResDef.h"
#endif

#ifdef WGUI_STATUS_ICON_SHOW_TIME_IN_RIGHT
#include "wgui_clock.h"
#include "wgui_categories_res.h"
static MMI_BOOL wgui_status_icon_clock_create = MMI_FALSE;
static S32 t_x1, t_y1, t_x2, t_y2;
extern U8 PhnsetGetTimeFormat(void);
U32 static wgui_clock_status_icon_id = WGUI_CLOCK_INVALID_INDEX;
#endif/* WGUI_STATUS_ICON_SHOW_TIME_IN_RIGHT */


/* Externals for graphics context switching  */
extern bitmap main_LCD_device_bitmap;
extern bitmap sub_LCD_device_bitmap;

#if defined(__MMI_MAINLCD_176X220__) || defined (__MMI_MAINLCD_240X320__) || defined (__MMI_MAINLCD_240X400__) || defined (__MMI_MAINLCD_320X240__) || defined (__MMI_MAINLCD_320X480__)
extern MMI_BOOL if_full_screen_hide_status_icons(void);
#endif 

extern MMI_BOOL mmi_flight_mode_is_status_icon_visible(S16 icon_ID, U32 icon_flag);

void UI_set_main_LCD_graphics_context(void);
void UI_set_sub_LCD_graphics_context(void);
extern bitmap *current_LCD_device_bitmap;


MMI_BOOL wgui_status_icon_blocking = MMI_FALSE;

void wgui_status_icon_set_block(MMI_BOOL blocking);

static U8 IsReArrangeNeeded = 0;

static gdi_handle wgui_status_bar_alpha_layer = GDI_NULL_HANDLE;

/* Moved from down HIMANSHU modified */
extern S32 MAIN_LCD_device_width;
extern S32 MAIN_LCD_device_height;

/* This is the customizable list of status icons                        */
/* Use (x,y)=(0,0) for automatic positioning. Automatic positioning is        */
/* only valid with Icon bars having STATUS_ICON_BAR_ARRANGE_XXXXXXXX       */
/* Set width=0, height=0 for all icons. These values are autocalculated    */
/* If there is no icon, use IMAGE_ID_NULL for the MMI_ID_TYPE value           */
/* FLAGS:   Set status icon specific flags here. Usually this value is 0      */
/* STATE:   Set the default state of a multi-state status icon here           */
/* GROUP:   Set the group number for grouped icons here.                */
/*       0 = Icon is not grouped                                  */
/* PRIORITY:   Set the status icon priority here. If there are more status    */
/*          icons than what can be displayed in the status icon bar        */
/*          area, priority is used to determine which icons are            */
/*          displayed and which are not.                          */
/*          0 = Icon is always displayed                          */
/* IMAGE:   Pointer to Icons image data. Autocalculated. Always set to NULL      */
/* NFRAMES: Number of frames in the image. Autocalculated. Always set to 0    */

/* !!NOTE:  Currently the priority field is dummy, the icons need to be       */
/* manually arranged according to the desired priority                     */
/* Also, grouped icons need to be arranged consecutively in the list       */

MMI_status_icon MMI_status_icons[MAX_STATUS_ICONS] = 
{
    /* ICON_NUMBER    X     Y     WIDTH    HEIGHT      MMI_ID_TYPE                      FLAGS STATE GROUP PRIORITY IMAGE NFRAMES  */
#ifndef __MMI_DUAL_SIM_MASTER__
    /* 0  */ {MMI_UI_STATUSBAR_GAP, 2, 0, 0, IMG_SI_SIGNAL_STRENGTH, 0, 0, 0, 0, NULL, 0,},
#else	/* else __MMI_DUAL_SIM_MASTER__ */
    /* 0  */ {MMI_UI_STATUSBAR_GAP, 2, 0, 0, IMG_SI_MASTER_SIGNAL_CLOSED, 0, 0, 0, 0, NULL, 0,},
#endif	/* end __MMI_DUAL_SIM_MASTER__ */
    /* 1  */ {111, 4, 0, 0, IMG_SI_BATTERY_STRENGTH, 0, 0, 0, 0, NULL, 0,},
#ifdef __MMI_DUAL_SIM_MASTER__
/*2*/{0,	0,0,	0,IMG_SI_SLAVE_SIGNAL_CLOSED,	0,0,0,0,NULL,0,},
#endif	/* __MMI_DUAL_SIM_MASTER__ */

#ifdef __MMI_CALL_INDICATOR__
        {0, 0, 0, 0, IMG_SI_CALL_INDICATOR_ACTIVE, 0, 0, 0, 0, NULL, 0,},
#endif 

#ifdef __MMI_MOBILE_TV_PLAYER__
        {0, 0, 0, 0, IMG_SI_MTV_SIGNAL, 0, 0, 0, 0, NULL, 0,},
#endif 

#ifdef __MMI_WLAN_FEATURES__
    /* 54 */ {0, 0, 0, 0, IMG_ID_DTCNT_WLAN_STATUS_DISCONNECT, 0, 0, 0, 0, NULL, 0,},
#endif 
#ifndef __MMI_STATUS_ICON_SIGNAL_WITH_LINE_SWITCH__     /* 022505 Calvin added */
    /* 2  */ {0, 0, 0, 0, IMG_SI_LINE_L1, 0, 0, 1, 0, NULL, 0,},
    /* 3  */ {0, 0, 0, 0, IMG_SI_LINE_L2, 0, 0, 1, 0, NULL, 0,},
#endif /* __MMI_STATUS_ICON_SIGNAL_WITH_LINE_SWITCH__ */ 
#if defined(__GSM_RAT__) && defined(__WCDMA_RAT__) /* 112805 3G icon Calvin */
/* under construction !*/
/* under construction !*/
#endif /* defined(__GSM_RAT__) && defined(__WCDMA_RAT__) */ 
#ifdef __MMI_VOIP__
    /* 5  */ {0, 0, 0, 0, IMG_SI_VOIP, 0, 0, 0, 0, NULL, 0,},//091806 voip icon
#endif
#ifdef __MMI_AUDIO_PLAYER__
    /* 5  */ {0, 0, 0, 0, IMG_SI_AUDPLY, 0, 0, 0, 0, NULL, 0,},
#endif

#ifdef __MMI_MEDIA_PLAYER__
    /* 5  */ {0, 0, 0, 0, IMG_SI_MEDPLY, 0, 0, 0, 0, NULL, 0,},
#endif

    /* 6  */ {0, 0, 0, 0, IMG_SI_ROAMING_INDICATOR, 0, 0, 0, 0, NULL, 0,},
#ifdef __MMI_DUAL_SIM_MASTER__
/**/	{0,0,0,0,IMG_SI_SLAVE_ROAMING_INDICATOR,0,0,0,0,NULL,0,},
#endif	/* __MMI_DUAL_SIM_MASTER__ */
    /* 7  */ {0, 0, 0, 0, IMG_SI_CALL_DIVERT_L1L2, 0, 0, 2, 0, NULL, 0,},
    /* 8  */ {0, 0, 0, 0, IMG_SI_CALL_DIVERT_L1, 0, 0, 2, 0, NULL, 0,},
    /* 9  */ {0, 0, 0, 0, IMG_SI_CALL_DIVERT_L2, 0, 0, 2, 0, NULL, 0,},
#ifdef __MMI_DUAL_SIM_MASTER__
    /*3*/{0,	0,0,	0,IMG_SLAVE_CALL_FORWARD,0,	0,0,0,NULL,0,},
    {0,    0,0,    0,IMG_SLAVE_CALL_FORWARD_L1,0, 0,0,0,NULL,0,},
    {0,    0,0,    0,IMG_SLAVE_CALL_FORWARD_L2,0, 0,0,0,NULL,0,},
#endif	/* __MMI_DUAL_SIM_MASTER__ */
    /* 10 */ {0, 0, 0, 0, IMG_SI_MUTE, 0, 0, 0, 0, NULL, 0,},
    /* 11 */ {0, 0, 0, 0, IMG_SI_SMS_INDICATOR, 0, 0, 0, 0, NULL, 0,},
#ifdef __MMI_DUAL_SIM_MASTER__
    /**/     {0,0,0,0,IMG_SI_SLAVE_SMS_INDICATOR,0,0,0,0,	NULL,0,},
#endif	/* __MMI_DUAL_SIM_MASTER__ */
    /* 12 */ {0, 0, 0, 0, IMG_SI_MMS_UNREAD_INDICATOR, 0, 0, 0, 0, NULL, 0,},
    /* 13 */ {0, 0, 0, 0, IMG_SI_MMS_BUSY_INDICATOR, 0, 0, 0, 0, NULL, 0,},

#ifdef __MMI_DUAL_SIM_MASTER__
             {0, 0, 0, 0, IMG_SI_MMS_UNREAD_INDICATOR_SIM1, 0, 0, 0, 0, NULL, 0,},
             {0, 0, 0, 0, IMG_SI_MMS_UNREAD_INDICATOR_SIM2, 0, 0, 0, 0, NULL, 0,},
             {0, 0, 0, 0, IMG_SI_MMS_BUSY_INDICATOR_SIM1, 0, 0, 0, 0, NULL, 0,},
             {0, 0, 0, 0, IMG_SI_MMS_BUSY_INDICATOR_SIM2, 0, 0, 0, 0, NULL, 0,},
#endif  /* __MMI_DUAL_SIM_MASTER__ */

#ifdef  __UNIFIED_MESSAGE_BACKGROUND_SEND_SUPPORT__ 
    /* 11 */ {0, 0, 0, 0, IMG_SI_OUTBOX_INDICATOR, 0, 0, 0, 0, NULL, 0,},
#endif

    /* 11 */ {0, 0, 0, 0, IMG_SI_SMS_SENDING, 0, 0, 0, 0, NULL, 0,},

    /* 14 */ {0, 0, 0, 0, IMG_SI_VIBRATE, 0, 0, 3, 0, NULL, 0,},
    /* 15 */ {0, 0, 0, 0, IMG_SI_SILENT, 0, 0, 3, 0, NULL, 0,},
    /* 16 */ {0, 0, 0, 0, IMG_SI_RING, 0, 0, 3, 0, NULL, 0,},
    /* 17 */ {0, 0, 0, 0, IMG_SI_VIBRATE_AND_RING, 0, 0, 3, 0, NULL, 0,},
    /* 18 */ {0, 0, 0, 0, IMG_SI_VIBRATE_THEN_RING, 0, 0, 3, 0, NULL, 0,},
#ifndef __MMI_DUAL_SIM_MASTER__
             {0, 0, 0, 0, IMG_SI_CSD, 0, 0, 9, 0, NULL, 0,},   
    /* 19 */ {0, 0, 0, 0, IMG_SI_EDGE, 0, 0, 9, 0, NULL, 0,},
    /* 20 */ {0, 0, 0, 0, IMG_SI_GPRS_INDICATOR, 0, 0, 9, 0, NULL, 0,},    
    /* 21 */ {0, 0, 0, 0, IMG_SI_EDGEC, 0, 0, 10, 0, NULL, 0,},    
    /* 22 */ {0, 0, 0, 0, IMG_SI_GPRS_ATT_NO_PDP_INDICATOR, 0, 0, 10, 0, NULL, 0,},
#else
             {0, 0, 0, 0, IMG_SI_CSD_SIM1, 0, 0, 9, 0, NULL, 0,},
    /* 19 */ {0, 0, 0, 0, IMG_SI_EDGE_SIM1, 0, 0, 9, 0, NULL, 0,},
    /* 20 */ {0, 0, 0, 0, IMG_SI_GPRS_SERVICE_SIM1, 0, 0, 9, 0, NULL, 0,},    
    /* 21 */ {0, 0, 0, 0, IMG_SI_EDGEC_SIM1, 0, 0, 10, 0, NULL, 0,},    
    /* 22 */ {0, 0, 0, 0, IMG_SI_GPRS_ATT_NO_PDP_INDICATOR_SIM1, 0, 0, 10, 0, NULL, 0,},

             {0, 0, 0, 0, IMG_SI_CSD_SIM2, 0, 0, 9, 0, NULL, 0,},
    /* 19 */ {0, 0, 0, 0, IMG_SI_EDGE_SIM2, 0, 0, 9, 0, NULL, 0,},
    /* 20 */ {0, 0, 0, 0, IMG_SI_GPRS_SERVICE_SIM2, 0, 0, 9, 0, NULL, 0,},    
    /* 21 */ {0, 0, 0, 0, IMG_SI_EDGEC_SIM2, 0, 0, 10, 0, NULL, 0,},    
    /* 22 */ {0, 0, 0, 0, IMG_SI_GPRS_ATT_NO_PDP_INDICATOR_SIM2, 0, 0, 10, 0, NULL, 0,},
    
#endif /* __MMI_DUAL_SIM_MASTER__ */
    
#if (defined(OBIGO_Q05A) || defined(JATAAYU_SUPPORT)) && defined(WAP_SUPPORT)
    /* 23 */ {0, 0, 0, 0, IMG_SI_SECURITY, 0, 0, 0, 0, NULL, 0,},
#endif
    /* 24 */ {0, 0, 0, 0, IMG_SI_EARPHONE_INDICATOR, 0, 0, 0, 0, NULL, 0,},
    /* 25 */ {0, 0, 0, 0, IMG_SI_ALARM_ACTIVATED, 0, 0, 0, 0, NULL, 0,},
    /* 26 */ {0, 0, 0, 0, IMG_SI_VOICE_RECORD, 0, 0, 0, 0, NULL, 0,},
    /* 27 */ {0, 0, 0, 0, IMG_SI_KEYPAD_LOCK, 0, 0, 0, 0, NULL, 0,},
#ifndef __MMI_DUAL_SIM_MASTER__
    /* 28 */ {0, 0, 0, 0, IMG_SI_MISSED_CALL_INDICATOR, 0, 0, 0, 0, NULL, 0,},
#else
    /*  */    {0, 0, 0, 0, IMG_SI_CARD1MISSED_CALL_INDICATOR, 0, 0, 0, 0, NULL, 0,},
    /*  */    {0, 0, 0, 0, IMG_SI_CARD2MISSED_CALL_INDICATOR, 0, 0, 0, 0, NULL, 0,},
#endif/*__MMI_DUAL_SIM_MASTER__*/

    {0, 0, 0, 0, IMG_SI_UNREAD_EMAIL, 0, 0, 0, 0, NULL, 0,},

    /* 29 */ {0, 0, 0, 0, IMG_SI_UNREAD_VOICE_L1L2, 0, 0, 4, 0, NULL, 0,},
    /* 30 */ {0, 0, 0, 0, IMG_SI_UNREAD_VOICE_L1, 0, 0, 4, 0, NULL, 0,},
    /* 31 */ {0, 0, 0, 0, IMG_SI_UNREAD_VOICE_L2, 0, 0, 4, 0, NULL, 0,},
    /* 32 */ {0, 0, 0, 0, IMG_SI_UNREAD_FAX_L1L2, 0, 0, 5, 0, NULL, 0,},
    /* 33 */ {0, 0, 0, 0, IMG_SI_UNREAD_FAX_L1, 0, 0, 5, 0, NULL, 0,},
    /* 34 */ {0, 0, 0, 0, IMG_SI_UNREAD_FAX_L2, 0, 0, 5, 0, NULL, 0,},
    /* 35 */ {0, 0, 0, 0, IMG_SI_UNREAD_EMAIL_L1L2, 0, 0, 6, 0, NULL, 0,},
    /* 36 */ {0, 0, 0, 0, IMG_SI_UNREAD_EMAIL_L1, 0, 0, 6, 0, NULL, 0,},
    /* 37 */ {0, 0, 0, 0, IMG_SI_UNREAD_EMAIL_L2, 0, 0, 6, 0, NULL, 0,},
    
            {0, 0, 0, 0, IMG_SI_UNREAD_NET_L1, 0, 0, 7, 0, NULL, 0,},
            {0, 0, 0, 0, IMG_SI_UNREAD_NET_L2, 0, 0, 7, 0, NULL, 0,},
            {0, 0, 0, 0, IMG_SI_UNREAD_NET_L1L2, 0, 0, 7, 0, NULL, 0,},
        #ifdef __MMI_DUAL_SIM_MASTER__

            {0, 0, 0, 0, IMG_SI_UNREAD_VOICE_L1_MASTER, 0, 0, 0, 0, NULL, 0,},
            {0, 0, 0, 0, IMG_SI_UNREAD_VOICE_L2_MASTER, 0, 0, 0, 0, NULL, 0,},
            {0, 0, 0, 0, IMG_SI_UNREAD_VOICE_L1L2_MASTER, 0, 0, 0, 0, NULL, 0,},
            {0, 0, 0, 0, IMG_SI_UNREAD_FAX_L1_MASTER, 0, 0, 0, 0, NULL, 0,},
            {0, 0, 0, 0, IMG_SI_UNREAD_FAX_L2_MASTER, 0, 0, 0, 0, NULL, 0,},
            {0, 0, 0, 0, IMG_SI_UNREAD_FAX_L1L2_MASTER, 0, 0, 0, 0, NULL, 0,},
            {0, 0, 0, 0, IMG_SI_UNREAD_EMAIL_L1_MASTER, 0, 0, 0, 0, NULL, 0,},
            {0, 0, 0, 0, IMG_SI_UNREAD_EMAIL_L2_MASTER, 0, 0, 0, 0, NULL, 0,},
            {0, 0, 0, 0, IMG_SI_UNREAD_EMAIL_L1L2_MASTER, 0, 0, 0, 0, NULL, 0,},
            {0, 0, 0, 0, IMG_SI_UNREAD_NET_L1_MASTER, 0, 0, 0, 0, NULL, 0,},
            {0, 0, 0, 0, IMG_SI_UNREAD_NET_L2_MASTER, 0, 0, 0, 0, NULL, 0,},
            {0, 0, 0, 0, IMG_SI_UNREAD_NET_L1L2_MASTER, 0, 0, 0, 0, NULL, 0,},

            {0, 0, 0, 0, IMG_SI_UNREAD_VOICE_L1_SLAVE, 0, 0, 0, 0, NULL, 0,},
            {0, 0, 0, 0, IMG_SI_UNREAD_VOICE_L2_SLAVE, 0, 0, 0, 0, NULL, 0,},
            {0, 0, 0, 0, IMG_SI_UNREAD_VOICE_L1L2_SLAVE, 0, 0, 0, 0, NULL, 0,},
            {0, 0, 0, 0, IMG_SI_UNREAD_FAX_L1_SLAVE, 0, 0, 0, 0, NULL, 0,},
            {0, 0, 0, 0, IMG_SI_UNREAD_FAX_L2_SLAVE, 0, 0, 0, 0, NULL, 0,},
            {0, 0, 0, 0, IMG_SI_UNREAD_FAX_L1L2_SLAVE, 0, 0, 0, 0, NULL, 0,},
            {0, 0, 0, 0, IMG_SI_UNREAD_EMAIL_L1_SLAVE, 0, 0, 0, 0, NULL, 0,},
            {0, 0, 0, 0, IMG_SI_UNREAD_EMAIL_L2_SLAVE, 0, 0, 0, 0, NULL, 0,},
            {0, 0, 0, 0, IMG_SI_UNREAD_EMAIL_L1L2_SLAVE, 0, 0, 0, 0, NULL, 0,},
            {0, 0, 0, 0, IMG_SI_UNREAD_NET_L1_SLAVE, 0, 0, 0, 0, NULL, 0,},
            {0, 0, 0, 0, IMG_SI_UNREAD_NET_L2_SLAVE, 0, 0, 0, 0, NULL, 0,},
            {0, 0, 0, 0, IMG_SI_UNREAD_NET_L1L2_SLAVE, 0, 0, 0, 0, NULL, 0,},

#endif  /*__MMI_DUAL_SIM_MASTER__*/

#ifdef __MMI_AUTO_ANSWER_MACHINE__
		    	   {0, 0, 0, 0, IMG_SI_AUTOAM_REC, 0, 0, 0, 0, NULL, 0,},
		         {0, 0, 0, 0, IMG_SI_AUTOAM_UNREAD, 0, 0, 0, 0, NULL, 0,},
            {0, 0, 0, 0, IMG_SI_AUTOAM_ON, 0, 0, 0, 0, NULL, 0,},
#endif /* __MMI_AUTO_ANSWER_MACHINE__ */ 

    /* 38 */ {0, 0, 0, 0, IMG_SI_IMPS_MSG, 0, 0, 8, 0, NULL, 0,},
    /* 39 */ {0, 0, 0, 0, IMG_SI_IMPS_LOGIN, 0, 0, 8, 0, NULL, 0,},
    
#ifdef __MMI_DUAL_SIM_MASTER__
    /* 40 */ {0, 0, 0, 0, IMG_SI_WAP1, 0, 0, 0, 0, NULL, 0,},
    /* 40 */ {0, 0, 0, 0, IMG_SI_WAP2, 0, 0, 0, 0, NULL, 0,},
#else /* __MMI_DUAL_SIM_MASTER__ */
    /* 40 */ {0, 0, 0, 0, IMG_SI_WAP, 0, 0, 0, 0, NULL, 0,},
#endif /* __MMI_DUAL_SIM_MASTER__ */

    /* 41 */ {0, 0, 0, 0, IMG_SI_POC, 0, 0, 0, 0, NULL, 0,},
    /* 42 */ {0, 0, 0, 0, IMG_SI_BT, 0, 0, 0, 0, NULL, 0,},
    /* 43 */ {0, 0, 0, 0, IMG_SI_BT_CON, 0, 0, 0, 0, NULL, 0,},
    /* 44 */ {0, 0, 0, 0, IMG_SI_BT_ND, 0, 0, 0, 0, NULL, 0,},
    /* 45 */ {0, 0, 0, 0, IMG_SI_IR_INDICATOR, 0, 0, 0, 0, NULL, 0,},
    /* 46 */ {0, 0, 0, 0, IMG_SI_STOPWATCH, 0, 0, 0, 0, NULL, 0,},
                                                                                /*  */
                                                                                /* 021505 Calvin added */
#ifdef __MMI_DUAL_SIM_MASTER__
             {0, 0, 0, 0, IMG_SI_MASTER_NETWORK_CIPHER_GSM, 0, 0, 0, 0, NULL, 0,},

             {0, 0, 0, 0, IMG_SI_SLAVE_NETWORK_CIPHER_GSM, 0, 0, 0, 0, NULL, 0,},
#else
    /* 49 */ {0, 0, 0, 0, IMG_SI_NETWORK_CIPHER_GSM, 0, 0, 0, 0, NULL, 0,},
            
#endif

    /* 50 */ {0, 0, 0, 0, IMG_SI_NETWORK_CIPHER_GPRS, 0, 0, 0, 0, NULL, 0,},
#if defined(__J2ME__) && defined(J2ME_SUPPORT_BACKGROUND)
    /* 51 */ {0, 0, 0, 0, IMG_SI_JAVA_BG_VM, 0, 0, 0, 0, NULL, 0,},
    /* 51.1 */ {0, 0, 0, 0, IMG_SI_JAVA_BG_VM_AUDIO, 0, 0, 0, 0, NULL, 0,},
#endif
#ifdef __MMI_BG_SOUND_EFFECT__
    /* 52 */ {0, 0, 0, 0, IMG_SI_BGSND, 0, 0, 0, 0, NULL, 0,},
#endif 
#ifdef __MMI_AUDIO_REVERB_EFFECT__
    /* 53 */ {0, 0, 0, 0, IMG_SI_REVERBSND, 0, 0, 0, 0, NULL, 0,},
#endif 
#ifdef __MMI_AUDIO_SURROUND_EFFECT__
    /* 54 */ {0, 0, 0, 0, IMG_SI_SURROUNDSND, 0, 0, 0, 0, NULL, 0,},
#endif
#ifdef __MMI_HOMEZONE_STATUS_ICON__
    /* 55 */ {0, 0, 0, 0, IMG_SI_HZONE, 0, 0, 0, 0, NULL, 0,},
#ifdef __MMI_DUAL_SIM_MASTER__
    /* 55 */ {0, 0, 0, 0, IMG_SI_HZONE_SLAVE, 0, 0, 0, 0, NULL, 0,},
#endif
    /* 56 */ {0, 0, 0, 0, IMG_SI_CZONE, 0, 0, 0, 0, NULL, 0,},
#endif /* __MMI_HOMEZONE_STATUS_ICON__ */ 
#ifdef __CTM_SUPPORT__
    /* 57 */ {0, 0, 0, 0, IMG_SI_TTY_INDICATOR, 0, 0, 0, 0, NULL, 0,},
#endif 

    {0, 0, 0, 0, IMG_SI_EMAIL_REF, 0, 0, 0, 0, NULL, 0,},

#ifdef __MMI_PICT_BRIDGE_SUPPORT__
    /* 58 */ {0, 0, 0, 0, IMG_SI_PRINT, 0, 0, 11, 0, NULL, 0,},
    /* 59 */ {0, 0, 0, 0, IMG_SI_PRINT_FAIL, 0, 0, 11, 0, NULL, 0,},
#endif


#if defined(__AGPS_SUPPORT__) && defined(__SUPL_SUPPORT__)
            {0, 0, 0, 0, IMG_SI_AGPS_STATE, 0, 0, 0, 0, NULL, 0,},
#endif

#ifdef __MMI_SYNCML_STATUS_ICON_SUPPORT__
             {0, 0, 0, 0, IMG_SI_SYNCML, 0, 0, 0, 0, NULL, 0,},
#endif

#ifdef __MMI_PROV_INBOX_SUPPORT__
            {0, 0, 0, 0, IMG_SI_PROVMSG, 0, 0, 0, 0, NULL, 0,},
#ifdef __MMI_DUAL_SIM_MASTER__
            {0, 0, 0, 0, IMG_SI_PROVMSG_SIM1, 0, 0, 0, 0, NULL, 0,},
            {0, 0, 0, 0, IMG_SI_PROVMSG_SIM2, 0, 0, 0, 0, NULL, 0,},
#endif
#endif
#if defined(__QQIM_SUPPORT__)
            {0, 0, 0, 0, IMG_SI_QQIM_AWAY, 0, 0, 0, 0, NULL, 0,},
            {0, 0, 0, 0, IMG_SI_QQIM_INVISIBLE, 0, 0, 0, 0, NULL, 0,},
            {0, 0, 0, 0, IMG_SI_QQIM_MSG, 0, 0, 0, 0, NULL, 0,},
            {0, 0, 0, 0, IMG_SI_QQIM_OFFLINE, 0, 0, 0, 0, NULL, 0,},
            {0, 0, 0, 0, IMG_SI_QQIM_ONLINE, 0, 0, 0, 0, NULL, 0,},
#endif

    /* Sublcd Start */
    /* 60 */ {0, 0, 0, 0, IMG_SLSI_SIGNAL_STRENGTH, 0, 0, 0, 0, NULL, 0,},
    /* 61 */ {36, 0, 0, 0, IMG_SLSI_BATTERY_STRENGTH, 0, 0, 0, 0, NULL, 0,},
    /* 62 */ {11, 2, 0, 0, IMG_SLSI_SMS_INDICATOR, 0, 0, 0, 0, NULL, 0,},
    /* 63 */ {24, 1, 0, 0, IMG_SLSI_RING, 0, 0, 7, 0, NULL, 0,},
    /* 64 */ {24, 1, 0, 0, IMG_SLSI_VIBRATE, 0, 0, 7, 0, NULL, 0,},
    /* 65 */ {24, 1, 0, 0, IMG_SLSI_VIBRATE_AND_RING, 0, 0, 7, 0, NULL, 0,},
    /* 66 */ {24, 1, 0, 0, IMG_SLSI_VIBRATE_THEN_RING, 0, 0, 7, 0, NULL, 0,},
    /* 67 */ {24, 1, 0, 0, IMG_SLSI_SILENT, 0, 0, 7, 0, NULL, 0,},
    /* 68 */ {24, 1, 0, 0, IMG_SLSI_ALARM, 0, 0, 0, 2, NULL, 0,},
    /* 69 */ {24, 1, 0, 0, IMG_SLSI_MISSED_CALL, 0, 0, 0, 2, NULL, 0,},

/* Vogins Start */
#ifdef __VRE30_AM__
	{0, 0, 0, 0, IMG_SI_VRE_DEFAULT, 0, 0, 0, 0, NULL, 0,},    
	{0, 0, 0, 0, IMG_SI_VRE_DEFAULT, 0, 0, 0, 0, NULL, 0,},    
	{0, 0, 0, 0, IMG_SI_VRE_DEFAULT, 0, 0, 0, 0, NULL, 0,},    
	{0, 0, 0, 0, IMG_SI_VRE_DEFAULT, 0, 0, 0, 0, NULL, 0,},    
	{0, 0, 0, 0, IMG_SI_VRE_DEFAULT, 0, 0, 0, 0, NULL, 0,},
#endif
/* Vogins End */
};

/* Pool of status icons used for the horizontal and vertical   */
/* status icon bars                                */

/* optimize code size */
#define MMI_N_STATUS_ICONS_POOL1 ( sizeof(MMI_status_icons_pool1)/sizeof(MMI_status_icons_pool1[0]) )   /* 32 */
/* store id of main lcd icon */
const S16 MMI_status_icons_pool1[] = 
{
    STATUS_ICON_SIGNAL_STRENGTH,
    STATUS_ICON_BATTERY_STRENGTH,
#ifdef __MMI_DUAL_SIM_MASTER__
    STATUS_ICON_SLAVE_SIGNAL_STRENGTH,
#endif	/* __MMI_DUAL_SIM_MASTER__ */

#ifdef __MMI_CALL_INDICATOR__
    STATUS_ICON_CALL_INDICATOR,
#endif /* __MMI_CALL_INDICATOR__ */

#ifdef __MMI_MOBILE_TV_PLAYER__
    STATUS_ICON_MTV_SIGNAL,
#endif 

#ifdef __MMI_WLAN_FEATURES__
    STATUS_ICON_WLAN_SIGNAL,
#endif /* __MMI_WLAN_FEATURES__ */
#ifndef __MMI_STATUS_ICON_SIGNAL_WITH_LINE_SWITCH__
    STATUS_ICON_ALTERNATE_LINE_L1,
    STATUS_ICON_ALTERNATE_LINE_L2,
#endif /* __MMI_STATUS_ICON_SIGNAL_WITH_LINE_SWITCH__ */ 
#if defined(__GSM_RAT__) && defined(__WCDMA_RAT__)
/* under construction !*/
/* under construction !*/
#endif /* defined(__GSM_RAT__) && defined(__WCDMA_RAT__) */ 
#ifdef __MMI_VOIP__
    STATUS_ICON_VOIP,
#endif
#ifdef __MMI_AUDIO_PLAYER__
    STATUS_ICON_AUDPLY,
#endif

#ifdef __MMI_MEDIA_PLAYER__
    STATUS_ICON_MEDPLY,
#endif

    STATUS_ICON_ROAMING,
#ifdef __MMI_DUAL_SIM_MASTER__
    STATUS_ICON_SLAVE_ROAMING,
#endif	/* __MMI_DUAL_SIM_MASTER__ */
    STATUS_ICON_CALL_DIVERT_L1L2,
    STATUS_ICON_CALL_DIVERT_L1,
    STATUS_ICON_CALL_DIVERT_L2,
#ifdef __MMI_DUAL_SIM_MASTER__
    STATUS_ICON_SLAVE_CALLFORWARD,
    STATUS_ICON_SLAVE_CALLFORWARD_L1,
    STATUS_ICON_SLAVE_CALLFORWARD_L2,
#endif	/* __MMI_DUAL_SIM_MASTER__ */
    STATUS_ICON_MUTE,
    STATUS_ICON_INCOMING_SMS,
#ifdef __MMI_DUAL_SIM_MASTER__
    STATUS_ICON_SLAVE_INCOMING_SMS,
#endif	/* __MMI_DUAL_SIM_MASTER__ */
    STATUS_ICON_MMS_UNREAD,
    STATUS_ICON_MMS_BUSY,
    
#ifdef __MMI_DUAL_SIM_MASTER__
        STATUS_ICON_MMS_UNREAD_SIM_1,
        STATUS_ICON_MMS_UNREAD_SIM_2,
        STATUS_ICON_MMS_BUSY_SIM_1,
        STATUS_ICON_MMS_BUSY_SIM_2,
#endif  /* __MMI_DUAL_SIM_MASTER__ */

#ifdef __UNIFIED_MESSAGE_BACKGROUND_SEND_SUPPORT__
    STATUS_ICON_OUTBOX_INDICATOR,
#endif
	STATUS_ICON_SMS_SENDING,
    STATUS_ICON_VIBRATION,
    STATUS_ICON_SILENT,
    STATUS_ICON_RING,
    STATUS_ICON_VIBRATE_AND_RING,
    STATUS_ICON_VIBRATE_THEN_RING,
    STATUS_ICON_CSD,
    STATUS_ICON_EDGE,
    STATUS_ICON_GPRS_SERVICE,
    STATUS_ICON_EDGEC,
    STATUS_ICON_GPRS_ATT_NO_PDP_INDICATOR, 

#ifdef __MMI_DUAL_SIM_MASTER__
    STATUS_ICON_SLAVE_CSD,
    STATUS_ICON_SLAVE_EDGE, // for SIM2
    STATUS_ICON_SLAVE_GPRS_SERVICE,
    STATUS_ICON_SLAVE_EDGEC,
    STATUS_ICON_SLAVE_GPRS_ATT_NO_PDP_INDICATOR,
#endif

#if (defined(OBIGO_Q05A) || defined(JATAAYU_SUPPORT)) && defined(WAP_SUPPORT)
    STATUS_ICON_SECURITY,
#endif
    STATUS_ICON_EARPHONE_INDICATOR,
    STATUS_ICON_ALARM,
    STATUS_ICON_VOICE_RECORD,
    STATUS_ICON_KEYPAD_LOCK,
    STATUS_ICON_MISSED_CALL,
#ifdef __MMI_DUAL_SIM_MASTER__
    STATUS_ICON_CARD2MISSED_CALL,
#endif/*__MMI_DUAL_SIM_MASTER__*/

    STATUS_ICON_UNREAD_EMAIL,

    STATUS_ICON_UNREAD_VOICE_L1L2,
    STATUS_ICON_UNREAD_VOICE_L1,
    STATUS_ICON_UNREAD_VOICE_L2,
    STATUS_ICON_UNREAD_FAX_L1L2,
    STATUS_ICON_UNREAD_FAX_L1,
    STATUS_ICON_UNREAD_FAX_L2,
    STATUS_ICON_UNREAD_EMAIL_L1L2,
    STATUS_ICON_UNREAD_EMAIL_L1,
    STATUS_ICON_UNREAD_EMAIL_L2,
    
    STATUS_ICON_UNREAD_NET_L1,
    STATUS_ICON_UNREAD_NET_L2,
    STATUS_ICON_UNREAD_NET_L1L2,

    #ifdef __MMI_DUAL_SIM_MASTER__
    STATUS_ICON_UNREAD_VOICE_L1_MASTER,
    STATUS_ICON_UNREAD_VOICE_L2_MASTER,
    STATUS_ICON_UNREAD_VOICE_L1L2_MASTER,
    STATUS_ICON_UNREAD_FAX_L1_MASTER,
    STATUS_ICON_UNREAD_FAX_L2_MASTER,
    STATUS_ICON_UNREAD_FAX_L1L2_MASTER,
    STATUS_ICON_UNREAD_EMAIL_L1_MASTER,
    STATUS_ICON_UNREAD_EMAIL_L2_MASTER,
    STATUS_ICON_UNREAD_EMAIL_L1L2_MASTER,
    STATUS_ICON_UNREAD_NET_L1_MASTER,
    STATUS_ICON_UNREAD_NET_L2_MASTER,
    STATUS_ICON_UNREAD_NET_L1L2_MASTER,
    
    STATUS_ICON_UNREAD_VOICE_L1_SLAVE,
    STATUS_ICON_UNREAD_VOICE_L2_SLAVE,
    STATUS_ICON_UNREAD_VOICE_L1L2_SLAVE,
    STATUS_ICON_UNREAD_FAX_L1_SLAVE,
    STATUS_ICON_UNREAD_FAX_L2_SLAVE,
    STATUS_ICON_UNREAD_FAX_L1L2_SLAVE,
    STATUS_ICON_UNREAD_EMAIL_L1_SLAVE,
    STATUS_ICON_UNREAD_EMAIL_L2_SLAVE,
    STATUS_ICON_UNREAD_EMAIL_L1L2_SLAVE,
    STATUS_ICON_UNREAD_NET_L1_SLAVE,
    STATUS_ICON_UNREAD_NET_L2_SLAVE,
    STATUS_ICON_UNREAD_NET_L1L2_SLAVE,
#endif  /*__MMI_DUAL_SIM_MASTER__*/

#ifdef __MMI_AUTO_ANSWER_MACHINE__
    STATUS_ICON_AUTOAM_REC,
    STATUS_ICON_AUTOAM_UNREAD,
    STATUS_ICON_AUTOAM_ON,
#endif /* __MMI_AUTO_ANSWER_MACHINE__ */ 

    STATUS_ICON_IMPS_MSG,
    STATUS_ICON_IMPS_LOGIN,
#ifdef __MMI_DUAL_SIM_MASTER__
        STATUS_ICON_WAP1,
        STATUS_ICON_WAP2,
#else /* __MMI_DUAL_SIM_MASTER__ */
    STATUS_ICON_WAP,
#endif /* __MMI_DUAL_SIM_MASTER__ */
    STATUS_ICON_POC,                        
    STATUS_ICON_BT,
    STATUS_ICON_BT_CON,
    STATUS_ICON_BT_ND,
    STATUS_ICON_IR,
    STATUS_ICON_STOP_WATCH,
    STATUS_ICON_NETWORK_CIPHER_GSM,
#ifdef __MMI_DUAL_SIM_MASTER__
    STATUS_ICON_SLAVE_NETWORK_CIPHER_GSM,
#endif
    STATUS_ICON_NETWORK_CIPHER_GPRS
#if defined(__J2ME__) && defined(J2ME_SUPPORT_BACKGROUND)
    ,
    STATUS_ICON_JAVA_BG_VM,
    STATUS_ICON_JAVA_BG_VM_AUDIO
#endif
#ifdef __MMI_BG_SOUND_EFFECT__
        ,
    STATUS_ICON_BG_SOUND
#endif /* __MMI_BG_SOUND_EFFECT__ */ 
#ifdef __MMI_AUDIO_REVERB_EFFECT__
        ,
    STATUS_ICON_REVERB_SOUND
#endif /* __MMI_AUDIO_REVERB_EFFECT__ */ 
#ifdef __MMI_AUDIO_SURROUND_EFFECT__
        ,
    STATUS_ICON_AUD_SURROUND
#endif
#ifdef __MMI_HOMEZONE_STATUS_ICON__
        ,
    STATUS_ICON_HOMEZONE,
    
#ifdef __MMI_DUAL_SIM_MASTER__
    STATUS_ICON_SLAVE_HOMEZONE,
#endif

    STATUS_ICON_CITYZONE
#endif /* __MMI_HOMEZONE_STATUS_ICON__ */ 
#ifdef __CTM_SUPPORT__
        ,
    STATUS_ICON_TTY_INDICATOR
#endif /* __CTM_SUPPORT__ */ 

    ,
    STATUS_ICON_EMAIL_REF

#ifdef __MMI_PICT_BRIDGE_SUPPORT__
        ,
    STATUS_ICON_PRINT,
    STATUS_ICON_PRINT_FAIL
#endif /* __MMI_PICT_BRIDGE_SUPPORT__ */

#if defined(__AGPS_SUPPORT__) && defined(__SUPL_SUPPORT__)
    ,
    STATUS_ICON_AGPS_STATE
#endif

#ifdef __MMI_SYNCML_STATUS_ICON_SUPPORT__
    ,STATUS_ICON_SYNCML
#endif

#ifdef __MMI_PROV_INBOX_SUPPORT__
    ,STATUS_ICON_PROVMSG_STATE
#ifdef __MMI_DUAL_SIM_MASTER__
    ,STATUS_ICON_PROVMSG_SIM1,
    STATUS_ICON_PROVMSG_SIM2
#endif
#endif
#if defined(__QQIM_SUPPORT__)
    ,
    STATUS_ICON_QQIM_AWAY,
    STATUS_ICON_QQIM_INVISIBLE,
    STATUS_ICON_QQIM_MSG,
    STATUS_ICON_QQIM_OFFLINE,
    STATUS_ICON_QQIM_ONLINE
#endif
/* Vogins Start */
#ifdef __VRE30_AM__
	,
	STATUS_ICON_VRE_DEFAULT,    
	STATUS_ICON_VRE_1,    
	STATUS_ICON_VRE_2,    
	STATUS_ICON_VRE_3,    
	STATUS_ICON_VRE_4 
#endif
/* Vogins End */
};

#define MMI_N_STATUS_ICONS_POOL2 ( sizeof(MMI_status_icons_pool2)/sizeof(MMI_status_icons_pool2[0]) )   /* 8 */

const S16 MMI_status_icons_pool2[] = 
{
    STATUS_ICON_SUBLCD_SIGNAL_STRENGTH,
    STATUS_ICON_SUBLCD_BATTERY_STRENGTH,
    STATUS_ICON_SUBLCD_SMS,
    STATUS_ICON_SUBLCD_RING,
    STATUS_ICON_SUBLCD_VIBRATE,
    STATUS_ICON_SUBLCD_VIBRATE_AND_RING,
    STATUS_ICON_SUBLCD_VIBRATE_THEN_RING,
    STATUS_ICON_SUBLCD_SILENT,
    STATUS_ICON_SUBLCD_ALARM,
    STATUS_ICON_SUBLCD_MISSED_CALL
};

#ifdef __MMI_LCD_PARTIAL_ON__
#define MMI_N_STATUS_ICONS_POOL_PARTIAL_DISPLAY ( sizeof(MMI_status_icons_pool_partial_display)/sizeof(MMI_status_icons_pool_partial_display[0]) )

/* For incoming events */
const S16 MMI_status_icons_pool_partial_display[] = 
{
    STATUS_ICON_INCOMING_SMS,
#ifdef __MMI_DUAL_SIM_MASTER__
        STATUS_ICON_SLAVE_INCOMING_SMS,
#endif /* __MMI_DUAL_SIM_MASTER__ */
    STATUS_ICON_MMS_UNREAD,
    
#ifdef __MMI_DUAL_SIM_MASTER__
            STATUS_ICON_MMS_UNREAD_SIM_1,
            STATUS_ICON_MMS_UNREAD_SIM_2,
#endif  /* __MMI_DUAL_SIM_MASTER__ */

    STATUS_ICON_MISSED_CALL,
#ifdef __MMI_DUAL_SIM_MASTER__
    STATUS_ICON_CARD2MISSED_CALL,
#endif/*__MMI_DUAL_SIM_MASTER__*/

    STATUS_ICON_UNREAD_EMAIL,

    STATUS_ICON_UNREAD_VOICE_L1L2,
    STATUS_ICON_UNREAD_VOICE_L1,
    STATUS_ICON_UNREAD_VOICE_L2,
    STATUS_ICON_UNREAD_EMAIL_L1L2,
    STATUS_ICON_UNREAD_EMAIL_L1,
    STATUS_ICON_UNREAD_EMAIL_L2,
    STATUS_ICON_UNREAD_NET_L1,
    STATUS_ICON_UNREAD_NET_L2,
    STATUS_ICON_UNREAD_NET_L1L2,
    
#ifdef __MMI_DUAL_SIM_MASTER__
    STATUS_ICON_UNREAD_VOICE_L1_MASTER,
    STATUS_ICON_UNREAD_VOICE_L2_MASTER,
    STATUS_ICON_UNREAD_VOICE_L1L2_MASTER,
    STATUS_ICON_UNREAD_FAX_L1_MASTER,
    STATUS_ICON_UNREAD_FAX_L2_MASTER,
    STATUS_ICON_UNREAD_FAX_L1L2_MASTER,
    STATUS_ICON_UNREAD_EMAIL_L1_MASTER,
    STATUS_ICON_UNREAD_EMAIL_L2_MASTER,
    STATUS_ICON_UNREAD_EMAIL_L1L2_MASTER,
    STATUS_ICON_UNREAD_NET_L1_MASTER,
    STATUS_ICON_UNREAD_NET_L2_MASTER,
    STATUS_ICON_UNREAD_NET_L1L2_MASTER,
    
    STATUS_ICON_UNREAD_VOICE_L1_SLAVE,
    STATUS_ICON_UNREAD_VOICE_L2_SLAVE,
    STATUS_ICON_UNREAD_VOICE_L1L2_SLAVE,
    STATUS_ICON_UNREAD_FAX_L1_SLAVE,
    STATUS_ICON_UNREAD_FAX_L2_SLAVE,
    STATUS_ICON_UNREAD_FAX_L1L2_SLAVE,
    STATUS_ICON_UNREAD_EMAIL_L1_SLAVE,
    STATUS_ICON_UNREAD_EMAIL_L2_SLAVE,
    STATUS_ICON_UNREAD_EMAIL_L1L2_SLAVE,
    STATUS_ICON_UNREAD_NET_L1_SLAVE,
    STATUS_ICON_UNREAD_NET_L2_SLAVE,
    STATUS_ICON_UNREAD_NET_L1L2_SLAVE,
#endif  /*__MMI_DUAL_SIM_MASTER__*/

#ifdef __MMI_AUTO_ANSWER_MACHINE__
		STATUS_ICON_AUTOAM_REC,
		STATUS_ICON_AUTOAM_UNREAD,
#endif /* __MMI_AUTO_ANSWER_MACHINE__ */ 

    STATUS_ICON_IMPS_MSG,
    
#ifdef __MMI_DUAL_SIM_MASTER__
    STATUS_ICON_WAP1,
    STATUS_ICON_WAP2
#else /* __MMI_DUAL_SIM_MASTER__ */
    STATUS_ICON_WAP
#endif /* __MMI_DUAL_SIM_MASTER__ */
    
};
#endif /* __MMI_LCD_PARTIAL_ON__ */ 

S16 MMI_status_icons_list1[MMI_N_STATUS_ICONS_LIST1];

/* store icons id of vertical  status bar of main lcd */
S16 MMI_status_icons_list2[MMI_N_STATUS_ICONS_LIST2];

/* store icons id of horizontal status bar of sub lcd  */
S16 MMI_status_icons_list3[MMI_N_STATUS_ICONS_LIST3];

#ifdef __MMI_LCD_PARTIAL_ON__
S16 MMI_status_icons_list_partial_display[MMI_N_STATUS_ICONS_PARTIAL_DISPLAY];
S16 MMI_status_icons_partial_display_n_icons;
#endif /* __MMI_LCD_PARTIAL_ON__ */ 

/* This is the customizable list of status icon bars        */

MMI_status_icon_bar MMI_status_icon_bars[MAX_STATUS_ICON_BARS];
static S32 status_icon_display_area = DEFAULT_BOTH;

/* Globals  */
/* Hide functions need to take x1,y1,x2,y2 as parameters */
void (*MMI_hide_status_icon_bars[MAX_STATUS_ICON_BARS]) (void);
void (*MMI_redraw_integrated_status_icon_bars) (void);
U8 status_icon_display_ON = 0;
U8 status_icon_blink_animate_ON = 0;
U8 MMI_status_icons_blink_animate_ON_state = 0;

MMI_BOOL g_wgui_status_bar_integrated_disabled = MMI_FALSE;

#define SUBLCD_ALPHA_VALUE 30

#ifdef __MMI_TOUCH_SCREEN__
FuncPtr MMI_status_icon_pen_event_hdlr[MAX_STATUS_ICONS][WGUI_STATUS_ICON_BAR_PEN_EVENT_MAX];
S16 pen_down_status_icon_id = -1;
#endif /* __MMI_TOUCH_SCREEN__ */ 

extern void draw_wallpaper(void);
extern U16 GetActiveScreenId(void);

#ifdef __MMI_SUB_WALLPAPER__
extern MMI_BOOL is_sub_idle_screen;
extern void cat312_hide_status_icon_bar(void);
#endif /* __MMI_SUB_WALLPAPER__ */ 

GDI_HANDLE wgui_status_icon_display_layer = GDI_LAYER_EMPTY_HANDLE;

extern idle_context_struct g_idle_context;

#define TEST_STATUS_ICON_DISPLAY_ON(flags)   (flags & (STATUS_ICON_ANIMATE|STATUS_ICON_REVERSE_ANIMATE|STATUS_ICON_FORWARD_ANIMATE|STATUS_ICON_BLINK|STATUS_ICON_DISABLED|STATUS_ICON_DISPLAY))

#ifdef __MMI_SUBLCD__
extern S32 SUB_LCD_device_width;
extern S32 SUB_LCD_device_height;
#endif /* __MMI_SUBLCD__ */ 

extern S32 Sub_lcd_title_height;


static void (*status_icon_update_callback)(void) = NULL;


#ifdef __MMI_UI_STATUS_BAR_AT_BOTTOM__
pBOOL g_status_bar_at_bottom = FALSE;

/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_whether_status_bar_at_bottom
 * DESCRIPTION
 *  
 * PARAMETERS
 *  void
 * RETURNS
 *  
 *****************************************************************************/
pBOOL wgui_status_icon_whether_status_bar_at_bottom(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    return g_status_bar_at_bottom;
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_set_status_bar_at_bottom
 * DESCRIPTION
 *  
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_set_status_bar_at_bottom(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    g_status_bar_at_bottom = TRUE;
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_reset_status_bar_at_bottom
 * DESCRIPTION
 *  
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_reset_status_bar_at_bottom(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    g_status_bar_at_bottom = FALSE;
}
#endif /* __MMI_UI_STATUS_BAR_AT_BOTTOM__ */ 


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_register_hide_status_icon_bar
 * DESCRIPTION
 *  register hide function of status bar
 * PARAMETERS
 *  status_bar_ID       [IN]        Bar id
 *  f                   [IN]        )(void)      :- function pointer
 *  horizontal(?)       [IN]        Bar :- sublcd
 *  vertical(?)         [IN]        Bar main lcd
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_register_hide_status_icon_bar(U8 status_bar_ID, void (*f) (void))
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    /* register the hide function of status bar */
    MMI_hide_status_icon_bars[status_bar_ID] = f;
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_clear_hide_status_icon_bars
 * DESCRIPTION
 *  clear hide function of status bar
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_clear_hide_status_icon_bars(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    S32 i;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    /* set the hide function  to dummy function */
    for (i = 0; i < MAX_STATUS_ICON_BARS; i++)
    {
        if (MMI_status_icon_bars[i].graphics_context == current_LCD_device_bitmap)
        {
            MMI_hide_status_icon_bars[i] = UI_dummy_function;
        }
    }
}


void wgui_status_icon_set_rotate_flag_for_wap(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_status_icon_bars[0].rotate_flag_for_wap = MMI_TRUE;
}


void wgui_status_icon_reset_rotate_flag_for_wap(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_status_icon_bars[0].rotate_flag_for_wap = MMI_FALSE;
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_arrange_status_icons
 * DESCRIPTION
 *  calulate the position of status icon of both main lcd (horizontal/vertical)
 *  and sub lcd
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_arrange_status_icons(void)
{
    S32 x1, y1, x2, y2, x, y, cx, cy;
    S32 i, j, k = 0, group_ID;
    U8 done = 0, inner_done, display_flag;
    S32 old_i; 

#ifdef WGUI_STATUS_ICON_SHOW_TIME_IN_RIGHT
    S32 w, h;
	U32 temp_clock_status_icon_id = WGUI_CLOCK_INVALID_INDEX;
#endif /* WGUI_STATUS_ICON_SHOW_TIME_IN_RIGHT */

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    /* Status bar 1: Horizontal bar  */

#ifdef __MMI_NONBLOCKING_DECODE__
    if(wgui_status_icon_blocking == MMI_TRUE)
    {
        return;
    }
#endif

    MMI_status_icons_list1[0] = STATUS_ICON_SIGNAL_STRENGTH;
    MMI_status_icons_list1[1] = STATUS_ICON_BATTERY_STRENGTH;
    MMI_status_icon_bars[0].n_icons = 2;

    x1 = MMI_status_icon_bars[0].x1;    /* start x position of horizontal status bar main lcd */
    y1 = MMI_status_icon_bars[0].y1;    /* start y position of horizontal status bar main lcd */
    x2 = MMI_status_icon_bars[0].x2;    /* end x position of horizontal status bar main lcd */
    y2 = MMI_status_icon_bars[0].y2;    /* end y position of horizontal status bar main lcd */

    if (MMI_status_icon_bars[0].rotate_flag_for_wap)
    {
        MMI_status_icons[MMI_status_icons_list1[1]].x = 
            MMI_status_icon_bars[0].x2 - MMI_status_icons[STATUS_ICON_BATTERY_STRENGTH].width - MMI_UI_STATUSBAR_GAP;
    }
    else
    {
        MMI_status_icons[MMI_status_icons_list1[1]].x = 
            MAIN_LCD_device_width - MMI_status_icons[STATUS_ICON_BATTERY_STRENGTH].width - MMI_UI_STATUSBAR_GAP;
    }

    /* Shift x1,x2 assuming signal strength and battery strength icons are at either ends  */
    x1 += MMI_status_icons[MMI_status_icons_list1[0]].width - 1 + STATUS_ICON_X_GAP + MMI_UI_STATUSBAR_GAP;
    x2 = MMI_status_icons[MMI_status_icons_list1[1]].x - STATUS_ICON_X_GAP;

#ifdef WGUI_STATUS_ICON_SHOW_TIME_IN_RIGHT
	wgui_clock_create(&temp_clock_status_icon_id, 
		0, 0, 0, 0, 
        GUI_CLOCK_MAINLCD_TYPE_DIGITAL_TIME_SMALL);

	wgui_clock_measure_digital_small_clock(temp_clock_status_icon_id, &w, &h);

	wgui_clock_close(temp_clock_status_icon_id);
            
    #ifndef WGUI_STATUS_ICON_ALIGN_RIGHT
        t_x1 = x2 - w;
        t_y1 = y1;
        t_x2 = x2;
        t_y2 = y2;
    #else /* WGUI_STATUS_ICON_ALIGN_RIGHT */
        t_x1 = x1;
        t_y1 = y1;
        t_x2 = t_x1 + w - 1;
        t_y2 = y2;
    #endif /* WGUI_STATUS_ICON_ALIGN_RIGHT */
#endif /* WGUI_STATUS_ICON_SHOW_TIME_IN_RIGHT */

#if(STATUS_ICONS_DRAW_FROM_BOTTOM_BASELINE)
    cy = y2 - y1 + 1;
#else 
    cy = (y2 - y1 + 1) >> 1;
#endif 

#if(STATUS_ICONS_DRAW_FROM_BOTTOM_BASELINE)
#ifdef __MMI_UI_STATUS_BAR_AT_BOTTOM__
    if (wgui_status_icon_whether_status_bar_at_bottom())
    {
        MMI_status_icons[0].y =
            UI_device_height - MMI_button_bar_height - 1 - (MMI_status_icons[STATUS_ICON_SIGNAL_STRENGTH].height + 2);
        MMI_status_icons[1].y =
            UI_device_height - MMI_button_bar_height - 1 - (MMI_status_icons[STATUS_ICON_BATTERY_STRENGTH].height + 2);
    }
    else
#endif /* __MMI_UI_STATUS_BAR_AT_BOTTOM__ */ 
    {
        MMI_status_icons[0].y = cy - MMI_status_icons[0].height - 1;
        MMI_status_icons[1].y = cy - MMI_status_icons[1].height - 1;
    }
#else /* (STATUS_ICONS_DRAW_FROM_BOTTOM_BASELINE) */ 
#ifdef __MMI_UI_STATUS_BAR_AT_BOTTOM__
    if (wgui_status_icon_whether_status_bar_at_bottom())
    {
        MMI_status_icons[0].y =
            UI_device_height - MMI_button_bar_height - 1 - MMI_status_bar_height - 2 +
            ((MMI_status_bar_height - MMI_status_icons[STATUS_ICON_SIGNAL_STRENGTH].height) >> 1);
        MMI_status_icons[1].y =
            UI_device_height - MMI_button_bar_height - 1 - MMI_status_bar_height - 2 +
            ((MMI_status_bar_height - MMI_status_icons[STATUS_ICON_SIGNAL_STRENGTH].height) >> 1);
    }
    else
#endif /* __MMI_UI_STATUS_BAR_AT_BOTTOM__ */ 
    {
        MMI_status_icons[0].y = cy - (MMI_status_icons[0].height >> 1);
        MMI_status_icons[1].y = cy - (MMI_status_icons[1].height >> 1);
    }
#endif /* (STATUS_ICONS_DRAW_FROM_BOTTOM_BASELINE) */ 

#ifndef WGUI_STATUS_ICON_ALIGN_RIGHT
    i = 2;
        x = x1 + STATUS_ICON_X_GAP;
    while (!done)
    {
        display_flag = 0;
        j = MMI_status_icons_pool1[i];
        old_i = i;  
        i++;
        if (i > MMI_N_STATUS_ICONS_POOL1)
        {
            break;
        }
        if (TEST_STATUS_ICON_DISPLAY_ON(MMI_status_icons[j].flags))
        {
            k = j;
            display_flag = 1;
        }
        if ((MMI_status_icons[j].group_ID > 0))
        {
            group_ID = MMI_status_icons[j].group_ID;
            j++;
            inner_done = 0;
            while (!inner_done)
            {
                if (group_ID != MMI_status_icons[j].group_ID)
                {
                    break;
                }
                if ((!display_flag) && TEST_STATUS_ICON_DISPLAY_ON(MMI_status_icons[j].flags))
                {
                    display_flag = 1;
                    k = j;
                }
                else
                {
                    i++;
                    j++;
                }
            }
        }
        if (display_flag)   /* if flagis set to display */
        {
        #ifdef WGUI_STATUS_ICON_SHOW_TIME_IN_RIGHT
            if ((x + MMI_status_icons[k].width) > x2 - w - STATUS_ICON_X_GAP)   
        #else/* WGUI_STATUS_ICON_SHOW_TIME_IN_RIGHT */
            if ((x + MMI_status_icons[k].width) > x2)   
        #endif/* WGUI_STATUS_ICON_SHOW_TIME_IN_RIGHT */
            {                                          
                i = old_i;                              
                break;
            }
        #if(STATUS_ICONS_DRAW_FROM_BOTTOM_BASELINE)
        #ifdef __MMI_UI_STATUS_BAR_AT_BOTTOM__
            if (wgui_status_icon_whether_status_bar_at_bottom())
            {
                y = UI_device_height - MMI_button_bar_height - 1 - MMI_status_icons[k].height - 2;
            }
            else
        #endif /* __MMI_UI_STATUS_BAR_AT_BOTTOM__ */ 
                y = cy - MMI_status_icons[k].height - 1;
        #else /* (STATUS_ICONS_DRAW_FROM_BOTTOM_BASELINE) */ 
        #ifdef __MMI_UI_STATUS_BAR_AT_BOTTOM__
            if (wgui_status_icon_whether_status_bar_at_bottom())
            {
                y = UI_device_height - MMI_button_bar_height - 1 - MMI_status_bar_height +
                    ((MMI_status_bar_height - MMI_status_icons[STATUS_ICON_SIGNAL_STRENGTH].height) >> 1);
            }
            else
        #endif /* __MMI_UI_STATUS_BAR_AT_BOTTOM__ */ 
                y = cy - (MMI_status_icons[k].height >> 1);
        #endif /* (STATUS_ICONS_DRAW_FROM_BOTTOM_BASELINE) */ 
            MMI_status_icons[k].x = x;
            MMI_status_icons[k].y = y;
            x += MMI_status_icons[k].width + STATUS_ICON_X_GAP;
            MMI_status_icons_list1[MMI_status_icon_bars[0].n_icons] = (S16) k;
            MMI_status_icon_bars[0].n_icons++;
            if (MMI_status_icon_bars[0].n_icons >= MMI_N_STATUS_ICONS_LIST1)
            {
                done = 1;
            }
        }
    }
#else /* WGUI_STATUS_ICON_ALIGN_RIGHT */
    i = 2;

#ifdef WGUI_STATUS_ICON_SHOW_TIME_IN_RIGHT
    x = x2 - STATUS_ICON_X_GAP - w;
#else/* WGUI_STATUS_ICON_SHOW_TIME_IN_RIGHT */
    x = x2 - STATUS_ICON_X_GAP;
#endif/* WGUI_STATUS_ICON_SHOW_TIME_IN_RIGHT */

    while (!done)
    {
        display_flag = 0;
        j = MMI_status_icons_pool1[i];
        old_i = i;  
        i++;
        if (i > MMI_N_STATUS_ICONS_POOL1)
        {
            break;
        }
        if (TEST_STATUS_ICON_DISPLAY_ON(MMI_status_icons[j].flags))
        {
            k = j;
            display_flag = 1;
        }
        if ((MMI_status_icons[j].group_ID > 0))
        {
            group_ID = MMI_status_icons[j].group_ID;
            j++;
            inner_done = 0;
            while (!inner_done)
            {
                if (group_ID != MMI_status_icons[j].group_ID)
                {
                    break;
                }
                if ((!display_flag) && TEST_STATUS_ICON_DISPLAY_ON(MMI_status_icons[j].flags))
                {
                    display_flag = 1;
                    k = j;
                }
                else
                {
                    i++;
                    j++;
                }
            }
        }
        if (display_flag)   /* if flagis set to display */
        {
            if ((x - MMI_status_icons[k].width) < x1)   
            {                                          
                i = old_i;                              
                break;
            }
    #if(STATUS_ICONS_DRAW_FROM_BOTTOM_BASELINE)
    #ifdef __MMI_UI_STATUS_BAR_AT_BOTTOM__
            if (wgui_status_icon_whether_status_bar_at_bottom())
            {
                y = UI_device_height - MMI_button_bar_height - 1 - MMI_status_icons[k].height - 2;
            }
            else
    #endif /* __MMI_UI_STATUS_BAR_AT_BOTTOM__ */ 
                y = cy - MMI_status_icons[k].height - 1;
    #else /* (STATUS_ICONS_DRAW_FROM_BOTTOM_BASELINE) */ 
    #ifdef __MMI_UI_STATUS_BAR_AT_BOTTOM__
            if (wgui_status_icon_whether_status_bar_at_bottom())
            {
                y = UI_device_height - MMI_button_bar_height - 1 - MMI_status_bar_height +
                    ((MMI_status_bar_height - MMI_status_icons[STATUS_ICON_SIGNAL_STRENGTH].height) >> 1);
            }
            else
    #endif /* __MMI_UI_STATUS_BAR_AT_BOTTOM__ */ 
                y = cy - (MMI_status_icons[k].height >> 1);
    #endif /* (STATUS_ICONS_DRAW_FROM_BOTTOM_BASELINE) */ 
            MMI_status_icons[k].x = x - MMI_status_icons[k].width;
            MMI_status_icons[k].y = y;
            x -= MMI_status_icons[k].width + STATUS_ICON_X_GAP;
            MMI_status_icons_list1[MMI_status_icon_bars[0].n_icons] = (S16) k;
            MMI_status_icon_bars[0].n_icons++;
            if (MMI_status_icon_bars[0].n_icons >= MMI_N_STATUS_ICONS_LIST1)
            {
                done = 1;
            }
        }
    }
#endif /* WGUI_STATUS_ICON_ALIGN_RIGHT */

    /* Status bar 2: Vertical bar */

    MMI_status_icon_bars[1].n_icons = 0;
    x1 = MMI_status_icon_bars[1].x1;    /* start x position of vertical  status bar main lcd */
    y1 = MMI_status_icon_bars[1].y1;    /* start y position of vertical status bar main lcd */
    x2 = MMI_status_icon_bars[1].x2;    /* end x position of vertical status bar main lcd */
    y2 = MMI_status_icon_bars[1].y2;    /* end y position of horizontal status bar main lcd */
    cx = (x2 - x1 + 1) >> 1;
#ifdef __MMI_UI_STATUS_BAR_AT_BOTTOM__
    if (wgui_status_icon_whether_status_bar_at_bottom())
    {
    #ifdef __MMI_MAINLCD_128X160__
        y = UI_device_height - MMI_button_bar_height - MMI_status_bar_height -
            ((MMI_status_icons[1].height + (STATUS_ICON_Y_GAP) << 1) + STATUS_ICON_Y_GAP + 2) - 1;
    #else /* __MMI_MAINLCD_128X160__ */ 
        y = UI_device_height - MMI_button_bar_height - MMI_status_bar_height -
            ((MMI_status_icons[1].height + (STATUS_ICON_Y_GAP) << 1) + STATUS_ICON_Y_GAP) - 1;
    #endif /* __MMI_MAINLCD_128X160__ */ 
    }
    else
#endif /* __MMI_UI_STATUS_BAR_AT_BOTTOM__ */ 
    {
        y = STATUS_ICON_Y_GAP;
    }
    done = 0;
    while (!done)
    {
        display_flag = 0;
        j = MMI_status_icons_pool1[i];
        old_i = i;
        i++;
        if (i > MMI_N_STATUS_ICONS_POOL1)
        {
            break;
        }
        if (TEST_STATUS_ICON_DISPLAY_ON(MMI_status_icons[j].flags))     /* check if the flag is on /off */
        {
            k = j;
            display_flag = 1;
        }
        if ((MMI_status_icons[j].group_ID > 0))
        {
            group_ID = MMI_status_icons[j].group_ID;
            j++;
            inner_done = 0;
            while (!inner_done)
            {
                if (group_ID != MMI_status_icons[j].group_ID)
                {
                    break;
                }
                if ((!display_flag) && TEST_STATUS_ICON_DISPLAY_ON(MMI_status_icons[j].flags))
                {
                    display_flag = 1;
                    k = j;
                }
                else
                {
                    i++;
                    j++;
                }
            }
        }
        if (display_flag)
        {
        #ifdef __MMI_UI_STATUS_BAR_AT_BOTTOM__
            if ((y - MMI_status_icons[k].height) < y1)
        #else /* __MMI_UI_STATUS_BAR_AT_BOTTOM__ */ 
            if ((y1 + y + MMI_status_icons[k].height) > y2) 
        #endif /* __MMI_UI_STATUS_BAR_AT_BOTTOM__ */ 
            {                         
                i = old_i;
                break;
            }
            x = cx - (MMI_status_icons[k].width >> 1);
            MMI_status_icons[k].x = x;  /* set the x and y position of icon */
            MMI_status_icons[k].y = y;
        #ifdef __MMI_UI_STATUS_BAR_AT_BOTTOM__
            if (wgui_status_icon_whether_status_bar_at_bottom())
            {
                y = MMI_status_icons[k].y - (MMI_status_icons[k].height + STATUS_ICON_Y_GAP);
            }
        #else /* __MMI_UI_STATUS_BAR_AT_BOTTOM__ */ 
            y += MMI_status_icons[k].height + STATUS_ICON_Y_GAP;
        #endif /* __MMI_UI_STATUS_BAR_AT_BOTTOM__ */ 
            MMI_status_icons_list2[MMI_status_icon_bars[1].n_icons] = (S16) k;
            MMI_status_icon_bars[1].n_icons++;
            if (MMI_status_icon_bars[1].n_icons >= MMI_N_STATUS_ICONS_LIST2)
            {
                done = 1;
            }
        }
    }

    /* Status bar 3: Sub LCD status bar */

    /* arrange status icons in sublcd by priority */
    MMI_status_icons_list3[0] = STATUS_ICON_SUBLCD_SIGNAL_STRENGTH;
    MMI_status_icons_list3[1] = STATUS_ICON_SUBLCD_BATTERY_STRENGTH;
    MMI_status_icon_bars[2].n_icons = 2;

    x1 = MMI_status_icon_bars[2].x1;    /* start x position of status bar sub lcd */
    y1 = MMI_status_icon_bars[2].y1;    /* start y position of status bar sub lcd */
    x2 = MMI_status_icon_bars[2].x2;    /* end x position of status bar sub lcd */
    y2 = MMI_status_icon_bars[2].y2;    /* end y position of status bar sub lcd */

    /* Shift x1,x2 assuming signal strength and battery strength icons are at either ends  */
    x1 += MMI_status_icons[MMI_status_icons_list3[0]].width - 1 + SUBLCD_STATUS_ICON_X_GAP;
    x2 = MMI_status_icons[MMI_status_icons_list3[1]].x - SUBLCD_STATUS_ICON_X_GAP;

#if(STATUS_ICONS_DRAW_FROM_BOTTOM_BASELINE)
    cy = y2 - y1 - 1;
#else 
    cy = (y2 - y1 + 1) >> 1;
#endif 

    x = x1 + SUBLCD_STATUS_ICON_X_GAP;

#if(STATUS_ICONS_DRAW_FROM_BOTTOM_BASELINE)
    cy = MMI_status_icon_bars[2].y2 - MMI_status_icon_bars[2].y1 - 1;
#endif 

    done = 0;
    i = 2;
    while (!done)
    {
        display_flag = 0;
        j = MMI_status_icons_pool2[i];
        i++;
        if (i > MMI_N_STATUS_ICONS_POOL2)
        {
            break;
        }
        if (TEST_STATUS_ICON_DISPLAY_ON(MMI_status_icons[j].flags))
        {
            k = j;
            display_flag = 1;
        }
        if ((MMI_status_icons[j].group_ID > 0))
        {
            group_ID = MMI_status_icons[j].group_ID;
            j++;
            inner_done = 0;
            while (!inner_done)
            {
                if (group_ID != MMI_status_icons[j].group_ID)
                {
                    break;
                }
                if ((!display_flag) && TEST_STATUS_ICON_DISPLAY_ON(MMI_status_icons[j].flags))
                {
                    display_flag = 1;
                    k = j;
                }
                else
                {
                    i++;
                    j++;
                }
            }
        }
        if (display_flag)
        {
            /*arrange status icons in sublcd by priority*/
            if (x + MMI_status_icons[k].width > x2) /* 022205 Calvin modified */
            {
                break;
            }
            MMI_status_icons_list3[MMI_status_icon_bars[2].n_icons] = (S16) k;
            MMI_status_icon_bars[2].n_icons++;
        #if(STATUS_ICONS_DRAW_FROM_BOTTOM_BASELINE)
            y = cy - MMI_status_icons[k].height;
        #else 
            y = cy - (MMI_status_icons[k].height >> 1);
        #endif 
            MMI_status_icons[k].x = x;
            MMI_status_icons[k].y = y;
            x += MMI_status_icons[k].width + SUBLCD_STATUS_ICON_X_GAP;
        }
    }

#ifdef __MMI_LCD_PARTIAL_ON__
    /* Extra status bar: Used in LCD partial display */
    MMI_status_icons_partial_display_n_icons = 0;
    done = 0;
    i = 0;
    while (!done)
    {
        display_flag = 0;
        j = MMI_status_icons_pool_partial_display[i];
        i++;

        if (i > MMI_N_STATUS_ICONS_POOL_PARTIAL_DISPLAY)
        {
            break;
        }

        if (TEST_STATUS_ICON_DISPLAY_ON(MMI_status_icons[j].flags))
        {
            k = j;
            display_flag = 1;
        }
        if (display_flag)
        {
            MMI_status_icons_list_partial_display[MMI_status_icons_partial_display_n_icons] = (S16) k;
            MMI_status_icons_partial_display_n_icons++;
            if (MMI_status_icons_partial_display_n_icons >= MMI_N_STATUS_ICONS_PARTIAL_DISPLAY)
            {
                done = 1;
            }
        }
    }
#endif /* __MMI_LCD_PARTIAL_ON__ */ 
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_hide_status_icon_bars
 * DESCRIPTION
 *  call hide function of status bar (horizontal/vertical status bar of main
 *  lcd and horizontal status bar of sub lcd
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_hide_status_icon_bars(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    S32 i;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    for (i = 0; i < MAX_STATUS_ICON_BARS; i++)
    {
        if (MMI_status_icon_bars[i].flags & STATUS_ICON_BAR_DISPLAY)    /* stopwatch icon changed because re-arragne earlier */
        {
            if (MMI_status_icon_bars[i].graphics_context == &main_LCD_device_bitmap)
            {
                if (status_icon_display_area != SUBLCD_ONLY)
                {
                    S32 cx1, cy1, cx2, cy2;
                    
                    gdi_layer_push_clip();
                    wgui_status_icon_get_status_icon_bar_clip((U8)i, &cx1, &cy1, &cx2, &cy2);
                    gdi_layer_set_clip(cx1, cy1, cx2, cy2);
                    if (wgui_status_bar_alpha_layer != GDI_NULL_HANDLE)
                    {
                        gdi_push_and_set_alpha_blending_source_layer(wgui_status_bar_alpha_layer);
                    }
                    MMI_hide_status_icon_bars[i] ();    /* execute the hide function of status bar */
                    if (wgui_status_bar_alpha_layer != GDI_NULL_HANDLE)
                    {
                        gdi_pop_and_restore_alpha_blending_source_layer();
                    }
                    gdi_layer_pop_clip();
                }
            }
            else if (MMI_status_icon_bars[i].graphics_context == &sub_LCD_device_bitmap)
            {
                if (status_icon_display_area != MAINLCD_ONLY)
                {
                    S32 cx1, cy1, cx2, cy2;
                    
                    gdi_layer_push_clip();
                    wgui_status_icon_get_status_icon_bar_clip((U8)i, &cx1, &cy1, &cx2, &cy2);
                    gdi_layer_set_clip(cx1, cy1, cx2, cy2);
                    if (wgui_status_bar_alpha_layer != GDI_NULL_HANDLE)
                    {
                        gdi_push_and_set_alpha_blending_source_layer(wgui_status_bar_alpha_layer);
                    }
                    MMI_hide_status_icon_bars[i] ();    /* execute the hide function of status bar */
                    
                    if (wgui_status_bar_alpha_layer != GDI_NULL_HANDLE)
                    {
                        gdi_pop_and_restore_alpha_blending_source_layer();
                    }
                    gdi_layer_pop_clip();
                }
            }
        }
    }
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_refresh_fixed_status_icon
 * DESCRIPTION
 *  refrsh particulr icon of status bar
 * PARAMETERS
 *  icon_id     [IN]        
 *  bar_id      [IN]        
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_refresh_fixed_status_icon(S16 icon_id, S16 bar_id)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
#ifdef __MMI_NONBLOCKING_DECODE__    
    if(wgui_status_icon_blocking == MMI_TRUE)
    {
        return;
    }
#endif

    if ((status_icon_display_ON) && (MMI_status_icon_bars[bar_id].flags & STATUS_ICON_BAR_DISPLAY))
    {
        S32 x1, x2, y1, y2, cx1, cx2, cy1, cy2;

        /* backup active lcd */
        bitmap *saved_graphics_context = current_LCD_device_bitmap;

        wgui_status_icon_arrange_status_icons();

        /* set drawing region */
        x1 = MMI_status_icons[icon_id].x;   /* x start position of icon */
#ifdef __MMI_UI_STATUS_BAR_AT_BOTTOM__
        y1 = MMI_status_icons[icon_id].y;   /* y start poition of icon */
#else
        y1 = MMI_status_icon_bars[bar_id].y + MMI_status_icons[icon_id].y;   /* y start poition of icon */
#endif

        x2 = x1 + MMI_status_icons[icon_id].width - 1;  /* x end position of icon */
        y2 = y1 + MMI_status_icons[icon_id].height - 1; /* y end position of icon */

        /* set active lcd */
        if (bar_id == 2)
        {
            UI_set_sub_LCD_graphics_context();  /* if barid is equal to 2 then switch the graphic context to sub lcd. */
        }
        else
        {
            UI_set_main_LCD_graphics_context(); /* switch graphic context to main lcd if bar id is eqaul to 0 or 1 */
        }

        if (wgui_status_icon_display_layer != GDI_LAYER_EMPTY_HANDLE)
        {
            gdi_layer_push_and_set_active(wgui_status_icon_display_layer);
        }
        /* lock frame buffer */
        gdi_layer_lock_frame_buffer();
        
        if(wgui_status_icon_get_status_icon_bar_layer((U8)bar_id)!=GDI_ERROR_HANDLE)
        {
            gdi_layer_push_and_set_active(wgui_status_icon_get_status_icon_bar_layer((U8)bar_id));
        }

        /* set clip region */
        gui_push_clip();
        gui_set_clip(x1, y1, x2, y2);

        if (bar_id == 2)    /* SUBLCD status icons */
        {
        #ifdef __MMI_SUB_WALLPAPER__
            if (is_sub_idle_screen)
            {
                gdi_draw_solid_rect(x1, y1, x2, y2, GDI_COLOR_TRANSPARENT);
            }
            else
            {
                wgui_status_icon_show_sublcd_status_background(x1, y1, x2, y2, MMI_TRUE);
            }
        #else /* __MMI_SUB_WALLPAPER__ */ 
            wgui_status_icon_show_sublcd_status_background(x1, y1, x2, y2, MMI_TRUE);
        #endif /* __MMI_SUB_WALLPAPER__ */ 
        }
        else
        {
            
            wgui_status_icon_get_status_icon_bar_clip(0, &cx1, &cy1, &cx2, &cy2);
            wgui_status_icon_set_status_icon_bar_clip(0, x1, y1, x2, y2);
            gdi_layer_push_clip();
            gdi_layer_set_clip(x1, y1, x2, y2);
            if (wgui_status_bar_alpha_layer != GDI_NULL_HANDLE)
            {
                gdi_push_and_set_alpha_blending_source_layer(wgui_status_bar_alpha_layer);
            }
            MMI_hide_status_icon_bars[bar_id]();
            if (wgui_status_bar_alpha_layer != GDI_NULL_HANDLE)
            {
                gdi_pop_and_restore_alpha_blending_source_layer();
            }
            gdi_layer_pop_clip();
            wgui_status_icon_set_status_icon_bar_clip(0, cx1, cy1, cx2, cy2);
                    }
                    
        /* show the icon */
    #ifdef __MMI_DOWNLOADABLE_THEMES_SUPPORT__
        if (mmi_tm_does_theme_have_alt_image())
        {
            if (mmi_tm_is_theme_dirty_bit_set(GetImageIndex(MMI_status_icons[icon_id].icon_ID)))
            {
                MMI_status_icons[icon_id].icon_data = get_image(MMI_status_icons[icon_id].icon_ID);
                /* No need to calculate Image width, height and frames as they will not change */
            }
        }
    #endif /* __MMI_DOWNLOADABLE_THEMES_SUPPORT__ */ 
            if (wgui_status_bar_alpha_layer != GDI_NULL_HANDLE)
            {
                gdi_push_and_set_alpha_blending_source_layer(wgui_status_bar_alpha_layer);
            }
            gdi_image_draw_frames_blend2layers(
                x1,
                y1,
                MMI_status_icons[icon_id].icon_data,
                MMI_status_icons[icon_id].state);
            if (wgui_status_bar_alpha_layer != GDI_NULL_HANDLE)
            {
                gdi_pop_and_restore_alpha_blending_source_layer();
            }

        /* restoer clip region */
        gui_pop_clip();
        
        if(wgui_status_icon_get_status_icon_bar_layer((U8)bar_id)!=GDI_ERROR_HANDLE)
        {
            gdi_layer_pop_and_restore_active();
        }

        /* unlock and blt frame buffer */
        gdi_layer_unlock_frame_buffer();

        
    #ifdef __MMI_VUI_ENGINE__
    
        if (MMI_status_icon_bars[0].venus_update_screen_callback)
        {
            MMI_status_icon_bars[0].venus_update_screen_callback(MMI_status_icon_bars[0].venus_update_screen_handle);
        }
        else
        {
            gui_BLT_double_buffer(x1, y1, x2, y2);
        }
        
    #else /* __MMI_VUI_ENGINE__ */
    
        gui_BLT_double_buffer(x1, y1, x2, y2);

    #endif /* __MMI_VUI_ENGINE__ */


        /* 061506 double buffer Start */
        if(wgui_status_icon_get_status_icon_bar_type((U8)bar_id)&STATUS_ICON_BAR_ENABLE_DOUBLE_BUFFER)
        {
            gdi_layer_push_and_set_active(wgui_status_icon_get_status_icon_bar_layer((U8)bar_id));
            gdi_layer_toggle_double();
            gdi_layer_copy_double();
            gdi_layer_pop_and_restore_active();
        }

        if (wgui_status_icon_display_layer != GDI_LAYER_EMPTY_HANDLE)
        {
            gdi_layer_pop_and_restore_active();
        }

        /* restore active lcd according to backuped lcd */
        if (saved_graphics_context == &main_LCD_device_bitmap)
        {
            UI_set_main_LCD_graphics_context();
        }
        else if (saved_graphics_context == &sub_LCD_device_bitmap)
        {
            UI_set_sub_LCD_graphics_context();
        }
    }
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_refresh_status_icon_display
 * DESCRIPTION
 *  refrsh all status bar icons
 * PARAMETERS
 *  void
 *  S16(?)      [IN]        Bar_id
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_refresh_status_icon_display(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    S32 idx = 0;
    S32 is_status_layer_pushed = 0;
    
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
#ifdef __MMI_NONBLOCKING_DECODE__
    if(wgui_status_icon_blocking == MMI_TRUE)
    {
        return;
    }
#endif

    if (status_icon_display_ON)
    {
        /* backup active lcd */
        bitmap *saved_graphics_context = current_LCD_device_bitmap;
        S32 i;

        if (wgui_status_icon_display_layer != GDI_LAYER_EMPTY_HANDLE)
        {
            gdi_layer_push_and_set_active(wgui_status_icon_display_layer);
            is_status_layer_pushed = 1;
        }
        /* lock frame buffer */
        gui_lock_double_buffer();

        /* hide all status bars */
        wgui_status_icon_hide_status_icon_bars();

        /* show all status bar icons */
        wgui_status_icon_show_status_icons();

        /* unlock frame buffer */
        gui_unlock_double_buffer();

        for (i = 0; i < MAX_STATUS_ICON_BARS; i++)
        {
            if (MMI_status_icon_bars[i].flags & STATUS_ICON_BAR_DISPLAY)
            {
                if (MMI_status_icon_bars[i].graphics_context == &main_LCD_device_bitmap)
                {
                    if (status_icon_display_area != SUBLCD_ONLY)
                    {
                        UI_set_main_LCD_graphics_context(); /* switch to main lcd graphic context */
                        
                    #ifdef __MMI_VUI_ENGINE__
    
                        if (MMI_status_icon_bars[0].venus_update_screen_callback)
                        {
                            MMI_status_icon_bars[0].venus_update_screen_callback(MMI_status_icon_bars[0].venus_update_screen_handle);
                        }
                        else
                        {
                            gui_BLT_double_buffer(
                                MMI_status_icon_bars[i].x1,
                                MMI_status_icon_bars[i].y1,
                                MMI_status_icon_bars[i].x2,
                                MMI_status_icon_bars[i].y2);
                         }
                        
                    #else /* __MMI_VUI_ENGINE__ */
                    
                        gui_BLT_double_buffer(
                            MMI_status_icon_bars[i].x1,
                            MMI_status_icon_bars[i].y1,
                            MMI_status_icon_bars[i].x2,
                            MMI_status_icon_bars[i].y2);
                    
                    #endif /* __MMI_VUI_ENGINE__ */
                    
                    }
                }
                else if (MMI_status_icon_bars[i].graphics_context == &sub_LCD_device_bitmap)
                {
                    if (status_icon_display_area != MAINLCD_ONLY)
                    {
                        UI_set_sub_LCD_graphics_context();  /* switch to sub lcd graphic context */
                        gui_BLT_double_buffer(
                            MMI_status_icon_bars[i].x1,
                            MMI_status_icon_bars[i].y1,
                            MMI_status_icon_bars[i].x2,
                            MMI_status_icon_bars[i].y2);
                    }
                }
            }
        }

        for (idx=0; idx<MAX_STATUS_ICON_BARS; idx++)
        {
            if(wgui_status_icon_get_status_icon_bar_type((U8)idx)&STATUS_ICON_BAR_ENABLE_DOUBLE_BUFFER)
            {
                gdi_layer_push_and_set_active(wgui_status_icon_get_status_icon_bar_layer((U8)idx));
                gdi_layer_toggle_double();
                gdi_layer_copy_double();
                gdi_layer_pop_and_restore_active();
            }
        }
    
        /* restore active lcd according to backuped lcd */
        if (saved_graphics_context == &main_LCD_device_bitmap)
        {
            UI_set_main_LCD_graphics_context();
        }
        else if (saved_graphics_context == &sub_LCD_device_bitmap)
        {
            UI_set_sub_LCD_graphics_context();
        }
        if (is_status_layer_pushed == 1)
        {
            gdi_layer_pop_and_restore_active();
        }
    }
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_animate_status_icon
 * DESCRIPTION
 *  animate teh status icon
 * PARAMETERS
 *  icon_ID         [IN]        
 *  icon_id(?)      [IN]        
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_animate_status_icon1(S16 icon_ID)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if ((icon_ID < 0) || (icon_ID >= MAX_STATUS_ICONS))
    {
        return;
    }

    /* set the flaf of icon animated */
    MMI_status_icons[icon_ID].flags |= STATUS_ICON_ANIMATE; /* set the flag of icon to animate */

    /* set teh currenty frame to zero */
    MMI_status_icons[icon_ID].current_frame = 0;

    wgui_status_icon_setup_status_icon_blink_animate();
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_forward_animate_status_icon
 * DESCRIPTION
 *  show the animation of icon such that it is moving forward direction
 * PARAMETERS
 *  icon_ID         [IN]        
 *  icon_id(?)      [IN]        
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_forward_animate_status_icon(S16 icon_ID)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if ((icon_ID < 0) || (icon_ID >= MAX_STATUS_ICONS))
    {
        return;
    }

    /* set teh flag of icon to forward animate */
    MMI_status_icons[icon_ID].flags |= STATUS_ICON_FORWARD_ANIMATE;

    wgui_status_icon_setup_status_icon_blink_animate();
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_reverse_animate_status_icon
 * DESCRIPTION
 *  show the animation of icon such that it is moving in reverse ditrection
 * PARAMETERS
 *  icon_ID         [IN]        
 *  icon_id(?)      [IN]        
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_reverse_animate_status_icon(S16 icon_ID)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if ((icon_ID < 0) || (icon_ID >= MAX_STATUS_ICONS))
    {
        return;
    }

    /* set the flag of icon to reverse animate */
    MMI_status_icons[icon_ID].flags |= STATUS_ICON_REVERSE_ANIMATE;

    wgui_status_icon_setup_status_icon_blink_animate();
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_blink_status_icon
 * DESCRIPTION
 *  set teh flag of icon as blink and shw the blinking icon
 * PARAMETERS
 *  icon_ID         [IN]        
 *  icon_id(?)      [IN]        
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_blink_status_icon1(S16 icon_ID)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if ((icon_ID < 0) || (icon_ID >= MAX_STATUS_ICONS))
    {
        return;
    }

    /* Hide status icon in flight mode */
    if (!mmi_flight_mode_is_status_icon_visible(icon_ID, MMI_status_icons[icon_ID].flags | STATUS_ICON_BLINK))
    {
        return;
    }

    /* set teh flag of icon to blink */
    MMI_status_icons[icon_ID].flags |= STATUS_ICON_BLINK;

    wgui_status_icon_setup_status_icon_blink_animate();
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_disable_status_icon
 * DESCRIPTION
 *  set teh flag of icon as disable
 * PARAMETERS
 *  icon_ID         [IN]        
 *  icon_id(?)      [IN]        
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_disable_status_icon(S16 icon_ID)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if ((icon_ID < 0) || (icon_ID >= MAX_STATUS_ICONS))
    {
        return;
    }

    /* set the flag of icon to disable */
    MMI_status_icons[icon_ID].flags |= STATUS_ICON_DISABLED;
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_enable_status_icon
 * DESCRIPTION
 *  set teh flag of icon as enable
 * PARAMETERS
 *  icon_ID         [IN]        
 *  icon_id(?)      [IN]        
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_enable_status_icon(S16 icon_ID)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if ((icon_ID < 0) || (icon_ID >= MAX_STATUS_ICONS))
    {
        return;
    }

    /* set teh flag of icn to enbale */
    MMI_status_icons[icon_ID].flags &= ~STATUS_ICON_DISABLED;
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_set_status_icon_state
 * DESCRIPTION
 *  set the state of icon equal to valeu pass as parameter
 * PARAMETERS
 *  icon_ID         [IN]        
 *  state           [IN]        
 *  icon_id(?)      [IN]        
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_set_status_icon_state(S16 icon_ID, U8 state)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    /* set teh flag of icon equal to valeu pass as paraeter */
    MMI_status_icons[icon_ID].state = state;
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_reset_status_icon_state
 * DESCRIPTION
 *  set the state of icon equal to zero default value
 * PARAMETERS
 *  icon_ID         [IN]        
 *  icon_id(?)      [IN]        
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_reset_status_icon_state(S16 icon_ID)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    /* set teh flag of icon to default 0 value */
    MMI_status_icons[icon_ID].state = 0;
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_fill_status_bar_color
 * DESCRIPTION
 *  fill teh clor of status bar as grey
 * PARAMETERS
 *  bar_id      [IN]        
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_fill_status_bar_color(U8 bar_id)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    S32 x1, y1, x2, y2;
    color c;
    UI_filled_area *f;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    /* set clip region */
    gui_push_clip();
    wgui_status_icon_get_status_icon_bar_clip(bar_id, &x1, &y1, &x2, &y2);   /* get teh clip values of bar_id */
    gui_set_clip(x1, y1, x2, y2);

    /* draw status icon bar */
    if (current_MMI_theme != NULL)
    {
        f = current_MMI_theme->status_icon_bar_filler;
        if (wgui_is_wallpaper_on_bottom() == MMI_TRUE)
        {
            gdi_draw_solid_rect(x1, y1, x2, y2, GDI_COLOR_TRANSPARENT);
        }
        else if (f != NULL)
        {
            gui_draw_filled_area(x1, y1, x2, y2, f);
        }
        else
        {
            gdi_draw_solid_rect(x1, y1, x2, y2, GDI_COLOR_TRANSPARENT);
        }
    }
    else
    {
	c = gui_color(216, 212, 212);
        gui_fill_rectangle(x1, y1, x2, y2, c);  /* fill the icon position with grey color */
    }

    /* restore clip region */
    gui_pop_clip();
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_redraw_status_icon_bar
 * DESCRIPTION
 *  redarw only the staus bar whose id is equl to valeu pass as parameter
 * PARAMETERS
 *  j               [IN]        
 *  bar_id(?)       [IN]        
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_redraw_status_icon_bar(U8 j)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    /* backup active lcd */
    bitmap *saved_graphics_context = current_LCD_device_bitmap;
    S32 i, id, sbx, sby;
    S32 ix1, iy1;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
#ifdef __MMI_NONBLOCKING_DECODE__
    if(wgui_status_icon_blocking == MMI_TRUE)
    {
        return;
    }
#endif

    if (MMI_status_icon_bars[j].flags & STATUS_ICON_BAR_DISPLAY)
    {
        /* set active lcd according to different status icon bar */
        if (MMI_status_icon_bars[j].graphics_context == &main_LCD_device_bitmap)        /* check the graphicd context of bar id */
        {
            UI_set_main_LCD_graphics_context();
        }
        else if (MMI_status_icon_bars[j].graphics_context == &sub_LCD_device_bitmap)
        {
            UI_set_sub_LCD_graphics_context();
        }

        sbx = MMI_status_icon_bars[j].x;    /* store the start offset x position of bar */
        sby = MMI_status_icon_bars[j].y;    /* store the start offset y position of bar */

        if (MMI_status_icon_bars[j].icon_list != NULL)
        {
            if (wgui_status_bar_alpha_layer != GDI_NULL_HANDLE)
            {
                gdi_push_and_set_alpha_blending_source_layer(wgui_status_bar_alpha_layer);
            }
            for (i = 0; i < MMI_status_icon_bars[j].n_icons; i++)
            {
                id = MMI_status_icon_bars[j].icon_list[i];
            #ifdef __MMI_DOWNLOADABLE_THEMES_SUPPORT__

                if (mmi_tm_does_theme_have_alt_image())
                {
                    if (mmi_tm_is_theme_dirty_bit_set(GetImageIndex((U16)MMI_status_icons[id].icon_ID)))
                    {
                        MMI_status_icons[j].icon_data = get_image((U16)MMI_status_icons[id].icon_ID);
                        /* No need to calculate Image width, height and frames as they will not change */
                    }
                }
            #endif /* __MMI_DOWNLOADABLE_THEMES_SUPPORT__ */ 

                if (MMI_status_icons[id].icon_ID != IMAGE_ID_NULL)
                {
                    ix1 = MMI_status_icons[id].x + sbx; /* start x position of icon */
                    iy1 = MMI_status_icons[id].y + sby; /* start y position of icon */

                    if (MMI_status_icons[id].flags & STATUS_ICON_ANIMATE)
                    {
                        gdi_image_draw_frames_blend2layers(
                            ix1,
                            iy1,
                            MMI_status_icons[id].icon_data,
                            (U16) MMI_status_icons[id].current_frame);   
                    }
                    else if (MMI_status_icons[id].flags & STATUS_ICON_BLINK)
                    {
                        /* show blinking icon */
                        if (MMI_status_icons_blink_animate_ON_state)
                        {
                            gdi_image_draw_blend2layers(ix1, iy1, MMI_status_icons[id].icon_data);
                        }
                    }
                    else
                    {
                        /* show normal icon */
                        if (MMI_status_icons[id].state > 0)
                            /* the icon have many frames for different levels */
                        {
                            gdi_image_draw_frames_blend2layers(
                                ix1,
                                iy1,
                                MMI_status_icons[id].icon_data,
                                (U16) MMI_status_icons[id].state);
                        }
                        else
                            /* the icon has only one level */
                        {
                            gdi_image_draw_blend2layers(ix1, iy1, MMI_status_icons[id].icon_data);
                        }
                    }
                }
            }
            if (wgui_status_bar_alpha_layer != GDI_NULL_HANDLE)
            {
                gdi_pop_and_restore_alpha_blending_source_layer();
            }
        }
    }

    /* restore active lcd according to backuped lcd */
    if (saved_graphics_context == &main_LCD_device_bitmap)
    {
        UI_set_main_LCD_graphics_context(); /* set graphic context to main lcd */
    }
    else if (saved_graphics_context == &sub_LCD_device_bitmap)
    {
        UI_set_sub_LCD_graphics_context();  /* set graphic context to sub lcd */
    }
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_set_status_icon_display
 * DESCRIPTION
 *  set status icons display state (MAINLCD_ONLY, SUBLCD_ONLY, DEFAULT_BOTH)
 * PARAMETERS
 *  state       [IN]        
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_set_status_icon_display(S32 state)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    status_icon_display_area = state;
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_reset_status_icon_display
 * DESCRIPTION
 *  reset status icons display to DEFAULT_BOTH
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_reset_status_icon_display(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    status_icon_display_area = DEFAULT_BOTH;
}


#ifdef WGUI_STATUS_ICON_SHOW_TIME_IN_RIGHT
/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_time_small_hide_callback
 * DESCRIPTION
 *  
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
static void wgui_status_icon_time_small_hide_callback(S32 x1, S32 y1, S32 x2, S32 y2)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    S32 cx1, cy1, cx2, cy2;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    wgui_status_icon_get_status_icon_bar_clip(0, &cx1, &cy1, &cx2, &cy2);
    wgui_status_icon_set_status_icon_bar_clip(0, x1, y1, x2, y2);
    if (wgui_status_bar_alpha_layer != GDI_NULL_HANDLE)
    {
        gdi_push_and_set_alpha_blending_source_layer(wgui_status_bar_alpha_layer);
    }
    MMI_hide_status_icon_bars[0]();
    if (wgui_status_bar_alpha_layer != GDI_NULL_HANDLE)
    {
        gdi_pop_and_restore_alpha_blending_source_layer();
    }
    wgui_status_icon_set_status_icon_bar_clip(0, cx1, cy1, cx2, cy2);
}
#endif /* WGUI_STATUS_ICON_SHOW_TIME_IN_RIGHT */


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_show_status_icons
 * DESCRIPTION
 *  show all three status bar
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_show_status_icons(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    S32 i, j, id, sbx, sby;
    U8 blink_animate_flag = 0;
    S32 x1, y1, x2, y2;
    S32 ix1, iy1;

    /* backup active lcd */
    bitmap *saved_graphics_context = current_LCD_device_bitmap;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/

#ifdef __MMI_NONBLOCKING_DECODE__
    if(wgui_status_icon_blocking == MMI_TRUE)
    {
        return;
    }
#endif 

    if (status_icon_update_callback != NULL)
    {
        status_icon_update_callback();
    }

    if (IsReArrangeNeeded)
    {
        wgui_status_icon_arrange_status_icons();
        IsReArrangeNeeded = 0;
    }

    for (j = 0; j < MAX_STATUS_ICON_BARS; j++)
    {
        if (MMI_status_icon_bars[j].flags & STATUS_ICON_BAR_DISPLAY)
        {
            if (MMI_status_icon_bars[j].graphics_context == &main_LCD_device_bitmap)
            {
                if (status_icon_display_area == SUBLCD_ONLY)
                {
                    continue;
                }
                UI_set_main_LCD_graphics_context(); /* set graphic context to main lcd */
           /*     if ((!mmi_idle_is_on_idle_screen()) || (mmi_idle_is_on_idle_screen() && (!mmi_bootup_is_idle_screen_reached())))*/
                {
                    if(wgui_status_icon_get_status_icon_bar_layer((U8)j)!=GDI_ERROR_HANDLE)
                    {
                        gdi_layer_push_and_set_active(wgui_status_icon_get_status_icon_bar_layer((U8)j));
                    }
                    {
                        S32 cx1, cy1, cx2, cy2;
                        
                        gdi_layer_push_clip();
                        wgui_status_icon_get_status_icon_bar_clip((U8)j, &cx1, &cy1, &cx2, &cy2);
                        gdi_layer_set_clip(cx1, cy1, cx2, cy2);
                        if (wgui_status_bar_alpha_layer != GDI_NULL_HANDLE)
                        {
                            gdi_push_and_set_alpha_blending_source_layer(wgui_status_bar_alpha_layer);
                        }
                    MMI_hide_status_icon_bars[j] ();    /* call hide function of status bar */
                        if (wgui_status_bar_alpha_layer != GDI_NULL_HANDLE)
                        {
                            gdi_pop_and_restore_alpha_blending_source_layer();
                        }
                        gdi_layer_pop_clip();
                    }

                    #ifdef WGUI_STATUS_ICON_SHOW_TIME_IN_RIGHT

                    if (!wgui_status_icon_clock_create)
                    {
                        wgui_clock_create(&wgui_clock_status_icon_id, 
                            t_x1, t_y1, t_x2 - t_x1 + 1, t_y2 - t_y1 + 1, 
                            GUI_CLOCK_MAINLCD_TYPE_DIGITAL_TIME_SMALL);

                    #ifdef __MMI_VUI_ENGINE__
    
                        if (MMI_status_icon_bars[0].venus_update_screen_callback)
                        {
                            if(wgui_status_icon_get_status_icon_bar_layer(0)!=GDI_ERROR_HANDLE)
                            {
                                wgui_clock_set_target_layer(wgui_clock_status_icon_id, 
                                    wgui_status_icon_get_status_icon_bar_layer(0));
                            }
                            else
                            {
                                gdi_handle act;
                            
                                gdi_layer_get_active(&act);
                                wgui_clock_set_target_layer(wgui_clock_status_icon_id, 
                                    act);
                            }
                        }
                        else
                        {
                    #endif /* __MMI_VUI_ENGINE__ */

                        
                        if(wgui_status_icon_get_status_icon_bar_layer((U8)j)!=GDI_ERROR_HANDLE)
                        {
                            wgui_clock_set_target_layer(wgui_clock_status_icon_id, 
                                get_status_icon_bar_layer((U8)j));
                        }
                        else
                        {
                            gdi_handle act;
                            
                            gdi_layer_get_active(&act);
                            wgui_clock_set_target_layer(wgui_clock_status_icon_id, 
                                act);
                        }
                    #ifdef __MMI_VUI_ENGINE__
                        }
                    #endif
                        wgui_clock_set_hide_function(wgui_clock_status_icon_id, wgui_status_icon_time_small_hide_callback);
                        wgui_status_icon_clock_create = MMI_TRUE;
                    }
                    if (wgui_status_bar_alpha_layer != GDI_NULL_HANDLE)
                    {
                        gdi_push_and_set_alpha_blending_source_layer(wgui_status_bar_alpha_layer);
                        wgui_clock_set_abm_layer(wgui_clock_status_icon_id, wgui_status_bar_alpha_layer);
                    }
                    gdi_layer_lock_frame_buffer();
                    wgui_clock_show_clock(wgui_clock_status_icon_id);
                    gdi_layer_unlock_frame_buffer();
                    if (wgui_status_bar_alpha_layer != GDI_NULL_HANDLE)
                    {
                        gdi_pop_and_restore_alpha_blending_source_layer();
                    }
                    #endif /* WGUI_STATUS_ICON_SHOW_TIME_IN_RIGHT */

                    if(wgui_status_icon_get_status_icon_bar_layer((U8)j)!=GDI_ERROR_HANDLE)
                    {
                        gdi_layer_pop_and_restore_active();
                    }
                }
            }
            else if (MMI_status_icon_bars[j].graphics_context == &sub_LCD_device_bitmap)
            {
                if (status_icon_display_area == MAINLCD_ONLY)
                {
                    continue;
                }
                if(wgui_status_icon_get_status_icon_bar_layer((U8)j)!=GDI_ERROR_HANDLE)
                {
                    gdi_layer_push_and_set_active(wgui_status_icon_get_status_icon_bar_layer((U8)j));
                }
                {
                    S32 cx1, cy1, cx2, cy2;
                                            
                    gdi_layer_push_clip();
                    wgui_status_icon_get_status_icon_bar_clip((U8)j, &cx1, &cy1, &cx2, &cy2);
                    gdi_layer_set_clip(cx1, cy1, cx2, cy2);
                    if (wgui_status_bar_alpha_layer != GDI_NULL_HANDLE)
                    {
                        gdi_push_and_set_alpha_blending_source_layer(wgui_status_bar_alpha_layer);
                    }
                MMI_hide_status_icon_bars[j] ();    /* call hide function of sub lcd status bar */
                    if (wgui_status_bar_alpha_layer != GDI_NULL_HANDLE)
                    {
                        gdi_pop_and_restore_alpha_blending_source_layer();
                    }
                    gdi_layer_pop_clip();
                }
                if(wgui_status_icon_get_status_icon_bar_layer((U8)j)!=GDI_ERROR_HANDLE)
                {
                    gdi_layer_pop_and_restore_active();
                }
                UI_set_sub_LCD_graphics_context();  /* set grahic context to sub lcd */
            }

            if(wgui_status_icon_get_status_icon_bar_layer((U8)j)!=GDI_ERROR_HANDLE)
            {
                gdi_layer_push_and_set_active(wgui_status_icon_get_status_icon_bar_layer((U8)j));
            }
            
            wgui_status_icon_arrange_status_icons();
            
            /* change clip region when drawing layer is set */
            gui_push_clip();
            gui_reset_clip();

            /* store coordinates of status icon bar */
            x1 = MMI_status_icon_bars[j].x1;
            y1 = MMI_status_icon_bars[j].y1;
            x2 = MMI_status_icon_bars[j].x2;
            y2 = MMI_status_icon_bars[j].y2;

            /* set clip region */
            gui_set_clip(x1, y1, x2, y2);

            /* store icon offset coordinates */
            sbx = MMI_status_icon_bars[j].x;
            sby = MMI_status_icon_bars[j].y;

            if (MMI_status_icon_bars[j].icon_list != NULL)
            {
                if (wgui_status_bar_alpha_layer != GDI_NULL_HANDLE)
                {
                    gdi_push_and_set_alpha_blending_source_layer(wgui_status_bar_alpha_layer);
                }
                for (i = 0; i < MMI_status_icon_bars[j].n_icons; i++)
                {
                    id = MMI_status_icon_bars[j].icon_list[i];
                    if (MMI_status_icons[id].icon_ID != IMAGE_ID_NULL)
                    {
                    #ifdef __MMI_DOWNLOADABLE_THEMES_SUPPORT__

                        if (mmi_tm_does_theme_have_alt_image())
                        {
                            if (mmi_tm_is_theme_dirty_bit_set(GetImageIndex(MMI_status_icons[id].icon_ID)))
                            {
                                MMI_status_icons[id].icon_data = get_image(MMI_status_icons[id].icon_ID);
                                /* No need to calculate Image width, height and frames as they will not change */
                            }
                        }
                    #endif /* __MMI_DOWNLOADABLE_THEMES_SUPPORT__ */ 

                        if (MMI_status_icons[id].flags & STATUS_ICON_REVERSE_ANIMATE)
                        {
                            /* do nothing */
                        }
                        else if (MMI_status_icons[id].flags & STATUS_ICON_FORWARD_ANIMATE)
                        {
                            /* do nothing */
                        }
                        else if (MMI_status_icons[id].flags & STATUS_ICON_ANIMATE)
                        {
                            blink_animate_flag = 1;

                            /* store position of the icon */
                            ix1 = MMI_status_icons[id].x + sbx;
                            iy1 = MMI_status_icons[id].y + sby;

                            gdi_image_draw_frames_blend2layers(
                                    ix1,
                                    iy1,
                                    MMI_status_icons[id].icon_data,
                                    (U16) MMI_status_icons[id].current_frame);   

                            /* bettery removed to status_icon_timer_callback */
                        }
                        else if (MMI_status_icons[id].flags & STATUS_ICON_BLINK)
                        {
                            blink_animate_flag = 1;
                            if (MMI_status_icons_blink_animate_ON_state)
                            {
                                /* store position of the icon */
                                ix1 = MMI_status_icons[id].x + sbx;
                                iy1 = MMI_status_icons[id].y + sby;
                                gdi_image_draw_blend2layers(ix1, iy1, MMI_status_icons[id].icon_data);                             
                            }
                        }
                        else if (MMI_status_icons[id].flags & STATUS_ICON_DISPLAY)
                        {
                            /* store position of the icon */
                            ix1 = MMI_status_icons[id].x + sbx;
                            iy1 = MMI_status_icons[id].y + sby;

                            if (MMI_status_icons[id].state > 0)
                            {
                                gdi_image_draw_frames_blend2layers(
                                    ix1,
                                    iy1,
                                    MMI_status_icons[id].icon_data,
                                    (U16) MMI_status_icons[id].state);
                            }
                            else
                            {
                                gdi_image_draw_blend2layers(ix1, iy1, MMI_status_icons[id].icon_data);
                            }
                        }
                    }
                }
                if (wgui_status_bar_alpha_layer != GDI_NULL_HANDLE)
                {
                    gdi_pop_and_restore_alpha_blending_source_layer();
                }
            }
            /* change clip region when drawing layer is set */
            gui_pop_clip();
            
            if(wgui_status_icon_get_status_icon_bar_layer((U8)j)!=GDI_ERROR_HANDLE)
            {
                gdi_layer_pop_and_restore_active();
            }
        }
    }

    status_icon_display_ON = 1;
    if (blink_animate_flag) /* if icon is animated */
    {
        if (status_icon_blink_animate_ON == 0)
        {
            wgui_status_icon_setup_status_icon_blink_animate();
        }
        status_icon_blink_animate_ON = 1;
    }
    else
    {
        if (status_icon_blink_animate_ON == 1)
        {
            wgui_status_icon_setup_status_icon_blink_animate();
        }
        status_icon_blink_animate_ON = 0;
    }

    /* restore active lcd according to backuped lcd */
    if (saved_graphics_context == &main_LCD_device_bitmap)
    {
        UI_set_main_LCD_graphics_context();
    }
    else if (saved_graphics_context == &sub_LCD_device_bitmap)
    {
        UI_set_sub_LCD_graphics_context();
    }
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_hide_status_icons
 * DESCRIPTION
 *  set the flag of all icons to hide
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_hide_status_icons(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    S32 i;
    U8 flag = 1;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
#ifdef __MMI_NONBLOCKING_DECODE__
    if(wgui_status_icon_blocking == MMI_TRUE)
    {
        return;
    }
#endif
    for (i = 0; i < MAX_STATUS_ICON_BARS; i++)
    {
        if (MMI_status_icon_bars[i].flags & STATUS_ICON_BAR_DISPLAY)
        {
            if (MMI_status_icon_bars[i].graphics_context == current_LCD_device_bitmap)
            {
                S32 cx1, cy1, cx2, cy2;
                        
                #ifdef WGUI_STATUS_ICON_SHOW_TIME_IN_RIGHT
                wgui_clock_close(wgui_clock_status_icon_id);
                wgui_clock_status_icon_id = WGUI_CLOCK_INVALID_INDEX;
                wgui_status_icon_clock_create = MMI_FALSE;
                #endif /* WGUI_STATUS_ICON_SHOW_TIME_IN_RIGHT */
            
       
                gdi_layer_push_clip();
                wgui_status_icon_get_status_icon_bar_clip((U8)i, &cx1, &cy1, &cx2, &cy2);
                gdi_layer_set_clip(cx1, cy1, cx2, cy2);
                if (wgui_status_bar_alpha_layer != GDI_NULL_HANDLE)
                {
                    gdi_push_and_set_alpha_blending_source_layer(wgui_status_bar_alpha_layer);
                }
                /* call hide fuction of status bar */
                MMI_hide_status_icon_bars[i] ();
                if (wgui_status_bar_alpha_layer != GDI_NULL_HANDLE)
                {
                    gdi_pop_and_restore_alpha_blending_source_layer();
                }
                gdi_layer_pop_clip();

                /* set the flag of bar to disable */
                MMI_status_icon_bars[i].flags &= ~STATUS_ICON_BAR_DISPLAY;
            }
            else if (MMI_status_icon_bars[i].flags & STATUS_ICON_BAR_DISPLAY)
            {
                flag = 0;
            }
        }
    }
    if (flag)
    {
        status_icon_display_ON = 0;
        if (status_icon_blink_animate_ON)
        {
            status_icon_blink_animate_ON = 0;
            gui_cancel_timer(wgui_status_icon_status_icon_timer_callback);
        }
    }
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_close_status_icons
 * DESCRIPTION
 *  set statu sicon bar flag to not to display,clear status bar hide function s
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_close_status_icons(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    S32 i;
    U8 flag = 1;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
#if defined(__MMI_MAINLCD_176X220__) || defined (__MMI_MAINLCD_240X320__) || defined (__MMI_MAINLCD_240X400__) || defined (__MMI_MAINLCD_320X240__) || defined (__MMI_MAINLCD_320X480__)
#ifdef __MMI_VUI_ENGINE__
    if (MMI_status_icon_bars[0].venus_update_screen_callback)
    {
        MMI_status_icon_bars[0].venus_update_screen_callback = NULL;
        MMI_status_icon_bars[0].venus_update_screen_handle = NULL;
    #ifdef WGUI_STATUS_ICON_SHOW_TIME_IN_RIGHT
        wgui_clock_set_target_layer(wgui_clock_status_icon_id, 
                                    NULL);
        wgui_clock_set_abm_layer(wgui_clock_status_icon_id, GDI_NULL_HANDLE);                            
    #endif
    }
    else
    {
    return;
    }
#endif /* __MMI_VUI_ENGINE__ */
    return;
#endif /* __MMI_VUI_ENGINE__ */

#ifdef WGUI_STATUS_ICON_SHOW_TIME_IN_RIGHT
    wgui_clock_close(wgui_clock_status_icon_id);
    wgui_clock_status_icon_id = WGUI_CLOCK_INVALID_INDEX;
    wgui_status_icon_clock_create = MMI_FALSE;
#endif /* WGUI_STATUS_ICON_SHOW_TIME_IN_RIGHT */

    for (i = 0; i < MAX_STATUS_ICON_BARS; i++)
    {
        if (MMI_status_icon_bars[i].graphics_context == current_LCD_device_bitmap)
        {
            MMI_status_icon_bars[i].flags &= ~STATUS_ICON_BAR_DISPLAY;
        }
        else if (MMI_status_icon_bars[i].flags & STATUS_ICON_BAR_DISPLAY)
        {
            flag = 0;
        }
    }
    if (flag)
    {
        status_icon_display_ON = 0;
        if (status_icon_blink_animate_ON)
        {
            status_icon_blink_animate_ON = 0;
            gui_cancel_timer(wgui_status_icon_status_icon_timer_callback);
        }
    }
    wgui_status_icon_clear_hide_status_icon_bars();  /* clear function handlers of hide function */
    gui_hide_animations();          /* stop animation of icons */

    wgui_status_bar_alpha_layer = GDI_NULL_HANDLE;
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_status_icon_timer_callback
 * DESCRIPTION
 *  timer call back of animated icon
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_status_icon_timer_callback(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    S32 i;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    /* moved here from show and refresh functions */
    for (i = 0; i < MAX_STATUS_ICONS; i++)
    {
        if (MMI_status_icons[i].flags & STATUS_ICON_ANIMATE)
        {
            MMI_status_icons[i].current_frame++;
            if (MMI_status_icons[i].current_frame >= MMI_status_icons[i].n_frames)
            {
                MMI_status_icons[i].current_frame = 0;
            }
        }
    }

    if (!status_icon_blink_animate_ON)
    {
        gui_cancel_timer(wgui_status_icon_status_icon_timer_callback);
    }

    if (!status_icon_display_ON)
    {
        return;
    }

    if (MMI_status_icons_blink_animate_ON_state)
    {
        MMI_status_icons_blink_animate_ON_state = 0;
    }
    else
    {
        MMI_status_icons_blink_animate_ON_state = 1;
    }

#ifdef MMI_SHOW_STATUS_ICON_IN_TITLE
    if (!(wgui_status_icon_get_status_icon_bar_type(0) & STATUS_ICON_BAR_DISPLAY) && !(g_wgui_status_bar_integrated_disabled))
    {
        if (MMI_redraw_integrated_status_icon_bars)
        {
            MMI_redraw_integrated_status_icon_bars();
        }
        gui_start_timer(STATUS_ICON_TIMER_RATE, wgui_status_icon_status_icon_timer_callback);
    }
    else
#endif
    {
        /* redraw the status icons */
        wgui_status_icon_refresh_status_icon_display();

        if (status_icon_blink_animate_ON)
        {
            gui_start_timer(STATUS_ICON_TIMER_RATE, wgui_status_icon_status_icon_timer_callback);
        }
        
    }
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_setup_status_icon_blink_animate
 * DESCRIPTION
 *  timer call back of blinking icon
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_setup_status_icon_blink_animate(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    S32 i;
    U8 blink_animate_flag = 0;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (!status_icon_display_ON 
#ifdef MMI_SHOW_STATUS_ICON_IN_TITLE
    || (g_wgui_status_bar_integrated_disabled && MMI_redraw_integrated_status_icon_bars == NULL)
#endif
        )
    {
        return;
    }
#ifdef __MMI_NONBLOCKING_DECODE__
    if(wgui_status_icon_blocking == MMI_TRUE)
    {
        return;
    }
#endif

    for (i = 0; i < MAX_STATUS_ICONS; i++)
    {
        if ((MMI_status_icons[i].flags & STATUS_ICON_BLINK) || (MMI_status_icons[i].flags & STATUS_ICON_ANIMATE))
        {
            blink_animate_flag = 1;
            break;
        }
    }

    if (blink_animate_flag)
        /* start timer for blinking icon */
    {
        gui_start_timer(STATUS_ICON_TIMER_RATE, wgui_status_icon_status_icon_timer_callback);
    }
    else
        /* stop timer for blinking icon */
    {
        gui_cancel_timer(wgui_status_icon_status_icon_timer_callback);
    }
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_show_status_icon_bar
 * DESCRIPTION
 *  set the display flag of status bar
 * PARAMETERS
 *  status_bar_ID       [IN]        0 -horizontal bar main lcd
 *  horizontal(?)       [IN]        Bar sub lcd
 *  vertical(?)         [IN]        Bar main lcd
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_show_status_icon_bar(U8 status_bar_ID)
{
#ifdef __MMI_SUBLCD_SHOW_ANALOG_CLOCK__
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (status_bar_ID == 2)
    {
        return;
    }
#endif /* __MMI_SUBLCD_SHOW_ANALOG_CLOCK__ */ 
    /* set teh flag of bar id to display */
    MMI_status_icon_bars[status_bar_ID].flags |= STATUS_ICON_BAR_DISPLAY;
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_hide_status_icon_bar
 * DESCRIPTION
 *  hide the status bar
 * PARAMETERS
 *  status_bar_ID       [IN]        0 -horizontal bar main lcd
 *  horizontal(?)       [IN]        Bar sub lcd
 *  vertical(?)         [IN]        Bar main lcd
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_hide_status_icon_bar(U8 status_bar_ID)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_status_icon_bars[status_bar_ID].flags &= ~STATUS_ICON_BAR_DISPLAY;
    
#ifdef WGUI_STATUS_ICON_SHOW_TIME_IN_RIGHT
    wgui_clock_close(wgui_clock_status_icon_id);
    wgui_clock_status_icon_id = WGUI_CLOCK_INVALID_INDEX;
    wgui_status_icon_clock_create = MMI_FALSE;
#endif /* WGUI_STATUS_ICON_SHOW_TIME_IN_RIGHT */
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_set_status_icon_bar_type
 * DESCRIPTION
 *  hide the status bar
 * PARAMETERS
 *  status_bar_ID       [IN]        0 -horizontal bar main lcd
 *  flags               [IN]        
 *  horizontal(?)       [IN]        Bar sub lcd
 *  vertical(?)         [IN]        Bar main lcd
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_set_status_icon_bar_type(U8 status_bar_ID, U32 flags)
{
#ifdef __MMI_SUBLCD_SHOW_ANALOG_CLOCK__
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (status_bar_ID == 2)
    {
        return;
    }
#endif /* __MMI_SUBLCD_SHOW_ANALOG_CLOCK__ */ 
    MMI_status_icon_bars[status_bar_ID].flags = flags;
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_get_status_icon_bar_type
 * DESCRIPTION
 *  hide the status bar
 * PARAMETERS
 *  status_bar_ID       [IN]        0 -horizontal bar main lcd
 * RETURNS
 *  
 *****************************************************************************/
U32 wgui_status_icon_get_status_icon_bar_type(U8 status_bar_ID)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    return MMI_status_icon_bars[status_bar_ID].flags;
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_get_status_icon_bar_clip
 * DESCRIPTION
 *  get teh clipping valeu sof particaular status bar
 * PARAMETERS
 *  status_bar_ID       [IN]        0 -horizontal bar main lcd
 *  x1                  [IN]        Start x position
 *  y1                  [IN]        Start y position
 *  x2                  [IN]        End x position
 *  y2                  [IN]        End y position
 *  horizontal(?)       [IN]        Bar sub lcd
 *  vertical(?)         [IN]        Bar main lcd
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_get_status_icon_bar_clip(U8 status_bar_ID, S32 *x1, S32 *y1, S32 *x2, S32 *y2)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    *x1 = MMI_status_icon_bars[status_bar_ID].x1;
    *y1 = MMI_status_icon_bars[status_bar_ID].y1;
    *x2 = MMI_status_icon_bars[status_bar_ID].x2;
    *y2 = MMI_status_icon_bars[status_bar_ID].y2;
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_set_status_icon_bar_clip
 * DESCRIPTION
 *  set teh clipping valeu sof particaular status bar
 * PARAMETERS
 *  status_bar_ID       [IN]        0 -horizontal bar main lcd
 *  x1                  [IN]        Start x position
 *  y1                  [IN]        Start y position
 *  x2                  [IN]        End x position
 *  y2                  [IN]        End y position
 *  horizontal(?)       [IN]        Bar sub lcd
 *  vertical(?)         [IN]        Bar main lcd
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_set_status_icon_bar_clip(U8 status_bar_ID, S32 x1, S32 y1, S32 x2, S32 y2)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_status_icon_bars[status_bar_ID].x1 = x1;
    MMI_status_icon_bars[status_bar_ID].y1 = y1;
    MMI_status_icon_bars[status_bar_ID].x2 = x2;
    MMI_status_icon_bars[status_bar_ID].y2 = y2;

    if (status_bar_ID == 1)
    {
        MMI_status_icon_bars[status_bar_ID].x = x1;
        MMI_status_icon_bars[status_bar_ID].y = y1;
    }
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_set_status_icon_bar_layer
 * DESCRIPTION
 *  set gdi layer handle
 * PARAMETERS
 *  status_bar_ID       [IN]        0 -horizontal bar main lcd
 *  gdi_layer               [IN]        
 *  horizontal(?)       [IN]        Bar sub lcd
 *  vertical(?)         [IN]        Bar main lcd
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_set_status_icon_bar_layer(U8 status_bar_ID, GDI_HANDLE gdi_layer)
{
#ifdef __MMI_SUBLCD_SHOW_ANALOG_CLOCK__
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (status_bar_ID == 2)
    {
        return;
    }
#endif /* __MMI_SUBLCD_SHOW_ANALOG_CLOCK__ */ 
    MMI_status_icon_bars[status_bar_ID].layer_handle= gdi_layer;
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_get_status_icon_bar_layer
 * DESCRIPTION
 *  get gdi layer handle
 * PARAMETERS
 *  status_bar_ID       [IN]        0 -horizontal bar main lcd
 * RETURNS
 *  
 *****************************************************************************/
U32 wgui_status_icon_get_status_icon_bar_layer(S32 status_bar_ID)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    return MMI_status_icon_bars[status_bar_ID].layer_handle;
}


/*****************************************************************************
 * FUNCTION
*  wgui_status_icon_reset_status_icon_bar_layer
* DESCRIPTION
*  get gdi layer handle
* PARAMETERS
*  status_bar_ID       [IN]        0 -horizontal bar main lcd
* RETURNS
*  
*****************************************************************************/
void wgui_status_icon_reset_status_icon_bar_layer(U8 status_bar_ID)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_status_icon_bars[status_bar_ID].layer_handle = GDI_NULL_HANDLE;
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_reset_all_status_icon_bar_layer
 * DESCRIPTION
 *  get gdi layer handle
 * PARAMETERS
 *  status_bar_ID       [IN]        0 -horizontal bar main lcd
 * RETURNS
 *  
*****************************************************************************/
void wgui_status_icon_reset_all_status_icon_bar_layer(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    U8 i;
    
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    for(i = 0; i < MAX_STATUS_ICON_BARS; i++)
    {
        if(MMI_status_icon_bars[i].graphics_context == current_LCD_device_bitmap)
        {
            MMI_status_icon_bars[i].layer_handle = GDI_NULL_HANDLE;
        }
    }
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_reset_status_icons
 * DESCRIPTION
 *  Function to be used by wgui_categories.
 *  This must be called in the exit function of the category
 *  screens that have displayed status icons and / or registered
 *  functions for hiding them.
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_reset_status_icons(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    wgui_status_icon_hide_status_icons();
    wgui_status_icon_clear_hide_status_icon_bars();
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_initialize_status_icons
 * DESCRIPTION
 *  Called once during bootup to initialize Status icons
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_initialize_status_icons(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    S32 i;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_status_icon_bars[0].x = 0;
    MMI_status_icon_bars[0].y = 0;
    MMI_status_icon_bars[0].x1 = 0;
    MMI_status_icon_bars[0].y1 = 0;
    MMI_status_icon_bars[0].x2 = MAIN_LCD_device_width - 1;
    MMI_status_icon_bars[0].y2 = MMI_status_bar_height - 1;

    /* set the cliping values of vertical status bar of main lcd */
    MMI_status_icon_bars[1].x = MAIN_LCD_device_width - 17;
    MMI_status_icon_bars[1].y = MMI_status_bar_height;
    MMI_status_icon_bars[1].x1 = MAIN_LCD_device_width - 17;
    MMI_status_icon_bars[1].y1 = MMI_status_bar_height;
    MMI_status_icon_bars[1].x2 = MAIN_LCD_device_width - 1;
#ifdef __MMI_KLG__
    MMI_status_icon_bars[1].y2 = MAIN_LCD_device_height >> 1;
#else 
    MMI_status_icon_bars[1].y2 = MAIN_LCD_device_height - MMI_button_bar_height - 1;
#endif 

    /* set the hide functions of statuas bar to dummy function  */
    for (i = 0; i < MAX_STATUS_ICON_BARS; i++)
    {
        MMI_hide_status_icon_bars[i] = UI_dummy_function;
    }

    for (i = 0; i < MAX_STATUS_ICONS; i++)
    {
        MMI_status_icons[i].icon_data = get_image(MMI_status_icons[i].icon_ID);
        MMI_status_icons[i].n_frames = (S16) gui_image_n_frames(MMI_status_icons[i].icon_data);
        gui_measure_image(
            MMI_status_icons[i].icon_data, 
            &MMI_status_icons[i].width, 
            &MMI_status_icons[i].height);
    }

    /* set the start x position of battery icon of horizontal man lcd status bar */
    MMI_status_icons[STATUS_ICON_BATTERY_STRENGTH].x =
        MAIN_LCD_device_width - 1 - MMI_status_icons[STATUS_ICON_BATTERY_STRENGTH].width - MMI_UI_STATUSBAR_GAP;

    MMI_status_icon_bars[0].rotate_flag_for_wap = MMI_FALSE;

#ifdef __MMI_SUBLCD__
    /* set the cliping values of horizontal status bar of sub lcd */
    MMI_status_icon_bars[2].x = 0;
    MMI_status_icon_bars[2].y = 0;
    MMI_status_icon_bars[2].x1 = 0;
    MMI_status_icon_bars[2].y1 = 0;
    MMI_status_icon_bars[2].x2 = SUB_LCD_device_width - 1;
    MMI_status_icon_bars[2].y2 = 14;
    MMI_status_icons[STATUS_ICON_SUBLCD_SIGNAL_STRENGTH].x = 1;
    MMI_status_icons[STATUS_ICON_SUBLCD_BATTERY_STRENGTH].x =
        SUB_LCD_device_width - 1 - MMI_status_icons[STATUS_ICON_SUBLCD_BATTERY_STRENGTH].width;
#endif /* __MMI_SUBLCD__ */ 

    wgui_status_bar_alpha_layer = GDI_NULL_HANDLE;

#ifdef __MMI_VUI_ENGINE__
    MMI_status_icon_bars[0].venus_update_screen_callback = NULL;
    MMI_status_icon_bars[0].venus_update_screen_handle = NULL;
    #ifdef WGUI_STATUS_ICON_SHOW_TIME_IN_RIGHT
    wgui_clock_set_target_layer(wgui_clock_status_icon_id, 
                                    NULL);
    wgui_clock_set_abm_layer(wgui_clock_status_icon_id, GDI_NULL_HANDLE);                                
    #endif
#endif /* __MMI_VUI_ENGINE__ */


}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_move_status_icons
 * DESCRIPTION
 *  move status icon bar
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_move_status_icons(S32 index, S32 x1, S32 y1, S32 x2, S32 y2)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_status_icon_bars[index].x = x1;
    MMI_status_icon_bars[index].y = y1;
    MMI_status_icon_bars[index].x1 = x1;
    MMI_status_icon_bars[index].y1 = y1;
    MMI_status_icon_bars[index].x2 = x2;
    MMI_status_icon_bars[index].y2 = y2;
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_show_status_icon
 * DESCRIPTION
 *  show particular icon
 * PARAMETERS
 *  icon_ID     [IN]        
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_show_status_icon(S16 icon_ID)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if ((icon_ID < 0) || (icon_ID >= MAX_STATUS_ICONS))
    {
        return;
    }

    /* Hide status icon in flight mode */
    if (!mmi_flight_mode_is_status_icon_visible(icon_ID, MMI_status_icons[icon_ID].flags | STATUS_ICON_DISPLAY))
    {
        return; /* the status icon is invisible for some reason */
    }

    /* set the flag of icon to display on */
    MMI_status_icons[icon_ID].flags |= STATUS_ICON_DISPLAY;

    /* set rearrange flag on */
    IsReArrangeNeeded = 1;
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_hide_status_icon
 * DESCRIPTION
 *  hide particular icon
 * PARAMETERS
 *  icon_ID     [IN]        
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_hide_status_icon(S16 icon_ID)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if ((icon_ID < 0) || (icon_ID >= MAX_STATUS_ICONS))
    {
        return;
    }

    /* set teh flag of icon to hide ...defualt value */
    MMI_status_icons[icon_ID].flags = 0;

    /* set rearrange flag on */
    IsReArrangeNeeded = 1;

    /* Hide status icon in flight mode */
    mmi_flight_mode_is_status_icon_visible(icon_ID, 0);
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_update_status_icons
 * DESCRIPTION
 *  rearrange and refresh status icons
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_update_status_icons(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
#ifdef __MMI_NONBLOCKING_DECODE__
    if (wgui_status_icon_blocking)
    {
        return;
    }
#endif

    if (IsReArrangeNeeded)
    {
        wgui_status_icon_arrange_status_icons();
        IsReArrangeNeeded = 0;
    }

    wgui_status_icon_refresh_status_icon_display();
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_update_SubLCD_status_icons
 * DESCRIPTION
 *  rearrange and refresh SubLCD status icons
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_update_SubLCD_status_icons(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    wgui_status_icon_set_status_icon_display(SUBLCD_ONLY);
    wgui_status_icon_update_status_icons();
    wgui_status_icon_reset_status_icon_display();
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_change_status_icon_level
 * DESCRIPTION
 *  change the level of status icon like battery icon ,signal icon
 * PARAMETERS
 *  icon_ID                 [IN]        
 *  level_percentage        [IN]        Level of increase
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_change_status_icon_level(S16 icon_ID, U8 level_percentage)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    S32 n_frames = MMI_status_icons[icon_ID].n_frames;
    S32 frame_number;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (n_frames <= 1)
    {
        return;
    }

    frame_number = (level_percentage * (n_frames)) / 100;

    /* check frame number not bigger than frames of icons */
    if (frame_number > (n_frames - 1))
    {
        frame_number = n_frames - 1;
    }

    /* check frame number more than 0 */
    if (frame_number < 0)
    {
        frame_number = 0;
    }

    /* if level is the same, no need to refresh the icon */
    if (frame_number == MMI_status_icons[icon_ID].state)
    {
        return;
    }

    wgui_status_icon_set_status_icon_state(icon_ID, (U8) frame_number);

    if ((icon_ID == STATUS_ICON_SIGNAL_STRENGTH) || (icon_ID == STATUS_ICON_BATTERY_STRENGTH))
    {
#ifdef MMI_SHOW_STATUS_ICON_IN_TITLE
        if (!(wgui_status_icon_get_status_icon_bar_type(0) & STATUS_ICON_BAR_DISPLAY) && !(g_wgui_status_bar_integrated_disabled))
        {
            if (MMI_redraw_integrated_status_icon_bars)
            {
                MMI_redraw_integrated_status_icon_bars();
            }
        }
        else
#endif
        {
            wgui_status_icon_refresh_fixed_status_icon(icon_ID, 0);
        }
    }
    else if ((icon_ID == STATUS_ICON_SUBLCD_SIGNAL_STRENGTH) || (icon_ID == STATUS_ICON_SUBLCD_BATTERY_STRENGTH))
    {
        wgui_status_icon_refresh_fixed_status_icon(icon_ID, 2);
    }
    else
    {
        wgui_status_icon_refresh_status_icon_display();
    }
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_change_status_icon_image
 * DESCRIPTION
 *  change the image of status icon like signal icon with line switch
 * PARAMETERS
 *  icon_ID         [IN]        
 *  image_ID        [IN]        
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_change_status_icon_image(S16 icon_ID, U16 image_ID)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_status_icons[icon_ID].icon_data = get_image(image_ID);
    MMI_status_icons[icon_ID].n_frames = (S16) gui_image_n_frames(MMI_status_icons[icon_ID].icon_data);
#ifdef __MMI_NONBLOCKING_DECODE__
    if(wgui_status_icon_blocking == MMI_TRUE)
    {
        MMI_status_icons[icon_ID].width = -1;
        MMI_status_icons[icon_ID].height = -1;
    }
    else
#endif
    {
        gui_measure_image(
            MMI_status_icons[icon_ID].icon_data,
            &MMI_status_icons[icon_ID].width,
            &MMI_status_icons[icon_ID].height);
    }
    MMI_status_icons[icon_ID].icon_ID = image_ID;

    IsReArrangeNeeded = 1;
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_animate_status_icon
 * DESCRIPTION
 *  animate particular icon
 * PARAMETERS
 *  icon_ID     [IN]        
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_animate_status_icon(S16 icon_ID)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    wgui_status_icon_show_status_icon(icon_ID);

    /* animate the icon */
    wgui_status_icon_animate_status_icon1(icon_ID);
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_blink_status_icon
 * DESCRIPTION
 *  blink particular icon
 * PARAMETERS
 *  icon_ID     [IN]        
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_blink_status_icon(S16 icon_ID)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_status_icons_blink_animate_ON_state = 0;
    wgui_status_icon_show_status_icon(icon_ID);
    wgui_status_icon_update_status_icons();

    /* blink the icon */
    wgui_status_icon_blink_status_icon1(icon_ID);
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_arrange_status_icons_sublcd
 * DESCRIPTION
 *  arrange status icon of sub lcd
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_arrange_status_icons_sublcd(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    S32 i, j, k = 0, group_ID;
    U8 done = 0, inner_done, display_flag;

#if(STATUS_ICONS_DRAW_FROM_BOTTOM_BASELINE)
    S32 cy;
#endif 

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
#ifdef __MMI_SUBLCD_SHOW_ANALOG_CLOCK__
    return;
#endif 
    GDI_LOCK;
    MMI_status_icon_bars[2].n_icons = 0;
    MMI_status_icons_list3[0] = STATUS_ICON_SUBLCD_SIGNAL_STRENGTH;
    MMI_status_icons_list3[1] = STATUS_ICON_SUBLCD_BATTERY_STRENGTH;

#if(STATUS_ICONS_DRAW_FROM_BOTTOM_BASELINE)
    cy = MMI_status_icon_bars[2].y2 - MMI_status_icon_bars[2].y1 - 1;
#endif 

    done = 0;
    i = 0;
    while (!done)
    {
        display_flag = 0;
        j = MMI_status_icons_pool2[i];
        i++;
        if (i >= MMI_N_STATUS_ICONS_POOL2)
        {
            break;
        }
        if (TEST_STATUS_ICON_DISPLAY_ON(MMI_status_icons[j].flags))
        {
            k = j;
            display_flag = 1;
        }
        if ((MMI_status_icons[j].group_ID > 0))
        {
            group_ID = MMI_status_icons[j].group_ID;
            j++;
            inner_done = 0;
            while (!inner_done)
            {
                if (group_ID != MMI_status_icons[j].group_ID)
                {
                    break;
                }
                if ((!display_flag) && TEST_STATUS_ICON_DISPLAY_ON(MMI_status_icons[j].flags))
                {
                    display_flag = 1;
                    k = j;
                }
                else
                {
                    i++;
                    j++;
                }
            }
        }

        if (display_flag)
        {
            MMI_status_icons_list3[MMI_status_icon_bars[2].n_icons] = (S16) k;
            MMI_status_icon_bars[2].n_icons++;
        #if(STATUS_ICONS_DRAW_FROM_BOTTOM_BASELINE)
            MMI_status_icons[k].y = cy - MMI_status_icons[k].height;
        #endif 
        }
    }
    GDI_UNLOCK;
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_arrange_status_icons1
 * DESCRIPTION
 *  arrange status icon horizontal and vertical staus bar of main lcd
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_arrange_status_icons1(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    S32 x1, y1, x2, y2, x, y, cy;
    S32 i, j, k = 0, group_ID;
    U8 done = 0, inner_done, display_flag;
    S32 old_i;  

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    /* Status bar 1: Horizontal bar  */

    MMI_status_icons_list1[0] = STATUS_ICON_SIGNAL_STRENGTH;
    MMI_status_icons_list1[1] = STATUS_ICON_BATTERY_STRENGTH;
    MMI_status_icon_bars[0].n_icons = 2;

    x1 = MMI_status_icon_bars[0].x1;    /* store start x position of horizontal stats bar of main lcd */
    y1 = MMI_status_icon_bars[0].y1;    /* store start y position of horizontal stats bar of main lcd */
    x2 = MMI_status_icon_bars[0].x2;    /* store end x position of horizontal stats bar of main lcd */
    y2 = MMI_status_icon_bars[0].y2;    /* store end y position of horizontal stats bar of main lcd */

    /* Shift x1,x2 assuming signal strength and battery strength icons are at either ends  */
    x1 += MMI_status_icons[MMI_status_icons_list1[0]].width - 1 + STATUS_ICON_X_GAP;
    x2 = MMI_status_icons[MMI_status_icons_list1[1]].x - STATUS_ICON_X_GAP;
#if(STATUS_ICONS_DRAW_FROM_BOTTOM_BASELINE)
    cy = y2 - y1 - 1;
#else 
    cy = (y2 - y1 + 1) >> 1;
#endif 

#if(STATUS_ICONS_DRAW_FROM_BOTTOM_BASELINE)
    MMI_status_icons[0].y = cy - MMI_status_icons[0].height;
    MMI_status_icons[1].y = cy - MMI_status_icons[1].height;
#else /* (STATUS_ICONS_DRAW_FROM_BOTTOM_BASELINE) */ 
    MMI_status_icons[0].y = cy - (MMI_status_icons[0].height >> 1);
    MMI_status_icons[1].y = cy - (MMI_status_icons[1].height >> 1);
#endif /* (STATUS_ICONS_DRAW_FROM_BOTTOM_BASELINE) */ 

    i = 2;
    x = x1 + STATUS_ICON_X_GAP;
    while (!done)
    {
        display_flag = 0;
        j = MMI_status_icons_pool1[i];
        old_i = i;  
        i++;
        if (i > MMI_N_STATUS_ICONS_POOL1)
        {
            break;
        }
        if (TEST_STATUS_ICON_DISPLAY_ON(MMI_status_icons[j].flags))
        {
            k = j;
            display_flag = 1;
        }
        if ((MMI_status_icons[j].group_ID > 0))
        {
            group_ID = MMI_status_icons[j].group_ID;
            j++;
            inner_done = 0;
            while (!inner_done)
            {
                if (group_ID != MMI_status_icons[j].group_ID)
                {
                    break;
                }
                if ((!display_flag) && TEST_STATUS_ICON_DISPLAY_ON(MMI_status_icons[j].flags))
                {
                    display_flag = 1;
                    k = j;
                }
                else
                {
                    i++;
                    j++;
                }
            }
        }
        if (display_flag)
        {
            if ((x + MMI_status_icons[k].width + STATUS_ICON_X_GAP) > x2)
            {              
                i = old_i;  
                break;
            }
        #if(STATUS_ICONS_DRAW_FROM_BOTTOM_BASELINE)
            y = cy - MMI_status_icons[k].height;
        #else 
            y = cy - (MMI_status_icons[k].height >> 1);
        #endif 
            MMI_status_icons[k].x = x;  /* store x position of ion */
            MMI_status_icons[k].y = y;  /* stor y position of icon */
            x += MMI_status_icons[k].width + STATUS_ICON_X_GAP;
            MMI_status_icons_list1[MMI_status_icon_bars[0].n_icons] = (S16) k;
            MMI_status_icon_bars[0].n_icons++;
        }
    }

}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_show_status_icon1
 * DESCRIPTION
 *  set teh flag of icon to show
 * PARAMETERS
 *  icon_ID     [IN]        
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_show_status_icon1(S16 icon_ID)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if ((icon_ID < 0) || (icon_ID >= MAX_STATUS_ICONS))
    {
        return;
    }

    MMI_status_icons[icon_ID].flags |= STATUS_ICON_DISPLAY;
    wgui_status_icon_arrange_status_icons1();
}


/* sublcd background display. */
/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_show_sublcd_status_background
 * DESCRIPTION
 *  Displays the SUBLCD background screen
 * PARAMETERS
 *  start_x             [IN]        Left top position
 *  start_y             [IN]        Left top position
 *  end_x               [IN]        Right bottom position
 *  end_y               [IN]        Right bottom position
 *  is_status_bar       [IN]        Is status bar to draw or not.
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_show_sublcd_status_background(S32 start_x, S32 start_y, S32 end_x, S32 end_y, U8 is_status_bar)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    /* draw all screen of background display. */
#if defined(__MMI_SUBLCD__)
    GDI_LOCK;
#if (defined (__MMI_SUBLCD_COLOR__) || defined (__MMI_FULL_SUBLCD_COLOR__))

    if (!is_status_bar)
    {
    #ifdef __MMI_KLG__
        color c = gui_color(0, 0, 0);
    #else 
        color c = gui_color(255, 255, 255);
    #endif 

    #if defined(__MMI_SUBLCD_COLOR__)
        start_y = (U16) Sub_lcd_title_height;
    #endif 
        gui_push_clip();
        gui_set_clip(start_x, start_y, end_x, end_y);

        /* Always draw color, not draw filled area */
        /* gui_draw_filled_area(start_x, start_y, end_x, end_y, &f); */
        gui_fill_rectangle(start_x, start_y, end_x, end_y, c);
        gui_pop_clip();
    }
    else
    {
        /* No process for status basr */
    #ifdef __MMI_KLG__
        color c = gui_color(0, 0, 0);
    #else 
        color c = gui_color(255, 255, 255);
    #endif 
        gui_push_clip();
        gui_set_clip(start_x, start_y, end_x, end_y);
        gui_fill_rectangle(start_x, start_y, end_x, end_y, c);
        gui_pop_clip();
    }
#else /* (defined (__MMI_SUBLCD_COLOR__) || defined (__MMI_FULL_SUBLCD_COLOR__)) */ 
    {
        color c = gui_color(255, 255, 255);

        gui_push_clip();
        gui_set_clip(start_x, start_y, end_x, end_y);
        gui_fill_rectangle(start_x, start_y, end_x, end_y, c);
        gui_pop_clip();
    }
#endif /* (defined (__MMI_SUBLCD_COLOR__) || defined (__MMI_FULL_SUBLCD_COLOR__)) */ 
    GDI_UNLOCK;
#endif /* defined(__MMI_SUBLCD__) */ 

}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_show_mainlcd_status_bar_background
 * DESCRIPTION
 *  
 * PARAMETERS
 *  start_x     [IN]        
 *  start_y     [IN]        
 *  end_x       [IN]        
 *  end_y       [IN]        
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_show_mainlcd_status_bar_background(U16 start_x, U16 start_y, U16 end_x, U16 end_y)
/* Only for status icon bar 0 */
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    color c = {255, 255, 255, 100};

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    gdi_layer_push_clip();
    gui_set_clip_preset(start_x, start_y, end_x, end_y);
    gui_fill_rectangle(start_x, start_y, end_x, end_y, c);
    gdi_layer_pop_clip();
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_set_status_icons_display
 * DESCRIPTION
 *  
 * PARAMETERS
 *  state       [IN]        
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_set_status_icons_display(MMI_BOOL state)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (state)
    {
        U8 blink_animate_flag = 0;
        S32 i;

        status_icon_display_ON = 1;

        for (i = 0; i < MAX_STATUS_ICONS; i++)
        {
            if ((MMI_status_icons[i].flags & STATUS_ICON_BLINK) || (MMI_status_icons[i].flags & STATUS_ICON_ANIMATE))
            {
                blink_animate_flag = 1;
                break;
            }
        }

        if (blink_animate_flag == 1)
        {
            gui_start_timer(STATUS_ICON_TIMER_RATE, wgui_status_icon_status_icon_timer_callback);        /* start timer for blinking icon */
        }
    }
    else
    {
        status_icon_display_ON = 0;
        gui_cancel_timer(wgui_status_icon_status_icon_timer_callback);
    }
}

#ifdef __MMI_TOUCH_SCREEN__
MMI_BOOL gIsHandleStatusIconPenEvent = MMI_FALSE;


/*****************************************************************************
 * FUNCTION
 *  wgui_register_status_icon_pen_event_hdlr
 * DESCRIPTION
 *  register pen event handlers for status icons.
 * PARAMETERS
 *  icon_id         [IN]        Id of icon
 *  event_type      [IN]        
 *  f               [IN]        Event handler
 * RETURNS
 *  BOOL        if any status icon is touched(?)
 *****************************************************************************/
void wgui_register_status_icon_pen_event_hdlr(S16 icon_id, wgui_status_icon_bar_pen_enum event_type, FuncPtr f)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_status_icon_pen_event_hdlr[icon_id][event_type] = f;
    gIsHandleStatusIconPenEvent = MMI_TRUE;
}


/*****************************************************************************
 * FUNCTION
 *  wgui_execute_status_icon_pen_event_hdlr
 * DESCRIPTION
 *  execute pen event handlers for status icons.
 * PARAMETERS
 *  icon_id         [IN]        Id of icon
 *  event_type      [IN]        
 * RETURNS
 *  BOOL        if event handler exists
 *****************************************************************************/
/* Vogins Start */
/*ltz 0613*/
#if  defined(__VRE_V30__) && defined(__MMI_TOUCH_SCREEN__) //&& defined(__MMI_VUI_HOMESCREEN__)
extern signed int g_statusicon_id;
#endif
/* Vogins End */
MMI_BOOL wgui_execute_status_icon_pen_event_hdlr(S16 icon_id, wgui_status_icon_bar_pen_enum event_type)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (MMI_status_icon_pen_event_hdlr[icon_id][event_type] != NULL)
    {
/* Vogins Start */
/*ltz 0613*/
#if  defined(__VRE_V30__) && defined(__MMI_TOUCH_SCREEN__) //&& defined(__MMI_VUI_HOMESCREEN__)
		g_statusicon_id = icon_id;
#endif
/* Vogins End */
        MMI_status_icon_pen_event_hdlr[icon_id][event_type] ();
        return MMI_TRUE;
    }
    else
    {
        return MMI_FALSE;
    }
}


/*****************************************************************************
 * FUNCTION
 *  wgui_reset_status_icon_pen_event_hdlr
 * DESCRIPTION
 *  Reset pen event handlers for status icons.
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
/* Vogins Start */
#if defined(__VRE_V30__) && defined(__MMI_TOUCH_SCREEN__) /*&& !defined(__MMI_VUI_HOMESCREEN__)*/
extern int vm_vre_is_running(void);
#endif
/* Vogins End */
void wgui_reset_status_icon_pen_event_hdlr(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    U8 icon_id = 0, event_type = 0;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
/* Vogins Start */
#if defined(__VRE_V30__) && defined(__MMI_TOUCH_SCREEN__) /*&& !defined(__MMI_VUI_HOMESCREEN__)*/
    U8 in_check= MMI_FALSE;
#endif
/* Vogins End */


    for (event_type = 0; event_type < WGUI_STATUS_ICON_BAR_PEN_EVENT_MAX; event_type++)
        for (icon_id = 0; icon_id < MAX_STATUS_ICONS; icon_id++)
        {
/* Vogins Start */
#if defined(__VRE_V30__) && defined(__MMI_TOUCH_SCREEN__) /*&& !defined(__MMI_VUI_HOMESCREEN__)*/
            if (vm_vre_is_running())
            {
                switch (icon_id)
                {
                    case STATUS_ICON_VRE_1:
                    case STATUS_ICON_VRE_2:
                    case STATUS_ICON_VRE_3:
                    case STATUS_ICON_VRE_4:
                    case STATUS_ICON_VRE_DEFAULT:
                        in_check= MMI_TRUE;
                        continue;				
                }
            }
#else
            MMI_status_icon_pen_event_hdlr[icon_id][event_type] = NULL;
#endif
/* Vogins End */
        }
/* Vogins Start */
#if defined(__VRE_V30__) && defined(__MMI_TOUCH_SCREEN__) /*&& !defined(__MMI_VUI_HOMESCREEN__)*/
    if (in_check)
        gIsHandleStatusIconPenEvent = MMI_TRUE;
    else
        gIsHandleStatusIconPenEvent = MMI_FALSE;
#else
    gIsHandleStatusIconPenEvent = MMI_FALSE;
#endif
/* Vogins End */
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_translate_pen_position
 * DESCRIPTION
 *  Translate pen position for status icons.
 * PARAMETERS
 *  x       [IN]        Pen x-coordinate
 *  y       [IN]        Pen y-coordinate
 * RETURNS
 *  S16         icon_index
 *****************************************************************************/
S16 wgui_status_icon_translate_pen_position(S16 x, S16 y)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    U8 i = 0;
    S16 x1, y1, x2, y2;
    S16 icon_idx = -1, bar_idx = 0;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    for (bar_idx = 0; bar_idx < MAX_STATUS_ICON_BARS; bar_idx++)
    {
        if (MMI_status_icon_bars[bar_idx].flags & STATUS_ICON_BAR_DISPLAY &&
            MMI_status_icon_bars[bar_idx].icon_list != NULL)
        {
            x1 = MMI_status_icon_bars[bar_idx].x1;
            y1 = MMI_status_icon_bars[bar_idx].y1;
            x2 = MMI_status_icon_bars[bar_idx].x2;
            y2 = MMI_status_icon_bars[bar_idx].y2;
            if (x >= x1 && x <= x2 && y >= y1 && y <= y2)   /* inside status icon bar */
            {
                for (i = 0; i < MMI_status_icon_bars[bar_idx].n_icons; i++)
                {
                    icon_idx = MMI_status_icon_bars[bar_idx].icon_list[i];
                    if (MMI_status_icons[icon_idx].icon_ID != IMAGE_ID_NULL &&
                        MMI_status_icons[icon_idx].flags & STATUS_ICON_DISPLAY)
                    {
                        x1 = MMI_status_icons[icon_idx].x + MMI_status_icon_bars[bar_idx].x;
                        x2 = x1 + MMI_status_icons[icon_idx].width - 1;
                        y1 = MMI_status_icons[icon_idx].y + MMI_status_icon_bars[bar_idx].y;
                        y2 = y1 + MMI_status_icons[icon_idx].height - 1;
                        if (x >= x1 && x <= x2 && y >= y1 && y <= y2)   /* pen on status icon */
                        {
                            return icon_idx;
                        }
                    }
                }
            }
        }
    }
    return -1;
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_translate_pen_event
 * DESCRIPTION
 *  Translate pen events for status icons.
 * PARAMETERS
 *  pen_event       [IN]            Type of pen event
 *  x               [IN]            Pen x-coordinate
 *  y               [IN]            Pen y-coordinate
 *  icon_id         [IN/OUT]        Id of icon be touched
 *  event_type      [?]             
 * RETURNS
 *  BOOL        if any status icon is touched
 *****************************************************************************/
BOOL wgui_status_icon_translate_pen_event(
        mmi_pen_event_type_enum pen_event,
        S16 x,
        S16 y,
        S16 *icon_id,
        wgui_status_icon_bar_pen_enum *event_type)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    gui_wait_longpress_enum w;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    *event_type = WGUI_STATUS_ICON_BAR_PEN_NONE;
    *icon_id = -1;

    if (gIsHandleStatusIconPenEvent == MMI_FALSE)
    {
        return FALSE;
    }

    if (pen_down_status_icon_id < 0 && pen_event != MMI_PEN_EVENT_DOWN)
    {
        return FALSE;
    }

    if (pen_event == MMI_PEN_EVENT_DOWN)
    {
        pen_down_status_icon_id = wgui_status_icon_translate_pen_position(x, y);

        if (pen_down_status_icon_id < 0)
        {
            return FALSE;
        }
    }

    *icon_id = pen_down_status_icon_id;

    if (MMI_status_icon_pen_event_hdlr[pen_down_status_icon_id][WGUI_STATUS_ICON_BAR_PEN_LONG_PRESS_ICON] != NULL)
    {
        w = gui_pen_wait_longpress(pen_event, x, y);
        if (w == GUI_WAIT_LONGPRESS_READY)
        {
            *event_type = WGUI_STATUS_ICON_BAR_PEN_LONG_PRESS_ICON;
        }
    }

    if (pen_event == MMI_PEN_EVENT_UP && *event_type == WGUI_STATUS_ICON_BAR_PEN_NONE)
    {
        if (pen_down_status_icon_id == wgui_status_icon_translate_pen_position(x, y))
        {
            *event_type = WGUI_STATUS_ICON_BAR_PEN_SELECT_ICON;
        }
    }

    return TRUE;
}

#endif /* __MMI_TOUCH_SCREEN__ */ 


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_get_status_icon_flag
 * DESCRIPTION
 *  
 * PARAMETERS
 *  icon_ID     [IN]        
 * RETURNS
 *  
 *****************************************************************************/
U32 wgui_status_icon_get_status_icon_flag(S16 icon_ID)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    return MMI_status_icons[icon_ID].flags;
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_set_status_icon_flag
 * DESCRIPTION
 *  
 * PARAMETERS
 *  icon_ID     [IN]        
 *  value       [IN]        
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_set_status_icon_flag(S16 icon_ID, U32 value)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if ((icon_ID < 0) || (icon_ID >= MAX_STATUS_ICONS))
    {
        return;
    }

    /* set the flag to the specific value */
    MMI_status_icons[icon_ID].flags = value;

    /* set rearrange flag on */
    IsReArrangeNeeded = 1;
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_set_display_layer
 * DESCRIPTION
 *  
 * PARAMETERS
 *  display_layer       [IN]        
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_set_display_layer(GDI_HANDLE display_layer)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    wgui_status_icon_display_layer = display_layer;
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_get_display_layer
 * DESCRIPTION
 *  
 * PARAMETERS
 *  void
 * RETURNS
 *  
 *****************************************************************************/
GDI_HANDLE wgui_status_icon_get_display_layer(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    return wgui_status_icon_display_layer;
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_reset_display_layer
 * DESCRIPTION
 *  
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_reset_display_layer(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    wgui_status_icon_display_layer = GDI_LAYER_EMPTY_HANDLE;
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_integrated_get_area
 * DESCRIPTION
 *  
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_integrated_get_area(S16 icon_ID, S32 *width, S32 *height)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
#ifdef MMI_SHOW_STATUS_ICON_IN_TITLE
    if (!(wgui_status_icon_get_status_icon_bar_type(0) & STATUS_ICON_BAR_DISPLAY) && 
        (MMI_status_icons[icon_ID].flags & STATUS_ICON_DISPLAY) &&
        !(g_wgui_status_bar_integrated_disabled))
    {
        *width = MMI_status_icons[icon_ID].width;
        *height = MMI_status_icons[icon_ID].height;
    }
    else
#endif
    {
        *width = 0;
        *height = 0;
    }
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_integrated_show
 * DESCRIPTION
 *  
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_integrated_show(S16 icon_ID, S32 x, S32 y)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
#ifdef MMI_SHOW_STATUS_ICON_IN_TITLE    
    S32 oldx, oldy;
#endif

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
#ifdef MMI_SHOW_STATUS_ICON_IN_TITLE
    if (!(wgui_status_icon_get_status_icon_bar_type(0) & STATUS_ICON_BAR_DISPLAY) &&
        !(g_wgui_status_bar_integrated_disabled))
    {
        if (MMI_status_icons[icon_ID].flags & STATUS_ICON_ANIMATE)
        {
            gdi_image_draw_frames_blend2layers(
                x,
                y,
                MMI_status_icons[icon_ID].icon_data,
                (U16) MMI_status_icons[icon_ID].current_frame);   
        }
        else if (MMI_status_icons[icon_ID].flags & STATUS_ICON_DISPLAY)
        {
            oldx = MMI_status_icons[icon_ID].x;
            oldy = MMI_status_icons[icon_ID].y;
            MMI_status_icons[icon_ID].x = x;
            MMI_status_icons[icon_ID].y = y;
            /* store position of the icon */ 
            if (MMI_status_icons[icon_ID].state > 0)
            {
                gdi_image_draw_frames_blend2layers(
                    x,
                    y,
                    MMI_status_icons[icon_ID].icon_data,
                    (U16) MMI_status_icons[icon_ID].state);   
            }
            else
            {
                gdi_image_draw_blend2layers(x, y, MMI_status_icons[icon_ID].icon_data);
            }
            MMI_status_icons[icon_ID].x = oldx;
            MMI_status_icons[icon_ID].y = oldy;
        }
    }
#endif
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_integrated_register_redraw
 * DESCRIPTION
 *  
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_integrated_register_redraw(void (*f) (void))
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (!g_wgui_status_bar_integrated_disabled)
        MMI_redraw_integrated_status_icon_bars = f;
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_integrated_Disalbed
 * DESCRIPTION
 *  
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_integrated_enable(MMI_BOOL enable)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    U8 blink_animate_flag = 0, i;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    g_wgui_status_bar_integrated_disabled = !enable;
    
    if (!g_wgui_status_bar_integrated_disabled)
    {
        for (i = 0; i < MAX_STATUS_ICONS; i++)
        {
            if ((MMI_status_icons[i].flags & STATUS_ICON_BLINK) || (MMI_status_icons[i].flags & STATUS_ICON_ANIMATE))
            {
                blink_animate_flag = 1;
                break;
            }
        }
        if (blink_animate_flag)
        {
            gui_start_timer(STATUS_ICON_TIMER_RATE, wgui_status_icon_status_icon_timer_callback);
        }
    }
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_set_block
 * DESCRIPTION
 *  
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_set_block(MMI_BOOL blocking)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    U8 blink_animate_flag = 0;
    S32 i;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    wgui_status_icon_blocking = blocking;
    if(wgui_status_icon_blocking == MMI_TRUE)
    {
        for (i = 0; i < MAX_STATUS_ICONS; i++)
        {
            if ((MMI_status_icons[i].flags & STATUS_ICON_BLINK) || (MMI_status_icons[i].flags & STATUS_ICON_ANIMATE))
            {
                blink_animate_flag = 1;
                break;
            }
        }
        
        if (blink_animate_flag)
        {
            gui_cancel_timer(wgui_status_icon_status_icon_timer_callback);
        }    
        
    }
    else
    {
        for (i = 0; i < MAX_STATUS_ICONS; i++)
        {
            if ((MMI_status_icons[i].flags & STATUS_ICON_BLINK) || (MMI_status_icons[i].flags & STATUS_ICON_ANIMATE))
            {
                blink_animate_flag = 1;
            }
            if (MMI_status_icons[i].width == -1)
            {
                gui_measure_image(
                    MMI_status_icons[i].icon_data, 
                    &MMI_status_icons[i].width, 
                    &MMI_status_icons[i].height);
            }
        }
        
        if (blink_animate_flag)
            /* start timer for blinking icon */
        {
            gui_start_timer(STATUS_ICON_TIMER_RATE, wgui_status_icon_status_icon_timer_callback);
        }
        else
            /* stop timer for blinking icon */
        {
            gui_cancel_timer(wgui_status_icon_status_icon_timer_callback);
        }
        
        wgui_status_icon_arrange_status_icons();
        IsReArrangeNeeded = 0;
        wgui_status_icon_refresh_status_icon_display();
    }
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_unlock_redrawing
 * DESCRIPTION
 * Unlock all the GDI operation for  Async image decode.
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_unlock_redrawing(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    wgui_status_icon_set_block(MMI_FALSE);
    gui_remove_pre_cleanup_hook(wgui_status_icon_unlock_redrawing);
}


/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_lock_redrawing
 * DESCRIPTION
 * Lock all the GDI operation for  Async image decode.
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_lock_redrawing(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    wgui_status_icon_set_block(MMI_TRUE);
    gui_add_pre_cleanup_hook(wgui_status_icon_unlock_redrawing);
}


void wgui_status_icon_register_update_callback(void (*f)(void))
{
    status_icon_update_callback = f;
}

void wgui_status_icon_reset_update_callback(void)
{
    status_icon_update_callback = NULL;
}

void wgui_status_icon_set_alpha_layer(gdi_handle alpha_layer)
{
    wgui_status_bar_alpha_layer = alpha_layer;
#ifdef WGUI_STATUS_ICON_SHOW_TIME_IN_RIGHT
    if(wgui_clock_status_icon_id != WGUI_CLOCK_INVALID_INDEX)
    {
        wgui_clock_set_abm_layer(wgui_clock_status_icon_id, alpha_layer);
    }  
#endif /* WGUI_STATUS_ICON_SHOW_TIME_IN_RIGHT */
}

void wgui_status_icon_reset_alpha_layer(void)
{
    wgui_status_bar_alpha_layer = GDI_NULL_HANDLE;
}

#ifdef __MMI_VUI_ENGINE__
/*****************************************************************************
 * FUNCTION
 *  wgui_status_icon_bar_set_venus_callback
 * DESCRIPTION
 *  Set venus update callback and handle
 * PARAMETERS
 *  update_screen_callback      [IN]        venus update callback
 *  update_screen_handle        [IN]        venus update handle
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_status_icon_bar_set_venus_callback(
        void (*update_screen_callback)(void *handle),
        void *update_screen_handle)
{
    MMI_status_icon_bars[0].venus_update_screen_callback = update_screen_callback;
    MMI_status_icon_bars[0].venus_update_screen_handle = update_screen_handle;
}
#endif /* __MMI_VUI_ENGINE__ */

/* Vogins Start */
#ifdef __VRE_V30__
U16 GetIconImageId(S16 icon_ID)
{
	return MMI_status_icons[icon_ID].icon_ID;
}

void ChangeStatusIconImageData(S16 icon_ID, PU8 image_data)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_status_icons[icon_ID].icon_data = image_data;
    MMI_status_icons[icon_ID].n_frames = (S16) gui_image_n_frames(MMI_status_icons[icon_ID].icon_data);
#ifdef __MMI_NONBLOCKING_DECODE__
    if(wgui_status_icon_blocking == MMI_TRUE)
    {
        MMI_status_icons[icon_ID].width = -1;
        MMI_status_icons[icon_ID].height = -1;
    }
    else
#endif
    {
        gui_measure_image(
            MMI_status_icons[icon_ID].icon_data,
            &MMI_status_icons[icon_ID].width,
            &MMI_status_icons[icon_ID].height);
    }

    IsReArrangeNeeded = 1;
}
#endif
/* Vogins End */

