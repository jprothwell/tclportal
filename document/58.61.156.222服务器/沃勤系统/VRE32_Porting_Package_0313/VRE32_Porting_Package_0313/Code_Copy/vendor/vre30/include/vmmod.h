/***********************************************************************
  ...
  * 生命周期的功能。在VRE里的所有模块都将有三个状态。
  * 这三个状态分别对应:
  *
  * (1) INIT_STATE
  * 在VRE还没有启动的时候各个模块处于该状态。在该状态下
  * 各个模块提供的服务是可选择的。VRE中的各个功能模块可
  ...
  * 状态下。在该状态下，VRE内部的模块应该提供全部的服务功能。
  *
  * (3) FREEZING_STATE
  * 在模块收到了EVT_MOD_ACTIVE消息，VRE模块就应该切换到FREEZING_STATE
  * 状态，在该状态下模块对外提供的服务是不确定的，不被保证的。
  * 应用程序在收到VRE发出的INACTIVE消息后不应该再调用VRE任何模块提供
...
/**
 * 注册一个VRE模块。VRE中的每个模块都应该调用这个函数把自己注册到系统中。
 * 在进入VRE的主函数前，VRE管理器会按模块注册的顺序依次把模块的生命周期
 * 事件通知各个模块。当前的模块处理函数的返回值只有在模块处理EVT_MOD_INIT
 * 的时才有效，返回0，表示成功处理了该生命周期消息，否则返回错误码。当
 * 有一个模块在处理EVT_MOD_INIT的时候返回错误，那么模块管理器则会发送给已
 * 进入RUNNING_STATE状态的模块EVT_MOD_RELEASE消息，并且退出VRE系统。
 * 
 * 
