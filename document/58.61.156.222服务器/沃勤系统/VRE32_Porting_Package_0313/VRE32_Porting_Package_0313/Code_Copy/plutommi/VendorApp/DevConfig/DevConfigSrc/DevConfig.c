...
#include "MMI_features_video.h" /* video bitstream */
...
void mmi_devconfig_get_system_config(mmi_devconfig_system_config_struct * p_sys_config)
{
    ...
    g_system_config.dual_sim_master = MMI_TRUE;
#endif
    ...
    /* memory buffer size */
#if defined(__VRE__)
    g_system_config.memory_size = ((VRE_SYS_MEMORY_SIZE) << 10);
#endif
...
#if defined(__MMI_WAP_PROF__)    //WAP_SUPPORT
...
#if defined(__MMI_EM_SOCKET__)   //TCPIP_SUPPORT
    g_system_config.newwork_protocol.tcpip_support = MMI_TRUE;
#endif
