...
public class AllApps2D
    ...
    public static class HomeButton extends ImageButton {
        public HomeButton(Context context, AttributeSet attrs) {
            super(context, attrs);
            Log.d(TAG,"HomeButton");
        }
        @Override
        public View focusSearch(int direction) {
        	Log.d(TAG,"HomeButton focusSearch");
...
//            }
    ...
    public void zoom(float zoom, boolean animate) {
        ...
        if (isVisible()) {
        	Log.d(TAG,"zoom isVisible");
            getParent().bringChildToFront(this);
            setVisibility(View.VISIBLE);
            mGrid.setAdapter(mAppsAdapter);
            if (animate) {
            	Log.d(TAG,"zoom isVisible animate="+animate);
                startAnimation(AnimationUtils.loadAnimation(getContext(), R.anim.all_apps_2d_fade_in));
            } else {
            	Log.d(TAG,"zoom isVisible onAnimationEnd");
                onAnimationEnd();
            }
        } else {
            if (animate) {
            	Log.d(TAG,"zoom animate");
    ...
    protected void onAnimationEnd() {
        if (!isVisible()) {
        	Log.d(TAG,"zoom onAnimationEnd isVisible");
            setVisibility(View.GONE);
